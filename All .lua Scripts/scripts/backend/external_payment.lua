...
function ExternalPayment:update_account_store_status()
	if is_xbox_live() then
...
function ExternalPayment:payment_options()
	local language = "en"
...
function ExternalPayment:reconcile_pending_txns()
...
function ExternalPayment:reconcile_dlc(store_ids)
	return get_platform_token():next(function (token)
		return Managers.backend:authenticate():next(function (account)
...
function ExternalPayment:init_txn(payment_option)
	return Managers.backend:authenticate():next(function (account)
...
function ExternalPayment:finalize_txn(order_id)
	return get_platform_token():next(function (token)
		return Managers.backend:authenticate():next(function (account)
...
function ExternalPayment:fail_txn(order_id)
	return Managers.backend:authenticate():next(function (account)
		local builder = BackendUtilities.url_builder():path("/store/"):path(account.sub):path("/payments/"):path(order_id):query("platform", get_payment_platform())
...
function ExternalPayment:_decorate_option(option, platform_entitlements)
	option.description = {
		type = "currency",
...
function ExternalPayment:get_options()
