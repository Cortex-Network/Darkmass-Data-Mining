...
function MasterItems.default_inventory(archetype_name, game_mode_settings_or_nil)
...
function MasterItems.find_fallback_item_id(slot)
	local fallback_name = slot and FALLBACK_ITEMS_BY_SLOT[slot]

	return fallback_name
end

function MasterItems.find_fallback_item(slot)
...
function MasterItems.get_item_instance(gear, gear_id)
	if not gear then
...
function MasterItems.create_preview_item_instance(item)
...
function MasterItems.get_item(id)
	assert_backend()

	return MasterItems.get_cached()[id]
end

function MasterItems.item_exists(id)
	assert_backend()
