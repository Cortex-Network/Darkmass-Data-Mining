...
function RegionLatency:pre_get_region_latencies()
	if not self._promise then
		self._promise = self:_do_refresh()
	end
end

function RegionLatency:get_region_latencies()
	if not self._promise then
		self._promise = self:_do_refresh()
	end
...
function RegionLatency:_ping_targets(timeout, regions)
	local targets = {}
...
function RegionLatency:_recursive_ping(timeout, ping_count, regions, ping_responses, promise)
...
function RegionLatency:_do_refresh()
