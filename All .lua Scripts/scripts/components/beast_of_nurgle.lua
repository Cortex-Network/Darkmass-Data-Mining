...
function BeastOfNurgle:init(unit, is_server)
...
function BeastOfNurgle:editor_init(unit)
...
function BeastOfNurgle:enable(unit)
end

function BeastOfNurgle:disable(unit)
end

function BeastOfNurgle:destroy(unit)
	self:_reparent_spline_joints(unit)
end

function BeastOfNurgle:editor_destroy(unit)
	if self._in_level_editor then
		self:_reparent_spline_joints(unit)
...
function BeastOfNurgle:update(unit, dt, t)
...
function BeastOfNurgle:editor_update(unit, dt, t)
	if self._in_level_editor then
		self:_update_spline(unit, dt)
		self:_update_joints(unit, dt)
...
function BeastOfNurgle:get_spline_position_rotation(spline_index)
...
function BeastOfNurgle:get_last_spline_position_rotation()
...
function BeastOfNurgle:editor_world_transform_modified(unit)
end

function BeastOfNurgle:editor_toggle_debug_draw(enable)
	self._should_debug_draw = enable
end

local COLLISION_FILTER = "filter_minion_mover"

function BeastOfNurgle:_ray_cast(from, to)
...
function BeastOfNurgle:_init_spline(unit)
...
function BeastOfNurgle:_set_default_spline(unit)
...
function BeastOfNurgle:_update_spline(unit, dt)
	self._target_transform = Unit.world_pose(unit, self._target_joint)
...
function BeastOfNurgle:_reparent_spline_joints(unit)
	for _, spline_j in pairs(self._spline_joints) do
		Unit.scene_graph_link(unit, spline_j, 1)
	end
end

function BeastOfNurgle:_unparent_spline_joints(unit)
	for _, spline_j in pairs(self._spline_joints) do
		Unit.scene_graph_link(unit, spline_j, nil)
	end
end

function BeastOfNurgle:_update_joints(unit, dt)
...
function BeastOfNurgle:_get_bezier_transform(p1, c1, c2, p2, up1, up2, t)
...
function BeastOfNurgle:_get_spline_components(tm1, tm2)
...
function BeastOfNurgle:_editor_debug_draw(unit)
