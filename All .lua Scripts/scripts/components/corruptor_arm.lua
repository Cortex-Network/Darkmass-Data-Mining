...
function CorruptorArm:init(unit)
	self._unit = unit
...
function CorruptorArm:destroy(unit)
	self:_destroy_children(unit)
end

function CorruptorArm:enable(unit)
end

function CorruptorArm:disable(unit)
end
...
function CorruptorArm:editor_init(unit)
...
function CorruptorArm:editor_destroy(unit)
	if self._in_level_editor then
...
function CorruptorArm:_initialize_arm(unit)
...
function CorruptorArm:_bezier_setup(unit)
	self:_spline_position(unit, false, 0, unit, 0, 1)
	self:_place_meshes(unit)
end

function CorruptorArm:last_joint_position()
...
function CorruptorArm:_spline_position(unit, multi_mesh, mesh_increment_add, attach_mesh, mesh_rotation, current_end_position)
...
function CorruptorArm:_create_meshes(unit)
...
function CorruptorArm:_place_meshes(unit)
...
function CorruptorArm:_set_arm_visible(unit, visible)
	for i, mesh in ipairs(self._attached_meshes) do
		Unit.set_visibility(mesh, "main", visible)
	end
end

function CorruptorArm:_set_node_visible(unit, visible)
	Unit.set_visibility(unit, "main", visible)
end

function CorruptorArm:_destroy_children(unit)
	local attached_meshes = self._attached_meshes
...
function CorruptorArm:editor_world_transform_modified(unit)
...
function CorruptorArm:editor_toggle_debug_draw(enable)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function CorruptorArm:_editor_debug_draw(unit)
	self._drawer:reset()
...
function CorruptorArm.events:set_animation_pos(position)
...
function CorruptorArm.events:demolition_stage_start()
	self._corruptor_arm_extension:activate()
end

function CorruptorArm.events:demolition_stage_finished()
