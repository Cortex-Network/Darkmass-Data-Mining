...
function FootIk:editor_init(unit)
...
function FootIk:_instantiate_ik_data()
...
function FootIk:enable(unit)
end

function FootIk:disable(unit)
end

function FootIk:destroy(unit)
end

function FootIk:sampling(surface_sample, past_surface_sample, leaning_sample, past_leaning_sample, distance_warp_sample)
...
function FootIk:_get_feet_grounded_weights()
...
function FootIk:feet_are_on_ground()
...
function FootIk:_distance_warp_sampling(left_ref_pos, right_ref_pos, distance_warp_sample)
...
function FootIk:_raycast_surface(from_pose, ray_distance, is_left)
...
function FootIk:_get_handle_movement_to_surface(hit_distance, weight, hit_type)
...
function FootIk:_get_handle_orientation_to_surface(orient_ref, weight, orient_ref_parent, surface_normal)
...
function FootIk:_lean_in_acceleration_sampling(leaning_sample, past_leaning_sample)
...
function FootIk:update(unit, dt, t)
...
function FootIk:editor_update(unit, dt, t)
...
function FootIk:_refresh_slider_values()
...
function FootIk:editor_property_changed(unit)
	self:_refresh_slider_values()
end

function FootIk:editor_toggle_debug_draw(enable)
	self._should_debug_draw = enable
end

function FootIk:_editor_debug_draw()
	if self._should_debug_draw then
		self._drawer:reset()
		self._drawer:update(self._world)
	else
...
function FootIk:editor_destroy(unit)
