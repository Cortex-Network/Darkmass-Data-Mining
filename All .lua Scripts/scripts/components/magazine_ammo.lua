...
function MagazineAmmo:editor_init(unit)
...
function MagazineAmmo:init(unit)
	self._anim_speed = self:get_data(unit, "anim_speed")
	self._use_simple_animation_length = self:get_data(unit, "use_simple_animation_length")
	self._dismantled = self:get_data(unit, "dismantled")
	self._dismantled_ammo_mask = self:get_data(unit, "dismantled_ammo_mask")
...
function MagazineAmmo:enable(unit)
	self:_update_ammo_representation(unit, false)
end

function MagazineAmmo:disable(unit)
end

function MagazineAmmo:destroy(unit)
end

function MagazineAmmo:_update_ammo_representation(unit, is_animated)
...
function MagazineAmmo:set_ammo(unit, ammo, max_ammo)
	ammo = ammo - 1
	max_ammo = max_ammo - 1
