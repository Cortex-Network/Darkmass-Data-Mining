...
function MonsterSpawner:init(unit, is_server, nav_world)
...
function MonsterSpawner:disable(unit)
end

function MonsterSpawner:editor_init(unit)
...
function MonsterSpawner:editor_destroy(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function MonsterSpawner:editor_update(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function MonsterSpawner:editor_world_transform_modified(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function MonsterSpawner:editor_property_changed(unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
function MonsterSpawner:_refresh_debug_draw(unit)
...
function MonsterSpawner:editor_selection_changed(unit, selected)
...
function MonsterSpawner:_editor_debug_draw(unit, show_main_path_connection)
	local drawer = self._drawer
...
function MonsterSpawner:_debug_draw_main_path_connection(unit, drawer)
	local gui = self._gui
...
function MonsterSpawner:_debug_draw_witch_spawner(unit, drawer)
...
function MonsterSpawner:editor_toggle_debug_draw(enable)
	if not rawget(_G, "LevelEditor") then
		return
	end
