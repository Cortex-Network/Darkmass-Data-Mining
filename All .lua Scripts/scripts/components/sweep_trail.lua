...
function SweepTrail:editor_init(unit)
	self._in_editor = true

	self:init(unit)
end

function SweepTrail:enable(unit)
	self:set_critical_strike(unit, false)
	self:set_powered(unit, false)
end

function SweepTrail:disable(unit)
end

function SweepTrail:destroy(unit)
end

function SweepTrail:set_critical_strike(unit, is_enabled, in_3p)
...
function SweepTrail:set_powered(unit, is_enabled)
