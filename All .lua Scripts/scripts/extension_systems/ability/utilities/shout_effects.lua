...
function ShoutEffects:init(equiped_ability_effect_scripts_context, ability_template)
...
function ShoutEffects:destroy()
	self:_destroy_effects()
end

function ShoutEffects:update(unit, dt, t)
...
function ShoutEffects:_spawn_effects(unit)
	...
	if fx_extension then
		...
		local effect_id = fx_extension:spawn_particles(effect_name, position, nil, , optional_variable_name, optional_variable_value)
		self._effect_id = effect_id
	end
end

function ShoutEffects:_destroy_effects()
...
function ShoutEffects:_update_effect_positions(target_position, effect_id)
