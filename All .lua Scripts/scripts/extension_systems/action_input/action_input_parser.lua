...
function ActionInputParser:init(unit, action_component_name, action_component, config_data, debug_index)
...
function ActionInputParser:_format_and_initialize_action_inputs(action_input_type, templates, sequences_ring_buffer, action_input_queue_ring_buffer, hierarchy_position_ring_buffer)
...
function ActionInputParser:on_reload()
...
function ActionInputParser:peek_next_input()
...
function ActionInputParser:last_action_auto_completed()
	return self._last_action_auto_completed
end

function ActionInputParser:consume_next_input(t)
...
function ActionInputParser:clear_input_queue_and_sequences()
	local ring_buffer_index = self._ring_buffer_index
	local input_queue = self._action_input_queue[ring_buffer_index]
...
function ActionInputParser:action_transitioned_with_automatic_input(action_input, t)
...
function ActionInputParser:bot_queue_action_input(action_input, raw_input)
...
function ActionInputParser:bot_queue_request_is_consumed(global_bot_request_id)
	local buffer_index = global_bot_request_id % BOT_REQUEST_RING_BUFFER_MAX + 1
	local request = self._bot_action_input_request_queue[buffer_index]
...
function ActionInputParser:bot_queue_clear_requests(id)
	self._num_bot_action_input_requests = 0
...
function ActionInputParser:pack_input_sequences_and_queue(input_sequences_is_running_table, input_sequences_current_element_index_table, input_sequences_element_start_t_table, input_queue_hierarchy_position_table, input_queue_produced_by_hierarchy_table, input_queue_action_input_table, input_queue_raw_input_table, hierarchy_position_table)
	local ring_buffer_index = self._ring_buffer_index
...
function ActionInputParser:_fill_table_with_authoritative_hierarchy_position(table, input_queue_index, input_queue_produced_by_hierarchy, action_input_network_lookups, input_queue_hierarchy_position, input_queue, base_hierarchy, max_hierarchy_depth, no_action_input)
...
function ActionInputParser:mispredict_happened(fixed_frame, input_sequences_is_running, input_sequences_current_element_index, input_sequences_element_start_t, input_qeueue_action_input, input_queue_raw_input, input_queue_produced_by_hierarchy, input_queue_hierarchy_position, input_queue_first_entry_became_first_entry_t, hierarchy_position)
...
function ActionInputParser:update(dt, t)
end

function ActionInputParser:fixed_update(unit, dt, t, fixed_frame)
...
function ActionInputParser:_update_buffering(old_input_queue, new_input_queue, t, sequence_configs, hierarchy_position, sequences, base_hierarchy, network_lookup)
...
function ActionInputParser:_update_bot_action_input_requests(request_queue, num_requests, this_frames_input_queue, this_frames_hierarchy_position, t)
	local NO_ACTION_INPUT = self._NO_ACTION_INPUT
	local NO_RAW_INPUT = self._NO_RAW_INPUT
	local template_name = self._action_component.template_name
	local sequence_configs = self._ACTION_INPUT_SEQUENCE_CONFIGS[template_name]
...
function ActionInputParser:_update_sequences(dt, t, template_name, hierarchy_position, sequences, input_queue)
...
function ActionInputParser:_handle_hierarchy_transition(transition, hierarchy_position, action_input, base_hierarchy, current_hierarchy)
...
function ActionInputParser:_reset_hierarchy_position(hierarchy_position)
	for i = 1, self._MAX_HIERARCHY_DEPTH do
		hierarchy_position[i] = self._NO_ACTION_INPUT
	end
end

function ActionInputParser:_progress_hierarchy_position(hierarchy_position, action_input)
	for i = 1, self._MAX_HIERARCHY_DEPTH do
...
function ActionInputParser:_regress_hierarchy_position(hierarchy_position)
	local NO_ACTION_INPUT = self._NO_ACTION_INPUT
...
function ActionInputParser:_this_frames_inputs(input_extension)
...
function ActionInputParser:_has_running_sequences(sequences)
	for i = 1, self._MAX_ACTION_INPUT_SEQUENCES do
		local sequence = sequences[i]
...
function ActionInputParser:_evaluate_element(element_config_or_nil, this_frames_input, sequence, t)
...
function ActionInputParser:_evaluate_input(input_config, this_frames_input)
...
function ActionInputParser:_progress_input_sequence(sequence, t, sequence_config, this_frames_input_queue, raw_input, hierarchy_position)
...
function ActionInputParser:_stop_running_sequence(sequence)
	sequence[IS_RUNNING] = false
	sequence[CURRENT_ELEMENT_INDEX] = 1
end

function ActionInputParser:_queue_action_input(action_input_queue, sequence_config, t, raw_input, hierarchy_position, base_hierarchy)
...
function ActionInputParser:_manipulate_queue_by_max_queue(action_input_queue, action_input_name, max_queue)
...
function ActionInputParser:_manipulate_queue_by_no_space(action_input_queue, action_input_name, hierarchy_position)
	local MAX_ACTION_INPUT_QUEUE = self._MAX_ACTION_INPUT_QUEUE
	local NO_ACTION_INPUT = self._NO_ACTION_INPUT
	local NO_RAW_INPUT = self._NO_RAW_INPUT
	local MAX_HIERARCHY_DEPTH = self._MAX_HIERARCHY_DEPTH
...
function ActionInputParser:_hierarchy_depth(hierarchy_position)
	local NO_ACTION_INPUT = self._NO_ACTION_INPUT
	local MAX_HIERARCHY_DEPTH = self._MAX_HIERARCHY_DEPTH
...
function ActionInputParser:_clear_action_input_queue_hierarchy(hierarchy_position)
	local NO_ACTION_INPUT = self._NO_ACTION_INPUT
...
function ActionInputParser:_clear_action_input_queue(input_queue, start_index)
...
function ActionInputParser:_clear_action_input_queue_from_matching_hierarchy_position(input_queue, action_input, hierarchy_position, base_hierarchy)
...
function ActionInputParser:_same_hierarchy_position(hieararchy_a, hierarchy_b)
...
function ActionInputParser:_hierarchy_position_is_base(hierarchy_position)
