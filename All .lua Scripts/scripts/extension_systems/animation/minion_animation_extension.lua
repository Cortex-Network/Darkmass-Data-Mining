...
function MinionAnimationExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
...
function MinionAnimationExtension:game_object_initialized(game_session, game_object_id)
	self._game_object_id = game_object_id
	self._game_session = game_session
end

function MinionAnimationExtension:hot_join_sync(unit, sender, channel)
...
function MinionAnimationExtension:anim_event(event_name, optional_except_channel_id)
...
function MinionAnimationExtension:anim_event_with_variable_float(event_name, variable_name, variable_value)
	local unit = self._unit
	local game_object_id = self._game_object_id
	local event_index = Unit.animation_event(unit, event_name)
	local variable_index = Unit.animation_find_variable(unit, variable_name)
...
function MinionAnimationExtension:set_variable(name, value)
	local unit = self._unit
...
function MinionAnimationExtension:update(unit, ...)
