...
function PlayerUnitAnimationExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function PlayerUnitAnimationExtension:extensions_ready(world, unit)
...
function PlayerUnitAnimationExtension:anim_event(event_name)
...
function PlayerUnitAnimationExtension:anim_event_with_variable_float(event_name, variable_name, variable_value)
	local unit = self._unit
...
function PlayerUnitAnimationExtension:anim_event_with_variable_floats(event_name, ...)
	local unit = self._unit
...
function PlayerUnitAnimationExtension:anim_event_with_variable_int(event_name, variable_name, variable_value)
	local unit = self._unit
	local variable_index = Unit.animation_find_variable(unit, variable_name)
...
function PlayerUnitAnimationExtension:anim_event_1p(event_name)
...
function PlayerUnitAnimationExtension:anim_event_with_variable_float_1p(event_name, variable_name, variable_value)
...
function PlayerUnitAnimationExtension:anim_event_with_variable_floats_1p(event_name, ...)
	local first_person_unit = self._first_person_unit
	local num_params = select("#", ...)
...
function PlayerUnitAnimationExtension:inventory_slot_wielded(weapon_template, t)
...
function PlayerUnitAnimationExtension:anim_variable_id(anim_variable)
	return self._anim_variable_ids_third_person[anim_variable]
end

function PlayerUnitAnimationExtension:anim_variable_id_1p(anim_variable)
	return self._anim_variable_ids_first_person[anim_variable]
end

function PlayerUnitAnimationExtension:fixed_update(unit, dt, t, frame)
	PlayerUnitAnimationState.record_animation_state(self._animation_state_component, unit, self._first_person_unit)
end
...
function PlayerUnitAnimationExtension:server_correction_occurred(unit, from_frame, to_frame, simulated_components)
	local first_person_unit = self._first_person_unit
...
function PlayerUnitAnimationExtension:destroy()
end
