...
function BotBehaviorExtension:init(extension_init_context, unit, extension_init_data, game_object_data)
	self._unit = unit
...
function BotBehaviorExtension:_set_nav_spawn_points()
	local main_path_manager = Managers.state.main_path
	local nav_spawn_points = main_path_manager:nav_spawn_points()
	self._nav_spawn_points = nav_spawn_points
end

function BotBehaviorExtension:_init_brain(unit, breed, blackboard, behavior_tree_name)
...
function BotBehaviorExtension:_init_blackboard_components(blackboard, physics_world, gestalts_or_nil)
...
function BotBehaviorExtension:destroy()
...
function BotBehaviorExtension:extensions_ready(world, unit)
...
function BotBehaviorExtension:update(unit, dt, t, ...)
...
function BotBehaviorExtension:_update_ammo(unit)
...
function BotBehaviorExtension:_update_health_deployables(unit)
	local pickup_component = self._pickup_component
...
function BotBehaviorExtension:_update_health_stations(unit, dt, t)
	local perception_component = self._perception_component
...
function BotBehaviorExtension:_verify_target_ally_aid_destination(unit)
...
function BotBehaviorExtension:_handle_doors(unit)
...
function BotBehaviorExtension:_update_movement_target(unit, dt, t)
	local self_position = POSITION_LOOKUP[unit]
...
function BotBehaviorExtension:_is_within_hold_position_radius(hold_position, hold_position_max_distance_sq, new_position)
...
function BotBehaviorExtension:_update_cover(unit, self_position, follow_component)
...
function BotBehaviorExtension:_in_line_of_fire(self_unit, self_position, take_cover_targets, taking_cover_from)
	local changed = false
...
function BotBehaviorExtension:_find_cover(taking_cover_from, self_position)
...
function BotBehaviorExtension:_refresh_destination(t, self_position, previous_destination, hold_position, hold_position_max_distance_sq, bot_group_data, navigation_extension, follow_component, perception_component)
	local ALIVE = ALIVE
...
function BotBehaviorExtension:_alter_target_position(nav_world, traverse_logic, perception_component, self_position, target_unit, target_position, reason)
...
function BotBehaviorExtension:_enemy_path_allowed(enemy_unit)
...
function BotBehaviorExtension:_find_pickup_position_on_nav_mesh(nav_world, traverse_logic, self_position, pickup_unit, pickup_attempt)
...
function BotBehaviorExtension:new_destination_distance_check(self_position, previous_destination, new_destination, navigation_extension)
...
function BotBehaviorExtension:clear_failed_paths()
	table.clear(self._attempted_ally_paths)
	table.clear(self._attempted_enemy_paths)
end

function BotBehaviorExtension:cb_cover_path_result(cover_hash, success, destination)
	if not success then
		local cover_data = self._cover_data
...
function BotBehaviorExtension:ally_path_status(ally_unit)
	local path_status = self._attempted_ally_paths[ally_unit]

	return path_status
end

function BotBehaviorExtension:cb_ally_path_result(ally_unit, success, destination)
...
function BotBehaviorExtension:cb_enemy_path_result(enemy_unit, success, destination)
...
function BotBehaviorExtension:cb_health_pickup_path_result(pickup_unit, success, destination)
	if pickup_unit == self._last_health_pickup_attempt.unit then
		self._last_health_pickup_attempt.path_failed = not success
	end
end

function BotBehaviorExtension:cb_mule_pickup_path_result(pickup_unit, success, destination)
	if pickup_unit == self._last_mule_pickup_attempt.unit then
		self._last_mule_pickup_attempt.path_failed = not success
	end
end

function BotBehaviorExtension:brain()
	return self._brain
end

function BotBehaviorExtension:running_action()
	return self._brain:running_action()
end

function BotBehaviorExtension:hit_by_attacker_projectile(attacking_unit)
	local hit_by_projectile = self._hit_by_projectile[attacking_unit] or false

	return hit_by_projectile
end

function BotBehaviorExtension:set_hit_by_attacker_projectile(attacking_unit)
	self._hit_by_projectile[attacking_unit] = true
end

local DEFAULT_STAY_NEAR_PLAYER_RANGE = 5

function BotBehaviorExtension:set_stay_near_player(stay_near_player, max_range)
...
function BotBehaviorExtension:should_stay_near_player()
	return self._stay_near_player, self._stay_near_player_range
end

local HOLD_POSITION_EPSILON = 0.05

function BotBehaviorExtension:set_hold_position(hold_position, max_allowed_distance)
...
function BotBehaviorExtension:hold_position()
...
function BotBehaviorExtension:is_taking_cover()
	local cover_data = self._cover_data
