...
function BtChaosDaemonhostWarpSweepAction:init_values(blackboard, action_data, node_data)
	local behavior_component = Blackboard.write_component(blackboard, "behavior")
	behavior_component.warp_sweep_cooldown = 0
end

function BtChaosDaemonhostWarpSweepAction:enter(unit, breed, blackboard, scratchpad, action_data, t)
	local behavior_component = Blackboard.write_component(blackboard, "behavior")
...
function BtChaosDaemonhostWarpSweepAction:leave(unit, breed, blackboard, scratchpad, action_data, t, reason, destroy)
...
function BtChaosDaemonhostWarpSweepAction:run(unit, breed, blackboard, scratchpad, action_data, dt, t)
...
function BtChaosDaemonhostWarpSweepAction:_start_attacking(scratchpad, action_data, t)
...
function BtChaosDaemonhostWarpSweepAction:_update_attacking(unit, action_data, scratchpad, t)
...
function BtChaosDaemonhostWarpSweepAction:_deal_damage(unit, action_data, scratchpad)
				...
				if angle_to_target < angle then
					...
					if Breed.is_minion(breed) then
						...
						if not tags.monster then
							...
							ImpactEffect.play(hit_unit, nil, damage, damage_type, nil, result, to, nil, direction, unit, nil, , , damage_efficiency, damage_profile)
						end
					end
				end
			end
