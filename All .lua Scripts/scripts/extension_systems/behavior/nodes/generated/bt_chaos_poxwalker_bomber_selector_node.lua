...
local BtChaosPoxwalkerBomberSelectorNode = class("BtChaosPoxwalkerBomberSelectorNode", "BtNode")

function BtChaosPoxwalkerBomberSelectorNode:init(...)
	BtChaosPoxwalkerBomberSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtChaosPoxwalkerBomberSelectorNode:init_values(blackboard, action_data, node_data)
	BtChaosPoxwalkerBomberSelectorNode.super.init_values(self, blackboard, action_data, node_data)
...
function BtChaosPoxwalkerBomberSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtChaosPoxwalkerBomberSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	local node_approach = children[5]
	local is_running = last_leaf_node_running and last_running_node == node_approach
	local condition_result = nil
	...
	if condition_result then
		new_running_child_nodes[node_identifier] = node_approach

		return node_approach
	end
...
function BtChaosPoxwalkerBomberSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
...
return BtChaosPoxwalkerBomberSelectorNode
