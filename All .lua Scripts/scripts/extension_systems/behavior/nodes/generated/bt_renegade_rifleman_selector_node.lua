...
local BtRenegadeRiflemanSelectorNode = class("BtRenegadeRiflemanSelectorNode", "BtNode")

function BtRenegadeRiflemanSelectorNode:init(...)
	BtRenegadeRiflemanSelectorNode.super.init(self, ...)

	self._children = {}
end

function BtRenegadeRiflemanSelectorNode:init_values(blackboard, action_data, node_data)
	BtRenegadeRiflemanSelectorNode.super.init_values(self, blackboard, action_data, node_data)
...
function BtRenegadeRiflemanSelectorNode:add_child(node)
	self._children[#self._children + 1] = node
end

function BtRenegadeRiflemanSelectorNode:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	local node_cover_combat = children[7]
	...
	repeat
		...
		if not is_aggroed then
			...
			if not combat_ranges[combat_range] then
				...
				condition_result = has_cover
			end
		end
	until true

	if condition_result then
		local leaf_node = node_cover_combat:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)

		if leaf_node then
			new_running_child_nodes[node_identifier] = node_cover_combat
	...
	local node_far_combat = children[10]
	...
	local node_close_combat = children[11]
...
function BtRenegadeRiflemanSelectorNode:run(unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
...
return BtRenegadeRiflemanSelectorNode
