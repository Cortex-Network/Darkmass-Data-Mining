...
function BehaviorTree:init(lua_tree_node, name)
	self._root = nil
	self._name = name
	self._action_data = {}
...
function BehaviorTree:action_data()
...
function BehaviorTree:parse_lua_tree(lua_root_node)
	self._root = _create_btnode_from_lua_node(lua_root_node)

	self:parse_lua_node(lua_root_node, self._root)
end

function BehaviorTree:parse_lua_node(lua_node, parent)
