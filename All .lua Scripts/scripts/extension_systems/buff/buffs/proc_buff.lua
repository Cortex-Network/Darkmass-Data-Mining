...
function ProcBuff:init(context, template, start_time, instance_id, ...)
...
function ProcBuff:set_buff_component(buff_component, component_keys, component_index)
...
function ProcBuff:active_start_time()
	return self._active_start_time
end

function ProcBuff:set_active_start_time(active_start_time)
	self._active_start_time = active_start_time
end

function ProcBuff:is_active()
	local t = Managers.time:time("gameplay")

	return self:_is_active(t)
end

function ProcBuff:_is_active(t)
...
function ProcBuff:_active_duration()
...
function ProcBuff:duration_progress()
	local template = self._template
...
function ProcBuff:activate_percentage(t)
...
function ProcBuff:_is_cooling_down(t)
...
function ProcBuff:_cooldown_progress(t)
	local active_cooldown = self:_cooldown_duration()
	local active_start_time = self._active_start_time
...
function ProcBuff:_can_activate(t)
	local cooldown = self:_cooldown_duration()
...
function ProcBuff:_cooldown_duration()
	local template = self._template
...
function ProcBuff:update(dt, t, portable_random)
...
function ProcBuff:_can_add_stat_and_keywords(t)
	local template = self._template
...
function ProcBuff:update_keywords(current_key_words, t)
...
function ProcBuff:update_stat_buffs(current_stat_buffs, t)
...
function ProcBuff:update_proc_events(t, proc_events, num_proc_events, portable_random, local_portable_random)
	local template = self._template
...
function ProcBuff:_proc_chance(proc_event_name)
	local template = self._template
...
function ProcBuff:_start_proc_fx()
	local template_context = self._template_context
	local template = self._template
...
function ProcBuff:_start_proc_active_fx()
	local template_context = self._template_context
	local template = self._template
...
function ProcBuff:_stop_proc_active_fx(t)
	local template = self._template
	local template_context = self._template_context
	local world = template_context.world
	local wwise_world = template_context.wwise_world
...
function ProcBuff:force_predicted_proc()
	local template = self._template
