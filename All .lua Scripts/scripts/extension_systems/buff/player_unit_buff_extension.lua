...
function PlayerUnitBuffExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
...
function PlayerUnitBuffExtension:_init_components(buff_component)
...
function PlayerUnitBuffExtension:hot_join_sync(unit, sender, channel)
...
function PlayerUnitBuffExtension:fixed_update(unit, dt, t, fixed_frame)
...
function PlayerUnitBuffExtension:add_internally_controlled_buff(template_name, t, ...)
...
function PlayerUnitBuffExtension:add_externally_controlled_buff(template_name, t, ...)
	local template = BuffTemplates[template_name]
	local is_server = self._is_server
...
function PlayerUnitBuffExtension:_next_available_component_index(template)
...
function PlayerUnitBuffExtension:_add_predicted_buff(template, t, ...)
	local buff_component = self._buff_component
...
function PlayerUnitBuffExtension:remove_internally_controlled_buff_stack(template_name)
	local buff_instance = self._stacking_buffs[template_name]

	return buff_instance:remove_stack()
end

function PlayerUnitBuffExtension:_remove_internally_controlled_buff(local_index)
	local buff_instance = self._buffs_by_index[local_index]
	local template = buff_instance:template()
...
function PlayerUnitBuffExtension:_remove_predicted_buff(component_index)
...
function PlayerUnitBuffExtension:_on_add_buff(buff_instance)
	local buff_context = self._buff_context
...
function PlayerUnitBuffExtension:_on_remove_buff(buff_instance)
	local buff_context = self._buff_context
	local player = buff_context.player
...
function PlayerUnitBuffExtension:server_correction_occurred(unit, from_frame, to_frame, client_simulated_components)
	local buff_component = self._buff_component
...
function PlayerUnitBuffExtension:_remove_buff_from_server_correction(component_index)
	local component_buffs = self._component_buffs
...
function PlayerUnitBuffExtension:_add_buff_from_server_correction(template_name, t, component_index)
	local template = BuffTemplates[template_name]
	local buff_component = self._buff_component
	local index = self:_add_buff(template, t)
	local buff_instance = self._buffs_by_index[index]
...
function PlayerUnitBuffExtension:_start_fx(index, template)
...
function PlayerUnitBuffExtension:_stop_fx(index, template)
	local buff_context = self._buff_context
	local player_effects = template.player_effects
	local player = buff_context.player
	local is_human_controlled = player:is_human_controlled()
