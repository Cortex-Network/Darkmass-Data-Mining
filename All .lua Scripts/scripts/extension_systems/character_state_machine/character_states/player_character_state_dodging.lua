...
function PlayerCharacterStateDodging:init(character_state_init_context, ...)
...
function PlayerCharacterStateDodging:_on_enter_animation(unit, dodge_direction, estimated_dodge_time, animation_extension)
...
local function _calculate_dodge_total_time(specialization_dodge_template, diminishing_return_factor, weapon_dodge_template, buff_extension)
	...
	local base_distance = weapon_dodge_template.base_distance or specialization_dodge_template.base_distance
	...
	local dodge_speed_at_times = weapon_dodge_template.dodge_speed_at_times or specialization_dodge_template.dodge_speed_at_times
...
function PlayerCharacterStateDodging:on_enter(unit, dt, t, previous_state, params)
...
function PlayerCharacterStateDodging:on_exit(unit, t, next_state)
	local dodge_character_state_component = self._dodge_character_state_component
	local weapon_dodge_template = self._weapon_extension:dodge_template()
	local specialization_dodge_template = self._specialization_dodge_template
	local buff_extension = self._buff_extension
...
function PlayerCharacterStateDodging:fixed_update(unit, dt, t, next_state_params, fixed_frame)
	local time_in_dodge = t - self._character_state_component.entered_t
...
function PlayerCharacterStateDodging:_check_transition(unit, t, input_extension, next_state_params, still_dodging, wants_slide)
...
function PlayerCharacterStateDodging:_update_dodge(unit, dt, time_in_dodge, has_slide_input)
	local specialization_dodge_template = self._specialization_dodge_template
	local dodge_character_state_component = self._dodge_character_state_component
	local weapon_dodge_template = self._weapon_extension:dodge_template()
	...
	local speed_at_times = weapon_dodge_template.dodge_speed_at_times or specialization_dodge_template.dodge_speed_at_times
	local start_point = 1
	local diminishing_return_factor = _calculate_dodge_diminishing_return(dodge_character_state_component, weapon_dodge_template, self._buff_extension)
	local distance_scale = (weapon_dodge_template and weapon_dodge_template.distance_scale or 1) * diminishing_return_factor
