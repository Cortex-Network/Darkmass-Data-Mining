...
function PlayerCharacterStateFalling:init(...)
...
function PlayerCharacterStateFalling:on_enter(unit, dt, t, previous_state, params)
...
function PlayerCharacterStateFalling:on_exit(unit, t, next_state)
...
function PlayerCharacterStateFalling:fixed_update(unit, dt, t, next_state_params, fixed_frame)
...
function PlayerCharacterStateFalling:_update_falling_sound()
	local falling_character_state_component = self._falling_character_state_component
	local locomotion_component = self._locomotion_component
	local inair_state_component = self._inair_state_component
	local fx_extension = self._fx_extension
...
function PlayerCharacterStateFalling:_stuck_ray(position, direction)
	local hit = Raycast.cast(self._raycast_object, position, direction, RAY_LENGTH)

	return hit
end

function PlayerCharacterStateFalling:_verify_stuckness(position)
...
function PlayerCharacterStateFalling:_update_likely_stuck_hack(locomotion_component, dt, t, fixed_frame, input_extension)
...
function PlayerCharacterStateFalling:_check_transition(unit, t, next_state_params, input_source)
	local unit_data_extension = self._unit_data_extension
