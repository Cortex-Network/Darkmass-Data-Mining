...
function PlayerCharacterStateLadderClimbing:_on_enter_animation(unit, previous_state)
...
function PlayerCharacterStateLadderClimbing:init(character_state_init_context, ...)
...
function PlayerCharacterStateLadderClimbing:update(unit, dt, t)
...
function PlayerCharacterStateLadderClimbing:fixed_update(unit, dt, t, next_state_params, fixed_frame)
	local ladder_unit = Managers.state.unit_spawner:unit(self._ladder_character_state_component.ladder_unit_id, true)
	local velocity_current = self._locomotion_component.velocity_current
...
function PlayerCharacterStateLadderClimbing:_check_transition(unit, ladder_unit, t, next_state_params)
...
function PlayerCharacterStateLadderClimbing:_move_on_ladder(move_x, move_y, player_rotation, ladder_rotation, unit, ladder_unit)
...
function PlayerCharacterStateLadderClimbing:_have_reached_top(position, ladder_unit, velocity, climb_rotation)
	local top_pos = Unit.world_position(ladder_unit, Unit.node(ladder_unit, LADDER_TOP_NODE))
	local first_person_position = self._first_person_component.position
...
function PlayerCharacterStateLadderClimbing:_on_ladder_animation(unit, velocity, ladder_unit, animation_extension)
...
function PlayerCharacterStateLadderClimbing:_on_ladder_footsteps(t, unit, velocity, position_1p, position_3p, ladder_unit)
...
function PlayerCharacterStateLadderClimbing:_trigger_climb_sound(sound_alias, position)
	local _, event_name, has_husk_events = self._visual_loadout_extension:resolve_gear_sound(sound_alias)
	local is_predicted = true

	self._fx_extension:trigger_wwise_event_synced(event_name, has_husk_events, is_predicted, nil, , , position, nil, , , , )
end

function PlayerCharacterStateLadderClimbing:_should_climb_ladder(unit, t)
