...
function PlayerCharacterStateLadderTopEntering:on_enter(unit, dt, t, previous_state, params)
...
function PlayerCharacterStateLadderTopEntering:_velocity_wanted(ladder_unit, unit, duration)
	local enter_end_node = Unit.has_node(ladder_unit, LADDER_ENTER_END_NODE) and Unit.node(ladder_unit, LADDER_ENTER_END_NODE)
	local top_node = Unit.node(ladder_unit, LADDER_TOP_NODE)
	local exit_pos = Unit.world_position(ladder_unit, enter_end_node or top_node)
...
function PlayerCharacterStateLadderTopEntering:on_exit(unit, t, next_state)
	local ladder_character_state_component = self._ladder_character_state_component
...
function PlayerCharacterStateLadderTopEntering:fixed_update(unit, dt, t, next_state_params, fixed_frame)
	local next_state = self:_check_transition(t, next_state_params)

	return next_state
end

function PlayerCharacterStateLadderTopEntering:_check_transition(t, next_state_params)
