...
function PlayerCharacterStateLunging:init(character_state_init_context, ...)
...
function PlayerCharacterStateLunging:_play_animation(animation_extension, anim_event)
...
function PlayerCharacterStateLunging:on_exit(unit, t, next_state)
	...
	if self._is_server and on_finish_explosion then
		...
		Explosion.create_explosion(self._world, self._physics_world, position + direction * forward_offset + Vector3.up() * vertical_offset, impact_normal, unit, explosion_template, power_level, charge_level, attack_type, nil, , , , self._hit_enemy_units)
	end
...
function PlayerCharacterStateLunging:fixed_update(unit, dt, t, next_state_params, fixed_frame)
	local lunge_character_state_component = self._lunge_character_state_component
	local time_in_lunge = t - self._character_state_component.entered_t
...
function PlayerCharacterStateLunging:_check_transition(unit, t, input_extension, next_state_params, max_mass_hit, still_lunging, lunge_template, time_in_lunge)
...
function PlayerCharacterStateLunging:_update_lunge(unit, dt, time_in_lunge, lunge_template)
	local lunge_character_state_component = self._lunge_character_state_component
...
function PlayerCharacterStateLunging:_update_enemy_hit_detection(unit, lunge_template)
	...
	for ii = 1, num_actors do
		...
		if side_system:is_enemy(unit, hit_unit) and not hit_enemy_units[hit_unit] then
			...
			ImpactEffect.play(hit_unit, hit_actor, damage_dealt, damage_type, nil, attack_result, hit_world_position, nil, attack_direction, unit, nil, , , damage_efficiency, damage_profile)
...
function PlayerCharacterStateLunging:_update_timing_anims(timing_anims, time_in_lunge)
