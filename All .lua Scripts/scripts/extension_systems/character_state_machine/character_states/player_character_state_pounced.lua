...
function PlayerCharacterStatePounced:init(character_state_init_context, ...)
...
function PlayerCharacterStatePounced:fixed_update(unit, dt, t, next_state_params, fixed_frame)
	local is_server = self._is_server
...
function PlayerCharacterStatePounced:_check_transition(unit, t, next_state_params)
...
function PlayerCharacterStatePounced:_init_player_vo(t)
	self._next_vo_trigger_time_t = t + DialogueSettings.pounced_vo_start_delay_t
	self._vo_sequence = 1
end

function PlayerCharacterStatePounced:_update_vo(t)
