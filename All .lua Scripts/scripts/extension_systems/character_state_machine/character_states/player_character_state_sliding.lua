...
function PlayerCharacterStateSliding:init(character_state_init_context, ...)
...
function PlayerCharacterStateSliding:on_enter(unit, dt, t, previous_state, params)
...
function PlayerCharacterStateSliding:on_exit(unit, t, next_state)
...
function PlayerCharacterStateSliding:fixed_update(unit, dt, t, next_state_params, fixed_frame)
	local first_person_extension = self._first_person_extension
...
function PlayerCharacterStateSliding:_do_material_query()
	local length = 0.3
...
function PlayerCharacterStateSliding:update(unit, dt, t)
end

function PlayerCharacterStateSliding:_check_transition(unit, t, next_state_params, input_source, is_crouching, commit_period_over, max_mass_hit)
...
function PlayerCharacterStateSliding:_update_enemy_hit_detection(unit, damage_settings)
	...
	for i = 1, num_actors do
		...
		if side_system:is_enemy(unit, hit_unit) and not hit_enemy_units[hit_unit] then
			...
			ImpactEffect.play(hit_unit, hit_actor, damage_dealt, damage_type, nil, attack_result, hit_position, nil, attack_direction, unit, nil, , , damage_efficiency, damage_profile)
