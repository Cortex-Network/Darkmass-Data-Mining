...
function AcceleratedLocalSpaceMovement.speed_function(speed, wanted_speed, wanted_other_speed, acceleration, deceleration, dt)
...
function AcceleratedLocalSpaceMovement.wanted_movement(player_character_constants, input_source, locomotion_steering_component, movement_settings_component, first_person_component, is_crouching, velocity_current, dt)
...
function AcceleratedLocalSpaceMovement.wants_move(input_source)
	local move_input = input_source:get("move")

	return Vector3_length_squared(move_input) > 0
end

function AcceleratedLocalSpaceMovement.set_wanted_movement(locomotion_steering_component, move_direction, move_speed, new_x, new_y)
...
function AcceleratedLocalSpaceMovement.refresh_local_move_variables(max_move_speed, locomotion_steering_component, locomotion_component, first_person_component)
