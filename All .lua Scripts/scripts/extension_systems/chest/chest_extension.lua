...
function ChestExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function ChestExtension:setup_from_component(locked)
	if locked then
		self._current_state = STATES.locked
	end
end

function ChestExtension:hot_join_sync(unit, sender, channel)
...
function ChestExtension:update(unit, dt, t)
	local finished = true
...
function ChestExtension:reserve_pickup(index, pickup_name)
	self._containing_pickups[index] = pickup_name
end

function ChestExtension:current_state()
	return self._current_state
end

function ChestExtension:rpc_set_chest_state(state)
	self:set_current_state(state)
end

function ChestExtension:set_current_state(state)
...
function ChestExtension:is_interactable()
	return self._current_state == STATES.closed
end

function ChestExtension:open()
