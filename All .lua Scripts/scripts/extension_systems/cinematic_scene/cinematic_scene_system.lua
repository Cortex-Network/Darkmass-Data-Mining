...
function CinematicSceneSystem:init(extension_init_context, system_init_data, ...)
...
function CinematicSceneSystem:_on_preload_cinematic(preload_id)
...
function CinematicSceneSystem:_on_spawn_group_loaded()
...
function CinematicSceneSystem:destroy(...)
...
function CinematicSceneSystem:on_gameplay_post_init(level)
	self._level = level
...
function CinematicSceneSystem:rpc_cinematic_intro_played()
	self._intro_played = true
end

function CinematicSceneSystem:intro_played()
	return self._intro_played
end

function CinematicSceneSystem:_can_play(cinematic_name, check_currently_playing, check_sub_cinematics, check_cinematics_setup)
...
function CinematicSceneSystem:_cinematic_played(cinematic_name, cinematic_category)
...
function CinematicSceneSystem:_send_cinematic_played_flow_event(cinematic_name)
	local sub_cinematics = self._cinematics[cinematic_name]
	local sub_cinematics_setup = self._cinematics_setups[cinematic_name]
...
function CinematicSceneSystem:_on_level_loaded(cinematic_name)
end
...
function CinematicSceneSystem:request_play_cutscene(cinematic_name)
	local cinematic_name_id = NetworkLookup.cinematic_scene_names[cinematic_name]

	Managers.connection:send_rpc_server("rpc_request_play_cutscene", cinematic_name_id)
end

function CinematicSceneSystem:_on_level_prepared(cinematic_name, client_channel_id)
...
function CinematicSceneSystem:_play_cutscene(cinematic_name, client_channel_id)
...
function CinematicSceneSystem:current_cinematic_name()
	return self._current_cinematic_name
end

function CinematicSceneSystem:_queue_cinematics(cinematic_name, client_channel_id)
	local sub_cinematics = self._cinematics[cinematic_name]
	local sub_cinematics_setup = self._cinematics_setups[cinematic_name]
...
function CinematicSceneSystem:_initialize_cinematic(cinematic_name)
	local sub_cinematics = self._cinematics[cinematic_name]
...
function CinematicSceneSystem:_uninitialize_cinematic(cinematic_name)
	Managers.state.cinematic:unload_levels(cinematic_name)
	table.clear(self._cinematics_setups[cinematic_name])
end

function CinematicSceneSystem:_initialize_sub_cinematics(cinematic_name, sub_cinematics)
...
function CinematicSceneSystem:_get_free_slot_id()
...
function CinematicSceneSystem:_player_slot_id(unique_id)
	local spawn_slots = self._spawn_slots
...
function CinematicSceneSystem:_clear_spawn_slots()
	local spawn_slots = self._spawn_slots
...
function CinematicSceneSystem:_setup_spawn_slots(world, cinematic_name, camera, player_spawner_units)
...
function CinematicSceneSystem:_update_player_slots(dt, t, input_service)
	local spawn_slots = self._spawn_slots
...
function CinematicSceneSystem:_assign_player_slots(cinematic_name)
...
function CinematicSceneSystem:_assign_player_to_slot(cinematic_name, player, slot)
...
function CinematicSceneSystem:_reset_spawn_slot(slot)
	local profile_spawner = slot.profile_spawner
...
function CinematicSceneSystem:_initialize_cutscene_characters(cinematic_name)
...
function CinematicSceneSystem:_uninitialize_cutscene_characters(cinematic_name)
	local cutscene_character_system = Managers.state.extension:system("cutscene_character_system")

	cutscene_character_system:uninitialize_characters_for_cinematic(cinematic_name)
end

function CinematicSceneSystem:_set_cinematic_name(cinematic_name)
	self._current_cinematic_name = cinematic_name
end

function CinematicSceneSystem:client_set_scene(cinematic_name)
	self:_initialize_cutscene_characters(cinematic_name)
	self:_activate_view(cinematic_name)
	self:_set_cinematic_name(cinematic_name)
...
function CinematicSceneSystem:client_unset_scene(cinematic_name)
	self:_uninitialize_cutscene_characters(cinematic_name)
	self:_activate_view(CINEMATIC_NAMES.none)
	self:_set_cinematic_name(CINEMATIC_NAMES.none)
end

function CinematicSceneSystem:is_active()
	return self._current_cinematic_name ~= CINEMATIC_NAMES.none
end

function CinematicSceneSystem:is_cinematic_active(cinematic_name)
	return self._current_cinematic_name == cinematic_name
end

function CinematicSceneSystem:intro_loading_started()
	return self._intro_loading_started
end

function CinematicSceneSystem:_activate_view(cinematic_name)
	...
	if ui_manager then
		...
		if activate and not is_active then
			...
			if not ui_manager:view_active(view) then
				...
				ui_manager:open_view(view, nil, , , , view_context, view_settings_override)
			end
...
function CinematicSceneSystem:rpc_play_cutscene(channel_id, cinematic_name_id)
	local cinematic_name = NetworkLookup.cinematic_scene_names[cinematic_name_id]

	self:play_cutscene(cinematic_name)
end

function CinematicSceneSystem:rpc_request_play_cutscene(channel_id, cinematic_name_id)
	local cinematic_name = NetworkLookup.cinematic_scene_names[cinematic_name_id]
