...
function CoherencySystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
...
function CoherencySystem:on_remove_extension(unit, extension_name)
...
function CoherencySystem:_exit_removed_extension(exit_unit, exit_coherency_extension)
...
function CoherencySystem:add_external_buff(target_unit, buff_name)
...
function CoherencySystem:remove_external_buff(target_unit, index)
	local unit_to_extension_map = self._unit_to_extension_map
	local target_extension = unit_to_extension_map[target_unit]
...
local function _should_be_in_coherency_with_all(unit)
	...
	if is_invisible then
		...
		local have_special_rule = specialization_extension and specialization_extension:has_special_rule(special_rules.in_coherence_when_invisible)

		if have_special_rule then
			return true
		end
	end
...
function CoherencySystem:update(context, dt, t, ...)
