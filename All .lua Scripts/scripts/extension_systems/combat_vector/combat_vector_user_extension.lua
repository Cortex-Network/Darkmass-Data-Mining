...
function CombatVectorUserExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function CombatVectorUserExtension:destroy()
	self:_check_and_release_location()
end

function CombatVectorUserExtension:_init_blackboard_components(blackboard)
...
function CombatVectorUserExtension:_get_best_location_type(unit_position, claimed_location_counters, nav_mesh_locations, flank_positions, combat_vector_position, combat_range)
	local config = self._config
	local can_flank = config.can_flank
	local best_location_type = nil
	local lowest_amount = math.huge
	local vector_type = nil

	if can_flank then
		local closest_distance = Vector3.distance(unit_position, combat_vector_position:unbox())
		local closest_flank_vector_type = nil
...
function CombatVectorUserExtension:_get_new_location(unit_position, locations, combat_range)
	...
	local max_dist = self._new_location_max_distance or math.huge
	local config = self._config
...
function CombatVectorUserExtension:look_for_new_location(min_distance, max_distance, new_combat_range)
...
function CombatVectorUserExtension:switch_range(new_combat_range)
	self._combat_vector_component.range = new_combat_range
end

function CombatVectorUserExtension:update(unit, dt, t, context, nav_mesh_locations, claimed_location_counters, changed, flank_positions, combat_vector_position)
	...
	local combat_vector_component = self._combat_vector_component
	local behavior_component = self._behavior_component
	local current_combat_range = behavior_component.combat_range
	local previous_combat_range = self._previous_combat_range
...
function CombatVectorUserExtension:_check_and_release_location()
...
function CombatVectorUserExtension:get_vector_type()
