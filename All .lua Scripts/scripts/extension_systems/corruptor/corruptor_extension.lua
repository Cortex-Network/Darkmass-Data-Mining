...
function CorruptorExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function CorruptorExtension:extensions_ready(world, unit)
	...
	if self._is_server then
		...
		self._health_extension:add_damage(diff, nil, , , , , unit)
	end
end

function CorruptorExtension:hot_join_sync(unit, sender, channel_id)
...
function CorruptorExtension:setup_from_component(use_trigger)
	self._use_trigger = use_trigger
end

function CorruptorExtension:destroy()
	if self._is_server then
...
function CorruptorExtension:awake()
...
function CorruptorExtension:expose()
...
function CorruptorExtension:damaged(damage)
...
function CorruptorExtension:died()
...
function CorruptorExtension:activate_segment_units()
...
function CorruptorExtension:set_to_demolition_target()
	local target_extension = self._mission_objective_target_extension
...
function CorruptorExtension:update(unit, dt, t)
...
function CorruptorExtension:use_trigger()
	return self._use_trigger
end

function CorruptorExtension:_start_effect_template(effect_template_data_name)
...
function CorruptorExtension:_stop_effect_template(effect_template_data_name)
	local effect_template_data = self._effect_template_data[effect_template_data_name]
...
function CorruptorExtension:_update_explosion(unit, dt)
...
function CorruptorExtension:_update_closest_target()
...
function CorruptorExtension:_update_eye_constraint(unit, dt, closest_target, closest_distance)
...
function CorruptorExtension:_update_exposed(dt, t, closest_target, closest_distance)
	if self._current_state ~= STATES.exposed then
		return
	end
...
function CorruptorExtension:set_eye_activated(activated)
	if self._is_server then
...
function CorruptorExtension:set_eye_hidden(hidden)
	if self._is_server then
