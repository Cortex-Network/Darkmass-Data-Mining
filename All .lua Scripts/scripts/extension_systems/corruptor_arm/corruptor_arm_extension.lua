...
function CorruptorArmExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function CorruptorArmExtension:extensions_ready(world, unit)
	self._mission_objective_target_extension = ScriptUnit.extension(unit, "mission_objective_target_system")
end

function CorruptorArmExtension:setup_from_component(activation_delay, arm_length, corruptor_arm_component)
...
function CorruptorArmExtension:hot_join_sync(unit, sender, channel_id)
...
function CorruptorArmExtension:destroy()
...
function CorruptorArmExtension:activate()
...
function CorruptorArmExtension:deactivate()
...
function CorruptorArmExtension:update(unit, dt, t)
	local target_units = self._target_units
...
function CorruptorArmExtension:_animation_update(unit, dt)
...
function CorruptorArmExtension:set_animation_target(target, speed_multiplier_type, optional_hot_join_animation_pos)
...
function CorruptorArmExtension:has_lost_pustules()
	return self._has_lost_pustules
end

function CorruptorArmExtension:_fully_extended(unit)
...
function CorruptorArmExtension:_start_extending()
...
function CorruptorArmExtension:_stop_extending()
...
function CorruptorArmExtension:_update_extending()
	local position = self._corruptor_arm_component:last_joint_position()
...
function CorruptorArmExtension:_spawn_targets(unit)
	local unit_spawner_manager = Managers.state.unit_spawner
...
function CorruptorArmExtension:_fully_retracted()
