...
function CutsceneCharacterExtension:setup_from_component(cinematic_name, character_type, breed_name, prop_items, slot, inventory_animation_event)
...
function CutsceneCharacterExtension:_is_valid_for_cutscene(cinematic_name)
...
function CutsceneCharacterExtension:_set_loadout(items)
	if not Managers.state or not Managers.state.extension then
		return
	end
...
function CutsceneCharacterExtension:_on_package_loaded(player_unique_id, items, id)
