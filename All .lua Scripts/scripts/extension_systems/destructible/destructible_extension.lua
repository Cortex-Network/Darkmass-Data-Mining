...
function DestructibleExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function DestructibleExtension:destroy()
	self:_disable_nav_volume()
end

function DestructibleExtension:_disable_nav_volume()
...
function DestructibleExtension:setup_from_component(depawn_timer_duration, despawn_when_destroyed, mass, speed, force_direction_type, start_visible, is_nav_gate, broadphase_radius)
...
function DestructibleExtension:setup_stages()
	local unit = self._unit
...
function DestructibleExtension:hot_join_sync(unit, sender)
	self:_sync_server_state(sender)
end

function DestructibleExtension:update(unit, dt, t)
...
function DestructibleExtension:set_enabled(value)
	local actor = Unit.actor(self._unit, "c_destructible")

	Actor.set_scene_query_enabled(actor, value)
end

function DestructibleExtension:set_physics_disabled(physics_disabled)
	self._physics_disabled = physics_disabled
end

function DestructibleExtension:physics_disabled()
	return self._physics_disabled
end

function DestructibleExtension:set_visibility(is_visible)
	local unit = self._unit
...
function DestructibleExtension:add_damage(damage, hit_actor, attack_direction)
	local unit = self._unit
...
function DestructibleExtension:_setup_nav_gate(unit, nav_world, is_server)
...
function DestructibleExtension:force_destruct()
...
function DestructibleExtension:set_lights_enabled(are_lights_enabled)
	local visibility_info = self._visibility_info
	visibility_info.lights_enabled = are_lights_enabled
...
function DestructibleExtension:rpc_sync_destructible(current_stage_index, visible, silent_transition)
	local unit = self._unit
...
function DestructibleExtension:_sync_server_state(peer_id)
	local unit = self._unit
...
function DestructibleExtension:broadphase_radius()
