...
function DestructibleUtilities.setup_stages(unit, parts_mass, parts_speed, force_direction_type, health_extension)
...
function DestructibleUtilities.add_damage(unit, destructible_parameters, visibility_info, damage, attack_direction)
	local current_stage_index = destructible_parameters.current_stage_index
...
function DestructibleUtilities.calculate_total_health(destructible_parameters)
	local current_stage_index = destructible_parameters.current_stage_index
	local health = 0
...
function DestructibleUtilities.set_stage(unit, destructible_parameters, new_stage, visibility_info, silent_transition)
	local current_stage_index = destructible_parameters.current_stage_index
...
function DestructibleUtilities.set_lights_enabled(unit, destructible_parameters, visibility_info)
...
function DestructibleUtilities.is_destructible(unit)
	local destructible_ext = ScriptUnit.has_extension(unit, "destructible_system")

	return destructible_ext ~= nil
end

function DestructibleUtilities.set_physics_simulation(unit, val)
end
