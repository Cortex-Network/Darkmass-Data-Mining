...
function DialogueEventQueue:init(unit_extension_data, dialogues, query_database)
...
function DialogueEventQueue:update_new_events(dt, t)
...
function DialogueEventQueue:append_event(unit, event_name, event_data, identifier)
	if self:is_category_filtered() then
...
function DialogueEventQueue:filter_events_under(category, duration)
	if self:is_category_filtered() then
...
function DialogueEventQueue:_append_event_implementation(unit, event_name, event_data, identifier)
...
function DialogueEventQueue:_pop_event(event_data_target)
...
function DialogueEventQueue:_purge_events()
...
function DialogueEventQueue:is_category_filtered()
	return self._current_time < self._filtering_end_time
end

function DialogueEventQueue:filtering_end_time()
