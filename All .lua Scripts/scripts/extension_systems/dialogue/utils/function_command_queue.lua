local FunctionCommandQueue = class("FunctionCommandQueue")

function FunctionCommandQueue:init(max_num_args)
	self.command_queue = {}
	self.command_index = 0
	self.command_stride = max_num_args + 1
end

function FunctionCommandQueue:run_commands()
...
function FunctionCommandQueue:cleanup_destroyed_unit(unit)
	local command_queue = self.command_queue
	local stride = self.command_stride
...
function FunctionCommandQueue:queue_function_command(function_call, ...)
