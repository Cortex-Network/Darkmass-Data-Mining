...
function DoorExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function DoorExtension:destroy()
...
function DoorExtension:setup_from_component(type, start_state, open_duration, close_duration, allow_closing, self_closing_time, blocked_time, open_type, control_panel_props, control_panels_active, ignore_broadphase)
...
function DoorExtension:_spawn_control_panels(control_panel_props, control_panels_active)
...
function DoorExtension:_unspawn_control_panels()
	local unit_spawner_manager = Managers.state.unit_spawner
...
function DoorExtension:set_control_panel_active(val)
	local control_panel_units = self._control_panel_units
...
function DoorExtension:_setup_nav_layer(unit, start_state)
...
function DoorExtension:update(unit, dt, t)
	if self._is_server then
...
function DoorExtension:_should_nav_block()
...
function DoorExtension:_normalized_anim_time()
...
function DoorExtension:_anim_time()
	local unit = self._unit
...
function DoorExtension:_anim_duration()
	local state = self._current_state

	return self._anim_data[state].duration
end

function DoorExtension:_is_playing_anim()
	local anim_time = self:_normalized_anim_time()
...
function DoorExtension:_update_external_dependencies()
...
function DoorExtension:_set_nav_block(block)
...
function DoorExtension:can_close(use_proximity_check)
...
function DoorExtension:_minion_proximity_check()
...
function DoorExtension:open(state, interactor_unit, optional_closing_time)
...
function DoorExtension:_get_open_state_from_interactor(interactor_unit)
	if self._type == TYPES.two_states then
		return STATES.open
	else
		local unit = self._unit
...
function DoorExtension:_set_server_state(new_state)
	local current_state = self._current_state
...
function DoorExtension:_do_flow_calls(state)
	local unit = self._unit
...
function DoorExtension:_sync_server_state(peer_id, state)
	local unit = self._unit
	local unit_level_index = Managers.state.unit_spawner:level_index(unit)
...
function DoorExtension:increment_num_attackers()
	self._num_attackers = self._num_attackers + 1
end

function DoorExtension:decrement_num_attackers()
	self._num_attackers = self._num_attackers - 1
end

function DoorExtension:num_attackers()
	return self._num_attackers
end

function DoorExtension:ignore_broadphase()
	return self._ignore_broadphase
end

function DoorExtension:rpc_sync_door_state(new_state)
	self:_do_flow_calls(new_state)
	self:_set_current_state(new_state)
end
