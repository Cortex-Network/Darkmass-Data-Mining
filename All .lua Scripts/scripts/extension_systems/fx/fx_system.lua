...
function FxSystem:init(extension_system_creation_context, ...)
...
function FxSystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
...
function FxSystem:on_remove_extension(unit, extension_name)
...
function FxSystem:hot_join_sync(sender, channel)
	local running_template_effects = self._running_template_effects
...
function FxSystem:update(context, dt, t, ...)
...
function FxSystem:start_template_effect(template, optional_unit, optional_node, optional_position)
	local template_name = template.name
...
function FxSystem:_start_template_effect(template_effect, template, optional_unit, optional_node, optional_position)
	if not DEDICATED_SERVER then
		local template_data = template_effect.template_data
		local template_context = self._template_context
...
function FxSystem:stop_template_effect(global_effect_id)
	local template_effects = self._template_effects
...
function FxSystem:_stop_template_effect(template_effect, template)
	local template_data = template_effect.template_data
	local template_context = self._template_context
...
function FxSystem:play_impact_fx(impact_fx, position, direction, source_parameters, provoking_unit, optional_target_unit, optional_node_index, optional_hit_normal, optional_will_be_predicted, local_only_or_nil)
...
function FxSystem:play_surface_impact_fx(hit_position, hit_direction, source_parameters, provoking_unit, optional_hit_normal, damage_type, hit_type, optional_will_be_predicted)
	if self._is_server then
		local provoking_unit_id = Managers.state.unit_spawner:game_object_id(provoking_unit)
...
function FxSystem:play_shotshell_surface_impact_fx(fire_position, hit_positions, hit_normals, source_parameters, provoking_unit, damage_type, hit_type, optional_will_be_predicted)
	if self._is_server then
		local provoking_unit_id = Managers.state.unit_spawner:game_object_id(provoking_unit)
...
function FxSystem:trigger_vfx(vfx_name, position, optional_rotation)
...
function FxSystem:trigger_wwise_event(event_name, optional_position, optional_unit, optional_node, optional_parameter_name, optional_parameter_value)
	local wwise_world = self._wwise_world
	local source_id = nil
...
function FxSystem:trigger_ground_impact_fx(ground_impact_fx_template, impact_material_or_nil, impact_position, impact_normal)
...
function FxSystem:delete_impact_fx_unit(unit)
...
function FxSystem:delete_units()
...
function FxSystem:trigger_flow_event(unit, event_name)
...
function FxSystem:rpc_start_template_effect(channel_id, buffer_index, template_id, optional_unit_id, optional_node, optional_position)
...
function FxSystem:rpc_stop_template_effect(channel_id, buffer_index)
	local template_effects = self._template_effects
	local template_effect = template_effects[buffer_index]
	local template = template_effect.template
...
function FxSystem:rpc_play_impact_fx(channel_id, impact_fx_name_id, position, direction, provoking_unit_id, optional_target_unit_id, optional_node_index, optional_hit_normal, provoking_unit_has_particle_group)
...
function FxSystem:rpc_play_surface_impact_fx(channel_id, hit_position, hit_direction, provoking_unit_id, optional_hit_normal, damage_type_id, hit_type_id, provoking_unit_has_particle_group)
	local provoking_unit = Managers.state.unit_spawner:unit(provoking_unit_id)
...
function FxSystem:rpc_play_shotshell_surface_impact_fx(channel_id, fire_position, hit_positions, hit_normals, provoking_unit_id, damage_type_id, hit_type_id, provoking_unit_has_particle_group)
	local provoking_unit = Managers.state.unit_spawner:unit(provoking_unit_id)
...
function FxSystem:rpc_trigger_vfx(channel_id, vfx_id, position, optional_rotation)
	local vfx_name = NetworkLookup.vfx[vfx_id]

	World.create_particles(self._world, vfx_name, position, optional_rotation)
end

function FxSystem:rpc_trigger_2d_wwise_event(channel_id, event_id)
	local event_name = NetworkLookup.sound_events_2d[event_id]

	WwiseWorld.trigger_resource_event(self._wwise_world, event_name)
end

function FxSystem:rpc_trigger_wwise_event(channel_id, event_id, optional_position, optional_unit_id, optional_node, optional_parameter_id, optional_parameter_value)
...
function FxSystem:rpc_trigger_flow_event(channel_id, unit_id, event_id)
...
function FxSystem:rpc_projectile_trigger_fx(channel_id, unit_id, event_type_id)
...
function _play_impact_fx_template(t, world, wwise_world, unit_to_extension_map, spawned_impact_fx_units, impact_fx, position, direction, source_parameters, provoking_unit, optional_particle_group_id, optional_target_unit, optional_node_index, optional_hit_normal, only_decal)
	...
	if decal and optional_hit_normal then
		...
		_create_projection_decal(t, decal, position, decal_rotation, optional_hit_normal, nil, )
	end
