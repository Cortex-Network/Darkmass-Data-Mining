...
function MinionFxExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
...
function MinionFxExtension:extensions_ready(world, unit)
	self._visual_loadout_extension = ScriptUnit.extension(unit, "visual_loadout_system")
end

function MinionFxExtension:game_object_initialized(game_session, game_object_id)
...
function MinionFxExtension:should_play_husk_effect()
	return true
end

function MinionFxExtension:trigger_inventory_wwise_event(event_name, inventory_slot_name, fx_source_name, optional_target_unit, optional_is_ranged_attack)
...
function MinionFxExtension:_trigger_inventory_wwise_event(event_name, inventory_slot_name, fx_source_name, optional_target_unit, is_ranged_attack)
...
function MinionFxExtension:trigger_inventory_vfx(vfx_name, inventory_slot_name, fx_source_name)
...
function MinionFxExtension:_trigger_inventory_vfx(vfx_name, inventory_slot_name, fx_source_name)
	local visual_loadout_extension = self._visual_loadout_extension
...
function MinionFxExtension:rpc_trigger_minion_inventory_wwise_event(channel_id, go_id, event_id, inventory_slot_id, fx_source_name_id, optional_target_unit_id, optional_is_ranged_attack)
...
function MinionFxExtension:rpc_trigger_minion_inventory_vfx(channel_id, go_id, vfx_id, inventory_slot_id, fx_source_name_id)
...
function MinionFxExtension:trigger_unit_line_fx(line_effect, inventory_slot_name, fx_source_name, end_position)
...
function MinionFxExtension:_trigger_unit_line_fx(line_effect, inventory_slot_name, fx_source_name, end_position)
	local visual_loadout_extension = self._visual_loadout_extension
	local world = self._world
	local inventory_item = visual_loadout_extension:slot_item(inventory_slot_name)
	local attachment_unit, node = MinionVisualLoadout.attachment_unit_and_node_from_node_name(inventory_item, fx_source_name)
...
function MinionFxExtension:_trigger_wwise_event_on_line(event_name, start_position, end_position, append_husk_to_event_name)
...
function MinionFxExtension:rpc_trigger_minion_fx_line(channel_id, game_object_id, line_effect_id, inventory_slot_name_id, fx_source_name_id, end_position)
