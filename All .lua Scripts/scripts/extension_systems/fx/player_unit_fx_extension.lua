...
function PlayerUnitFxExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
...
function PlayerUnitFxExtension:game_object_initialized(session, id)
	self._game_object_id = id
end

function PlayerUnitFxExtension:extensions_ready(world, unit)
...
function PlayerUnitFxExtension:_create_particles_wrapper(world, particle_name, position, rotation, scale)
	return World.create_particles(world, particle_name, position, rotation, scale, self._player_particle_group_id)
end

function PlayerUnitFxExtension:hot_join_sync(unit, peer, channel_id)
...
function PlayerUnitFxExtension:destroy(unit)
...
function PlayerUnitFxExtension:destroy_particle_group()
	World.destroy_particle_group(self._world, self._player_particle_group_id)
end

function PlayerUnitFxExtension:update(unit, dt, t)
...
function PlayerUnitFxExtension:post_update(unit, dt, t)
	self:_update_aligned_vfx()
end

function PlayerUnitFxExtension:_update_first_person_mode_all_sources(first_person_mode)
	local wwise_world = self._wwise_world
...
function PlayerUnitFxExtension:_update_first_person_mode_looping_particles(first_person_mode)
...
function PlayerUnitFxExtension:_update_first_person_mode(source_name, first_person_mode)
...
function PlayerUnitFxExtension:_update_targeted_by_special()
...
function PlayerUnitFxExtension:_update_aligned_vfx()
...
function PlayerUnitFxExtension:_update_moving_sfx(dt, t)
	local moving_sfx = self._moving_sfx
...
function PlayerUnitFxExtension:_update_looping_particle_variables(dt, t)
	local world = self._world
...
function PlayerUnitFxExtension:server_correction_occurred(unit, from_frame, to_frame, simulated_components)
...
function PlayerUnitFxExtension:register_sound_source(source_name, parent_unit, attachments, node_name)
	local sources = self._sources
...
function PlayerUnitFxExtension:_register_sound_source(sources, source_name, parent_unit, attachments, node_name)
...
function PlayerUnitFxExtension:move_sound_source(source_name, parent_unit, attachments, node_name)
	local sources = self._sources
...
function PlayerUnitFxExtension:sound_source(source_name)
	local sound_source = self._sources[source_name]

	return sound_source
end

function PlayerUnitFxExtension:vfx_spawner(spawner_name)
	local vfx_spawner = self._vfx_spawners[spawner_name]

	return vfx_spawner
end

function PlayerUnitFxExtension:vfx_spawner_unit_and_node(spawner_name)
	local vfx_spawner = self._vfx_spawners[spawner_name]
...
function PlayerUnitFxExtension:sfx_spawner_unit_and_node(spawner_name)
	local source_cache = self._wwise_source_node_cache[spawner_name]

	return source_cache.unit, source_cache.node
end

function PlayerUnitFxExtension:vfx_spawner_pose(spawner_name)
	local vfx_spawner = self._vfx_spawners[spawner_name]
	local unit = vfx_spawner.unit
	local node = vfx_spawner.node
...
function PlayerUnitFxExtension:unregister_sound_source(source_name)
	local sources = self._sources
...
function PlayerUnitFxExtension:_unregister_sound_source(source_name)
	local wwise_source_node_cache = self._wwise_source_node_cache
	local wwise_world = self._wwise_world
...
function PlayerUnitFxExtension:wwise_player_state()
...
function PlayerUnitFxExtension:trigger_wwise_event(event_name, append_husk_to_event_name, ...)
...
function PlayerUnitFxExtension:trigger_wwise_events_local_and_husk(local_event_name, husk_event_name, optional_occlusion, optional_unit, optional_node_index, optional_position, optional_rotation, optional_parameter_name, optional_parameter_value, optional_switch_name, optional_switch_value)
...
function PlayerUnitFxExtension:trigger_wwise_event_synced(event_name, append_husk_to_event_name, is_predicted, optional_occlusion, optional_unit, optional_node_index, optional_position, optional_rotation, optional_parameter_name, optional_parameter_value, optional_switch_name, optional_switch_value)
	if self._unit_data_extension.is_resimulating then
		return
	end
...
function PlayerUnitFxExtension:_trigger_wwise_event_synced(event_name, append_husk_to_event_name, optional_occlusion, optional_unit, optional_node_index, optional_position, optional_rotation, optional_parameter_name, optional_parameter_value, optional_switch_name, optional_switch_value)
	local wwise_world = self._wwise_world
	local source_id = nil
...
function PlayerUnitFxExtension:trigger_wwise_event_server_controlled(event_name, append_husk_to_event_name, optional_position, optional_parameter_name, optional_parameter_value)
...
function PlayerUnitFxExtension:_trigger_wwise_event_server_controlled(event_name, append_husk_to_event_name, optional_position, optional_parameter_name, optional_parameter_value)
	local wwise_world = self._wwise_world
	local source_id = nil
...
function PlayerUnitFxExtension:spawn_unit_fx_line(line_effect, is_critical_strike, source_name, end_position, link, orphaned_policy, scale, append_husk_to_event_name)
...
function PlayerUnitFxExtension:_spawn_unit_fx_line(line_effect, is_critical_strike, spawner_name, end_position, link, orphaned_policy, scale, append_husk_to_event_name)
...
function PlayerUnitFxExtension:_trigger_wwise_event_on_line(event_name, start_position, end_position, append_husk_to_event_name)
	local sound_position = nil
	local player = Managers.player:local_player(1)
...
function PlayerUnitFxExtension:should_play_husk_effect()
	return (not self._is_local_unit or not self._player:is_human_controlled()) and not self._is_in_first_person_mode
end

function PlayerUnitFxExtension:set_targeted_by_special(special_unit, targeted)
	local specials_targeting_me = self._specials_targeting_me
...
function PlayerUnitFxExtension:spawn_gear_particle_effect_with_source(particle_alias, external_properties, source_name, link, orphaned_policy, position_offset, rotation_offset, scale)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:trigger_gear_wwise_event(sound_alias, external_properties, ...)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:trigger_gear_wwise_event_with_source(sound_alias, external_properties, source_name, sync_to_clients, include_client)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:trigger_gear_wwise_event_with_position(sound_alias, external_properties, sound_position, sync_to_clients, include_client)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:trigger_voice_wwise_event_with_source(event_name, source_name, append_husk_to_event_name, include_client)
...
function PlayerUnitFxExtension:trigger_wwise_event_with_source(event_name, source_name, append_husk_to_event_name, include_client)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:trigger_wwise_event_non_synced(dialogue_extension, event_name, source_name, append_husk_to_event_name)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:_resolve_wwise_event(event_name, append_husk_to_event_name)
	local wwise_event_name = nil
...
function PlayerUnitFxExtension:set_source_parameter(parameter_name, parameter_value, source_name)
	local source = self._sources[source_name]
...
function PlayerUnitFxExtension:trigger_exclusive_wwise_event(event_name, optional_position, optional_except_sender, optional_parameter_name, optional_parameter_value)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:trigger_exclusive_gear_wwise_event(sound_alias, external_properties, optional_position, optional_except_sender, optional_parameter_name, optional_parameter_value)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:spawn_exclusive_particle(particle_name, position, rotation, scale)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:trigger_looping_wwise_event(sound_alias, optional_source_name)
	local is_server = self._is_server
...
function PlayerUnitFxExtension:_trigger_looping_wwise_event(sound_alias, optional_source_name)
...
function PlayerUnitFxExtension:stop_looping_wwise_event(sound_alias)
	local is_server = self._is_server
	local is_local = self._is_local_unit
	local config = PlayerCharacterLoopingSoundAliases[sound_alias]
	local trigger_event = true
...
function PlayerUnitFxExtension:_stop_looping_wwise_event(sound_alias, force_stop)
	local data = self._looping_sounds[sound_alias]
	local config = PlayerCharacterLoopingSoundAliases[sound_alias]
...
function PlayerUnitFxExtension:is_looping_wwise_event_playing(looping_sound_alias)
...
function PlayerUnitFxExtension:register_vfx_spawner(spawner_name, parent_unit, attachments, node_name, should_add_3p_node)
...
function PlayerUnitFxExtension:_register_vfx_spawner(spawners, spawner_name, parent_unit, attachments, node_name, should_add_3p_node)
	if attachments then
...
function PlayerUnitFxExtension:move_vfx_spawner(spawner_name, parent_unit, attachments, node_name)
	local spawners = self._vfx_spawners
...
function PlayerUnitFxExtension:unregister_vfx_spawner(spawner_name)
	self._vfx_spawners[spawner_name] = nil
end

function PlayerUnitFxExtension:stop_looping_wwise_events_for_source_on_mispredict(source_name)
	local looping_sounds = self._looping_sounds
...
function PlayerUnitFxExtension:spawn_looping_particles(looping_particle_alias, optional_spawner_name, external_properties)
	local is_server = self._is_server
	local is_local = self._is_local_unit
	local link = true
	local position_offset, rotation_offset, scale = nil
...
function PlayerUnitFxExtension:_spawn_looping_particles(looping_particle_alias, optional_spawner_name, external_properties, link, position_offset, rotation_offset, scale)
	local data = self._looping_particles[looping_particle_alias]
	local particle_config = PlayerCharacterLoopingParticleAliases[looping_particle_alias]
...
function PlayerUnitFxExtension:stop_looping_particles(looping_particle_alias, should_fade_kill)
	local is_server = self._is_server
	local is_local = self._is_local_unit
...
function PlayerUnitFxExtension:_stop_looping_particles(looping_particle_alias, should_fade_kill)
	local data = self._looping_particles[looping_particle_alias]
	local id = data.id
...
function PlayerUnitFxExtension:is_looping_particles_playing(looping_particle_alias)
	local component = self._looping_particles_components[looping_particle_alias]

	return component and component.is_playing
end

function PlayerUnitFxExtension:spawn_particles(particle_name, position, rotation, scale, optional_variable_name, optional_variable_value)
	local world = self._world
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:spawn_unit_particles(particle_name, spawner_name, link, orphaned_policy, position_offset, rotation_offset, scale, all_clients)
	local is_resim = self._unit_data_extension.is_resimulating
...
function PlayerUnitFxExtension:_spawn_unit_particles(particle_name, spawner_name, link, orphaned_policy, position_offset, rotation_offset, scale, test)
	local world = self._world
...
function PlayerUnitFxExtension:destroy_player_particles(particle_id)
...
function PlayerUnitFxExtension:stop_player_particles(particle_id)
...
function PlayerUnitFxExtension:rpc_play_player_sound(channel_id, game_object_id, event_id, source_id, append_husk_to_event_name)
...
function PlayerUnitFxExtension:rpc_play_player_sound_with_position(channel_id, game_object_id, event_id, sound_position, append_husk_to_event_name)
	local event_name = NetworkLookup.player_character_sounds[event_id]
...
function PlayerUnitFxExtension:rpc_play_server_controlled_player_sound(channel_id, game_object_id, event_id, append_husk_to_event_name, optional_position, optional_parameter_name_id, optional_parameter_value)
	local event_name = NetworkLookup.player_character_sounds[event_id]
...
function PlayerUnitFxExtension:rpc_set_source_parameter(channel_id, game_object_id, source_id, parameter_id, parameter_value)
...
function PlayerUnitFxExtension:rpc_play_exclusive_player_sound(channel_id, game_object_id, event_id, optional_position)
	local event_name = NetworkLookup.player_character_sounds[event_id]

	self:trigger_exclusive_wwise_event(event_name, optional_position)
end

function PlayerUnitFxExtension:rpc_spawn_exclusive_particle(channel_id, game_object_id, particle_id, position, rotation, scale)
...
function PlayerUnitFxExtension:rpc_play_looping_player_sound(channel_id, game_object_id, sound_alias_id, optional_source_id)
...
function PlayerUnitFxExtension:rpc_stop_looping_player_sound(channel_id, game_object_id, sound_alias_id)
	local sound_alias = NetworkLookup.player_character_looping_sound_aliases[sound_alias_id]
	local data = self._looping_sounds[sound_alias]
...
function PlayerUnitFxExtension:rpc_spawn_player_particles(channel_id, game_object_id, particle_name_id, particle_spawner_id, link, position_offset, rotation_offset, scale, particle_id)
...
function PlayerUnitFxExtension:rpc_destroy_player_particles(channel_id, game_object_id, key_id)
...
function PlayerUnitFxExtension:rpc_stop_player_particles(channel_id, game_object_id, key_id)
	local particle_id = self._player_particles[key_id]
...
function PlayerUnitFxExtension:rpc_spawn_player_particles_with_variable(channel_id, game_object_id, particle_name_id, particle_spawner_id, link, position_offset, rotation_offset, scale, variable_name_id, variable_value)
	local particle_name = NetworkLookup.player_character_particles[particle_name_id]
	local spawner_name = NetworkLookup.player_character_fx_sources[particle_spawner_id]
	local world = self._world
	local particle_id = nil
...
function PlayerUnitFxExtension:rpc_spawn_looping_player_particles(channel_id, game_object_id, looping_particle_alias_id, particle_name_id, optional_particle_spawner_id, link)
...
function PlayerUnitFxExtension:rpc_stop_looping_particles(channel_id, game_object_id, looping_particle_alias_id, should_fade_kill)
	local looping_particle_alias = NetworkLookup.player_character_looping_particle_aliases[looping_particle_alias_id]
	local data = self._looping_particles[looping_particle_alias]
	local id = data.id
...
function PlayerUnitFxExtension:rpc_spawn_player_fx_line(channel_id, game_object_id, line_effect_id, is_critical_strike, source_id, end_position, link, scale, append_husk_to_event_name)
