...
function GadgetSystem:init(context, system_init_data, ...)
...
function GadgetSystem:destroy()
	if self._is_client then
		self._network_event_delegate:unregister_events(unpack(RPCS_CLIENT))
	end
end

function GadgetSystem:add_meta_buff(player, buff_name, start_time, lerp_value, slot_name)
...
function GadgetSystem:rpc_add_meta_buff(channel_id, peer_id, local_player_id, buff_template_id, buff_instance_id, optional_lerp_value, slot_name_id)
...
function GadgetSystem:_add_meta_buff(player, buff_name, buff_instance_id, start_time, lerp_value, slot_name)
...
function GadgetSystem:meta_buffs()
	return self._buffs
end

function GadgetSystem:remove_meta_buff(player, buff_instance_id)
...
function GadgetSystem:rpc_remove_meta_buff(channel_id, peer_id, local_player_id, buff_instance_id)
	local player = Managers.player:player(peer_id, local_player_id)
...
function GadgetSystem:_remove_meta_buff(buff_instance_id)
...
function GadgetSystem:_update_meta_stat_buffs(player)
...
function GadgetSystem:_reset_stat_buffs(player)
	local current_stat_buffs = self._stat_buffs[player]
...
function GadgetSystem:_init_stat_buffs(player)
...
function GadgetSystem:stat_buffs(player)
	local stat_buffs = self._stat_buffs[player] or self:_default_stat_buffs(player)

	return stat_buffs
end

function GadgetSystem:_default_stat_buffs(player)
	self:_init_stat_buffs(player)
...
function GadgetSystem:update(dt, t)
end
