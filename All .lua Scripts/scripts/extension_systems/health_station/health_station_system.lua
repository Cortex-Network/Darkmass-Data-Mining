...
function HealthStationSystem:init(context, system_init_data, ...)
...
function HealthStationSystem:_fetch_settings(mission, circumstance_name)
...
function HealthStationSystem:on_gameplay_post_init(level)
	if self._is_server then
		self:_distribute_charges_to_stations()
	end
end

function HealthStationSystem:destroy()
	if not self._is_server then
		self._network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function HealthStationSystem:update(context, dt, t, ...)
	HealthStationSystem.super.update(self, context, dt, t, ...)
end

function HealthStationSystem:hot_join_sync(sender, channel)
...
function HealthStationSystem:_distribute_charges_to_stations()
...
function HealthStationSystem:get_closest_health_station(position)
	local unit_to_extension_map = self._unit_to_extension_map
...
function HealthStationSystem:rpc_health_station_use(channel_id, level_unit_id)
	local unit_spawner_manager = Managers.state.unit_spawner
...
function HealthStationSystem:rpc_health_station_hot_join(channel_id, level_unit_id, charge_amount, socket_object_id, battery_id, battery_is_level_unit)
	local unit_spawner_manager = Managers.state.unit_spawner
...
function HealthStationSystem:rpc_health_station_on_socket_spawned(channel_id, level_unit_id, socket_object_id)
	local unit_spawner_manager = Managers.state.unit_spawner
	local health_station_unit = unit_spawner_manager:unit(level_unit_id, true)
	local health_station_extension = self._unit_to_extension_map[health_station_unit]
...
function HealthStationSystem:rpc_health_station_on_battery_spawned(channel_id, level_unit_id, battery_id, battery_is_level_unit)
	local unit_spawner_manager = Managers.state.unit_spawner
	local health_station_unit = unit_spawner_manager:unit(level_unit_id, true)
	local health_station_extension = self._unit_to_extension_map[health_station_unit]
...
function HealthStationSystem:rpc_health_station_sync_charges(channel_id, level_unit_id, charge_amount)
	local unit_spawner_manager = Managers.state.unit_spawner
	local unit = unit_spawner_manager:unit(level_unit_id, true)
	local health_station_extension = self._unit_to_extension_map[unit]
