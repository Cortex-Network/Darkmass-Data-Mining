...
function ViewInteraction:init(template)
...
function ViewInteraction:start(world, interactor_unit, unit_data_component, t, interactor_is_server)
	...
	if is_local_player then
		...
		if not ui_manager:view_active(ui_interaction) and not ui_manager:is_view_closing(ui_interaction) then
			...
			ui_manager:open_view(ui_interaction, nil, , , , context)
		end
	end
end
...
function ViewInteraction:_check_view_requirements(interactor_unit, ui_interaction)
	local player_unit_spawn_manager = Managers.state.player_unit_spawn
	local player = player_unit_spawn_manager:owner(interactor_unit)
...
function ViewInteraction:interactor_condition_func(interactor_unit, interactee_unit)
...
function ViewInteraction:interactee_show_marker_func(interactor_unit, interactee_unit)
	local interactee_extension = ScriptUnit.extension(interactee_unit, "interactee_system")
	local ui_interaction = interactee_extension:ui_interaction()
	local ui_manager = Managers.ui
...
function ViewInteraction:hud_block_text(interactor_unit, interactee_unit, interactable_actor_node_index)
	local interactee_extension = ScriptUnit.extension(interactee_unit, "interactee_system")
	local ui_interaction = interactee_extension:ui_interaction()
