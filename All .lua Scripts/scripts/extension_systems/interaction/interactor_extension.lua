...
function InteractorExtension:init(extension_init_context, unit, extension_init_data, game_object_data, ...)
...
function InteractorExtension:destroy()
...
function InteractorExtension:_init_action_components(unit_data_extension)
...
function InteractorExtension:_reset_interaction()
	local interaction_component = self._interaction_component
...
function InteractorExtension:set_bot_interaction_unit(target_unit, target_actor, interaction_type)
...
function InteractorExtension:calculate_duration_buff(interactee_extension)
...
function InteractorExtension:calculate_speed_buff(interactee_extension)
	local interaction_type = interactee_extension:interaction_type()
...
function InteractorExtension:calculate_duration(interactee_extension)
...
function InteractorExtension:fixed_update(unit, dt, t, fixed_frame, context)
...
function InteractorExtension:_check_current_state(unit, dt, t, chosen_target, state)
...
function InteractorExtension:cancel_interaction(t)
...
function InteractorExtension:_start_interaction_timer(t)
	local interaction_component = self._interaction_component
...
function InteractorExtension:_find_reachable_object(interactor_unit)
...
function InteractorExtension:_check_valid_ongoing_interaction(target_unit, target_node)
...
function InteractorExtension:_check_valid_interaction_target(target_unit)
...
function InteractorExtension:_distance_to_interact(start_pos, direction, max_distance, interact_unit, interact_node)
...
function InteractorExtension:_check_collision_clear(start_pos, direction, distance)
...
function InteractorExtension:can_interact(target_unit, interaction_type)
	local interaction_component = self._interaction_component
	local state = interaction_component.state
...
function InteractorExtension:_can_interact(target_unit, optional_interaction_type)
	local unit = self._unit
	local interactee_extension = ScriptUnit.extension(target_unit, "interactee_system")
...
function InteractorExtension:interaction(optional_interaction_template_name)
...
function InteractorExtension:show_counter_ui()
	return self._show_counter_ui
end

function InteractorExtension:show_interaction_ui()
	return self._show_interaction_ui
end

function InteractorExtension:target_unit()
	return self._interaction_component.target_unit
end

function InteractorExtension:focus_unit()
	return self._focus_unit
end

function InteractorExtension:interaction_progress()
	local unit = self._unit
	local unit_data_extension = ScriptUnit.extension(unit, "unit_data_system")
...
function InteractorExtension:hud_description()
	local interaction_component = self._interaction_component
	local target_unit = interaction_component.target_unit
...
function InteractorExtension:hud_block_text()
	local interaction_component = self._interaction_component
	local target_unit = interaction_component.target_unit
	local actor_node_index = interaction_component.target_actor_node_index
...
function InteractorExtension:marker_offset()
	local interaction_component = self._interaction_component
	local target_unit = interaction_component.target_unit
	local actor_node_index = interaction_component.target_actor_node_index
	local interaction = self:interaction()
...
function InteractorExtension:is_interacting()
	local interaction_component = self._interaction_component
	local state = interaction_component.state
