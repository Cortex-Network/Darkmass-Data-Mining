...
function PlayerInteracteeExtension:init(extension_init_context, unit, extension_init_data, game_object_data, game_object_id)
...
function PlayerInteracteeExtension:hot_join_sync(unit, sender, channel)
...
function PlayerInteracteeExtension:game_object_initialized(session, game_object_id)
	self._unit_id = game_object_id
end

function PlayerInteracteeExtension:_set_interaction_contexts(interaction_contexts)
...
function PlayerInteracteeExtension:can_interact(interactor_unit, optional_interaction_type)
...
function PlayerInteracteeExtension:set_active(is_active)
...
function PlayerInteracteeExtension:active()
	return self._active
end

function PlayerInteracteeExtension:interaction_type()
	local unit = self._unit
	local objects = self._interaction_objects
...
function PlayerInteracteeExtension:show_marker(interactor_unit)
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:display_start_event()
	return false
end

function PlayerInteracteeExtension:ui_interaction_type()
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:interaction_icon()
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:interaction_length()
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:action_text()
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:description()
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:block_text()
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:hold_required()
	local unit_has_context = self._unit_has_context
...
function PlayerInteracteeExtension:started(interactor_unit)
	if self._is_server then
		local unit_id = self._unit_id
		local is_level_unit = false
...
function PlayerInteracteeExtension:stopped(result)
	if self._is_server then
		local unit_id = self._unit_id
		local is_level_unit = false
...
function PlayerInteracteeExtension:_trigger_flow_event(event_name)
