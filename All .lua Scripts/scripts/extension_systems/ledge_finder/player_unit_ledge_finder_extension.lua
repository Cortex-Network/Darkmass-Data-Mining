...
function PlayerUnitLedgeFinderExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function PlayerUnitLedgeFinderExtension:_new_ledge_data()
	local ledge_data = {
		left = Vector3Box(),
		right = Vector3Box(),
		forward = Vector3Box(),
...
function PlayerUnitLedgeFinderExtension:extensions_ready(world, unit)
	self._locomotion_extension = ScriptUnit.extension(unit, "locomotion_system")
end

function PlayerUnitLedgeFinderExtension:ledges()
	local ledges = self._ledge_ring_buffer[_ring_buffer_index(self._fixed_frame)]

	return ledges.num_ledges, ledges.ledge_data
end

function PlayerUnitLedgeFinderExtension:fixed_update(unit, dt, t, fixed_frame)
	self._fixed_frame = fixed_frame
...
function PlayerUnitLedgeFinderExtension:_try_find_ledges(ledges)
...
function PlayerUnitLedgeFinderExtension:_find_potential_ledge_points(origin, ray_direction, potential_ledge_points, ledge_point_unit_map)
...
function PlayerUnitLedgeFinderExtension:_pair_potential_ledge_points(ledges, potential_left_ledge_points, potential_right_ledge_points, ledge_point_unit_map)
	local up = Vector3.up()
	local down = -up
...
function PlayerUnitLedgeFinderExtension:_finalize_ledges(ledges, player_position)
