...
function LegacyV2ProximitySystem:init(extension_system_creation_context, system_init_data, system_name, extension_list, ...)
...
function LegacyV2ProximitySystem:_spread_start_times()
...
function LegacyV2ProximitySystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function LegacyV2ProximitySystem:register_extension_update(unit, extension_name, extension)
	if not legacy_extension_lookup[extension_name] then
		LegacyV2ProximitySystem.super.register_extension_update(self, unit, extension_name, extension)
	end
end

function LegacyV2ProximitySystem:extensions_ready(world, unit, extension_name)
	if extension_name == "PlayerProximityExtension" then
...
function LegacyV2ProximitySystem:set_unit_local(unit)
	if not self._unit_extension_data[unit] then
		LegacyV2ProximitySystem.super.set_unit_local(self, unit)
	end
end

function LegacyV2ProximitySystem:add_distance_based_vo_query(source_unit, concept_name, query_data)
...
function LegacyV2ProximitySystem:update(...)
end

function LegacyV2ProximitySystem:physics_async_update(context, dt, t)
...
function LegacyV2ProximitySystem:_update_nearby_enemies(side)
	if DEDICATED_SERVER then
		return
	end
...
function LegacyV2ProximitySystem:_process_distance_based_vo_query()
...
function LegacyV2ProximitySystem:_make_async_raycast_to_center(raycast_object, unit, unit_position, nearby_unit)
...
function LegacyV2ProximitySystem:_async_raycast_result_cb(id, hits, num_hits, data)
	if num_hits <= 0 then
		return
	end
...
function LegacyV2ProximitySystem:post_update(...)
