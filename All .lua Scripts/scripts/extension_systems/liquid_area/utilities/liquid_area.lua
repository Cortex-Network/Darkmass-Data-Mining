...
function LiquidArea.try_create(position, flow_direction, nav_world, liquid_area_template, optional_source_unit, optional_max_liquid, optional_not_on_other_liquids, optional_source_item)
	...
	local liquid_unit = Managers.state.unit_spawner:spawn_network_unit(nil, UNIT_TEMPLATE_NAME, nav_mesh_position, nil, , liquid_area_template, flow_direction, optional_source_unit, optional_max_liquid, nil, optional_source_item)

	return liquid_unit
end

function LiquidArea.start_paint()
	local liquid_area_system = Managers.state.extension:system("liquid_area_system")
...
function LiquidArea.paint(liquid_paint_id, max_liquid_paint_distance, position, nav_world, liquid_area_template, allow_liquid_unit_creation, optional_brush_size, optional_not_on_other_liquids, optional_source_unit, optional_source_item)
	local liquid_unit = nil
	local created_new_liquid_unit = false
	local liquid_area_system = Managers.state.extension:system("liquid_area_system")
	local traverse_logic = liquid_area_system:traverse_logic()
	local nav_mesh_position = NavQueries.position_on_mesh_with_outside_position(nav_world, traverse_logic, position, NAV_MESH_ABOVE, NAV_MESH_BELOW, NAV_MESH_LATERAL, DISTANCE_FROM_NAV_MESH)
	...
	if not liquid_unit and allow_liquid_unit_creation then
		...
		local flow_direction, max_liquid = nil
		liquid_unit = Managers.state.unit_spawner:spawn_network_unit(nil, UNIT_TEMPLATE_NAME, nav_mesh_position, nil, , liquid_area_template, flow_direction, optional_source_unit, max_liquid, liquid_paint_id, optional_source_item)
	end

	return liquid_unit, created_new_liquid_unit
end

function LiquidArea.stop_paint(liquid_paint_id)
	local liquid_area_system = Managers.state.extension:system("liquid_area_system")
