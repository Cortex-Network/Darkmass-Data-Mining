...
function PlayerUnitLocomotionExtension:init(extension_init_context, unit, extension_init_component, game_object_component_or_game_session, game_object_id_or_nil)
	self._unit = unit
...
function PlayerUnitLocomotionExtension:game_object_initialized(session, object_id)
...
function PlayerUnitLocomotionExtension:destroy()
	Managers.state.out_of_bounds:unregister_soft_oob_unit(self._unit, self)
end

function PlayerUnitLocomotionExtension:_update_soft_oob(dt, locomotion_component)
...
function PlayerUnitLocomotionExtension:_update_movement(unit, dt, t, locomotion_component, steering_component, inair_component)
...
function PlayerUnitLocomotionExtension:_do_minion_collision(unit, dt, dragged_velocity, current_position)
...
function PlayerUnitLocomotionExtension:_update_script_driven_movement(unit, dt, t, locomotion_component, steering_component, current_position, calculate_fall_velocity, on_ground, mover)
	local velocity_current = locomotion_component.velocity_current
...
function PlayerUnitLocomotionExtension:_update_script_driven_hub_movement(unit, dt, t, locomotion_component, steering_component, current_position, calculate_fall_velocity, mover)
	local velocity_wanted = steering_component.velocity_wanted
	local velocity_current = locomotion_component.velocity_current
...
function PlayerUnitLocomotionExtension:_handle_push_velocity(velocity_wanted, on_ground, dt, t)
...
function PlayerUnitLocomotionExtension:_update_new_push_velocity(t)
	local locomotion_push = self._locomotion_push_component
	local new_velocity = locomotion_push.new_velocity

	if Vector3.length_squared(new_velocity) > 0 and locomotion_push.time_to_apply <= t then
...
function PlayerUnitLocomotionExtension:_apply_push_velocity(velocity_wanted, dt)
	local locomotion_push = self._locomotion_push_component
...
function PlayerUnitLocomotionExtension:_decay_push_velocity(on_ground, dt, final_velocity)
	local locomotion_push = self._locomotion_push_component
	local push_velocity = locomotion_push.velocity
...
function PlayerUnitLocomotionExtension:visual_link(parent_unit, parent_node_name, node_name)
...
function PlayerUnitLocomotionExtension:visual_unlink()
...
function PlayerUnitLocomotionExtension:hot_join_sync(unit, sender, _)
...
function PlayerUnitLocomotionExtension:fixed_update(unit, dt, t, frame)
...
function PlayerUnitLocomotionExtension:_update_rotation(unit, dt, t, locomotion_component, steering_component, force_rotation_component)
...
function PlayerUnitLocomotionExtension:update(unit, dt, t)
	local locomotion_component = self._locomotion_component
...
function PlayerUnitLocomotionExtension:post_update(unit, dt, t)
	local locomotion_component = self._locomotion_component
	local locomotion_steering_component = self._locomotion_steering_component
...
function PlayerUnitLocomotionExtension:_calculate_rotation(unit, dt, t, data, steering_component, force_rotation_component, look_rotation, velocity_wanted, velocity_current, current_rotation, current_target_rotation)
...
function PlayerUnitLocomotionExtension:get_parent_unit()
	return self._locomotion_component.parent_unit
end

function PlayerUnitLocomotionExtension:set_parent_unit(unit)
	local locomotion_component = self._locomotion_component
	local unit_id = NetworkConstants.invalid_level_unit_id
	local is_level_unit = nil
...
function PlayerUnitLocomotionExtension:extensions_ready(world, unit)
...
function PlayerUnitLocomotionExtension:server_correction_occurred(unit, from_frame)
...
function PlayerUnitLocomotionExtension:_on_soft_oob()
