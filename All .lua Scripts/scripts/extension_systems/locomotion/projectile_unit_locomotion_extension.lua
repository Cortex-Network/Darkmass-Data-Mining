...
function ProjectileUnitLocomotionExtension:init(extension_init_context, unit, extension_init_data, game_object_data)
	...
	elseif starting_state == locomotion_states.sticky then
		self:switch_to_sticky(nil, , position, rotation)
...
function ProjectileUnitLocomotionExtension:has_been_marked_for_deletion()
...
function ProjectileUnitLocomotionExtension:fixed_update(unit, dt, t)
...
function ProjectileUnitLocomotionExtension:_update_out_of_bounds()
...
function ProjectileUnitLocomotionExtension:update(unit, dt, t)
...
function ProjectileUnitLocomotionExtension:switch_to_true_flight(position, rotation, direction, speed, angular_velocity, target_unit, target_position)
...
function ProjectileUnitLocomotionExtension:switch_to_manual_state_helper(position, rotation, direction, speed, angular_velocity, target_unit, target_position)
	local projectile_unit = self._projectile_unit
...
function ProjectileUnitLocomotionExtension:_update_manual_physics(unit, dt, t)
	local dynamic_actor_id = self._dynamic_actor_id
...
function ProjectileUnitLocomotionExtension:_update_true_flight(unit, dt, t)
	local dynamic_actor_id = self._dynamic_actor_id
...
function ProjectileUnitLocomotionExtension:_update_engine_physics(unit)
	local dynamic_actor_id = self._dynamic_actor_id
	local dynamic_actor = Unit.actor(unit, dynamic_actor_id)
...
function ProjectileUnitLocomotionExtension:switch_to_sticky(sticking_to_unit, sticking_to_actor_index, world_position, world_rotation)
...
function ProjectileUnitLocomotionExtension:switch_to_sleep(position, rotation)
...
function ProjectileUnitLocomotionExtension:_apply_changes(state, position, rotation, velocity, angular_momentum, target_position, target_unit, target_hit_zone)
	local projectile_unit = self._projectile_unit
	local dynamic_actor = nil
...
function ProjectileUnitLocomotionExtension:owner_unit()
	return self._owner_unit
end

function ProjectileUnitLocomotionExtension:radius()
	return self._radius
end

function ProjectileUnitLocomotionExtension:mass()
	return self._mass
end

function ProjectileUnitLocomotionExtension:is_moving()
	return moving_locomotion_states[self._current_state]
end

function ProjectileUnitLocomotionExtension:previous_and_current_positions()
...
function ProjectileUnitLocomotionExtension:current_rotation_and_direction()
	local integration_data = self._integration_data
...
function ProjectileUnitLocomotionExtension:current_speed()
	local state = self._current_state
...
function ProjectileUnitLocomotionExtension:sticking_to_unit()
	local sticking_to_unit = self._sticking_to_unit
...
function ProjectileUnitLocomotionExtension:current_state()
	return self._current_state
end

function ProjectileUnitLocomotionExtension:locomotion_template()
