...
function LuggableSocketExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function LuggableSocketExtension:setup_from_component(consume_luggable, is_side_mission_socket, lock_offset_node)
...
function LuggableSocketExtension:_fetch_actor_size(unit, actor_name)
...
function LuggableSocketExtension:hot_join_sync(socketed_unit)
	if socketed_unit then
		self:socket_luggable(socketed_unit)
	end
end

function LuggableSocketExtension:update(unit, dt, t)
	if self._is_server and self._is_temp_locked then
		self:_check_temp_locked_socket(dt)
	end
end

function LuggableSocketExtension:_retrieve_overlapping_with_luggables()
...
function LuggableSocketExtension:is_overlapping_with_luggable(luggable_unit)
...
function LuggableSocketExtension:add_overlapping_unit(luggable_unit)
	self._overlapping_units[luggable_unit] = true
end

function LuggableSocketExtension:_has_same_mission_objective(unit)
...
function LuggableSocketExtension:socket_luggable(luggable, socket_lock_time)
	local socket_unit = self._unit
...
function LuggableSocketExtension:_check_temp_locked_socket(dt)
...
function LuggableSocketExtension:_lock_socket(luggable)
...
function LuggableSocketExtension:unlock_socket()
	self._locked_luggable = nil
	local socket_objective_target_ext = self._socket_objective_target_ext
...
function LuggableSocketExtension:is_socketable(unit)
	return self._locked_luggable == nil and self:_has_same_mission_objective(unit)
end

function LuggableSocketExtension:is_occupied()
	return self._locked_luggable ~= nil
end

function LuggableSocketExtension:socketed_unit()
	return self._locked_luggable
end

function LuggableSocketExtension:consume_luggable()
	return self._consume_luggable
end

function LuggableSocketExtension:set_socket_visibility(value)
	local socket_unit = self._unit
