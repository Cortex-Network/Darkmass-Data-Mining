...
function MinigameDecodeSymbols:init(unit, is_server, seed, context)
...
function MinigameDecodeSymbols:hot_join_sync(sender, channel)
...
function MinigameDecodeSymbols:start(player)
...
function MinigameDecodeSymbols:stop()
	local is_server = self._is_server
...
function MinigameDecodeSymbols:setup_game()
...
function MinigameDecodeSymbols:_calculate_cursor_time(t)
...
function MinigameDecodeSymbols:is_completed()
	local stage_amount = self._decode_symbols_stage_amount
	local current_stage = self._decode_current_stage
...
function MinigameDecodeSymbols:current_decode_target()
...
function MinigameDecodeSymbols:set_current_stage(stage)
...
function MinigameDecodeSymbols:set_start_time(time)
	self._decode_start_time = time
end

function MinigameDecodeSymbols:set_symbols(symbols)
	self._symbols = table.shallow_copy(symbols)
end

function MinigameDecodeSymbols:set_target(stage, target)
	self._decode_targets[stage] = target
end

function MinigameDecodeSymbols:current_stage()
	return self._decode_current_stage
end

function MinigameDecodeSymbols:sweep_duration()
	return self._decode_symbols_sweep_duration
end

function MinigameDecodeSymbols:start_time()
	return self._decode_start_time
end

function MinigameDecodeSymbols:symbols()
