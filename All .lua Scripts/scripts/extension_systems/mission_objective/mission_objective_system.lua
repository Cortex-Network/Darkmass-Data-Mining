...
function MissionObjectiveSystem:init(context, system_init_data, ...)
...
function MissionObjectiveSystem:objective_definition(objective_name)
	return self._objective_definitions[objective_name]
end

function MissionObjectiveSystem:objective_list(peer_id)
...
function MissionObjectiveSystem:active_objective(objective_name, peer_id)
	local objective_list = self:objective_list(peer_id)

	return objective_list:get_active_objective(objective_name)
end

function MissionObjectiveSystem:has_active_objective(objective_name, peer_id)
	peer_id = peer_id or GLOBAL_LIST_INDEX
...
function MissionObjectiveSystem:pre_update(dt, t)
...
function MissionObjectiveSystem:update(system_context, dt, t)
	if self._is_server then
		for peer_id, objective_list in pairs(self._objective_lists) do
			local active_objectives = objective_list:active_objectives()
...
function MissionObjectiveSystem:start_mission_objective(objective_name, progression, second_progression, increment, stage, peer_id)
	peer_id = peer_id or GLOBAL_LIST_INDEX
...
function MissionObjectiveSystem:_setup_mission_objective(objective_name, peer_id)
	local mission_objective_data = self:objective_definition(objective_name)
	local mission_objective_type = mission_objective_data.mission_objective_type
	local objective_list = self:objective_list(peer_id)
...
function MissionObjectiveSystem:start_mission_objective_stage(objective_name, stage, peer_id)
	if self._is_server then
		local objective_name_id = NetworkLookup.mission_objective_names[objective_name]
...
function MissionObjectiveSystem:evaluate_level_end_objectives()
	for peer_id, objective_list in pairs(self._objective_lists) do
...
function MissionObjectiveSystem:end_mission_objective(objective_name, peer_id)
	peer_id = peer_id or GLOBAL_LIST_INDEX
	local objective_list = self:objective_list(peer_id)
	local objective = objective_list:get_active_objective(objective_name)
...
function MissionObjectiveSystem:_proc_side_mission_objective_complete_buffs(player)
...
function MissionObjectiveSystem:external_update_mission_objective(objective_name, dt, increment, peer_id)
	if self._is_server then
		local objective = self:active_objective(objective_name, peer_id)
...
function MissionObjectiveSystem:_propagate_objective_progression(objective, peer_id)
...
function MissionObjectiveSystem:propagate_objective_increment(objective, peer_id)
	local objective_name = objective:name()
	local objective_name_id = NetworkLookup.mission_objective_names[objective_name]
...
function MissionObjectiveSystem:is_current_active_objective(objective_name, peer_id)
	return self:active_objective(objective_name, peer_id) ~= nil
end

function MissionObjectiveSystem:get_active_objective(objective_name, peer_id)
	return self:active_objective(objective_name, peer_id)
end

function MissionObjectiveSystem:get_active_objectives(peer_id)
	local objective_list = self:objective_list(peer_id)

	return objective_list:active_objectives()
end

function MissionObjectiveSystem:get_completed_objectives(peer_id)
	local objective_list = self:objective_list(peer_id)

	return objective_list:completed_objectives()
end

function MissionObjectiveSystem:get_objective_progress(objective_name, peer_id)
...
function MissionObjectiveSystem:is_objective_complete(objective_name, peer_id)
	local objective_list = self:objective_list(peer_id)
...
function MissionObjectiveSystem:_override_ui_string(objective_name, new_ui_header, new_ui_description)
...
function MissionObjectiveSystem:_change_active_objective_ui_string(is_header, new_ui_string, objective_name)
...
function MissionObjectiveSystem:set_objective_show_counter(objective_name, show)
	local active_objective = self:get_active_objective(objective_name)
...
function MissionObjectiveSystem:on_player_unit_spawn(player, unit, is_respawn)
...
function MissionObjectiveSystem:store_grimoire()
	local active_objective = self:get_active_objective("side_mission_grimoire")
...
function MissionObjectiveSystem:get_objective_event_music()
...
function MissionObjectiveSystem:get_objective_event_type_music()
	for _, objective_list in pairs(self._objective_lists) do
		local active_objectives = objective_list:active_objectives()
...
function MissionObjectiveSystem:get_objective_event_music_progress()
	for _, objective_list in pairs(self._objective_lists) do
		local active_objectives = objective_list:active_objectives()
...
function MissionObjectiveSystem:sound_event(sound_event)
	if self._is_server then
...
function MissionObjectiveSystem:_distribute_sound_event(sound_event)
...
function MissionObjectiveSystem:register_music_event_listener(listener)
	self._music_event_listener = listener
end

function MissionObjectiveSystem:register_objective_synchronizer(objective_name, objective_unit, peer_id)
...
function MissionObjectiveSystem:get_objective_synchronizer_unit(objective_name, peer_id)
	local objective_list = self:objective_list(peer_id)

	return objective_list:objective_registered_synchronizer(objective_name)
end

function MissionObjectiveSystem:register_objective_unit(objective_name, objective_unit, objective_stage, peer_id)
...
function MissionObjectiveSystem:unregister_objective_unit(objective_name, objective_unit, objective_stage, peer_id)
	if not self:objective_definition(objective_name) then
		return
	end
...
function MissionObjectiveSystem:add_marker(objective_name, unit, peer_id)
	local objective = self:active_objective(objective_name, peer_id)
...
function MissionObjectiveSystem:remove_marker(objective_name, unit, peer_id)
	local objective = self:active_objective(objective_name, peer_id)
...
function MissionObjectiveSystem:enable_unit(objective_name, unit, stage, peer_id)
	stage = stage or 1
...
function MissionObjectiveSystem:disable_unit(objective_name, unit, stage, peer_id)
	stage = stage or 1
	local objective_list = self:objective_list(peer_id)
	local registered_units = objective_list:objective_registered_units(objective_name)
	local stage_units = registered_units[stage]
...
function MissionObjectiveSystem:hot_join_sync(sender, channel)
	local sorting_table = {}
...
function MissionObjectiveSystem:flow_callback_register_objective_unit(objective_name, objective_unit, peer_id)
	local objective_target = ScriptUnit.has_extension(objective_unit, "mission_objective_target_system")

	if objective_target then
		objective_target:setup_from_external(objective_name, nil, , , , , false, peer_id)
	else
		self:register_objective_unit(objective_name, objective_unit, peer_id)
	end
end

function MissionObjectiveSystem:flow_callback_start_mission_objective(objective_name, peer_id)
	local objective = self:active_objective(objective_name, peer_id)
	...
	self:start_mission_objective(objective_name, nil, , , , peer_id)
end

function MissionObjectiveSystem:flow_callback_update_mission_objective(objective_name, peer_id)
...
function MissionObjectiveSystem:flow_callback_end_mission_objective(objective_name, peer_id)
	local objective = self:active_objective(objective_name, peer_id)
...
function MissionObjectiveSystem:flow_callback_override_ui_string(objective_name, new_ui_header, new_ui_description)
	self:_override_ui_string(objective_name, new_ui_header, new_ui_description)
end

function MissionObjectiveSystem:flow_callback_set_objective_show_counter(objective_name, show)
	self:set_objective_show_counter(objective_name, show)
end

function MissionObjectiveSystem:send_rpc_to_objective_list(peer_id, rpc_name, ...)
	peer_id = peer_id or GLOBAL_LIST_INDEX
	local game_session = self._game_session
...
function MissionObjectiveSystem:rpc_update_mission_objective_progression(channel_id, objective_name_id, progression, peer_id)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
	local objective_list = self:objective_list(peer_id)
	local objective = objective_list:get_active_objective(objective_name)
...
function MissionObjectiveSystem:rpc_update_mission_objective_second_progression(channel_id, objective_name_id, progression)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
...
function MissionObjectiveSystem:rpc_update_mission_objective_increment(channel_id, objective_name_id, increment, max_increment)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
	local objective = self:active_objective(objective_name)
...
function MissionObjectiveSystem:rpc_end_mission_objective(channel_id, objective_name_id)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]

	self:end_mission_objective(objective_name)
end

function MissionObjectiveSystem:rpc_add_objective_marker(channel_id, objective_name_id, is_level_unit, level_unit_id)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
...
function MissionObjectiveSystem:rpc_remove_objective_marker(channel_id, objective_name_id, is_level_unit, level_unit_id)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
	local unit = Managers.state.unit_spawner:unit(level_unit_id, is_level_unit)
...
function MissionObjectiveSystem:rpc_mission_objective_override_ui_string(channel_id, objective_name_id, new_header_id, new_description_id)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]
...
function MissionObjectiveSystem:rpc_mission_objective_show_counter(channel_id, objective_name_id, show)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]

	self:set_objective_show_counter(objective_name, show)
end

function MissionObjectiveSystem:rpc_mission_sound_event(channel_id, music_event_id)
