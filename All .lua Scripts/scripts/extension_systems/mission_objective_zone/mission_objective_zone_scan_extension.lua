...
function MissionObjectiveZoneScanExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function MissionObjectiveZoneScanExtension:extensions_ready(world, unit)
	self._mission_objective_target_extension = ScriptUnit.has_extension(unit, "mission_objective_target_system")
end

function MissionObjectiveZoneScanExtension:destroy()
	if self._is_server then
		local event_manager = Managers.event
...
function MissionObjectiveZoneScanExtension:setup_from_component(num_scannable_objects, max_scannable_objects_per_player, zone_type, item_to_equip)
...
function MissionObjectiveZoneScanExtension:hot_join_sync(unit, sender, channel)
...
function MissionObjectiveZoneScanExtension:update(unit, dt, t)
...
function MissionObjectiveZoneScanExtension:_set_skull_to_wait_for_players()
...
function MissionObjectiveZoneScanExtension:_check_if_players_knocked_down_or_killed()
...
function MissionObjectiveZoneScanExtension:_select_scannable_units_for_event()
...
function MissionObjectiveZoneScanExtension:_last_scanned_object()
...
function MissionObjectiveZoneScanExtension:_set_servo_skull_target(server_skull_target_state, marker_type, is_server)
	self._mission_objective_zone_system:set_servo_skull_target_state(server_skull_target_state, marker_type, is_server)
end

function MissionObjectiveZoneScanExtension:register_scannable_unit(scannable_unit)
	local scannable_units = self._scannable_units
...
function MissionObjectiveZoneScanExtension:assign_scanned_object_to_player_and_bank(scannable_extension, player)
...
function MissionObjectiveZoneScanExtension:assign_scanned_object_to_player(scannable_extension, player)
	local players_with_scanned_objects = self._players_with_scanned_objects
...
function MissionObjectiveZoneScanExtension:release_scanned_object_from_player(player, player_incapacitated)
...
function MissionObjectiveZoneScanExtension:player_scanned_objects(player)
	local scanned_objects = self._scanned_objects_per_player[player] or 0

	return scanned_objects
end

function MissionObjectiveZoneScanExtension:scanned_objects_per_player()
	return self._scanned_objects_per_player
end

function MissionObjectiveZoneScanExtension:max_scannable_objects_per_player()
	return self._max_scannable_objects_per_player
end

function MissionObjectiveZoneScanExtension:add_scanned_points_to_player(player, scanned_points)
...
function MissionObjectiveZoneScanExtension:current_progression()
	return self._current_progression / self._num_scannables_in_zone
end

function MissionObjectiveZoneScanExtension:num_objets_banked()
	return self._current_progression
end

function MissionObjectiveZoneScanExtension:num_scannables_in_zone()
	return self._num_scannables_in_zone
end

function MissionObjectiveZoneScanExtension:selected_scannable_units()
	return self._selected_scannable_units
end

function MissionObjectiveZoneScanExtension:scannable_units()
	return self._scannable_units
end

function MissionObjectiveZoneScanExtension:_on_player_unit_despawned(player)
	local disconnect = true

	self:release_scanned_object_from_player(player, disconnect)
end

function MissionObjectiveZoneScanExtension:activate_zone()
...
function MissionObjectiveZoneScanExtension:_deactivate_zone()
	...
	if self._is_server then
		...
		if current_objective_name then
			...
			mission_objective_system:_override_ui_string(current_objective_name, nil, )
		end
	end
end

function MissionObjectiveZoneScanExtension:set_is_waiting_for_player_confirmation()
...
function MissionObjectiveZoneScanExtension:_vo_timer(dt)
...
function MissionObjectiveZoneScanExtension:_play_vo(player, scanning_vo_line, is_mission_giver_line)
...
function MissionObjectiveZoneScanExtension:reset_vo_timer()
	if self._is_server then
		self._vo_line_timer = self._vo_line_interval
	end
end

function MissionObjectiveZoneScanExtension:abort_vo_timer()
	if self._is_server then
		self._start_vo_line_timer = false
		self._vo_line_timer = self._vo_line_interval
	end
...
function MissionObjectiveZoneScanExtension:is_waiting_for_player_confirmation()
...
function MissionObjectiveZoneScanExtension:_equip_auspex_to_players()
...
function MissionObjectiveZoneScanExtension:_dequip_auspex_from_players()
	local t = Managers.time:time("gameplay")
	local players = Managers.player:players()
