...
function MissionObjectiveZoneSystem:init(context, system_init_data, ...)
...
function MissionObjectiveZoneSystem:on_gameplay_post_init(level)
...
function MissionObjectiveZoneSystem:destroy()
	if not self._is_server then
...
function MissionObjectiveZoneSystem:_group_units_by_objective_name()
	local unit_to_extension_map = self._unit_to_extension_map
...
function MissionObjectiveZoneSystem:_select_units_for_event()
	local zone_units = self._zone_units
	local seed = self._seed
...
function MissionObjectiveZoneSystem:_sort_units_by_id(units)
	if #units == 0 then
		return units
	end
...
function MissionObjectiveZoneSystem:spawn_servo_skull(position, rotation)
...
function MissionObjectiveZoneSystem:_register_servo_skull(servo_skull_unit)
...
function MissionObjectiveZoneSystem:_unregister_servo_skull()
	if self._is_server then
		local unit_spawner_manager = Managers.state.unit_spawner
		local servo_skull_unit = self._servo_skull_unit
...
function MissionObjectiveZoneSystem:_evaluate_next_step(objective_name, activate_zone)
	if self._is_server then
...
function MissionObjectiveZoneSystem:_follow_spline(objective_name)
...
function MissionObjectiveZoneSystem:_activate_zone(unit)
...
function MissionObjectiveZoneSystem:at_end_of_spline(last_spline)
	local is_server = self._is_server
...
function MissionObjectiveZoneSystem:set_servo_skull_target_state(servo_skull_target_state, marker_type, is_server)
...
function MissionObjectiveZoneSystem:_current_synchronizer()
	local mission_objective_system = self._mission_objective_system
	local objective_name = self._current_objective_name
	local synchronizer_unit = mission_objective_system:get_objective_synchronizer_unit(objective_name)
...
function MissionObjectiveZoneSystem:delete_units()
	if self._is_server then
		self:_unregister_servo_skull()
	end
end

function MissionObjectiveZoneSystem:retrieve_selected_units_for_event(objective_name)
	local selected_units = self._selected_zone_units[objective_name]

	return selected_units
end

function MissionObjectiveZoneSystem:start_event(objective_name)
...
function MissionObjectiveZoneSystem:register_objective_name(objective_name)
	self._current_objective_name = objective_name
end

function MissionObjectiveZoneSystem:register_scannable_unit(scannable_unit)
	local unit_to_extension_map = self._unit_to_extension_map
...
function MissionObjectiveZoneSystem:current_active_zone()
	local unit_to_extension_map = self._unit_to_extension_map
...
function MissionObjectiveZoneSystem:register_finished_zone()
...
function MissionObjectiveZoneSystem:reset_progression()
	self._progress = 0
end

function MissionObjectiveZoneSystem:progression()
	local progress = self._progress

	return progress
end

function MissionObjectiveZoneSystem:scannable_progression()
...
function MissionObjectiveZoneSystem:second_progression()
	local extension = self:current_active_zone()
...
function MissionObjectiveZoneSystem:seed()
	return self._seed
end

function MissionObjectiveZoneSystem:has_connected_spline_path(objective_name)
	local spline_follower_system = self._spline_follower_system
...
function MissionObjectiveZoneSystem:current_objective_name()
	local objective_name = self._current_objective_name

	return objective_name
end

function MissionObjectiveZoneSystem:event_completed()
	self:_unregister_servo_skull()
...
function MissionObjectiveZoneSystem:rpc_mission_objective_zone_system_hot_join_sync(channel_id, seed, game_object_id)
	self._seed = seed
...
function MissionObjectiveZoneSystem:rpc_mission_objective_zone_hot_join_sync(channel_id, level_unit_id)
...
function MissionObjectiveZoneSystem:rpc_mission_objective_zone_system_on_servo_skull_spawned(channel_id, game_object_id)
	local is_level_unit = false
...
function MissionObjectiveZoneSystem:rpc_mission_objective_zone_scan_add_player_scanned_object(channel_id, level_unit_id, peer_id, local_player_id, scanned_object_points)
	local is_level_unit = true
	local unit = Managers.state.unit_spawner:unit(level_unit_id, is_level_unit)
	local extension = self._unit_to_extension_map[unit]
...
function MissionObjectiveZoneSystem:rpc_mission_objective_zone_register_scannable_unit(channel_id, level_unit_id, level_scanable_id)
	local is_level_unit = true
	local unit = Managers.state.unit_spawner:unit(level_unit_id, is_level_unit)
...
function MissionObjectiveZoneSystem:rpc_mission_objective_zone_set_waiting_for_confirmation(channel_id, level_unit_id)
	local is_level_unit = true
	local unit = Managers.state.unit_spawner:unit(level_unit_id, is_level_unit)
	local extension = self._unit_to_extension_map[unit]
...
function MissionObjectiveZoneSystem:rpc_event_mission_objective_zone_follow_spline(channel_id, objective_name_id)
	local objective_name = NetworkLookup.mission_objective_names[objective_name_id]

	self:_follow_spline(objective_name)
end

function MissionObjectiveZoneSystem:rpc_event_mission_objective_zone_activate_zone(channel_id, level_unit_id)
	local is_level_unit = true
	local unit = Managers.state.unit_spawner:unit(level_unit_id, is_level_unit)
...
function MissionObjectiveZoneSystem:rpc_mission_objective_zone_finished(channel_id, unit_id)
	local is_level_unit = true
