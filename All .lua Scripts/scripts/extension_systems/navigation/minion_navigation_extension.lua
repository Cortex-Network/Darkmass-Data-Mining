...
function MinionNavigationExtension:init(extension_init_context, unit, extension_init_data, game_object_data)
...
function MinionNavigationExtension:_set_randomized_nav_tag_costs(nav_tag_cost_table, optional_randomized_nav_tag_costs)
...
function MinionNavigationExtension:_create_nav_bot(position, breed, traverse_logic, nav_world)
...
function MinionNavigationExtension:_init_blackboard_components(blackboard)
...
function MinionNavigationExtension:extensions_ready(world, unit)
...
function MinionNavigationExtension:destroy()
...
function MinionNavigationExtension:update(unit, t)
...
function MinionNavigationExtension:_update_destination(unit, nav_bot, t)
...
function MinionNavigationExtension:_update_desired_velocity(nav_bot)
...
function MinionNavigationExtension:_update_next_smart_object(nav_bot)
...
function MinionNavigationExtension:_update_dispersion(nav_bot)
...
function MinionNavigationExtension:update_position(unit)
	local nav_bot = self._nav_bot
...
function MinionNavigationExtension:traverse_logic()
	return self._traverse_logic
end

function MinionNavigationExtension:nav_world()
	return self._nav_world
end

function MinionNavigationExtension:set_enabled(enabled, max_speed)
...
function MinionNavigationExtension:set_avoidance_enabled(enabled)
	self._avoidance_enabled = enabled

	GwNavBot.set_use_avoidance(self._nav_bot, enabled)
end

function MinionNavigationExtension:add_movement_modifier(new_modifier)
...
function MinionNavigationExtension:remove_movement_modifier(id)
	local modifiers = self._movement_modifiers
	modifiers[id] = nil
...
function MinionNavigationExtension:_recalculate_max_speed()
...
function MinionNavigationExtension:set_max_speed(speed)
...
function MinionNavigationExtension:max_speed()
	return self._max_speed, self._max_speed_with_modifiers
end

function MinionNavigationExtension:set_nav_bot_position(position)
	GwNavBot.update_position(self._nav_bot, position)
end

function MinionNavigationExtension:move_to(position)
	self._wanted_destination:store(position)
end

function MinionNavigationExtension:stop()
	local unit = self._unit
	local position = POSITION_LOOKUP[unit]
...
function MinionNavigationExtension:failed_move_attempts()
	return self._failed_move_attempts
end

function MinionNavigationExtension:is_on_wait_time()
	local t = self._time_manager:time("gameplay")

	return t <= self._wait_timer
end

function MinionNavigationExtension:has_path()
	return self._has_path
end

function MinionNavigationExtension:is_computing_path()
	return self._is_computing_path
end

function MinionNavigationExtension:is_following_path()
	return self._is_following_path
end

function MinionNavigationExtension:destination()
	return self._wanted_destination:unbox()
end

function MinionNavigationExtension:distance_to_destination(optional_position)
...
function MinionNavigationExtension:distance_to_destination_sq(optional_position)
	local position = optional_position or Unit.local_position(self._unit, 1)
	local destination = self:destination()
...
function MinionNavigationExtension:has_reached_destination()
	local nav_bot = self._nav_bot

	return GwNavBot.has_reached_destination(nav_bot)
end

function MinionNavigationExtension:current_smart_object_data()
...
function MinionNavigationExtension:use_smart_object(do_use)
	local nav_bot = self._nav_bot
	local nav_smart_object_component = self._nav_smart_object_component
	local success = nil
...
function MinionNavigationExtension:is_using_smart_object()
	return self._is_using_smart_object
end

function MinionNavigationExtension:path_distance_to_next_smart_object(optional_max_distance)
...
function MinionNavigationExtension:allow_nav_tag_layers(layer_names, layer_allowed)
...
function MinionNavigationExtension:allow_nav_tag_layer(layer_name, layer_allowed)
	local layer_id = self._nav_mesh_manager:nav_tag_layer_id(layer_name)
	local nav_tag_cost_table = self._nav_tag_cost_table
...
function MinionNavigationExtension:set_nav_tag_layer_cost(layer_name, layer_cost)
	local layer_id = self._nav_mesh_manager:nav_tag_layer_id(layer_name)

	GwNavTagLayerCostTable.set_layer_cost_multiplier(self._nav_tag_cost_table, layer_id, layer_cost)
end

function MinionNavigationExtension:current_and_next_node_positions_in_path()
	local nav_bot = self._nav_bot
...
function MinionNavigationExtension:current_and_wanted_node_position_in_path(wanted_node_offset_index)
	local nav_bot = self._nav_bot
	local node_count = GwNavBot.get_path_nodes_count(nav_bot)
	local current_node_index = GwNavBot.get_path_current_node_index(nav_bot)
	local current_node_position = GwNavBot.get_path_node_pos(nav_bot, current_node_index)
...
function MinionNavigationExtension:remaining_distance_from_progress_to_end_of_path()
