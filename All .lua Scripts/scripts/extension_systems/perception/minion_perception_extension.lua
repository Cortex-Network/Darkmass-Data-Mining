...
function MinionPerceptionExtension:init(extension_init_context, unit, extension_init_data, game_object_data)
...
function MinionPerceptionExtension:_init_blackboard_components(blackboard)
...
function MinionPerceptionExtension:extensions_ready(world, unit)
...
function MinionPerceptionExtension:game_object_initialized(session, game_object_id)
...
function MinionPerceptionExtension:on_reload()
...
function MinionPerceptionExtension:last_los_position(target_unit)
...
function MinionPerceptionExtension:has_line_of_sight(target_unit)
	return self._line_of_sight_lookup[target_unit] or false
end

function MinionPerceptionExtension:has_line_of_sight_by_id(target_unit, id)
...
function MinionPerceptionExtension:immediate_line_of_sight_check(target_unit, optional_from_node_name, optional_to_node_name)
...
function MinionPerceptionExtension:cb_line_of_sight_hit(los_raycast_data, hit, hit_position)
...
function MinionPerceptionExtension:aggro()
...
function MinionPerceptionExtension:alert(enemy_unit)
	local perception_component = self._perception_component
...
function MinionPerceptionExtension:alert_nearby_allies(target_unit, optional_radius, optional_require_los, optional_max_distance_to_target)
	local unit = self._unit
...
function MinionPerceptionExtension:delayed_alert(enemy_unit, delay, optional_ignore_alerted_los)
...
function MinionPerceptionExtension:set_ignore_alerted_los(value)
	local perception_component = self._perception_component
	perception_component.ignore_alerted_los = value and true or false
end

function MinionPerceptionExtension:use_action_controlled_alert()
	return self._use_action_controlled_alert
end

function MinionPerceptionExtension:set_use_action_controlled_alert(value)
	self._use_action_controlled_alert = value
end

function MinionPerceptionExtension:set_threat_decay_enabled(enabled)
	self._threat_decay_disabled = not enabled
end

function MinionPerceptionExtension:force_new_target_attempt(optional_config)
	self._force_new_target_attempt_config = optional_config
	self._force_new_target_attempt = true
end

function MinionPerceptionExtension:_set_target_unit(target_unit)
	local perception_component = self._perception_component
...
function MinionPerceptionExtension:_update_aggro_state(target_changed, target_unit)
	if target_changed then
		local perception_component = self._perception_component
		local aggro_state = perception_component.aggro_state
...
function MinionPerceptionExtension:_update_target_selection(unit, side, perception_component, breed, dt, t, force_new_target_attempt, force_new_target_attempt_config)
...
function MinionPerceptionExtension:_update_line_of_sight(unit, target_units, los_collision_filter)
	local line_of_sight_queue = self._line_of_sight_queue
...
function MinionPerceptionExtension:_line_of_sight_check(unit, target_unit, los_collision_filter)
...
function MinionPerceptionExtension:destroy()
...
function MinionPerceptionExtension:_on_target_change(old_target_unit, new_target_unit)
...
function MinionPerceptionExtension:_update_priority_blackboard_status(unit)
	local perception_component = self._perception_component
...
function MinionPerceptionExtension:_decay_threat(unit, dt)
...
function MinionPerceptionExtension:threat(threat_unit)
	local threat_units = self._threat_units

	return threat_units[threat_unit]
end

function MinionPerceptionExtension:aggro_state()
	local perception_component = self._perception_component
	local aggro_state = perception_component.aggro_state
...
function MinionPerceptionExtension:add_threat(threat_unit, threat_to_add)
	local threat_config = self._threat_config
...
function MinionPerceptionExtension:update(unit, dt, t)
	local perception_component = self._perception_component
