...
function PerceptionSystem:init(...)
...
function PerceptionSystem:_init_line_of_sight_raycasts(physics_world)
...
function PerceptionSystem:physics_cb_line_of_sight_hit(los_data, id, hit, hit_position)
...
function PerceptionSystem:add_line_of_sight_raycast_query(unit, los_from_position, los_direction, los_distance, collision_filter)
...
function PerceptionSystem:register_extension_update(unit, extension_name, extension)
...
function PerceptionSystem:on_remove_extension(unit, extension_name)
	if extension_name ~= "BotPerceptionExtension" then
...
function PerceptionSystem:register_prioritized_unit_update(unit)
...
function PerceptionSystem:pre_update(context, dt, t)
	self:pre_update_extension("BotPerceptionExtension", dt, t, context)
end

function PerceptionSystem:update(context, dt, t)
...
function PerceptionSystem:on_reload()
	self:_preparse_bot_gestalt_target_selection_weights(BotGestaltTargetSelectionWeights)
end

function PerceptionSystem:_preparse_bot_gestalt_target_selection_weights(bot_gestalt_target_selection_weights)
