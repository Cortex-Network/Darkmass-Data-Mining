...
local function _calculate_score(breed, unit, target_unit, distance_sq, is_new_target, threat_units, t, perception_component, debug_target_weighting_or_nil)
	...
	if current_stacks > 0 and permanent_damage_taken_percent < 0.5 then
		score = score * current_stacks
	end
...
function target_selection_template.chaos_beast_of_nurgle(unit, side, perception_component, breed, target_units, line_of_sight_lookup, t, threat_units, force_new_target_attempt, force_new_target_attempt_config_or_nil, debug_target_weighting_or_nil)
