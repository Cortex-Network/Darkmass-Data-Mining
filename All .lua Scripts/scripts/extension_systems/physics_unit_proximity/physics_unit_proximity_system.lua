...
function PhysicsUnitProximitySystem:init(extension_system_creation_context, ...)
...
function PhysicsUnitProximitySystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
	local extension = {}
...
function PhysicsUnitProximitySystem:register_extension_update(unit, extension_name, extension)
	if extension_name == "PhysicsUnitProximityObserverExtension" then
		self._observers[unit] = -1
	end
end

function PhysicsUnitProximitySystem:on_remove_extension(unit, extension_name)
	if extension_name == "PhysicsUnitProximityObserverExtension" then
...
function PhysicsUnitProximitySystem:update(context, dt, t, ...)
...
function PhysicsUnitProximitySystem:post_update(context, dt, t, ...)
