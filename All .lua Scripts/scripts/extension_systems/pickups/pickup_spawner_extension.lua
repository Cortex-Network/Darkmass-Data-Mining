...
function PickupSpawnerExtension:setup_from_component(component, spawn_method, items, item_spawn_selection, spawn_nodes)
...
function PickupSpawnerExtension:on_gameplay_post_init(level)
	self._chest_extension = ScriptUnit.has_extension(self._unit, "chest_system")
end

function PickupSpawnerExtension:calculate_percentage_through_level()
...
function PickupSpawnerExtension:percentage_through_level()
	return self._percentage_through_level
end

function PickupSpawnerExtension:unit()
	return self._unit
end

function PickupSpawnerExtension:spawner_count()
	return #self._components
end

function PickupSpawnerExtension:can_spawn_pickup(component_index, pickup_name)
...
function PickupSpawnerExtension:_fetch_next_item(component_index)
	local component = self._components[component_index]
...
function PickupSpawnerExtension:_check_reserve(component_index, pickup_name)
...
function PickupSpawnerExtension:register_spawn_locations(node_list, distribution_type, pickup_settings)
	local num_components = #self._components
	local components = self._components
...
function PickupSpawnerExtension:spawn_guaranteed()
	local num_components = #self._components
	local components = self._components
...
function PickupSpawnerExtension:request_rubberband_pickup(component_index)
	local chest_extension = self._chest_extension
	local component = self._components[component_index]
...
function PickupSpawnerExtension:spawn_item(component_index)
...
function PickupSpawnerExtension:spawn_specific_item(component_index, pickup_name, check_reserve)
	component_index = component_index or 1
	local unit_item = nil
...
function PickupSpawnerExtension:spawned_item_picked_up(unit_item)
	Unit.flow_event(self._unit, "lua_item_picked_up")
end

function PickupSpawnerExtension:unspawn_item(item_unit)
	local pickup_system = self._pickup_system

	pickup_system:despawn_pickup(item_unit)
end

function PickupSpawnerExtension:spawn_position_rotation(component_index)
