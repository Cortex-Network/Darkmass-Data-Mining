...
function PickupSystem:extensions_ready(world, unit)
...
function PickupSystem:_shuffle(source)
	local seed = self._seed
	self._seed = table.shuffle(source, seed)
end

function PickupSystem:_check_main_path(unit)
...
function PickupSystem:_pickup_spawner_spawned(unit, pickup_spawner_extension)
...
function PickupSystem:get_rubberband_pickup(distribution_type, percentage_through_level, pickup_options)
	local AMMO = 1
	local GRENADE = 2
	local HEALTH = 3
...
function PickupSystem:_spawn_spread_pickups(distribution_type, pickup_pool)
	local num_spawners = #self._pickup_spawner_extensions
	local pickup_spawners = self._pickup_spawner_extensions
...
function PickupSystem:_check_spawn(spawner, pickup_type)
	local num_pickups_to_spawn = #pickups_to_spawn
	local spawner_extension = spawner.extension
...
function PickupSystem:spawn_pickup(pickup_name, position, rotation, pickup_spawner, placed_on_unit, spawn_interaction_cooldown)
...
function PickupSystem:player_spawn_pickup(pickup_name, position, rotation, player_session_id, placed_on_unit)
...
function PickupSystem:register_material_collected(pickup_unit, interactor_unit, type, size)
