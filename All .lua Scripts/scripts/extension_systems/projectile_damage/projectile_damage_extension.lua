...
function ProjectileDamageExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function ProjectileDamageExtension:_calculate_hit_mass()
...
function ProjectileDamageExtension:_reset_suppressed_units()
	self._suppressed_units = {}
end

function ProjectileDamageExtension:extensions_ready()
...
function ProjectileDamageExtension:fixed_update(unit, dt, t)
	...
	if fuse_started and fuse_damage_settings then
		...
		if new_life_time > fuse_time then
			...
			if fuse_liquid_area_template then
				LiquidArea.try_create(position, Vector3.down(), self._nav_world, fuse_liquid_area_template, owner_unit, nil, , weapon_item_or_nil)
			end
...
function ProjectileDamageExtension:on_impact(hit_position, hit_actor, hit_direction, hit_normal, current_speed, force_delete)
	local owner_unit = self._owner_unit
	local projectile_unit = self._projectile_unit
	local projectile_template = self._projectile_template
	local damage_settings = projectile_template.damage
	...
	if impact_damage_settings then
		...
		if not have_unit_been_hit and is_not_self then
			...
			if impact_liquid_area_template then
				LiquidArea.try_create(hit_position, hit_direction, self._nav_world, impact_liquid_area_template, owner_unit, nil, , weapon_item_or_nil)
...
function ProjectileDamageExtension:_spawn_cluster(cluster_settings, direction)
	local locomotion_extension = self._locomotion_extension
	local _, position = locomotion_extension:previous_and_current_positions()
	...
	for i = 1, number_of_clusters do
		...
		local projectile_unit, _ = Managers.state.unit_spawner:spawn_network_unit(nil, "item_projectile", position, random_rotation, material, item, projectile_template, starting_state, current_direction, speed, angular_velocity, owner_unit, is_critical_strike, origin_item_slot, nil, , , weapon_item_or_nil, fuse_override_time)
	end

	self._fx_extension:on_cluster()
end

function ProjectileDamageExtension:use_suppression()
	local projectile_template = self._projectile_template
	local damage_settings = projectile_template.damage
...
function ProjectileDamageExtension:on_suppression(hit_unit, hit_position)
	local owner_unit = self._owner_unit
	local projectile_template = self._projectile_template
	local damage_settings = projectile_template.damage
	local impact_damage_settings = damage_settings and damage_settings.impact
...
function ProjectileDamageExtension:get_origin_item_slot()
	return self._origin_item_slot
end

function ProjectileDamageExtension:_get_sticking_explosion_normal(sticking_to_unit, sticking_to_actor_index)
	local projectile_unit = self._projectile_unit
...
function ProjectileDamageExtension:_handle_explosion_achivements(player, explosion_attack_result_table)
