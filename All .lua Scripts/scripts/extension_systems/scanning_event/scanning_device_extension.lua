...
function ScanningDeviceExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function ScanningDeviceExtension:_setup_level_event_settings()
...
function ScanningDeviceExtension:extensions_ready(world, unit)
	if self._is_server then
		self:_create_game_object()
	end
end

function ScanningDeviceExtension:_create_game_object()
...
function ScanningDeviceExtension:on_game_object_created(game_object_id)
	self._go_id = game_object_id
end

function ScanningDeviceExtension:on_game_object_destroyed()
	self._go_id = nil
end

function ScanningDeviceExtension:enable()
	Unit.flow_event(self._unit, "lua_enable")
end

function ScanningDeviceExtension:disable()
	Unit.flow_event(self._unit, "lua_disable")
	self:_set_state(STATES.none)
end

function ScanningDeviceExtension:_enable_engine_fx()
	Unit.flow_event(self._unit, "lua_enable_engine_fx")
end

function ScanningDeviceExtension:_enable_scanning_fx()
	self._scanning_active = true

	Unit.flow_event(self._unit, "lua_enable_scanning_fx")
end

function ScanningDeviceExtension:_disable_scanning_fx()
	self._scanning_active = false

	Unit.flow_event(self._unit, "lua_disable_scanning_fx")
end

function ScanningDeviceExtension:travel_along_spline(spline, spline_name, end_position, end_rotation)
...
function ScanningDeviceExtension:_update_game_object()
	local game_session = self._game_session
...
function ScanningDeviceExtension:update(unit, dt, t)
...
function ScanningDeviceExtension:_move_to_spline(dt)
...
function ScanningDeviceExtension:_move_to_end_position(dt, direct_to_end_position)
	local end_position = self._end_position:unbox()
...
function ScanningDeviceExtension:_move_towards_position(delta, end_position, end_rotation, direct_to_end_position)
	local unit = self._unit
...
function ScanningDeviceExtension:_error_speed_calculation(dt, t, game, id, movement)
...
function ScanningDeviceExtension:_players_in_proximity()
...
function ScanningDeviceExtension:_set_state(state)
	self._current_state = state
end

function ScanningDeviceExtension:current_state()
	return self._current_state
end

function ScanningDeviceExtension:reached_end_of_spline()
	return self._current_movment_status == "end"
end

function ScanningDeviceExtension:finished_event()
	if self._is_server then
...
function ScanningDeviceExtension:_init_movement_spline(world, unit, spline_points, spline_name)
	local spline_curve = SplineCurve:new(spline_points, "Bezier", "SplineMovementHermiteInterpolatedMetered", spline_name, 10)

	return spline_curve
end

function ScanningDeviceExtension:past_spline_start_position()
	return self._past_spline_start_position
end

function ScanningDeviceExtension:at_end_position()
	return self._at_end_position
end

function ScanningDeviceExtension:hot_join_sync(past_spline_start_position, at_end_position, reached_end_of_spline)
