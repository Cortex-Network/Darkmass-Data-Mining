...
function ServoSkullExtension:init(extension_init_context, unit, extension_init_data, ...)
...
function ServoSkullExtension:setup_from_component(pulse_interval)
	self._pulse_interval = pulse_interval
end

function ServoSkullExtension:hot_join_sync(unit, sender)
...
function ServoSkullExtension:update(unit, dt, t)
...
function ServoSkullExtension:_zone_has_active_scannables()
...
function ServoSkullExtension:_recieve_scannables_from_active_zone()
...
function ServoSkullExtension:at_start_of_spline()
	self._pulse_timer = 0
...
function ServoSkullExtension:at_end_of_spline(last_spline)
	if self._is_server then
		local mission_objective_zone_system = self._mission_objective_zone_system
...
function ServoSkullExtension:player_nearby(player_nearby)
...
function ServoSkullExtension:do_pulse_fx()
...
function ServoSkullExtension:_do_pulse_feedback_fx_on_scannable(scannable_unit)
	if self._is_server then
		local script = ScriptUnit.has_extension(scannable_unit, "mission_objective_zone_scannable_system")
...
function ServoSkullExtension:set_servo_skull_state(servo_skull_state)
	if self._is_server then
		self._servo_skull_state = servo_skull_state
	end
end

function ServoSkullExtension:_vo_timer(dt)
...
function ServoSkullExtension:_play_vo(scanning_vo_line)
...
function ServoSkullExtension:_servo_skull_is_wandering()
