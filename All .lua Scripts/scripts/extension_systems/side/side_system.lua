...
function SideSystem:init(extension_system_creation_context, system_init_data, ...)
...
function SideSystem:destroy()
	Managers.event:unregister(self, "unit_died")
end

function SideSystem:_create_sides(side_compositions)
...
function SideSystem:_setup_relations(side_compositions, sides, side_lookup)
...
function SideSystem:on_add_extension(world, unit, extension_name, extension_init_data, game_object_data)
	local extension = {}
...
function SideSystem:register_extension_update(unit, extension_name, extension)
	extension.update_registered = true
end

function SideSystem:on_remove_extension(unit, extension_name)
...
function SideSystem:_event_unit_died(unit)
...
function SideSystem:sides()
	return self._sides
end

function SideSystem:side_names()
	return self._side_names
end

function SideSystem:get_side(side_id)
	return self._sides[side_id]
end

function SideSystem:get_side_from_name(name)
	return self._side_lookup[name]
end

function SideSystem:get_default_player_side_name()
	return self._default_player_side_name
end

function SideSystem:_add_unit_to_side(unit, side_id)
...
function SideSystem:_remove_unit_from_side(unit)
...
function SideSystem:_remove_unit_from_tag_units(unit, side_extension)
...
function SideSystem:is_ally(unit1, unit2)
...
function SideSystem:is_enemy(unit1, unit2)
	local side = self.side_by_unit[unit1]
...
function SideSystem:is_same_side(unit1, unit2)
...
function SideSystem:is_enemy_by_side(side1, side2)
	return side1.enemy_sides_lookup[side2] ~= nil
end

function SideSystem:pre_update(context, dt, t)
...
function SideSystem:add_aggroed_minion(unit)
	local side = self.side_by_unit[unit]
...
function SideSystem:remove_aggroed_minion(unit)
	local side = self.side_by_unit[unit]
	local enemy_sides = side:relation_sides("enemy")
	local num_enemy_sides = #enemy_sides
...
function SideSystem:_update_frame_tables(side)
...
function SideSystem:_update_enemy_frame_tables(side)
	local unit_extension_data = self._unit_extension_data
