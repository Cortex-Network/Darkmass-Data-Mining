...
function SlotSystem:init(extension_system_creation_context, ...)
...
function SlotSystem:on_gameplay_post_init(level)
...
function SlotSystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
	local extension = {}
...
function SlotSystem:_init_blackboard_values(unit, extension)
	local blackboard = BLACKBOARDS[unit]
...
function SlotSystem:register_extension_update(unit, extension_name, extension)
	if extension_name == "SlotExtension" then
		local target_units = self._target_units
...
function SlotSystem:on_remove_extension(unit, extension_name)
	local unit_extension_data = self._unit_extension_data
	local extension = unit_extension_data[unit]
...
function SlotSystem:do_slot_search(user_unit, set)
...
function SlotSystem:is_slot_searching(user_unit)
	local user_slot_extension = self._unit_extension_data[user_unit]
...
function SlotSystem:_improve_slot_position(user_unit, user_slot_extension, nav_world, traverse_logic, t)
...
function SlotSystem:user_unit_wait_slot_distance(user_unit)
	local user_slot_extension = self._unit_extension_data[user_unit]
...
function SlotSystem:user_unit_slot_position(user_unit)
	local user_slot_extension = self._unit_extension_data[user_unit]
...
function SlotSystem:user_unit_blocked_attack(user_unit)
	local user_slot_extension = self._unit_extension_data[user_unit]
...
function SlotSystem:user_unit_staggered(user_unit)
	local user_slot_extension = self._unit_extension_data[user_unit]
...
function SlotSystem:slots_count(unit, slot_type)
...
function SlotSystem:total_slots_count(unit, slot_type)
	local unit_extension = self._unit_extension_data[unit]
	local slot_data = unit_extension.all_slots[slot_type]
...
function SlotSystem:disabled_slots_count(unit, slot_type)
	local unit_extension = self._unit_extension_data[unit]
	local slot_data = unit_extension.all_slots[slot_type]
...
function SlotSystem:set_release_slot_lock(unit, release_slot_lock)
	local unit_extension = self._unit_extension_data[unit]
...
function SlotSystem:physics_async_update(context, dt, t)
	local target_units = self._target_units
	local target_units_n = #target_units
...
function SlotSystem:_update_target_slots(t, target_unit, target_units, unit_extension_data, target_slot_extension, nav_world, traverse_logic)
	local dist_sq = 0
...
function SlotSystem:_update_occupied_slots(unit_extension_data)
	local target_units = self._target_units
	local target_units_n = #target_units
...
function SlotSystem:_update_user_unit_blackboard_components(user_slot_extension)
	local slot = user_slot_extension.slot
...
function SlotSystem:_update_user_unit_slot(user_unit, user_slot_extension, target_units, unit_extension_data, nav_world, traverse_logic, t)
...
function SlotSystem:detach_user_unit(user_unit)
	local user_slot_extension = self._unit_extension_data[user_unit]
...
function SlotSystem:register_prioritized_user_unit_update(unit)
...
function SlotSystem:_prioritize_queued_units_on_slot(slot)
...
function SlotSystem:allow_nav_tag_layer(layer_name, layer_allowed)
...
function SlotSystem:is_traverse_logic_initialized()
