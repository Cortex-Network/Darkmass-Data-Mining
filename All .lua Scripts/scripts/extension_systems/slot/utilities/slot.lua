...
function Slot.create_slots(target_unit)
...
function Slot.remove(slot, unit_extension_data)
...
function Slot.detach_user_unit(user_unit, user_slot_extension, unit_extension_data)
...
function Slot.queue_position(unit_extension_data, slot, nav_world, distance_modifier, t, traverse_logic)
	local target_unit = slot.target_unit
...
function Slot.is_behind_target(slot, user_unit, target_unit_extension)
...
function Slot.clear_ghost_position(slot)
	slot.ghost_position:store(Vector3(0, 0, 0))
end

function Slot.set_ghost_position(target_unit_extension, slot, nav_world, traverse_logic)
	local slot_position = slot.absolute_position:unbox()
	local user_unit = slot.user_unit
	local user_unit_position = POSITION_LOOKUP[user_unit]
	local target_unit_position = target_unit_extension.position:unbox()
...
function Slot.release_check(slot, unit_extension_data)
...
function Slot.anchor(slot_type, target_unit, unit_extension_data)
	local target_unit_extension = unit_extension_data[target_unit]
	local slot_data = target_unit_extension.all_slots[slot_type]
	local target_slots = slot_data.slots
	local total_slots_count = slot_data.total_slots_count
...
local function _update_anchor_slot_position(slot, unit_extension_data, should_offset_slot, nav_world, traverse_logic, above, below, target_outside_navmesh)
	...
	if target_outside_navmesh then
		...
		position_on_navmesh, original_position = _get_reachable_slot_position_on_navmesh(slot, target_unit, target_unit_extension, target_position, wanted_position, nil, , should_offset_slot, nav_world, traverse_logic, above, below)
	end
...
local function _update_slot_position(target_unit, slot, slot_position, unit_extension_data, should_offset_slot, nav_world, traverse_logic, above, below, target_outside_navmesh)
	...
	if target_outside_navmesh then
		...
		position_on_navmesh, original_position = _get_reachable_slot_position_on_navmesh(slot, target_unit, target_unit_extension, target_position, slot_position, nil, , should_offset_slot, nav_world, traverse_logic, above, below)
	end
...
function Slot.update_target_slots_status(target_unit, target_units, unit_extension_data, nav_world, traverse_logic, outside_navmesh, t)
	local target_unit_extension = unit_extension_data[target_unit]
	local all_slots = target_unit_extension.all_slots
	...
	for i = 1, NUM_SLOT_TYPES do
		...
		for j = 1, total_slots_count do
			local slot = target_slots[j]
			local slot_position = slot.absolute_position:unbox()
			local is_on_navmesh = _update_slot_position(target_unit, slot, slot_position, unit_extension_data, should_offset_slot, nav_world, traverse_logic, nil, , outside_navmesh)
...
function Slot.update_target_slots_positions(target_unit, target_units, unit_extension_data, should_offset_slot, nav_world, traverse_logic, is_on_ladder, ladder_unit, bottom, top, target_outside_navmesh, t)
...
function Slot.find_best_slot(target_unit, target_units, user_unit, unit_extension_data, nav_world, t)
	local user_unit_position = POSITION_LOOKUP[user_unit]
	local user_unit_extension = unit_extension_data[user_unit]
	local target_unit_extension = unit_extension_data[target_unit]
...
function Slot.find_best_slot_to_wait_on(target_unit, user_unit, unit_extension_data, nav_world, t)
	local user_unit_extension = unit_extension_data[user_unit]
	local waiting_on_slot = user_unit_extension.wait_slot
	local target_unit_extension = unit_extension_data[target_unit]
	local slot_type = user_unit_extension.use_slot_type
...
function Slot.update_disabled_slots_count(target_units, unit_extension_data)
	local target_units_n = #target_units
...
function Slot.update_slot_sound(target_units)
	local target_units_n = #target_units
