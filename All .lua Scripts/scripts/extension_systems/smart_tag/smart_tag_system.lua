...
function SmartTagSystem:init(extension_system_creation_context, ...)
...
function SmartTagSystem:destroy()
	if self._is_server then
...
function SmartTagSystem:update(context, dt, t, ...)
	if self._is_server then
...
function SmartTagSystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
...
function SmartTagSystem:on_remove_extension(unit, extension_name)
...
function SmartTagSystem:set_tag(template_name, tagger_unit, target_unit, target_location)
...
function SmartTagSystem:set_contextual_unit_tag(tagger_unit, target_unit)
	local target_extension = self._unit_extension_data[target_unit]
...
function SmartTagSystem:cancel_tag(tag_id, remover_unit)
...
function SmartTagSystem:trigger_tag_interaction(tag_id, interactor_unit)
	local all_tags = self._all_tags
...
function SmartTagSystem:reply_tag(tag_id, replier_unit, reply_name)
	local tag = self._all_tags[tag_id]
...
function SmartTagSystem:unit_tag_id(unit)
	local extension = self._unit_extension_data[unit]
...
function SmartTagSystem:tag_by_id(tag_id)
	if not tag_id then
		return nil
	end
...
function SmartTagSystem:unit_tag(unit)
...
function SmartTagSystem:is_unit_tagged(unit)
	local tag_id = self:unit_tag_id(unit)

	return tag_id and true or false
end

function SmartTagSystem:location_tag_at_position(position, max_distance)
...
function SmartTagSystem:tagger_player_by_tag_id(tag_id)
	local tag = self._all_tags[tag_id]
...
function SmartTagSystem:_generate_tag_id()
...
function SmartTagSystem:_create_tag_locally(tag_id, template_name, tagger_unit, target_unit, target_location, replies, is_hotjoin_synced)
	local template = SmartTagSettings.templates[template_name]
...
function SmartTagSystem:_add_reply_locally(tag_id, replier_unit, reply)
	local tag = self._all_tags[tag_id]
...
function SmartTagSystem:_server_check_tag_group_limit(tagger_unit, group)
	local all_tags = self._all_tags
...
function SmartTagSystem:hot_join_sync(sender, channel)
...
function SmartTagSystem:rpc_request_set_smart_tag(channel_id, template_name_id, tagger_game_object_id, target_game_object_id, target_level_index, target_location)
...
function SmartTagSystem:rpc_set_smart_tag(channel_id, tag_id, template_name_id, tagger_game_object_id, target_game_object_id, target_level_index, target_location)
	local template_name = NetworkLookup.smart_tag_templates[template_name_id]
	local unit_spawner_manager = Managers.state.unit_spawner
...
function SmartTagSystem:rpc_set_smart_tag_hot_join(channel_id, tag_id, template_name_id, tagger_game_object_id, target_game_object_id, target_level_index, target_location, replier_array, reply_name_id_array)
	local template_name = NetworkLookup.smart_tag_templates[template_name_id]
	local unit_spawner_manager = Managers.state.unit_spawner
	local tagger_unit = tagger_game_object_id and unit_spawner_manager:unit(tagger_game_object_id)
	local target_unit = nil
...
function SmartTagSystem:rpc_remove_smart_tag(channel_id, tag_id, reason_id)
	local reason = REMOVE_TAG_REASONS_LOOKUP[reason_id]

	self:_remove_tag_locally(tag_id, reason)
end

function SmartTagSystem:rpc_request_cancel_smart_tag(channel_id, tag_id, remover_game_object_id)
	local tag = self._all_tags[tag_id]
...
function SmartTagSystem:rpc_request_smart_tag_reply(channel_id, tag_id, replier_game_object_id, reply_name_id)
	local tag = self._all_tags[tag_id]
...
function SmartTagSystem:rpc_smart_tag_reply(channel_id, tag_id, replier_game_object_id, reply_name_id)
	local replier_unit = Managers.state.unit_spawner:unit(replier_game_object_id)
	local reply_name = NetworkLookup.smart_tag_replies[reply_name_id]
	local reply = SmartTagSettings.replies[reply_name]
