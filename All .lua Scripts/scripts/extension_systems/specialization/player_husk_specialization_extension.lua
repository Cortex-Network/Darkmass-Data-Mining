...
function PlayerHuskSpecializationExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function PlayerHuskSpecializationExtension:_init_components()
...
function PlayerHuskSpecializationExtension:extensions_ready(world, unit)
...
function PlayerHuskSpecializationExtension:fixed_update(unit, dt, t, fixed_frame, context, ...)
...
function PlayerHuskSpecializationExtension:destroy()
	self._network_event_delegate:unregister_unit_events(self._game_object_id, unpack(RPCS))
end

function PlayerHuskSpecializationExtension:get_specialization_name()
	return self._specialization_name
end

function PlayerHuskSpecializationExtension:has_special_rule(special_rule_name)
	local active_special_rules = self._active_special_rules

	return active_special_rules[special_rule_name]
end

function PlayerHuskSpecializationExtension:_update_specialization_and_talents(specialization_name, talents)
	self._specialization_name = specialization_name
	self._talents = talents
...
function PlayerHuskSpecializationExtension:rpc_update_talents(channel_id, unit_id, specialization_name_id, talent_id_array)
