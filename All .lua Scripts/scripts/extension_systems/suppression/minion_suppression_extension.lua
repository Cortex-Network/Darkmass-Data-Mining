...
function MinionSuppressionExtension:init(extension_init_context, unit, extension_init_data, game_session, nil_or_game_object_id)
...
function MinionSuppressionExtension:_init_blackboard_components(blackboard, breed)
	local suppress_config = breed.suppress_config
...
function MinionSuppressionExtension:set_enabled(enabled)
	self._enabled = enabled
end

function MinionSuppressionExtension:game_object_initialized(game_session, game_object_id)
	self._game_object_id = game_object_id
end

function MinionSuppressionExtension:hot_join_sync(unit, sender, channel)
...
function MinionSuppressionExtension:update(unit, dt, t)
...
function MinionSuppressionExtension:add_suppress_value(value, suppression_type, attack_delay, direction, attacking_unit)
...
function MinionSuppressionExtension:_play_flinch_anim(breed, direction)
...
function MinionSuppressionExtension:_update_suppression(unit, blackboard, dt, t)
	local suppression_component = self._suppression_component
	local current_suppress_value = suppression_component.suppress_value
...
function MinionSuppressionExtension:_get_threshold_and_max_value()
...
function MinionSuppressionExtension:handle_unit_suppression(is_suppressed)
	self._is_suppressed = is_suppressed
end

function MinionSuppressionExtension:is_suppressed()
	return self._is_suppressed
end

function MinionSuppressionExtension:attack_delay()
	return self._attack_delay
end

function MinionSuppressionExtension:last_suppressing_unit()
