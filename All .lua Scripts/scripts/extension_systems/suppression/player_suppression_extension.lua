...
function PlayerSuppressionExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
...
function PlayerSuppressionExtension:extensions_ready(world, unit)
	self._buff_extension = ScriptUnit.extension(unit, "buff_system")
	self._mood_extension = ScriptUnit.has_extension(unit, "mood_system")
end

function PlayerSuppressionExtension:game_object_initialized(game_session, game_object_id)
	self._game_object_id = game_object_id
end

function PlayerSuppressionExtension:destroy()
	if not self._is_server then
		self._network_event_delegate:unregister_unit_events(self._game_object_id, unpack(CLIENT_RPCS))
	end
end

function PlayerSuppressionExtension:_init_components(unit_data_extension)
	self._movement_state_component = unit_data_extension:write_component("movement_state")
end

function PlayerSuppressionExtension:fixed_update(unit, dt, t)
...
function PlayerSuppressionExtension:_update_suppression_hits(dt, t)
...
function PlayerSuppressionExtension:_update_effects(dt, t)
...
function PlayerSuppressionExtension:add_suppression(suppression_hit_cost, hit_position)
	if not self._is_server then
		return
	end
...
function PlayerSuppressionExtension:_trigger_suppression_sounds(hit_position, num_suppression_hits)
...
function PlayerSuppressionExtension:rpc_player_suppressed(channel_id, unit_id, hit_position, num_suppression_hits, next_suppression_hit_clear_time)
	self:_trigger_suppression_sounds(hit_position, num_suppression_hits)
...
function PlayerSuppressionExtension:wwise_suppression_state()
	local num_suppression_hits = self._num_suppression_hits
...
function PlayerSuppressionExtension:clear_suppression()
