...
function MinionToughnessExtension:init(extension_init_context, unit, extension_init_data, game_object_data)
...
function MinionToughnessExtension:_init_blackboard_components(blackboard)
...
function MinionToughnessExtension:_initialize_toughness(unit, toughness_template)
...
function MinionToughnessExtension:_set_blackboard_values()
...
function MinionToughnessExtension:game_object_initialized(session, object_id)
...
function MinionToughnessExtension:destroy()
	local toughness_template = self._toughness_template
	local linked_actor_name = toughness_template.linked_actor
...
function MinionToughnessExtension:update(context, dt, t)
	self:_update_toughness(dt, t)
end

function MinionToughnessExtension:max_toughness()
	return self._max_toughness
end

function MinionToughnessExtension:toughness_damage()
	return self._toughness_damage
end

function MinionToughnessExtension:current_toughness_percent()
...
function MinionToughnessExtension:_update_toughness(dt, t)
	local toughness_template = self._toughness_template
...
function MinionToughnessExtension:add_damage(damage_amount, attack_result, hit_actor, damage_profile, attack_type, attack_direction, hit_world_position_or_nil)
	local toughness_template = self._toughness_template
	local toughness_damage = self._toughness_damage
	local max_toughness = self._max_toughness
...
function MinionToughnessExtension:stored_attacks()
	return self._stored_attacks
end

function MinionToughnessExtension:_explode(unit, explosion_template, power_level)
...
function MinionToughnessExtension:_store_toughness_attack_absorbed(damage_amount, impact_world_position)
	local attack = {
...
function MinionToughnessExtension:_set_linked_actor_active(linked_actor_name, active)
	local unit = self._unit
...
function MinionToughnessExtension:toughness_templates()
	return self._toughness_template, nil
end

function MinionToughnessExtension:is_stagger_immune()
	local toughness_template = self._toughness_template
