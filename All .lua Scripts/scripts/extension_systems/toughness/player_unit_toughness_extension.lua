...
function PlayerUnitToughnessExtension:init(extension_init_context, unit, extension_init_data, game_object_data)
	self._unit = unit
...
function PlayerUnitToughnessExtension:extensions_ready(world, unit)
	self._mood_extension = ScriptUnit.has_extension(unit, "mood_system")
end

function PlayerUnitToughnessExtension:game_object_initialized(session, object_id)
	self._game_session = session
	self._game_object_id = object_id
end

function PlayerUnitToughnessExtension:update(context, dt, t)
...
function PlayerUnitToughnessExtension:max_toughness_visual()
...
function PlayerUnitToughnessExtension:max_toughness()
	local buffs = self._buff_extension and self._buff_extension:stat_buffs()
...
function PlayerUnitToughnessExtension:toughness_damage()
	return self._toughness_damage
end

function PlayerUnitToughnessExtension:current_toughness_percent_visual()
...
function PlayerUnitToughnessExtension:current_toughness_percent()
	return 1 - self._toughness_damage / self:max_toughness()
end

function PlayerUnitToughnessExtension:_update_toughness(dt, t)
...
function PlayerUnitToughnessExtension:_initialize_toughness(toughness_template)
...
function PlayerUnitToughnessExtension:toughness_templates()
	local weapon_toughness_template = self._weapon_extension:toughness_template()

	return self._toughness_template, weapon_toughness_template
end

function PlayerUnitToughnessExtension:recover_toughness(recovery_type)
...
function PlayerUnitToughnessExtension:recover_percentage_toughness(fixed_percentage, ignore_stat_buffs, reason)
	if self:_toughness_regen_disabled() then
		return
	end
...
function PlayerUnitToughnessExtension:recover_max_toughness()
	if self:_toughness_regen_disabled() then
		return
	end
...
function PlayerUnitToughnessExtension:set_toughness_broken_time()
	local t = Managers.time:time("gameplay")
	self._toughness_broken_at_t = t
end

function PlayerUnitToughnessExtension:time_since_toughness_broken()
	local t = Managers.time:time("gameplay")

	return self._toughness_broken_at_t and t - self._toughness_broken_at_t or math.huge
end

function PlayerUnitToughnessExtension:add_damage(damage_amount, attack_result, hit_actor, damage_profile, attack_type, attack_direction, hit_world_position_or_nil)
...
function PlayerUnitToughnessExtension:_toughness_regen_disabled()
...
function PlayerUnitToughnessExtension:_record_stat(amount, reason)
