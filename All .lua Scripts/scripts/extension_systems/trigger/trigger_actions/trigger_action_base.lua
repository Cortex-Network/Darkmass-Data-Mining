...
function TriggerActionBase:init(is_server, volume_unit, parameters, trigger_action_name)
...
function TriggerActionBase:destroy()
end

function TriggerActionBase:on_activate(entering_unit, registered_units, dt, t)
...
function TriggerActionBase:on_deactivate(exiting_unit, registered_units)
	if not self._is_server then
		return
	end
...
function TriggerActionBase:on_unit_enter(entering_unit)
...
function TriggerActionBase:on_unit_exit(exiting_unit)
	local player_unit_spawn_manager = Managers.state.player_unit_spawn
...
function TriggerActionBase:_activate_on_unit(unit)
	local player_unit_spawn_manager = Managers.state.player_unit_spawn
...
function TriggerActionBase:_deactivate_on_unit(unit)
	local player_unit_spawn_manager = Managers.state.player_unit_spawn
	local player = player_unit_spawn_manager:owner(unit)
	local is_local_unit = not player or not player.remote
	local triggers_on_client = self._triggers_on_client
...
function TriggerActionBase:local_on_activate(unit)
end

function TriggerActionBase:local_on_deactivate(unit)
end

function TriggerActionBase:local_on_unit_enter(entering_unit)
	local volume_unit = self._volume_unit
...
function TriggerActionBase:local_on_unit_exit(exiting_unit)
	local volume_unit = self._volume_unit
