...
function TriggerExtension:init(extension_init_context, unit, ...)
	self._unit = unit
...
function TriggerExtension:setup_from_component(trigger_condition, condition_evaluates_bots, trigger_action, action_parameters, only_once, start_active, volume_type_or_nil, target_extension_name_or_nil)
...
function TriggerExtension:_register_volume()
	local unit = self._unit
...
function TriggerExtension:_unregister_volume()
	if self._volume_id ~= 0 then
...
function TriggerExtension:on_volume_enter(entering_unit, dt, t)
...
function TriggerExtension:on_volume_exit(exiting_unit)
...
function TriggerExtension:filter_passed(filter_unit)
...
function TriggerExtension:set_active(is_active)
	self._is_active = is_active
end

function TriggerExtension:local_action_activate(unit)
	self._trigger_action:local_on_activate(unit)
end

function TriggerExtension:local_action_deactivate(unit)
	self._trigger_action:local_on_deactivate(unit)
end

function TriggerExtension:local_action_on_unit_enter(entering_unit)
	self._trigger_action:local_on_unit_enter(entering_unit)
end

function TriggerExtension:local_action_on_unit_exit(exiting_unit)
	self._trigger_action:local_on_unit_exit(exiting_unit)
end

function TriggerExtension:update(unit, dt, t)
