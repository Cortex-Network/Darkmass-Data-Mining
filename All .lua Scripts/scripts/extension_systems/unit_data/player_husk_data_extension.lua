...
function PlayerHuskDataExtension:init(extension_init_context, unit, extension_init_data, game_session, game_object_id)
...
function PlayerHuskDataExtension:on_server_husk_data_state_game_object_created(game_object_id)
...
function PlayerHuskDataExtension:on_server_husk_hud_data_state_game_object_created(game_object_id)
...
function PlayerHuskDataExtension:breed()
	return self._breed
end

function PlayerHuskDataExtension:breed_name()
	local breed = self._breed

	return breed.name
end

function PlayerHuskDataExtension:archetype()
	return self._archetype
end

function PlayerHuskDataExtension:archetype_name()
	local archetype = self._archetype

	return archetype.name
end

function PlayerHuskDataExtension:specialization()
	return self._specialization
end

function PlayerHuskDataExtension:hit_zone(actor)
	return self._hit_zone_lookup[actor]
end

function PlayerHuskDataExtension:hit_zone_actors(hit_zone_name)
	return self._hit_zone_actors_lookup[hit_zone_name]
end

function PlayerHuskDataExtension:is_owned_by_death_manager()
	return false
end

function PlayerHuskDataExtension:has_component(component_name)
	return self._read_components[component_name] ~= nil
end

function PlayerHuskDataExtension:read_component(component_name)
	local read_component = self._read_components[component_name]

	return read_component
end

function PlayerHuskDataExtension:write_component(component_name)
	error("Writing to components is not supported on husks.")
end

function PlayerHuskDataExtension:is_local_unit()
	return false
end

function PlayerHuskDataExtension:pre_update()
	self:_read_server_husk_data_state()
end

function PlayerHuskDataExtension:_read_server_husk_data_state()
...
function PlayerHuskDataExtension:_create_read_component(component_name)
...
function PlayerHuskDataExtension:_initialize_components(components, components_husk_config, components_config, config)
