...
function PlayerUnitDataExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, nil_or_game_object_id)
	self._unit = unit
...
function PlayerUnitDataExtension:game_object_initialized(session, object_id)
...
function PlayerUnitDataExtension:on_server_data_state_game_object_created(session, game_object_id)
	self._server_data_state_game_object_id = game_object_id
end

local USERDATA_REPLACEMENT = {}

function PlayerUnitDataExtension:extensions_ready(world, unit)
...
function PlayerUnitDataExtension:_create_read_component(component_name)
...
function PlayerUnitDataExtension:has_component(component_name)
	return self._read_components[component_name] ~= nil
end

function PlayerUnitDataExtension:read_component(component_name)
...
function PlayerUnitDataExtension:write_component(component_name)
	return self._write_components[component_name]
end

function PlayerUnitDataExtension:_create_write_component(component_name)
	local config = self._component_config[component_name]
	local component = {
		__data = self._components[component_name],
		__blackboard = self._component_blackboard,
...
function PlayerUnitDataExtension:_setup_component_dependency(component_name, field_name, field_info, additional_data_config)
...
function PlayerUnitDataExtension:destroy()
...
function PlayerUnitDataExtension:breed()
	return self._breed
end

function PlayerUnitDataExtension:breed_name()
...
function PlayerUnitDataExtension:archetype_name()
...
function PlayerUnitDataExtension:specialization()
	return self._specialization
end

function PlayerUnitDataExtension:hit_zone(actor)
	return self._hit_zone_lookup[actor]
end

function PlayerUnitDataExtension:hit_zone_actors(hit_zone_name)
	return self._hit_zone_actors_lookup[hit_zone_name]
end

function PlayerUnitDataExtension:is_owned_by_death_manager()
	return false
end

function PlayerUnitDataExtension:last_received_fixed_frame()
	return self._last_received_frame
end

function PlayerUnitDataExtension:last_fixed_frame()
	return self._last_fixed_frame
end

function PlayerUnitDataExtension:is_local_unit()
	return self._is_local_unit
end

function PlayerUnitDataExtension:pre_update(unit, dt, t)
...
function PlayerUnitDataExtension:fixed_update(unit, dt, t, fixed_frame)
...
function PlayerUnitDataExtension:_copy_components(components, from, to)
...
function PlayerUnitDataExtension:post_update(unit, dt, t)
	if not self._is_server then
		return
	end
...
function PlayerUnitDataExtension:_read_server_unit_data_state(t)
	local server_data_state_game_object_id = self._server_data_state_game_object_id
