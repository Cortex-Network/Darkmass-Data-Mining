...
local unit_templates = {
	player_character = {
		...
		local_init = function (unit, config, template_context, game_object_data, player, breed, side_id, optional_starting_state, input_handler, random_seed, optional_damage, optional_permanent_damage, ...)
			...
			local is_social_hub = Managers.state.game_mode:is_social_hub()
			...
			local health = specialization.health
			local knocked_down_health = specialization.knocked_down_health
			local wounds = Managers.state.difficulty:player_wounds(specialization_name)
	...
	item_projectile = {
		...
		local_init = function (unit, config, template_context, game_object_data, item, projectile_template, starting_state, direction, speed, momentum_or_angular_velocity, owner_unit, is_critical_strike, origin_item_slot, charge_level, target_unit, target_position, weapon_item_or_nil, fuse_override_time)
			local is_server = template_context.is_server
			...
			config:add("ProjectileDamageExtension", {
				...
				fuse_override_time = fuse_override_time
			})
