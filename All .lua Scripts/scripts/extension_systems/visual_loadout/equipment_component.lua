...
function EquipmentComponent:init(world, item_definitions, unit_spawner, unit_3p, optional_extension_manager, optional_item_streaming_settings, optional_is_in_menu)
	self._world = world
...
function EquipmentComponent.initialize_equipment(slot_configuration, optional_slot_options)
	local equipment = {}
...
function EquipmentComponent:equip_item(unit_3p, unit_1p, slot, item, optional_existing_unit_3p, deform_overrides, optional_breed_name, optional_mission_template)
	slot.equipped = true
	slot.item = item
...
function EquipmentComponent:_slot_is_loaded(slot)
...
function EquipmentComponent:_spawn_item_units(slot, unit_3p, unit_1p, attach_settings, optional_mission_template)
...
function EquipmentComponent:unequip_item(slot)
	slot.equipped = false
	slot.item = nil
...
function EquipmentComponent:unequip_slot_dependencies(slot_config, equipment, slot_equip_order)
...
function EquipmentComponent:equip_slot_dependencies(equipment, slot_equip_order, items, body_deform_overrides, breed_name, character_unit_3p, character_unit_1p)
...
function EquipmentComponent:try_spawn_attachments(equipment, slot_equip_order, optional_mission_template)
...
function EquipmentComponent.resolve_profile_properties(equipment, wielded_slot, archetype_property, selected_voice_property)
	local properties = {
...
function EquipmentComponent.update_item_visibility(equipment, wielded_slot, unit_3p, unit_1p, first_person_mode)
...
function EquipmentComponent.wield_slot(slot, first_person_mode)
...
function EquipmentComponent.unwield_slot(slot, first_person_mode)
	local base_unit_1p = first_person_mode and slot.unit_1p
	local base_unit_3p = slot.unit_3p ~= nil and unit_alive(slot.unit_3p) and slot.unit_3p
	local base_unit = base_unit_1p or base_unit_3p or nil
...
function EquipmentComponent.send_component_event(slot, event_name, ...)
	local unit_3p = slot.unit_3p
