...
function PlayerHuskVisualLoadoutExtension:init(extension_init_context, unit, extension_init_data, game_session, game_object_id)
	self._unit = unit
...
function PlayerHuskVisualLoadoutExtension:extensions_ready(world, unit)
	WieldableSlotScripts.extensions_ready(self._wieldable_slot_scripts)
end

function PlayerHuskVisualLoadoutExtension:destroy()
...
function PlayerHuskVisualLoadoutExtension:update(unit, dt, t)
...
function PlayerHuskVisualLoadoutExtension:update_unit_position(unit, dt, t)
	local wielded_slot_name = self._wielded_slot
...
function PlayerHuskVisualLoadoutExtension:force_update_item_visibility()
	self:_update_item_visibility(self._is_in_first_person_mode)
end

function PlayerHuskVisualLoadoutExtension:_update_item_visibility(is_in_first_person_mode)
...
function PlayerHuskVisualLoadoutExtension:wield_slot(slot_name)
...
function PlayerHuskVisualLoadoutExtension:weapon_template_from_slot(slot_name)
	local slot = self._equipment[slot_name]

	return WeaponTemplate.weapon_template_from_item(slot and slot.item)
end

function PlayerHuskVisualLoadoutExtension:item_from_slot(slot_name)
	local slot = self._equipment[slot_name]
...
function PlayerHuskVisualLoadoutExtension:unit_3p_from_slot(slot_name)
	local slot = self._equipment[slot_name]
...
function PlayerHuskVisualLoadoutExtension:unwield_slot(slot_name)
	local slot_scripts = self._wieldable_slot_scripts[slot_name]
...
function PlayerHuskVisualLoadoutExtension:_equip_item_to_slot(slot_name, item, optional_existing_unit_3p)
	local equipment = self._equipment
...
function PlayerHuskVisualLoadoutExtension:rpc_player_equip_item_to_slot(channel_id, go_id, slot_id, item_id, optional_existing_unit_3p_id)
	local slot_name = NetworkLookup.player_inventory_slot_names[slot_id]
	local item_name = NetworkLookup.player_item_names[item_id]
...
function PlayerHuskVisualLoadoutExtension:rpc_player_unequip_item_from_slot(channel_id, go_id, slot_id)
	local slot_name = NetworkLookup.player_inventory_slot_names[slot_id]
	local equipment = self._equipment
	local slot = equipment[slot_name]
	local item = slot.item
...
function PlayerHuskVisualLoadoutExtension:source_fx_for_slot(slot_name)
	return self._fx_sources[slot_name]
end

function PlayerHuskVisualLoadoutExtension:resolve_gear_sound(sound_alias, optional_external_properties)
...
function PlayerHuskVisualLoadoutExtension:wieldable_slot_scripts_from_slot(slot_name)
	local wieldable_slot_scripts = self._wieldable_slot_scripts
...
function PlayerHuskVisualLoadoutExtension:wielded_weapon()
	local equipment = self._equipment
...
function PlayerHuskVisualLoadoutExtension:set_force_hide_wieldable_slot(slot_name, first_person, third_person)
	local slot = self._equipment[slot_name]
