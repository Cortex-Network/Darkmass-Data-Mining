...
function PlayerUnitVisualLoadoutExtension:init(extension_init_context, unit, extension_init_data, game_object_data_or_game_session, unit_spawn_parameter_or_game_object_id)
...
function PlayerUnitVisualLoadoutExtension:extensions_ready(world, unit)
...
function PlayerUnitVisualLoadoutExtension:game_object_initialized(session, object_id)
...
function PlayerUnitVisualLoadoutExtension:update(unit, dt, t)
...
function PlayerUnitVisualLoadoutExtension:post_update(unit, dt, t, context, ...)
...
function PlayerUnitVisualLoadoutExtension:fixed_update(unit, dt, t, frame)
	local inventory_component = self._inventory_component
	local wielded_slot_name = inventory_component.wielded_slot
...
function PlayerUnitVisualLoadoutExtension:update_unit_position(unit, dt, t)
	local inventory_component = self._inventory_component
	local wielded_slot_name = inventory_component.wielded_slot
	local wielded_slot_scripts = self._wieldable_slot_scripts[wielded_slot_name]
...
function PlayerUnitVisualLoadoutExtension:server_correction_occurred(unit, from_frame)
...
function PlayerUnitVisualLoadoutExtension:destroy()
...
function PlayerUnitVisualLoadoutExtension:hot_join_sync(unit, sender)
...
function PlayerUnitVisualLoadoutExtension:equip_item_to_slot(item, slot_name, optional_existing_unit_3p, t)
...
function PlayerUnitVisualLoadoutExtension:_equip_item_to_slot(item, slot_name, t, optional_existing_unit_3p, from_server_correction_occurred)
	local equipment = self._equipment
	local slot_config = self._slot_configuration[slot_name]
	local slot = equipment[slot_name]
...
function PlayerUnitVisualLoadoutExtension:unequip_item_from_slot(slot_name, fixed_frame)
	local from_server_correction_occurred = false
...
function PlayerUnitVisualLoadoutExtension:_unequip_item_from_slot(slot_name, from_server_correction_occurred, fixed_frame)
	local equipment = self._equipment
	local slot = equipment[slot_name]
	local item = slot.item
	local voice_fx_preset = item.voice_fx_preset
...
function PlayerUnitVisualLoadoutExtension:wield_slot(slot_name)
...
function PlayerUnitVisualLoadoutExtension:_wield_slot(slot_name)
	local equipment = self._equipment
	local slot = equipment[slot_name]
	local is_in_first_person_mode = self._is_in_first_person_mode
	local equipment_component = self._equipment_component
...
function PlayerUnitVisualLoadoutExtension:rewield_current_slot()
	local inventory = self._inventory_component
...
function PlayerUnitVisualLoadoutExtension:weapon_template_from_slot(slot_name)
	local slot = self._equipment[slot_name]
...
function PlayerUnitVisualLoadoutExtension:item_from_slot(slot_name)
	local slot = self._equipment[slot_name]
...
function PlayerUnitVisualLoadoutExtension:unit_3p_from_slot(slot_name)
	local slot = self._equipment[slot_name]
...
function PlayerUnitVisualLoadoutExtension:unwield_slot(slot_name)
	local inventory_component = self._inventory_component
...
function PlayerUnitVisualLoadoutExtension:_unwield_slot(slot_name)
	local equipment_component = self._equipment_component
	local equipment = self._equipment
	local slot_scripts = self._wieldable_slot_scripts[slot_name]
...
function PlayerUnitVisualLoadoutExtension:can_wield(slot_name)
	local slot_config = self._slot_configuration[slot_name]
...
function PlayerUnitVisualLoadoutExtension:slot_configuration()
	return self._slot_configuration
end

function PlayerUnitVisualLoadoutExtension:slot_configuration_by_type(slot_type)
	return self._slot_configuration_by_type[slot_type]
end

function PlayerUnitVisualLoadoutExtension:wielded_slot_configuration()
	local wielded_slot = self._inventory_component.wielded_slot

	return self._slot_configuration[wielded_slot]
end

function PlayerUnitVisualLoadoutExtension:is_wielding()
	return self._inventory_component.wielded_slot ~= self.NO_WIELDED_SLOT
end

function PlayerUnitVisualLoadoutExtension:source_fx_for_slot(slot_name)
	return self._fx_sources[slot_name]
end

function PlayerUnitVisualLoadoutExtension:force_update_item_visibility()
	self:_update_item_visibility(self._is_in_first_person_mode)
end

function PlayerUnitVisualLoadoutExtension:_update_item_visibility(first_person_mode)
...
function PlayerUnitVisualLoadoutExtension:resolve_gear_sound(sound_alias, optional_external_properties)
...
function PlayerUnitVisualLoadoutExtension:resolve_gear_particle(particle_alias, optional_external_properties)
	local properties = self._profile_properties

	return PlayerCharacterParticles.resolve_particle(particle_alias, properties, optional_external_properties)
end

function PlayerUnitVisualLoadoutExtension:current_wielded_slot_scripts()
	local inventory_component = self._inventory_component
	local current_wielded_slot = inventory_component.wielded_slot
...
function PlayerUnitVisualLoadoutExtension:wieldable_slot_scripts_from_slot(slot_name)
	local wieldable_slot_scripts = self._wieldable_slot_scripts

	return wieldable_slot_scripts[slot_name]
end

function PlayerUnitVisualLoadoutExtension:unit_and_attachments_from_slot(slot_name)
	local slot = self._equipment[slot_name]

	return slot.unit_1p, slot.unit_3p, slot.attachments_1p, slot.attachments_3p
end

function PlayerUnitVisualLoadoutExtension:_cache_node_names(weapon_template, slot_name)
...
function PlayerUnitVisualLoadoutExtension:_unit_and_node_from_node_name(slot_name, node_name)
...
function PlayerUnitVisualLoadoutExtension:unit_and_node_from_node_name(slot_name, node_name)
	local slot = self._equipment[slot_name]
	local cached_nodes = slot.cached_nodes
...
function PlayerUnitVisualLoadoutExtension:item_in_slot(slot_name)
	local slot = self._equipment[slot_name]
	local item = slot.item
...
function PlayerUnitVisualLoadoutExtension:slot_to_item(item)
	local equipment = self._equipment
...
function PlayerUnitVisualLoadoutExtension:wielded_weapon()
	local inventory = self._inventory_component
	local equipment = self._equipment
...
function PlayerUnitVisualLoadoutExtension:set_force_hide_wieldable_slot(slot_name, first_person, third_person)
	local slot = self._equipment[slot_name]
