...
function AimProjectileEffects:init(context, slot, weapon_template, fx_sources)
...
function AimProjectileEffects:_trajectory_settings_from_aim_action(action_settings, trajectory_settings)
...
function AimProjectileEffects:_trajectory_settings(t)
...
function AimProjectileEffects:update_unit_position(unit, dt, t)
	if not self._is_local_unit then
		return
	end
...
function AimProjectileEffects:_update_trajectory(trajectory_settings, dt, t)
...
function AimProjectileEffects:update_first_person_mode(first_person_mode)
end

function AimProjectileEffects:wield()
end

function AimProjectileEffects:unwield()
	self:_stop_trajectory()
end

function AimProjectileEffects:destroy()
	self:_stop_trajectory()
end
...
function AimProjectileEffects:_get_trajactory_data(integration_data, max_iterations, stop_on_impact, time_step_multiplier, max_number_of_bounces)
...
function AimProjectileEffects:_can_update_trajectory(trajectory_settings)
...
function AimProjectileEffects:_start_trajectory(trajectory_settings)
	self:_start_trajectory_spline(trajectory_settings)
end

function AimProjectileEffects:_stop_trajectory()
	self:_stop_trajectory_spline()
end

function AimProjectileEffects:_trajectory_is_active()
	local is_alive = nil
...
function AimProjectileEffects:_set_trajectory_positions(start_position, aim_data, number_of_aim_data, arc_offset, total_distance, arc_distances, dt)
...
function AimProjectileEffects:_can_update_trajectory_spline(trajectory_settings)
	return true
end

function AimProjectileEffects:_start_trajectory_spline(trajectory_settings)
...
function AimProjectileEffects:_stop_trajectory_spline()
...
function AimProjectileEffects:_trajectory_is_active_spline()
	return self._splines and #self._splines > 0
end

local position_tabel = {}

function AimProjectileEffects:_set_trajectory_positions_spline(start_position, aim_data, number_of_aim_data, arc_offset, total_distance, arc_distances, dt)
