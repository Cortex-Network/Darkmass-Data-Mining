...
function AuspexScanningEffects:init(context, slot, weapon_template, fx_sources)
...
function AuspexScanningEffects:wield()
end
...
function AuspexScanningEffects:update_unit_position(unit, dt, t)
	...
	local new_look = Quaternion.look(Matrix4x4.up(holo_orgin_world_pose), -Matrix4x4.forward(holo_orgin_world_pose))
	...
	if current_scan_mission_zone then
		...
		for i = 1, #scanable_units do
			...
			if is_current_active then
				...
				local world_holo_position = new_orgin + direction_on_holo_plane * holo_distance + holo_up * current_height
...
function AuspexScanningEffects:update_first_person_mode(first_person_mode)
...
function AuspexScanningEffects:has_outline_unit(unit, active)
	return self._has_outline_unit
end

function AuspexScanningEffects:_set_highlight_unit(unit, active)
	local scannable_unit_extension = unit and ScriptUnit.has_extension(unit, "mission_objective_zone_scannable_system")
...
function AuspexScanningEffects:__set_scanner_light(enebeled, scanner_light_components)
...
function AuspexScanningEffects:_set_scanner_light(enebeled, color)
...
function AuspexScanningEffects:_set_screen_active(enabled)
