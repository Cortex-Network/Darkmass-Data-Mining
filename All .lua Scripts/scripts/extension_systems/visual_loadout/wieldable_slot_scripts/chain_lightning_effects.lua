...
function ChainLightningEffects:init(context, slot, weapon_template, fx_sources)
...
function ChainLightningEffects:destroy()
	self:_reset()
end

function ChainLightningEffects:wield()
	self:_reset()
end

function ChainLightningEffects:unwield()
	self:_reset()
end

function ChainLightningEffects:fixed_update(unit, dt, t, frame)
end

function ChainLightningEffects:update_unit_position(unit, dt, t)
...
function ChainLightningEffects:_find_root_targets(t)
...
function ChainLightningEffects:_find_new_chain_targets(t, broadphase, enemy_side_names, max_angle, max_z_diff, max_jumps, radius, root_target, initial_travel_direction)
...
function ChainLightningEffects:_validate_targets(t)
...
function ChainLightningEffects:_find_new_targets(t)
...
function ChainLightningEffects:_update_fx()
...
function ChainLightningEffects:update_first_person_mode(first_person_mode)
...
function ChainLightningEffects:_update_vfx(t, world, current_particle_id, node_unit, node, target_unit, charge_level)
	if target_unit then
...
function ChainLightningEffects:_update_no_target_vfx(t, world, current_particle_id, has_any_particle_effect, node_unit, node)
...
function ChainLightningEffects:_update_hand_vfx(t, world, current_particle_id, node_unit, node, charge_level)
	if current_particle_id then
		return current_particle_id
	else
...
function ChainLightningEffects:_stop_vfx(world, particle_id)
	if particle_id then
...
function ChainLightningEffects:_reset()
	self._next_jump_time = 0
	self._target_1_right_particle_id = self:_stop_vfx(self._world, self._target_1_right_particle_id)
	self._target_2_right_particle_id = self:_stop_vfx(self._world, self._target_2_right_particle_id)
	self._target_3_right_particle_id = self:_stop_vfx(self._world, self._target_3_right_particle_id)
...
function ChainLightningEffects:_next_free_fx_table()
...
function ChainLightningEffects:_return_fx_table(fx_table)
	self._fx_tables[fx_table.index].active = false
end

function ChainLightningEffects:_update_sfx()
	local action_module_charge_component = self._action_module_charge_component
