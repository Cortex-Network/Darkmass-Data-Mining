...
function SweepTrail:init(context, slot, weapon_template, fx_sources, item)
...
function SweepTrail:fixed_update(unit, dt, t, frame)
end

function SweepTrail:update(unit, dt, t)
...
function SweepTrail:update_first_person_mode(first_person_mode)
	self._first_person_mode = first_person_mode
end

function SweepTrail:wield()
	local action_settings = Action.current_action_settings_from_component(self._weapon_action_component, self._weapon_actions)
...
function SweepTrail:destroy()
end

function SweepTrail:_update_windup(action_kind)
...
function SweepTrail:_update_trail_status(t, action_settings)
