...
function ThunderHammerEffects:destroy()
	self:_stop_sfx_loop()
	self:_stop_vfx_loop(true)
end

function ThunderHammerEffects:wield()
end

function ThunderHammerEffects:unwield()
	self:_stop_sfx_loop()
	self:_stop_vfx_loop(true)
end

function ThunderHammerEffects:fixed_update(unit, dt, t, frame)
end

function ThunderHammerEffects:update(unit, dt, t)
	self:_update_active()
end

function ThunderHammerEffects:update_first_person_mode(first_person_mode)
end

function ThunderHammerEffects:_update_active()
...
function ThunderHammerEffects:_start_sfx_loop()
...
function ThunderHammerEffects:_stop_sfx_loop()
	local wwise_world = self._wwise_world
	local sfx_source_id = self._fx_extension:sound_source(self._special_active_fx_source_name)
	local current_playing_id = self._looping_playing_id
...
function ThunderHammerEffects:_start_vfx_loop()
...
function ThunderHammerEffects:_stop_vfx_loop(destroy)
	local current_effect_id = self._looping_effect_id
