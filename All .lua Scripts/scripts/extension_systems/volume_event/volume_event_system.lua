...
function VolumeEventSystem:init(extension_system_creation_context, system_init_data, system_name, extension_list, ...)
...
function VolumeEventSystem:_require_level_volumes(level_name, volume_data)
...
function VolumeEventSystem:_create_event_volume_data(level_volumes)
...
function VolumeEventSystem:_add_nested_levels(level, levels)
...
function VolumeEventSystem:on_gameplay_post_init(main_level)
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:_register_level_volume(volume_name)
...
function VolumeEventSystem:register_unit_volume(unit, volume_name, extension_name, volume_type)
	local engine_volume_event_system = self._engine_volume_event_system
	local volume_events = volume_type_events[volume_type]
...
function VolumeEventSystem:unregister_unit_volume(volume_id, extension_name)
	local engine_volume_event_system = self._engine_volume_event_system

	VolumeEvent.unregister_volume(engine_volume_event_system, volume_id, extension_name)
end

function VolumeEventSystem:_unregister_level_volume(volume_name)
	local volume = self._level_volumes_by_name[volume_name]
	local engine_volume_event_system = self._engine_volume_event_system
...
function VolumeEventSystem:_update_traversal_cost(extension_name, volume_name, cost)
...
function VolumeEventSystem:_set_unit_nav_tag_layer_cost(unit, layer_name, layer_cost)
	local navigation_extension = ScriptUnit.extension(unit, "navigation_system")

	navigation_extension:set_nav_tag_layer_cost(layer_name, layer_cost)
end

function VolumeEventSystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
	local extension = {}
...
function VolumeEventSystem:extensions_ready(world, unit, extension_name)
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:on_remove_extension(unit, extension_name)
	self:_cleanup_extension(unit, extension_name)
end

function VolumeEventSystem:_cleanup_extension(unit, extension_name)
...
function VolumeEventSystem:update(context, dt, t)
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:destroy()
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:unregister_volumes_by_name(volume_name)
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:register_volumes_by_name(volume_name)
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:connect_unit_to_volume(volume_name, unit)
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:disconnect_unit_from_volume(volume_name, unit)
	if not self._is_server then
		return
	end
...
function VolumeEventSystem:engine_volume_event_system()
