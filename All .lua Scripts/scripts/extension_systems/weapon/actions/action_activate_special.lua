...
function ActionActivateSpecial:init(action_context, action_params, action_settings)
...
function ActionActivateSpecial:start(action_settings, t, time_scale, params)
	...
	if particle_name then
		...
		self._pre_activation_particle_id = self._fx_extension:spawn_unit_particles(particle_name, node_name, true, "stop", nil, , , false)
	end
end

function ActionActivateSpecial:fixed_update(dt, t, time_in_action)
...
function ActionActivateSpecial:finish(reason, data, t, time_in_action)
