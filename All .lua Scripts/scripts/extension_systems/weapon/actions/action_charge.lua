...
function ActionCharge:init(action_context, action_params, action_settings)
...
function ActionCharge:start(action_settings, t, time_scale, action_start_params)
...
function ActionCharge:update(dt, t)
...
function ActionCharge:_play_start_effects(t)
...
function ActionCharge:_play_charged_done_effects()
	local fx_extension = self._fx_extension
	local action_settings = self._action_settings
	local charge_effects = action_settings.charge_effects
	local fx_sources = self._fx_sources
...
function ActionCharge:finish(reason, data, t, time_in_action)
...
function ActionCharge:running_action_state(t, time_in_action)
	local charge_level = self._action_module_charge_component.charge_level
	local charge_template = self._weapon_extension:charge_template()
