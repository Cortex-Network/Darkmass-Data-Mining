...
function ActionPush:init(action_context, ...)
...
function ActionPush:start(...)
	ActionPush.super.start(self, ...)

	self._action_push_component.has_pushed = false
end

function ActionPush:fixed_update(dt, t, time_in_action)
...
function ActionPush:_play_push_particles(t)
