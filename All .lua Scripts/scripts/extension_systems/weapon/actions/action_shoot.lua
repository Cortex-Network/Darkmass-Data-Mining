...
function ActionShoot:init(action_context, action_params, action_settings)
...
function ActionShoot:start(action_settings, t, time_scale, params)
...
function ActionShoot:fixed_update(dt, t, time_in_action)
	local action_component = self._action_component
...
function ActionShoot:_next_fire_state(dt, t)
	local action_component = self._action_component
	local weapon_extension = self._weapon_extension
	local fire_rate_settings = _fire_rate_settings(weapon_extension)
...
function ActionShoot:_prepare_shooting(dt, t)
	local action_settings = self._action_settings
	local first_person_unit = self._first_person_unit
	local player_unit = self._player_unit
	local weapon_extension = self._weapon_extension
...
function ActionShoot:_spend_ammunition(dt, t, charge_level)
	local action_settings = self._action_settings
	local inventory_slot_component = self._inventory_slot_component
...
function ActionShoot:_add_heat(dt, t, charge_level)
...
function ActionShoot:_trigger_new_charge(t)
	local action_settings = self._action_settings
...
function ActionShoot:_shoot(position, rotation, power_level, charge_level)
end

function ActionShoot:_set_fire_state(t, new_fire_state)
	local action_component = self._action_component
	local shooting_status_component = self._shooting_status_component
...
function ActionShoot:finish(reason, data, t, time_in_action)
...
function ActionShoot:_rewind_ms(is_local_unit, player, position, direction, max_distance)
...
function ActionShoot:_trigger_wwise_event(event_name, tail_event_name, source_name, append_husk_to_event_name, charge_level_parameter_name, charge_level)
	local fx_extension = self._fx_extension
...
function ActionShoot:_play_shoot_sound()
	local action_settings = self._action_settings
	local muzzle_fx_source_name = self:_muzzle_fx_source()
...
function ActionShoot:_update_looping_shoot_sound()
	local looping_shoot_sound_component = self._looping_shoot_sound_component
...
function ActionShoot:_play_muzzle_flash_vfx(shoot_rotation, charge_level)
...
function ActionShoot:_play_muzzle_smoke()
	local fx = self._action_settings.fx
...
function ActionShoot:_check_for_auto_critical_strike()
...
function ActionShoot:_check_for_auto_critical_strike_end(t)
	local auto_fire = self._is_auto_fire_weapon
...
function ActionShoot:_update_sound_reflection()
...
function ActionShoot:_set_charge_animation_variable(first_person_unit, charge_level)
...
function ActionShoot:trigger_anim_event(anim_event, anim_event_3p)
...
function ActionShoot:_play_line_fx(line_effect, position, end_position)
...
function ActionShoot:_muzzle_fx_source()
	local action_settings = self._action_settings
...
function ActionShoot:_eject_fx_source()
	local action_settings = self._action_settings
	local fx = action_settings.fx
...
function ActionShoot:_handle_shot_concluded_stats()
	if Managers.stats.can_record_stats() then
