...
function ActionShootProjectile:init(action_context, action_params, action_settings)
...
function ActionShootProjectile:_shoot()
	...
	if self._is_server then
		local projectile_unit, _ = Managers.state.unit_spawner:spawn_network_unit(nil, "item_projectile", position, rotation, material, item, projectile_template, starting_state, direction, speed, momentum, owner_unit, is_critical_strike, origin_item_slot, nil, , , weapon_item)
	end
end

function ActionShootProjectile:_get_projectile_item()
	local action_settings = self._action_settings
	local fire_config = action_settings.fire_configuration
