...
function ActionSpawnProjectile:start(action_settings, t, ...)
	...
	if vfx_name then
		...
		self._particle_id = self._fx_extension:spawn_unit_particles(vfx_name, source_name, true, "stop", nil, , , false)
	end
...
function ActionSpawnProjectile:fixed_update(dt, t, time_in_action)
	if self._targeting_module then
...
function ActionSpawnProjectile:finish(reason, data, t, time_in_action)
...
function ActionSpawnProjectile:running_action_state(t, time_in_action)
	local action_settings = self._action_settings
	local use_ability_charge = action_settings.use_ability_charge
...
function ActionSpawnProjectile:_check_direction()
...
function ActionSpawnProjectile:_pay_ability_charge_cost()
	local action_settings = self._action_settings
	local ability_type = action_settings.ability_type
	local ability_extension = self._ability_extension
...
function ActionSpawnProjectile:_pay_for_projectile(t)
	local action_settings = self._action_settings
	local charge_component = self._charge_component
...
function ActionSpawnProjectile:_fire_projectile(t, time_difference_from_paying, projectile_locomotion_extension, offset)
	local action_settings = self._action_settings
	local unit = self._player_unit
