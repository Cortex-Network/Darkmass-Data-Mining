...
function ActionSweep:init(action_context, action_params, action_settings)
...
function ActionSweep:start(action_settings, t, time_scale, params)
...
function ActionSweep:_reset_sweep_component()
...
function ActionSweep:_calculate_max_hit_mass(damage_profile, power_level, charge_level, critical_strike)
...
function ActionSweep:server_correction_occurred()
	table.clear(self._hit_units)
...
function ActionSweep:finish(reason, data, t, time_in_action)
	table.clear(self._hit_units)
	...
	if special_implementation then
		special_implementation:on_action_finish(t, self._num_hit_enemies)
	end

	ActionSweep.super.finish(self, reason, data, t, time_in_action)
end

function ActionSweep:sensitivity_modifier(t)
	local action_settings = self._action_settings
	local special_active_at_start = self._weapon_action_component.special_active_at_start
	local hit_stickyness_settings = special_active_at_start and action_settings.hit_stickyness_settings_special_active or action_settings.hit_stickyness_settings
...
function ActionSweep:allow_chain_actions()
...
function ActionSweep:fixed_update(dt, t, time_in_action)
	local action_settings = self._action_settings

	self:_update_sweep(dt, t, time_in_action, action_settings, self._action_sweep_component)
end

function ActionSweep:_update_sweep(dt, t, time_in_action, action_settings, action_sweep_component)
...
function ActionSweep:_start_hit_stickyness(hit_stickyness_settings, t, attack_direction)
...
function ActionSweep:_stop_hit_stickyness(hit_stickyness_settings)
...
function ActionSweep:_is_currently_sticky()
	return self._action_sweep_component.is_sticky
end

function ActionSweep:_can_start_stickyness(hit_stickyness_settings, abort_attack, special_active)
	if not hit_stickyness_settings then
		return false
	end
...
function ActionSweep:_update_hit_stickyness(dt, t, action_sweep_component, hit_stickyness_settings)
	local stick_to_unit = action_sweep_component.sweep_aborted_unit
	...
	if not self._unit_data_extension.is_resimulating and damage and sticky_target_unit_alive then
		...
		if num_damage_instances_this_frame > 0 then
			...
			if attack_direction_override then
				attack_direction = _calculate_attack_direction(self._action_settings, nil, , player_rotation, attack_direction_override)
			end
			...
			for i = 1, num_damage_instances_this_frame do
				...
				if i == 1 then
					...
					ImpactEffect.play(stick_to_unit, stick_to_actor, damage_dealt, damage_type, hit_zone_name, attack_result, hit_world_position, hit_normal, attack_direction, self._player.player_unit, stickyness_impact_fx_data, nil, , damage_efficiency, damage_profile)
				end
			end
		end
	end
...
function ActionSweep:_handle_stickyness(stick_to_position)
...
function ActionSweep:_is_within_damage_window(time_in_action, action_settings, time_scale)
...
function ActionSweep:_exit_damage_window(t, num_hit_enemies)
	local special_implementation = self._weapon.special_implementation
...
function ActionSweep:_do_overlap(t, start_position, start_rotation, end_position, end_rotation, is_final_frame, action_settings, action_sweep_component)
	...
	local proximity_overlap_radius = ActionSweepSettings.proximity_overlap_radius

	PhysicsProximitySystem.prepare_for_overlap(physics_world, start_position, proximity_overlap_radius, rewind_ms)
...
function ActionSweep:_should_save_sweeps(action_settings, t, is_final_frame, num_new_entries, num_saved_entries)
...
function ActionSweep:_save_sweep_results(sweep_results, num_sweep_results, t, action_settings, num_saved_entries)
	local saved_entries = self._saved_entries

	if num_saved_entries == 0 then
		local num_frames_before_process = action_settings.num_frames_before_process
		self._time_before_processing_saved_entries = t + num_frames_before_process * GameParameters.fixed_time_step
	end
...
function ActionSweep:_merge_saved_entries(sweep_results, num_sweep_results, saved_entries, num_saved_entries)
...
function ActionSweep:_process_sweep_results(t, sweep_results, num_sweep_results, action_settings, attack_direction, action_sweep_component)
...
function ActionSweep:_pick_best_sweep_result_per_unit(sweep_results, num_sweep_results, hit_units)
...
function ActionSweep:_order_by_significance(sweep_units, sweep_results, actor_to_unit_map, action_settings)
...
function ActionSweep:_sweep_order(unit_table, sweep_units, actor_to_unit_map)
...
function ActionSweep:_context_aware_sweep_order(unit_table, sweep_units, actor_to_unit_map)
...
function ActionSweep:_current_max_hit_mass(inventory_slot_component)
...
function ActionSweep:_process_hit(t, hit_unit, hit_actor, hit_units, action_settings, hit_position, attack_direction, hit_zone_name_or_nil, hit_normal, action_sweep_component)
	...
	local amount_of_mass_hit = current_amount_of_mass_hit + target_hit_mass * (action_armor_hit_mass_mod or 1)
...
function ActionSweep:_do_damage_to_unit(damage_profile, hit_unit, hit_actor, hit_position, hit_normal, attack_direction, num_hit_enemies, hit_zone_name_or_nil, abort_attack, amount_of_mass_hit, damage_type, is_special_active)
...
function ActionSweep:_modify_sweep_position(position, rotation, weapon_half_extents, action_settings)
...
function ActionSweep:_run_sphere_sweeps(start_position, end_position, action_settings)
...
function ActionSweep:_run_sweeps(start_position, start_rotation, end_position, end_rotation, action_settings)
	...
	local sweep_results1 = PhysicsWorld.linear_obb_sweep(physics_world, sweep_1_start, sweep_1_end, sweep_1_extents, sweep_1_rot, max_num_hits1, "collision_filter", collision_filter, "report_initial_overlap")
	...
	local sweep_results2 = PhysicsWorld.linear_obb_sweep(physics_world, sweep_2_start, sweep_2_end, sweep_2_extents, sweep_2_rot, max_num_hits2, "collision_filter", collision_filter, "report_initial_overlap")
	...
	local sweep_results3 = PhysicsWorld.linear_obb_sweep(physics_world, sweep_3_start, sweep_3_end, sweep_3_extents, sweep_3_rot, max_num_hits3, "collision_filter", collision_filter, "report_initial_overlap")
...
function ActionSweep:_weapon_half_extents(weapon_template, action_settings)
...
function ActionSweep:_play_hit_animations(action_settings, abort_attack, armor_aborts_attack, special_active)
	...
	if abort_attack and special_active then
...
function ActionSweep:_play_hit_effects(damage_profile, hit_position, attack_direction, damage, result, damage_efficiency)
	local is_heavy = damage_profile.melee_attack_strength == melee_attack_strengths.heavy
