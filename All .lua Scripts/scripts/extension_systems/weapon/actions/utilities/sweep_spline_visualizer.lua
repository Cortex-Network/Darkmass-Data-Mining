...
function SweepSplineVisualizer.draw_splines(sweep_spline, drawer, time, combined_spline, control_splines, color)
	...
	while t < time do
		...
		local new_pos, _, new_outer_pos, new_inner_pos = sweep_spline:position_and_rotation(new_t / time)
		...
		prev_inner_pos = new_inner_pos
		prev_outer_pos = new_outer_pos
		prev_pos = new_pos
	end
end

function SweepSplineVisualizer.draw_point_on_spline(sweep_spline, drawer, t, show_rotation, color)
	color = color or Color.black()
	local pos, rot = sweep_spline:position_and_rotation(t)
