...
function PlayerUnitWeaponExtension:init(extension_init_context, unit, extension_init_data, ...)
	self._unit = unit
...
function PlayerUnitWeaponExtension:_init_action_components(unit_data_extension)
...
function PlayerUnitWeaponExtension:extensions_ready(world, unit)
...
function PlayerUnitWeaponExtension:on_player_unit_spawn(spawn_ammo_percentage)
...
function PlayerUnitWeaponExtension:on_player_unit_respawn(respawn_ammo_percentage)
	local unit_data_extension = self._unit_data_extension
...
function PlayerUnitWeaponExtension:_wielded_weapon(inventory_component, weapons)
	local wielded_slot = inventory_component.wielded_slot

	return weapons[wielded_slot]
end

function PlayerUnitWeaponExtension:update(unit, dt, t)
	self._action_handler:update(dt, t)
end

local NO_LERP_VALUES = {}

function PlayerUnitWeaponExtension:damage_profile_lerp_values(damage_profile_name_or_nil)
...
function PlayerUnitWeaponExtension:explosion_template_lerp_values(explosion_template_name_or_nil)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:fixed_update(unit, dt, t, fixed_frame)
...
function PlayerUnitWeaponExtension:on_wieldable_slot_equipped(item, slot_name, weapon_unit, fx_sources, t, optional_existing_unit_3p, from_server_correction_occurred)
...
function PlayerUnitWeaponExtension:on_wieldable_slot_unequipped(slot_name, from_server_correction_occurred)
...
function PlayerUnitWeaponExtension:on_slot_wielded(slot_name, t)
	local weapon = self._weapons[slot_name]
...
function PlayerUnitWeaponExtension:on_slot_unwielded(slot_name, t)
	local weapon = self._weapons[slot_name]
	local weapon_item = weapon.item
	local weapon_template = weapon.weapon_template
...
function PlayerUnitWeaponExtension:can_wield(slot_name)
	local weapons = self._weapons
...
function PlayerUnitWeaponExtension:_start_action(action_name, action_settings, t, used_input, transition_type)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:server_correction_occurred(unit)
	table.clear(action_params)
...
function PlayerUnitWeaponExtension:start_action(action_name, t)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
	local weapon_template = weapon.weapon_template
	local action_settings = Action.action_settings(weapon_template, action_name)
...
function PlayerUnitWeaponExtension:stop_action(reason, data, t, allow_reason_chain_action)
	local inventory_component = self._inventory_component
	local wielded_slot = inventory_component.wielded_slot
...
function PlayerUnitWeaponExtension:blocked_attack(attacking_unit, hit_world_position, block_broken, weapon_template, attack_type)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:get_shield_block_position(hit_world_position, attack_direction)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
	local weapon_template = weapon.weapon_template
...
function PlayerUnitWeaponExtension:_condition_func_params(wielded_slot)
...
function PlayerUnitWeaponExtension:update_weapon_actions(fixed_frame)
	local inventory_component = self._inventory_component
	local wielded_slot = inventory_component.wielded_slot
...
function PlayerUnitWeaponExtension:_update_weapon_special(weapon, dt, t)
...
function PlayerUnitWeaponExtension:_apply_buffs(buffs, buff_target, slot_name, inventory_slot_component, t, weapon_item)
...
function PlayerUnitWeaponExtension:_remove_buffs(buffs, buff_target, inventory_slot_component)
	local buff_list = buffs[buff_target]
	local num_buffs = #buff_list
...
function PlayerUnitWeaponExtension:_apply_stat_buffs(inventory_slot_component, slot_type)
	local buff_extension = self._buff_extension
...
function PlayerUnitWeaponExtension:_update_overheat(dt, t)
...
function PlayerUnitWeaponExtension:_update_stamina(dt, t, fixed_frame)
	Stamina.update(t, dt, self._stamina_component, self._specialization_stamina_template, self._unit, fixed_frame)
end

function PlayerUnitWeaponExtension:_update_ammo()
	local weapons = self._weapons
...
function PlayerUnitWeaponExtension:move_speed_modifier(t)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:has_running_action()
	return self._action_handler:has_running_action("weapon_action")
end

function PlayerUnitWeaponExtension:running_action_settings()
	return self._action_handler:running_action_settings("weapon_action")
end

function PlayerUnitWeaponExtension:weapon_template()
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:_weapon_tweak_template(template_type, template_name)
	local weapon = self:_wielded_weapon(self._inventory_component, self._weapons)
...
function PlayerUnitWeaponExtension:recoil_template()
...
function PlayerUnitWeaponExtension:sway_template()
...
function PlayerUnitWeaponExtension:spread_template()
...
function PlayerUnitWeaponExtension:suppression_template()
...
function PlayerUnitWeaponExtension:weapon_handling_template()
...
function PlayerUnitWeaponExtension:dodge_template()
...
function PlayerUnitWeaponExtension:movement_curve_modifier_template()
...
function PlayerUnitWeaponExtension:stagger_duration_modifier_template()
...
function PlayerUnitWeaponExtension:charge_template()
...
function PlayerUnitWeaponExtension:warp_charge_template()
...
function PlayerUnitWeaponExtension:sprint_template()
...
function PlayerUnitWeaponExtension:stamina_template()
...
function PlayerUnitWeaponExtension:toughness_template()
...
function PlayerUnitWeaponExtension:ammo_template()
...
function PlayerUnitWeaponExtension:burninating_template()
...
function PlayerUnitWeaponExtension:size_of_flame_template()
...
function PlayerUnitWeaponExtension:sensitivity_modifier()
	return self._action_handler:sensitivity_modifier("weapon_action", self._last_fixed_t)
end

function PlayerUnitWeaponExtension:rotation_contraints()
	return self._action_handler:rotation_contraints("weapon_action", self._last_fixed_t)
end

function PlayerUnitWeaponExtension:block_actions()
	self._action_handler:block_actions()
end

function PlayerUnitWeaponExtension:unblock_actions()
	self._action_handler:unblock_actions()
end

function PlayerUnitWeaponExtension:action_input_is_currently_valid(component_name, action_input, used_input, current_fixed_t)
	local inventory_component = self._inventory_component
	local wielded_slot = inventory_component.wielded_slot
