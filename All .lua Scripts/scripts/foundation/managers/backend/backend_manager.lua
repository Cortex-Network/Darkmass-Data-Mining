...
function BackendManager:init(default_headers_ctr)
...
function BackendManager:authenticated()
	return self._auth_cache and self._auth_cache.state == "fulfilled"
end

function BackendManager:authentication_failed()
...
function BackendManager:update(dt, t)
...
function BackendManager:on_reload(refreshed_resources)
	self.interfaces.master_data:on_reload(refreshed_resources)
end

function BackendManager:logout()
	if self._initialized then
		self._auth_cache = nil
...
function BackendManager:authenticate()
...
function BackendManager:get_auth_method()
	return Backend.get_auth_method()
end

function BackendManager:account_id()
...
function BackendManager:title_request(path, options)
...
function BackendManager:url_request(url, options)
...
function BackendManager:_not_initialized()
	local p = Promise:new()
...
function BackendManager:sync_time(t)
...
function BackendManager:sync_time_result(t, id, mapping)
...
function BackendManager:get_server_time(t)
...
function BackendManager:_time_sync_restart()
	self.server_time_game_start_epoch = nil
	self.time_sync_started_t = nil
	self.time_sync_state = TIME_SYNC_STATES.not_started
end

function BackendManager:failed_request()
