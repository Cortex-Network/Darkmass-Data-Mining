...
function BackendUtilities.fetch_embedded_links(body)
...
function BackendUtilities.wrap_paged_response(paged, prev_links)
...
function BackendUtilities.create_error(code, description)
	local t = {
		code = code,
...
function BackendUtilities.fetch_link(data, link_name, template_data)
...
function UrlBuilder:init(start_path)
	self.the_path = start_path or ""
	self.params = {}
end

function UrlBuilder:prefix_path(part)
	part = tostring(part)
...
function UrlBuilder:path(part)
	self.the_path = self.the_path .. tostring(part)

	return self
end

function UrlBuilder:query(param, value)
...
function BackendUtilities.url_builder(start_path)
	return UrlBuilder:new(start_path and tostring(start_path))
end

function BackendUtilities.make_account_title_request(hal_link, url_builder, options, requested_account_id, optional_top_node)
...
function BackendUtilities.build_account_path(hal_link, requested_account_id, optional_top_node)
...
function BackendUtilities.on_expiry_do(request_headers, on_expiry, pause_time)
