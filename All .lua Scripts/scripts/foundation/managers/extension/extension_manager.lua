...
function ExtensionManager:init(world, physics_world, wwise_world, nav_world, has_navmesh, level_name, circumstance_name, is_server, unit_templates, system_configuration, system_init_data, unit_category_list, network_event_delegate, fixed_frame_time, game_session, optional_soft_cap_out_of_bounds_units, use_time_slice)
...
function ExtensionManager:_units_added_and_registered()
...
function ExtensionManager:update_time_slice_init()
	return self._extension_system_holder:update_time_slice_init_systems()
end

function ExtensionManager:update_time_slice_post_init()
	return self._extension_system_holder:update_time_slice_post_init_systems()
end

function ExtensionManager:is_in_fixed_update()
	return self._in_fixed_update
end

function ExtensionManager:pre_update(dt, t)
	self._extension_system_holder:pre_update(dt, t)
end

function ExtensionManager:fixed_update(dt, t, frame)
...
function ExtensionManager:update()
	self._extension_system_holder:update()
end

function ExtensionManager:post_update()
	self._extension_system_holder:post_update()
end

function ExtensionManager:physics_async_update()
	self._extension_system_holder:physics_async_update()
end

function ExtensionManager:register_system(system, system_name, extension_list)
...
function ExtensionManager:system(system_name)
	return self._systems[system_name]
end

function ExtensionManager:has_system(system_name)
	return self._systems[system_name] ~= nil
end

function ExtensionManager:system_by_extension(extension_name)
	local system_name = self._extension_to_system_map[extension_name]

	return system_name and self._systems[system_name]
end

function ExtensionManager:units_by_category(category)
	local unit_map = self._unit_categories[category]

	return unit_map
end

function ExtensionManager:get_entities(extension_name)
	return self._extensions[extension_name] or EMPTY_TABLE
end

function ExtensionManager:units()
	return self._units
end

function ExtensionManager:destroy()
...
function ExtensionManager:add_unit_extensions_from_script_data(world, unit)
	local extension_config = ExtensionConfig:new()
...
function ExtensionManager:add_unit_extensions(world, unit, extension_config, game_object_data_or_session, ...)
...
function ExtensionManager:sync_unit_extensions(unit, session, object_id)
...
function ExtensionManager:hot_join_sync(sender, channel)
	self._extension_system_holder:hot_join_sync(sender, channel)
end

function ExtensionManager:on_gameplay_post_init(level, themes)
	self._extension_system_holder:on_gameplay_post_init(level, themes)
end

function ExtensionManager:init_time_slice_on_gameplay_post_init(level, themes)
	self._extension_system_holder:init_time_slice_on_gameplay_post_init(level, themes)
end

local TEMP_TABLE = {}

function ExtensionManager:register_unit(world, unit, optional_category)
	...
	if unit_template_name then
		...
		if self:add_unit_extensions_from_template(world, unit, init_function, unit_spawned_function_or_nil, nil, ) then
			TEMP_TABLE[1] = unit
...
function ExtensionManager:add_and_register_units(world, unit_list, num_units, optional_category)
...
function ExtensionManager:init_time_slice_add_and_register_level_units(world, unit_list, optional_category)
	local time_slice_unit_registration_data = self._time_slice_unit_registration_data
...
function ExtensionManager:update_time_slice_add_and_register_level_units()
	local time_slice_unit_registration_data = self._time_slice_unit_registration_data
...
function ExtensionManager:register_units_extensions(unit_list, num_units)
	local self_units = self._units
	local self_extensions = self._extensions
...
function ExtensionManager:remove_extensions_from_unit(unit, extensions_to_remove)
	local unit_extensions_list = self._unit_extensions_list
	local self_extensions = self._extensions
...
function ExtensionManager:remove_all_extensions_from_unit_except(unit, extension_exceptions)
	local unit_extensions_list = self._unit_extensions_list
	local extensions_list = unit_extensions_list[unit]
...
function ExtensionManager:unregister_unit_category(category)
	local unit_map = self._unit_categories[category]
	local i = 0
...
function ExtensionManager:unregister_units(units, num_units, unregister_from_level_units)
	local self_units = self._units
	local self_extensions = self._extensions
	local unit_extensions_list = self._unit_extensions_list
	local ignore_extensions_list = self._ignore_extensions_list
...
function ExtensionManager:add_ignore_extensions(ignore_extensions)
	local ignore_extensions_list = self._ignore_extensions_list
...
function ExtensionManager:unregister_unit(unit)
	TEMP_UNIT_TABLE[1] = unit

	self:unregister_units(TEMP_UNIT_TABLE, 1, true)
end

function ExtensionManager:initialize_client_fixed_frame(fixed_frame)
	self._extension_system_holder:initialize_client_fixed_frame(fixed_frame)
end

function ExtensionManager:latest_fixed_t()
...
function ExtensionManager:fixed_update_resimulate_unit(unit, from_frame, simulated_components)
	local extension_to_system_map = self._extension_to_system_map
	local extensions = self._units[unit]
...
function ExtensionManager:registered_level_units()
	return self._registered_level_units
end

function ExtensionManager:on_reload(refreshed_resources)
