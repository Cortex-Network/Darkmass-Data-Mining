...
function ExtensionSystemHolder:init(extension_system_creation_context, system_configuration, system_init_data, fixed_frame_time, use_time_slice)
...
function ExtensionSystemHolder:_init_systems(extension_system_creation_context, system_configuration, system_init_data)
...
function ExtensionSystemHolder:_add_system(context, system_init_data, name, class_name, has_pre_update, has_fixed_update, has_post_update, run_on_dedicated, run_in_runtime_loaded_level, extension_list, extension_list_ignore_on_dedicated)
...
function ExtensionSystemHolder:on_reload(refreshed_resources)
...
function ExtensionSystemHolder:pre_update(dt, t)
	self._dt = dt
	self._t = t
...
function ExtensionSystemHolder:fixed_update(dt, t, frame)
...
function ExtensionSystemHolder:initialize_client_fixed_frame(fixed_frame)
...
function ExtensionSystemHolder:latest_fixed_t()
	return self._system_update_context.fixed_frame * self._fixed_frame_time
end

local TEMP_SYSTEM_LIST = {}

function ExtensionSystemHolder:fixed_update_resimulate_unit(unit, from_frame, simulated_components, system_map, server_correction_system_map)
	local context = self._system_update_context
...
function ExtensionSystemHolder:hot_join_sync(sender, channel)
	for i, system in ipairs(self._systems) do
...
function ExtensionSystemHolder:on_gameplay_post_init(level, themes)
	for i, system in ipairs(self._systems) do
...
function ExtensionSystemHolder:is_init_ready()
...
function ExtensionSystemHolder:update_time_slice_init_systems()
	local init_data = self._init_data
...
function ExtensionSystemHolder:is_post_init_ready()
...
function ExtensionSystemHolder:init_time_slice_on_gameplay_post_init(level, themes)
	local post_init_data = self._post_init_data
...
function ExtensionSystemHolder:update_time_slice_post_init_systems()
	local post_init_data = self._post_init_data
...
function ExtensionSystemHolder:destroy()
