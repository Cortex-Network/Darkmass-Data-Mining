...
function FreeFlightManager:_setup_standard_camera(camera)
	camera.active = false
	camera.mode = "paused"
...
function FreeFlightManager:set_project_specific_teleporter(object)
	self._free_flight_teleporter = object
end

function FreeFlightManager:on_gameplay_shutdown()
	self:_clean_up()
end

function FreeFlightManager:_clean_up()
...
function FreeFlightManager:destroy()
	self:_clean_up()
...
function FreeFlightManager:set_input_source(source)
	self._input = source
end

function FreeFlightManager:_get_input()
...
function FreeFlightManager:update(dt, t)
...
function FreeFlightManager:_camera(camera_id)
...
function FreeFlightManager:_debug_print(str, ...)
	Log.debug(self.DEBUG_TAG, str, ...)
end

function FreeFlightManager:teleport_camera(camera_id, pos, rot)
...
function FreeFlightManager:camera_position_rotation(camera_id)
	local camera = self:_camera(camera_id)
...
function FreeFlightManager:_check_toggle(input)
...
function FreeFlightManager:is_in_free_flight()
	local global_camera = self._free_flight_cameras.global

	return global_camera.active
end

function FreeFlightManager:_check_camera(input, dt, camera_id, camera_data)
...
function FreeFlightManager:_exit_frustum_freeze(data, world, viewport)
...
function FreeFlightManager:_enter_frustum_freeze(data, world, viewport)
	local camera = nil
	local cam = ScriptViewport.camera(viewport)
...
function FreeFlightManager:_toggle_frustum_freeze(dt, data, world, viewport)
...
function FreeFlightManager:toggle_follow_path()
...
function FreeFlightManager:set_follow_path_speed(speed)
	self._follow_path:set_speed(speed)
end

function FreeFlightManager:_update_camera(input, dt, camera_data)
	local world = Managers.world:world(camera_data.viewport_world_name)
	local viewport = ScriptWorld.global_free_flight_viewport(world)
...
function FreeFlightManager:_handle_fov(input, camera)
...
function FreeFlightManager:_reset_dof(data)
...
function FreeFlightManager:_handle_dof(input, data, world)
...
function FreeFlightManager:_enter_global_free_flight(camera_data)
...
function FreeFlightManager:_exit_global_free_flight(camera_data)
	local world = Managers.world:world(camera_data.viewport_world_name)
...
function FreeFlightManager:_clear_free_flight_camera(camera_data)
	camera_data.active = false
	camera_data.viewport_world_name = nil
end
