...
function PlayerManager:set_network(is_server, network_event_delegate)
...
function PlayerManager:unset_network()
	if not self._is_server then
		self._network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function PlayerManager:claim_slot()
...
function PlayerManager:release_slot(slot)
	self._claimed_slots[slot] = nil
end

function PlayerManager:debug_remote_human_player_index(player)
	local index = 0
...
function PlayerManager:_generate_session_id(account_id, character_id)
...
function PlayerManager:player_from_session_id(session_id)
	return self._players_by_session_id[session_id]
end

function PlayerManager:_generate_unique_id(peer_id, local_player_id)
...
function PlayerManager:player_from_unique_id(unique_id)
	return self._players[unique_id]
end

function PlayerManager:on_game_state_enter(game_state_class, mapping, context)
...
function PlayerManager:init_time_slice_on_game_state_enter(game_state_class, mapping, context)
...
function PlayerManager:update_time_slice_on_game_state_enter()
...
function PlayerManager:on_game_state_exit(game_state_class)
	self._game_state = nil
	self._game_state_mapping = nil
	self._game_state_context = nil
...
function PlayerManager:on_reload(refreshed_resources)
	for player, game_state_object in pairs(self._game_state_objects) do
		game_state_object:on_reload(refreshed_resources)
	end
end

function PlayerManager:_add_player_game_state(player, mapping, context)
...
function PlayerManager:game_state_context()
	local game_state_context = self._game_state_context

	return game_state_context
end

function PlayerManager:_remove_player_game_state(player)
...
function PlayerManager:add_bot_player(player_class, channel_id, peer_id, local_player_id, profile, slot, ...)
...
function PlayerManager:add_human_player(player_class, channel_id, peer_id, local_player_id, profile, slot, account_id, ...)
...
function PlayerManager:add_player(player_class, channel_id, peer_id, local_player_id, profile, slot, account_id, ...)
...
function PlayerManager:player_exists(peer_id, local_player_id)
	local peer_table = self._players_by_peer[peer_id]

	return peer_table and peer_table[local_player_id or 1] or false
end

function PlayerManager:remove_player(peer_id, local_player_id)
...
function PlayerManager:_remove_human_player(unique_id, peer_id, local_player_id)
	self._human_players[unique_id] = nil
	self._num_human_players = self._num_human_players - 1
end

function PlayerManager:_remove_bot_player(unique_id)
	self._bot_players[unique_id] = nil
end

function PlayerManager:player(peer_id, local_player_id)
...
function PlayerManager:players_at_peer(peer_id)
...
function PlayerManager:bot_players()
	return self._bot_players
end

function PlayerManager:human_players()
	return self._human_players
end

function PlayerManager:human_player(unique_id)
	return self._human_players[unique_id]
end

function PlayerManager:player_by_unit(player_unit)
...
function PlayerManager:next_available_local_player_id(peer_id, start_index)
...
function PlayerManager:num_players()
	return self._num_players
end

function PlayerManager:num_human_players()
	return self._num_human_players
end

function PlayerManager:num_ready_human_players()
	local human_players = self._human_players
...
function PlayerManager:local_player(local_player_id)
	return self:player(Network.peer_id(), local_player_id)
end

function PlayerManager:local_player_safe(local_player_id)
...
function PlayerManager:print_players()
...
function PlayerManager:state_pre_update(dt, t)
...
function PlayerManager:state_fixed_update(dt, t, frame)
	for player, object in pairs(self._game_state_objects) do
...
function PlayerManager:state_update(dt, t)
	for player, object in pairs(self._game_state_objects) do
...
function PlayerManager:state_post_update(dt, t)
	for player, object in pairs(self._game_state_objects) do
...
function PlayerManager:state_render(dt, t)
	for player, object in pairs(self._game_state_objects) do
...
function PlayerManager:state_initialize_client_fixed_frame(frame)
	for player, object in pairs(self._game_state_objects) do
...
function PlayerManager:local_player_backend_profile()
...
function PlayerManager:create_sync_data(peer_id, include_profile_chunks)
...
function PlayerManager:create_players_from_sync_data(player_class, channel_id, peer_id, is_server, local_player_id_array, is_human_controlled_array, account_id_array, profile_chunks_array, player_session_id_array, slot_array)
