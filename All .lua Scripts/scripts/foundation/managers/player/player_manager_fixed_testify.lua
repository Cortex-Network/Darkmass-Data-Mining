...
function PlayerManagerFixedTestify.apply_weapon_progression_to_current_weapon_template(data, _, t)
...
function PlayerManagerFixedTestify.remove_weapon_progression_from_current_weapon_template(data, _, t)
	local player = data.player
	local weapon_system = Managers.state.extension:system("weapon_system")
	local new_modifiers = {}
...
function PlayerManagerFixedTestify.reset_grenade_charges(data, _, t)
	local player = data.player
	local player_unit = player.player_unit
	local unit_data_extension = ScriptUnit.extension(player_unit, "unit_data_system")
...
function PlayerManagerFixedTestify.reset_magazine_ammo(data, _, t)
	local player = data.player
	local player_unit = player.player_unit
	local unit_data_extension = ScriptUnit.extension(player_unit, "unit_data_system")
	local components = unit_data_extension._components
...
function PlayerManagerFixedTestify.trigger_animation_event(player_unit, animation_event)
...
function PlayerManagerFixedTestify.wait_for_action_completed(data, _, t)
	local player = data.player
	local player_unit = player.player_unit
	local unit_data_extension = ScriptUnit.extension(player_unit, "unit_data_system")
	local weapon_action_component = unit_data_extension:read_component("weapon_action")
...
function PlayerManagerFixedTestify.wield_slot(data, _, t)
