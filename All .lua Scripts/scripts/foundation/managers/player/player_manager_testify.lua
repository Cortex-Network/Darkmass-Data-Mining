...
function PlayerManagerTestify.teleport_bots_forward_on_main_path_if_blocked(bots_data, player_manager)
...
function PlayerManagerTestify.teleport_players_to_main_path_point(main_path_point, player_manager)
	local players = player_manager:players()
	local human_players = table.filter(players, function (player)
		return player:is_human_controlled()
	end)
...
function PlayerManagerTestify.teleport_player_to_position(position, player_manager)
	local players = player_manager:players()
	local human_players = table.filter(players, function (player)
		return player:is_human_controlled()
	end)
...
function PlayerManagerTestify.wait_for_bots_to_reach_position(wait_for_bots_position_data, player_manager)
...
function PlayerManagerTestify.wait_for_item_equipped(data, time)
	local TIMEOUT = 1
