...
function UnitSpawnerManager:_build_network_lookup(templates)
	local lookup = {}
	local i = 1
...
function UnitSpawnerManager:destroy()
...
function UnitSpawnerManager:set_gameobject_to_unit_creator_function(func)
	self._create_unit_from_gameobject_function = func
end

function UnitSpawnerManager:commit_and_remove_pending_units()
...
function UnitSpawnerManager:_add_pending_extensions()
...
function UnitSpawnerManager:mark_for_deletion(unit)
...
function UnitSpawnerManager:_remove_units_marked_for_deletion()
...
function UnitSpawnerManager:set_deletion_state(state)
	self._deletion_state = state
end

function UnitSpawnerManager:spawn_unit(unit_name, ...)
...
function UnitSpawnerManager:_create_unit_extensions(world, unit, extension_init_function, extension_unit_spawned_function_or_nil, game_object_data_or_session, is_husk, ...)
...
function UnitSpawnerManager:is_husk(unit)
	local is_husk = self._husk_units[unit]

	return is_husk
end

function UnitSpawnerManager:game_object_id_or_level_index(unit)
...
function UnitSpawnerManager:game_object_id(unit)
...
function UnitSpawnerManager:_add_network_unit(unit, game_object_id, is_husk)
	self._game_object_ids[unit] = game_object_id
	self._network_units[game_object_id] = unit
	self._husk_units[unit] = is_husk
end

function UnitSpawnerManager:_remove_network_unit(unit)
...
function UnitSpawnerManager:make_network_unit_local_unit(unit)
...
function UnitSpawnerManager:spawn_network_unit(unit_name, unit_template_name, position, rotation, material, ...)
...
function UnitSpawnerManager:_spawn_unit_with_extensions(unit_name, unit_template_name, position, rotation, material, game_object_data, ...)
...
function UnitSpawnerManager:_world_delete_units(units_list, num_units)
...
function UnitSpawnerManager:_world_delete_unit(unit)
	TEMP_UNIT_TABLE[1] = unit

	self:_world_delete_units(TEMP_UNIT_TABLE, 1)
end

function UnitSpawnerManager:spawn_husk_unit(game_object_id, owner_id)
...
function UnitSpawnerManager:register_runtime_loaded_level(level)
...
function UnitSpawnerManager:unregister_runtime_loaded_level(level)
	local level_name_hash = Level._name_hash_32(level)
	self._runtime_loaded_levels[level_name_hash] = nil
end

function UnitSpawnerManager:destroy_game_object_unit(game_object_id, owner_id)
