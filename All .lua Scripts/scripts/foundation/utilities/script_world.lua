...
function ScriptWorld.create_viewport(world, name, template, layer, camera_unit, position, rotation, add_shadow_cull_camera, shading_environment_name, shading_callback, mood_setting, render_targets)
...
function ScriptWorld.change_camera_unit(viewport, camera_unit, add_shadow_cull_camera)
	local camera = Unit.camera(camera_unit, "camera")
...
function ScriptWorld.has_viewport(world, name)
	local viewports = World.get_data(world, "viewports")

	return viewports[name] and true or false
end

function ScriptWorld.viewport(world, name, return_free_flight_viewport)
	local viewport = nil
...
function ScriptWorld.create_free_flight_viewport(world, overridden_viewport_name, template, shading_environment_name, shading_callback, mood_setting)
...
function ScriptWorld.update(world, dt, anim_callback, scene_callback)
...
function ScriptWorld.create_shading_environment(world, viewport, shading_environment_name, shading_callback, mood_setting)
...
function ScriptWorld.destroy_shading_environment(world, viewport)
	local shading_environment = Viewport.get_data(viewport, "shading_environment")
...
function ScriptWorld.spawn_level(world, name, object_sets, position, rotation, spawn_units, ignore_background)
...
function ScriptWorld._register_nested_levels(levels, parent, last_count)
...
function ScriptWorld._unregister_nested_levels(levels, parent_level)
...
function ScriptWorld.level_id(world, level)
	local levels = World.get_data(world, "levels")
...
function ScriptWorld.level_from_id(world, level_id)
	local levels = World.get_data(world, "levels")
...
function ScriptWorld.level_id_from_name(world, name)
	local levels = World.get_data(world, "levels")
...
function ScriptWorld.register_sub_levels_spawned_callback(world, level, cb)
	local levels = World.get_data(world, "levels")
...
function ScriptWorld.trigger_sub_levels_spawned(world, level)
	local levels = World.get_data(world, "levels")
