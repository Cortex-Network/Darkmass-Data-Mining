...
function GameplayStateRun:on_enter(parent, params)
...
function GameplayStateRun:on_exit()
...
function GameplayStateRun:_do_activate_boons()
	local shared_state = self._shared_state
	local is_server = shared_state.is_server
...
function GameplayStateRun:update(main_dt, main_t)
	local shared_state = self._shared_state
	local is_server = shared_state.is_server
	local is_dedicated_server = shared_state.is_dedicated_server
	self._fixed_frame_parsed = false
...
function GameplayStateRun:post_update(main_dt, main_t)
	local shared_state = self._shared_state
	local physics_world = shared_state.physics_world
	local is_server = shared_state.is_server
...
function GameplayStateRun:render()
	Managers.player:state_render(Managers.time:delta_time("main"), Managers.time:time("main"))
end

function GameplayStateRun:on_reload(refreshed_resources)
	Managers.state.extension:on_reload(refreshed_resources)
end

function GameplayStateRun:_register_run_network_events(is_server)
	if not is_server then
...
function GameplayStateRun:_unregister_run_network_events(is_server)
	if not is_server then
		local connection_manager = Managers.connection
		local network_event_delegate = connection_manager:network_event_delegate()
...
function GameplayStateRun:_music_notify_shutdown(is_dedicated_server)
	if not is_dedicated_server then
		Managers.wwise_game_sync:on_gameplay_shutdown()
	end
end

function GameplayStateRun:_player_notify_shutdown(gameplay_state)
	Managers.player:unset_network()
	Managers.player:on_game_state_exit(gameplay_state)
end

function GameplayStateRun:_unregister_network_events(is_server)
	local connection_manager = Managers.connection
	local network_event_delegate = connection_manager:network_event_delegate()
...
function GameplayStateRun:_destroy_nvidia_ai_agent(is_dedicated_server, out_shared_state)
	if GameParameters.nvidia_ai_agent and not is_dedicated_server then
...
function GameplayStateRun:_despawn_units(is_server, world, level, themes)
	local extension_manager = Managers.state.extension
...
function GameplayStateRun:_destroy_nav_world(out_shared_state)
...
function GameplayStateRun:_unregister_timer(is_server, out_shared_state)
	if is_server then
...
function GameplayStateRun:_deinit_frame_rate()
	Managers.frame_rate:relinquish_request("gameplay")
end

function GameplayStateRun:_handle_world_fullscreen_blur()
	local ui_manager = Managers.ui
...
function GameplayStateRun:_handle_world_enabled_state()
	local ui_manager = Managers.ui
...
function GameplayStateRun:_fixed_update(extension_manager, player_manager, player_unit_spawn_manager, t)
	local shared_state = self._shared_state
...
function GameplayStateRun:_handle_load_failures()
...
function GameplayStateRun:_handle_session_disconnects()
...
function GameplayStateRun:rpc_sync_clock(channel_id, time, offset)
