...
function StateMainMenu:on_enter(parent, params, creation_context)
...
function StateMainMenu:_stay_in_main_menu(profiles, selected_profile)
	self:_set_view_state_cb("main_menu")
	self:_set_profiles(profiles)
	self:_set_selected_profile(selected_profile)
end

function StateMainMenu:_register_menu_events()
...
function StateMainMenu:_unregister_menu_events()
...
function StateMainMenu:event_request_select_new_profile(profile)
	if profile ~= self._selected_profile then
		self:_set_selected_profile(profile)
	end
end

function StateMainMenu:event_request_delete_character(character_id)
...
function StateMainMenu:event_main_menu_entered()
...
function StateMainMenu:event_create_new_character_start()
	self:_create_new_character_start()
end

function StateMainMenu:event_create_new_character_continue(skip_onboarding)
	self:_next_character_create_view(skip_onboarding)
end

function StateMainMenu:event_create_new_character_back()
	self:_previous_character_create_view()
end

function StateMainMenu:_start_game_or_onboarding()
...
function StateMainMenu:event_continue_cb()
	local selected_profile = self._selected_profile
...
function StateMainMenu:_skip_prologue()
	return GameParameters.skip_prologue
end

function StateMainMenu:_refresh_profiles()
...
function StateMainMenu:_set_profiles(profiles)
...
function StateMainMenu:_set_selected_profile(profile)
...
function StateMainMenu:_create_new_character_start()
...
function StateMainMenu:new_character_create()
	self._character_create = CharacterCreate:new(self._item_definitions, self._gear)

	return self._character_create
end

function StateMainMenu:in_character_create_state()
	return self._character_create ~= nil
end

function StateMainMenu:_waiting_for_profile_synchronization()
...
function StateMainMenu:waiting_for_profile_synchronization()
	return self:_waiting_for_profile_synchronization()
end

function StateMainMenu:_previous_character_create_view()
...
function StateMainMenu:_next_character_create_view(skip_onboarding)
...
function StateMainMenu:set_wait_for_character_profile_upload(value)
	self._wait_for_character_profile_upload = value
end

function StateMainMenu:_on_profile_create_completed(created_profile)
	self._character_create_state_views = nil
...
function StateMainMenu:_close_current_character_create_state_views()
...
function StateMainMenu:_open_character_create_state_views(index)
	...
	if next_views then
		...
		for i = 1, #next_views do
			...
			Managers.ui:open_view(next_view, nil, , , , view_context)
...
function StateMainMenu:_close_current_state_views()
...
function StateMainMenu:_set_view_state_cb(state)
	...
	if new_state_views then
		...
		for i = 1, #new_state_views do
			...
			Managers.ui:open_view(view_name, nil, , , , view_context)
		end
	end
...
function StateMainMenu:_start_game()
	self:_unregister_menu_events()
...
function StateMainMenu:update(main_dt, main_t)
	local ui_manager = Managers.ui
...
function StateMainMenu:_start_onboarding(mission_name)
	self._onboarding_mission_name = mission_name

	self:_start_game()
end

function StateMainMenu:_should_open_news()
...
function StateMainMenu:on_exit()
...
function StateMainMenu:_rejoin_game()
	local selected_profile = self._selected_profile
...
function StateMainMenu:_show_reconnect_popup()
	local context = {
