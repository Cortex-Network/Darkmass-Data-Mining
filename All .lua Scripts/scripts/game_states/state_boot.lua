...
function StateBoot:on_enter(parent, params)
	...
	self._sm = GameStateMachine:new(self, params.states[1][1], sub_state_params)
...
function StateBoot:_create_startup_world()
...
function StateBoot:gui()
	return self._gui
end

function StateBoot:sub_states_done()
	self._done = true
end

function StateBoot:on_exit()
	self._sm:destroy()
