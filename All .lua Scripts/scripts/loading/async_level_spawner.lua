...
function AsyncLevelSpawner:init(world_name, level_name, parameters, frame_time_budget, optional_world)
...
function AsyncLevelSpawner:destroy()
	if self._level then
...
function AsyncLevelSpawner:update()
...
function AsyncLevelSpawner.setup_world(world_name, parameters)
	local flags = {
...
function AsyncLevelSpawner:level_name()
