...
function LoadingRemoteStateMachine:init(network_delegate, client_channel_id, spawn_queue, done_loading_level_func)
	local shared_state = {
		state = "loading",
...
function LoadingRemoteStateMachine:peer_id()
	return self._shared_state.client_peer_id
end

function LoadingRemoteStateMachine:state()
	return self._shared_state.state
end

function LoadingRemoteStateMachine:spawn_group_ready(spawn_group)
...
function LoadingRemoteStateMachine:update(dt)
