...
function LocalLevelState:init(state_machine, shared_state)
...
function LocalLevelState:update(dt)
...
function LocalLevelState:_spawn_level(level_name)
	local world_parameters = {
		timer_name = "gameplay",
		layer = 1
	}
	local shared_state = self._shared_state
	shared_state.level_spawner = AsyncLevelSpawner:new("level_world", level_name, world_parameters, GameplayInitTimeSlice.MAX_DT_IN_SEC, shared_state.world)
end
