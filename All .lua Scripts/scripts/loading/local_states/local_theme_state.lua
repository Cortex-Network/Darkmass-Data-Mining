...
local LocalThemeState = class("LocalThemeState")

function LocalThemeState:init(state_machine, shared_state)
	...
	if circumstance_name then
		...
		Log.error("LocalThemeState", "[init] 'default' circumstance name is not provided. %s", Script.callstack())
	end
end

function LocalThemeState:update(dt)
	return "load_done"
end
