...
function XboxPrivileges:init()
	self:reset()
end

function XboxPrivileges:reset()
...
function XboxPrivileges:fetch_all_privileges(user_id, fetch_done_cb)
	self:reset()
...
function XboxPrivileges:update_privileges(user_id)
	for _, privilege in pairs(DEFAULT_PRIVILEGES) do
		local has_privilege, deny_reason, resolution_required = XUser.check_privilege(user_id, XUserPrivilegeOptions.None, privilege)
		self._privileges_data[privilege] = {
			has_privilege = has_privilege,
...
function XboxPrivileges:_cb_user_privilege_done(user_id, privilege, async_task)
...
function XboxPrivileges:_cb_user_privilege_failed(user_id, privilege, result)
...
function XboxPrivileges:has_privilege(privilege)
...
function XboxPrivileges:has_error()
	return self._has_error
end

function XboxPrivileges:fetch_crossplay_restrictions()
...
function XboxPrivileges:has_crossplay_restriction(relation, restriction)
...
function XboxPrivileges:verify_user_restriction(xuid, restriction)
...
function XboxPrivileges:user_restriction_verified(xuid, restriction)
	if not XBOX_PERMISSION_LUT[restriction] then
		return
	end
...
function XboxPrivileges:user_has_restriction(xuid, restriction)
