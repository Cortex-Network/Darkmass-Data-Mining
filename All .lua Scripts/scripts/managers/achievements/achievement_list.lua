...
local function _add_meta_family(description, category, icon, ...)
	...
	_add_achievement_family_from_trigger(description, UITypes.meta, icon, category, triggers, nil, , false)
end
...
function AchievementList.achievement_exists(achievement_id)
	local index = AchievementList._lookup[achievement_id]

	return index ~= nil
end

function AchievementList.achievement_from_id(achievement_id)
	local index = AchievementList._lookup[achievement_id]
