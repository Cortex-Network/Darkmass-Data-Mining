...
function AchievementTracker:init(achievement_definitions, saving_strategy_class, optional_push_platform_progress)
...
function AchievementTracker:destroy()
	self:untrack_all()
	self._saving_strategy:delete()
end

function AchievementTracker:track_player(player)
...
function AchievementTracker:_stop_tracking(account_id)
...
function AchievementTracker:untrack_player(account_id)
...
function AchievementTracker:untrack_all()
...
function AchievementTracker:is_tracking(account_id)
	return self._tracked_players[account_id] ~= nil
end

function AchievementTracker:_unlock_achievement(account_id, achievement_id)
	local tracked_table = self._tracked_players[account_id]
...
function AchievementTracker:_progress_achievement(account_id, achievement_id)
	local achievement_index = self._achievement_definitions._lookup[achievement_id]
...
function AchievementTracker:_on_trigger(account_id, type, trigger_id, ...)
	local tracked_table = self._tracked_players[account_id]
...
function AchievementTracker:_on_achievement_unlock(account_id, achievement_id)
	self:_on_trigger(account_id, AchievementTypes.meta, achievement_id)
end

function AchievementTracker:_on_event_trigger(account_id, event_name, event_params)
	self:_on_trigger(account_id, AchievementTypes.event, event_name, event_params)
end

function AchievementTracker:_on_stat_trigger(id, trigger_id, trigger_value, ...)
