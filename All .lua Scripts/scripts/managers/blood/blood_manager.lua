...
function BloodManager:init(world, is_server, network_event_delegate)
	self._world = world
...
function BloodManager:_create_blood_ball_buffer()
...
function BloodManager:delete_units()
	Blood.destroy(self._blood_system)
end

function BloodManager:destroy()
...
function BloodManager:_update_blood_balls(dt, t)
...
function BloodManager:queue_blood_ball(position, direction, blood_ball_unit, optional_damage_type)
...
function BloodManager:remove_blood_ball(unit)
	Blood.despawn_blood_ball(self._blood_system, unit)
end

function BloodManager:play_screen_space_blood(fx_extension)
	fx_extension:spawn_exclusive_particle("content/fx/particles/screenspace/screen_blood_splatter", Vector3(0, 0, 1))
end

function BloodManager:blood_ball_collision(unit, position, normal, velocity, blood_type, remove_blood_ball)
	local blood_decals_enabled = self:_blood_decals_enabled()
	...
	if Managers.state.decal ~= nil then
		...
		Managers.state.decal:add_projection_decal(decal_unit_name, position, tangent_rotation, normal, extents, nil, , t)
	end
...
function BloodManager:_spawn_blood_ball(blood_ball_data)
...
function BloodManager:_update_weapon_blood(dt, t)
...
function BloodManager:_set_weapon_blood_intensity(visual_loadout_extension, slot_name, blood_amount)
...
function BloodManager:add_weapon_blood(player, slot_name, amount)
...
function BloodManager:rpc_add_weapon_blood(channel_id, game_object_id, slot_name_id, weapon_blood_amount_id)
...
function BloodManager:_blood_decals_enabled()
