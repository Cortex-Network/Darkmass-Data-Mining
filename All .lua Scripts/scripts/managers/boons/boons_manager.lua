...
function BoonsManager:init(is_server)
...
function BoonsManager:_on_player_unit_spawned(player)
...
function BoonsManager:update(dt, t)
end

function BoonsManager:request_equip_boon(player, gear_id)
	local peer_id = player:peer_id()
...
function BoonsManager:rpc_player_equip_boon_request(channel_id, peer_id, local_player_id, gear_id)
...
function BoonsManager:rpc_player_equip_boon_response(channel_id, success, optional_fail_reason)
...
function BoonsManager:_boon_can_be_equipped(boon_item)
	if self._boons_activated then
...
function BoonsManager:_equip_boon_on_server(player, item)
	self._equipped_boons[player] = item
end

function BoonsManager:activate_boons()
...
function BoonsManager:rpc_boons_activated(channel_id)
	self._boons_activated = true
end

function BoonsManager:_add_boon_effects_on_all_player()
...
function BoonsManager:_add_buff(player, name)
...
function BoonsManager:_player_have_buff(player, buff_template_name)
	local player_unit = player.player_unit
...
function BoonsManager:_sync_equipped_boons()
	local equipped_boons = self._equipped_boons
	local peer_ids_list = {}
...
function BoonsManager:rpc_sync_equipped_boons(channel_id, peer_ids_list, local_player_ids_list, boon_items_id_list)
...
function BoonsManager:equipped_boon(player)
	local boon_item = self._equipped_boons[player]

	return boon_item
end

function BoonsManager:get_boon_items_in_inventory(player, callback)
...
function BoonsManager:_send_rpc_server(rpc_name, peer_id, ...)
...
function BoonsManager:_send_rpc_client(rpc_name, peer_id, ...)
