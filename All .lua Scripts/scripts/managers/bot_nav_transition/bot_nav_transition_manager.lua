...
function BotNavTransitionManager:init(world, physics_world, nav_world, is_server)
	self._world = world
...
function BotNavTransitionManager:on_gameplay_post_init(level)
...
function BotNavTransitionManager:destroy()
...
function BotNavTransitionManager:clear_temp_transitions()
...
function BotNavTransitionManager:_destroy_transition(transitions, index)
...
function BotNavTransitionManager:create_transition(wanted_from, via, wanted_to, player_jumped, make_permanent)
	local nav_world = self._nav_world
	local traverse_logic = self._traverse_logic
	local drawer = nil
...
function BotNavTransitionManager:unregister_transition(index)
	local bot_nav_transitions = self._bot_nav_transitions

	self:_destroy_transition(bot_nav_transitions, index)
end

function BotNavTransitionManager:traverse_logic()
	return self._traverse_logic
end

function BotNavTransitionManager:transition_data(smart_object_id)
	local bot_nav_transitions = self._bot_nav_transitions
...
function BotNavTransitionManager:register_ladder(unit)
	local physics_world = self._physics_world
	local drawer = nil
	local data = {}
...
function BotNavTransitionManager:unregister_ladder(unit)
...
function BotNavTransitionManager:allow_nav_tag_layer(layer_name, layer_allowed)
...
function BotNavTransitionManager:set_nav_tag_layer_cost(layer_name, layer_cost)
	local layer_id = Managers.state.nav_mesh:nav_tag_layer_id(layer_name)
