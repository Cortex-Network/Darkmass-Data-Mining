...
function CameraManager:add_environment(shading_environment_extension)
...
function CameraManager:remove_environment(shading_environment_extension)
...
function CameraManager:_add_environment_blender(viewport_name)
...
function CameraManager:set_elevation_offset(offset, scale, min, max)
...
function CameraManager:create_viewport(viewport_name, camera_unit, position, rotation, shading_environment_name)
...
function CameraManager:delete_units()
	local world = self._world
...
function CameraManager:set_node_tree_root_near_range(viewport_name, tree_id, near_range)
	self._node_trees[viewport_name][tree_id].root_node:set_root_near_range(near_range)
end

function CameraManager:set_node_tree_root_far_range(viewport_name, tree_id, far_range)
	self._node_trees[viewport_name][tree_id].root_node:set_root_far_range(far_range)
end

function CameraManager:set_node_tree_root_dof_enabled(viewport_name, tree_id, dof_enabled)
	self._node_trees[viewport_name][tree_id].root_node:set_root_dof_enabled(dof_enabled)
end

function CameraManager:set_node_tree_root_focal_distance(viewport_name, tree_id, focal_distance)
	self._node_trees[viewport_name][tree_id].root_node:set_root_focal_distance(focal_distance)
end

function CameraManager:set_node_tree_root_focal_region(viewport_name, tree_id, focal_region)
	self._node_trees[viewport_name][tree_id].root_node:set_root_focal_region(focal_region)
end

function CameraManager:set_node_tree_root_focal_padding(viewport_name, tree_id, focal_padding)
	self._node_trees[viewport_name][tree_id].root_node:set_root_focal_padding(focal_padding)
end

function CameraManager:set_node_tree_root_focal_scale(viewport_name, tree_id, focal_scale)
...
function CameraManager:tree_node(viewport_name, tree_id, node_name)
...
function CameraManager:shading_callback(world, shading_env, viewport, default_shading_environment_resource)
...
function CameraManager:is_in_view(viewport_name, position)
...
function CameraManager:world_to_screen_position(viewport_name, world_position)
...
function CameraManager:has_camera(viewport_name)
	local world = self._world
...
function CameraManager:camera(viewport_name)
	local viewport = ScriptWorld.viewport(self._world, viewport_name)
...
function CameraManager:angular_velocity(viewport_name)
	return self._angular_velocity:unbox()
end

function CameraManager:has_viewport(viewport_name)
...
function CameraManager:update(dt, t, viewport_name, yaw, pitch, roll)
...
function CameraManager:set_fov_multiplier(multiplier)
...
function CameraManager:set_mood_blend_list(mood_blend_list)
...
function CameraManager:post_update(dt, t, viewport_name)
	local node_trees = self._node_trees[viewport_name]
...
function CameraManager:force_update_nodes(dt, viewport_name)
	local node_trees = self._node_trees[viewport_name]
	local data = self._variables[viewport_name]
...
function CameraManager:camera_effect_shake_event(event_name, source_unit_data)
	if not self._camera_shake_enabled then
		return
	end
...
function CameraManager:_update_camera(dt, t, viewport_name)
	local viewport = ScriptWorld.viewport(self._world, viewport_name)
	local camera = ScriptViewport.camera(viewport)
...
function CameraManager:_update_active_events(dt, viewport_name, camera_data)
...
function CameraManager:_apply_sequence_event(camera_data, t)
	local sequence_event_settings = self._sequence_event_settings
...
function CameraManager:_calculate_sequence_event_position(camera_data, new_values)
...
function CameraManager:_calculate_sequence_event_rotation(camera_data, new_values)
	local current_rot = camera_data.rotation
...
function CameraManager:_update_camera_properties(camera, shadow_cull_camera, current_node, camera_data, viewport_name)
...
function CameraManager:_update_angular_velocity(dt, viewport_name)
...
function CameraManager:listener_pose(viewport_name)
	local world = self._world
