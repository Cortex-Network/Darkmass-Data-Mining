...
function ChunkLodManager:init(world, mission, local_player)
...
function ChunkLodManager:update(dt, t)
...
function ChunkLodManager:current_chunk_name()
	return self._current_level_name
end

function ChunkLodManager:_try_update_level(hit_unit, force_show_all, is_using_cinematic_levels)
...
function ChunkLodManager:_do_level_check(force_show_all, t)
	local cinematic_manager = Managers.state.cinematic
	local cinematic_camera, is_testify_camera = cinematic_manager:active_camera()
...
function ChunkLodManager:_detect_level_unit(force_show_all, is_using_cinematic_levels)
...
function ChunkLodManager:_async_raycast_result_cb(id, hits, num_hits, data)
	if num_hits <= 0 then
		return
	end
...
function ChunkLodManager:_get_raycast_parameters()
...
function ChunkLodManager:_get_neighbours(level, ...)
	local i = 1
	local neighbours = {}
...
function ChunkLodManager:_update_level_lods(level, show_all)
...
function ChunkLodManager:enable()
	self._active = true

	self:_do_level_check(true, 0)
end

function ChunkLodManager:disable()
	self._active = false
	self._current_level_name = nil
