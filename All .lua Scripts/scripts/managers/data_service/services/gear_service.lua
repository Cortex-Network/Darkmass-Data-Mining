...
function GearService:init(backend_interface)
	self._backend_interface = backend_interface
end

function GearService:fetch_inventory(character_id, slot_filter_list, item_type_filter_list)
...
function GearService:fetch_inventory_paged(character_id, item_amount, slot_filter_list)
	local backend_interface = self._backend_interface
...
function GearService:fetch_account_items_paged(item_amount, slot_filter_list, chained_promise)
	local backend_interface = self._backend_interface
	...
	return gear_promise:next(function (gear_list)
		...
		return Promise.resolved({
			...
			next_page = function (data)
				return self:fetch_account_items_paged(nil, , gear_list.next_page(data))
			end
		})
	end):catch(function (errors)
		local gear_list_error = unpack(errors)
...
function GearService:attach_item_as_override(item_id, attach_point, gear_id)
	local backend_interface = self._backend_interface
