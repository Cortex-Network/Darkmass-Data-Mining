...
function TalentsService:init(backend_interface)
...
function TalentsService:set_talents(player, talent_names)
...
function TalentsService:set_specialization(player, specialization)
	local backend = self._backend_interface.characters
	local character_id = player:character_id()
...
function TalentsService:load_icons_for_profile(profile, reference_name, callback, prioritize)
...
function TalentsService:release_icons(load_id)
	if load_id then
		Managers.package:release(load_id)
	end
end

function TalentsService:mark_unlocked_group_as_new(character_id, talent_group_id)
...
function TalentsService:unmark_unlocked_group_as_new(character_id, talent_group_id)
	local save_manager = Managers.save
	local character_save_data = character_id and save_manager and save_manager:character_data(character_id)
...
function TalentsService:is_group_marked_as_new(character_id, talent_group_id)
	local save_manager = Managers.save
	local character_save_data = character_id and save_manager and save_manager:character_data(character_id)
