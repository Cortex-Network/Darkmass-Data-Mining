...
local function _spawn_flood_minions(horde, target_unit, nav_world, nav_spawn_points, path_position, num_groups, target_side_id, num_to_spawn, travel_distance)
	...
	for i = 1, #nearby_spawners do
		repeat
			...
			local i = spawner:add_spawns(breed_list, side_id, target_side_id, nil, , group_id)
		until true
	end
...
function horde_template.execute(physics_world, nav_world, side, target_side, composition, towards_combat_vector)
...
function horde_template.update(horde, dt, t)
