...
local function _try_find_occluded_position(nav_world, nav_spawn_points, side, target_side, occluded_spawn_range, num_spawn_groups, try_find_on_main_path, optional_main_path_offset)
	...
	local random_occluded_position = SpawnPointQueries.get_random_occluded_position(nav_world, nav_spawn_points, wanted_position, side, occluded_spawn_range, num_spawn_groups, MIN_DISTANCE_FROM_PLAYERS, nil, , only_search_forward)
...
function horde_template.execute(physics_world, nav_world, side, target_side, composition, towards_combat_vector, optional_main_path_offset)
	local target_side_id = target_side.side_id
	local main_path_manager = Managers.state.main_path
	...
	if should_spawn_patrol then
		for i = 1, num_positions do
			...
			local unit = minion_spawn_manager:spawn_minion(breed_name, spawn_position, spawn_rotation, side_id, aggro_states.passive, nil, , group_id, nil, , "trickle_horde")
		...
		for i = 1, num_positions do
			local breed_name = spawn_list[i]
			local spawn_position = flood_fill_positions[i]
			local unit = minion_spawn_manager:spawn_minion(breed_name, spawn_position, spawn_rotation, side_id, aggro_states.aggroed, target_unit, nil, group_id, nil, , "trickle_horde")
			spawned_minions[i] = unit
		end
	end
