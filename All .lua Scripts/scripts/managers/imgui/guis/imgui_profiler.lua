...
function ImguiProfiler:init()
	self:_reset()
end

function ImguiProfiler:_reset()
	self._filter = ""
...
function ImguiProfiler:is_persistent()
	return false
end

function ImguiProfiler:on_activated()
	LuaProfiler.CALCULATE_AVERAGE = true
end

function ImguiProfiler:on_deactivated()
	LuaProfiler.CALCULATE_AVERAGE = false
end

DO_RELOAD = true

function ImguiProfiler:update(dt, t)
	if DO_RELOAD then
		self:_reset()
	end
end

function ImguiProfiler:_draw()
end

function ImguiProfiler:post_update(dt, t)
	self:_post_draw()
end
...
function ImguiProfiler:_post_draw()
...
function ImguiProfiler:_draw_histogram()
	if not self._selected_scope then
		local label = string.upper("Select scope to generate histogram")
		local width, height = Imgui.get_window_size()

		Imgui.plot_histogram("", EMPTY_TABLE, nil, label, nil, , Vector2(width, 180))
...
function ImguiProfiler:_draw_filtered_scopes()
...
function ImguiProfiler:_draw_lookup_table(in_scope, is_top_scope, keep_in_scope)
...
function ImguiProfiler:_apply_filter(in_scope, keep_in_scope)
	local parent = in_scope.name
	local keep_in_scope = in_scope.keep_in_scope or keep_in_scope
