...
function MainPathManager:init(world, nav_world, level_name, level_seed, num_sides, is_server, use_nav_point_time_slice)
...
function MainPathManager:spawn_point_positions()
	return self._spawn_point_positions
end

function MainPathManager:nav_spawn_points()
	return self._nav_spawn_points
end

function MainPathManager:_calculate_travel_distances(main_path_segments)
...
function MainPathManager:_load_main_path_data(main_path_resource_name)
...
function MainPathManager:destroy()
...
function MainPathManager:crossroad_road_id(crossroads_id)
...
function MainPathManager:is_crossroad_segment_available(crossroads_id, road_id)
	local chosen_crossroads = self._chosen_crossroads
...
function MainPathManager:is_main_path_available()
	return self._main_path_version ~= nil
end

function MainPathManager:is_main_path_ready()
	return self._main_path_version ~= nil and self._nav_spawn_points ~= nil
end

function MainPathManager:main_path_segments()
	return self._main_path_segments
end

function MainPathManager:ahead_unit(side_id)
...
function MainPathManager:behind_unit(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function MainPathManager:furthest_travel_distance(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function MainPathManager:furthest_travel_percentage(side_id)
...
function MainPathManager:time_since_forward_travel_changed(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function MainPathManager:time_since_behind_travel_changed(side_id)
	local side_progress_on_path = self._side_progress_on_path
	local progress_on_path = side_progress_on_path[side_id]
...
function MainPathManager:segment_index_by_unit(unit)
	return self._segment_index_by_unit[unit]
end

function MainPathManager:node_index_by_nav_group_index(group_index)
	return self._group_to_main_path_index[group_index]
end

function MainPathManager:spawn_point_cost_table()
	return self._spawn_point_cost_table
end

function MainPathManager:allow_nav_tag_layer(layer_name, layer_allowed)
...
function MainPathManager:on_gameplay_post_init()
...
function MainPathManager:_generate_spawn_points()
...
function MainPathManager:_init_time_slice_nav_points(nav_world, nav_triangle_group, min_free_radius, min_distance_to_others, num_spawn_points_per_subgroup, nav_tag_cost_table, start_seed)
...
function MainPathManager:update_time_slice_spawn_points()
...
function MainPathManager:update_time_slice_generate_occluded_points()
	if self._main_path_version == nil or not self._is_server then
		return true
	end
...
function MainPathManager:update(dt, t)
	if self._main_path_version == nil then
		return
	end
...
function MainPathManager:_update_progress_on_path(t)
