...
function MechanismHub:all_players_ready()
end

function MechanismHub:failed_fetching_session_report()
end
...
function MechanismHub:_retry_join()
...
function MechanismHub:_show_retry_popup()
	local context = {
...
function MechanismHub:is_allowed_to_reserve_slots(peer_ids)
	return true
end

function MechanismHub:peers_reserved_slots(peer_ids)
end

function MechanismHub:peer_freed_slot(peer_id)
end
