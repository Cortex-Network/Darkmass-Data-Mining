...
function MinionDeathManager:init(is_server, network_event_delegate, soft_cap_out_of_bounds_units)
...
function MinionDeathManager:destroy()
	if not self._is_server then
		self._network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function MinionDeathManager:delete_units()
...
function MinionDeathManager:die(unit, attacking_unit_or_nil, attack_direction, hit_zone_name_or_nil, damage_profile, attack_type_or_nil, herding_template_or_nil, damage_type_or_nil)
...
function MinionDeathManager:set_dead(unit, attack_direction, hit_zone_name, damage_profile_name, do_ragdoll_push, herding_template_name)
...
function MinionDeathManager:update(dt, t)
...
function MinionDeathManager:_server_update_minions_awaiting_death(t)
	local minions_awaiting_death = self._minions_awaiting_death
...
function MinionDeathManager:_server_finalize_death(death_data)
...
function MinionDeathManager:_remove_extensions_on_death(unit, unit_id)
...
function MinionDeathManager:client_finalize_death(unit, unit_id)
	self:_remove_extensions_on_death(unit, unit_id)
...
function MinionDeathManager:rpc_minion_set_dead(channel_id, unit_id, attack_direction, hit_zone_id, damage_profile_id, do_ragdoll_push, herding_template_id_or_nil)
...
function MinionDeathManager:minion_ragdoll()
