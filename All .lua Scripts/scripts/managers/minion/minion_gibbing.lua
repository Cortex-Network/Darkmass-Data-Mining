...
function MinionGibbing:init(unit, breed, world, wwise_world, gib_template, visual_loadout_extension, random_seed, optional_gib_variations, optional_wounds_extension)
	self._gibs = {}
	self._flesh_gibs = {}
...
function MinionGibbing:spawn_gib_from_queue(unit, gib_settings, gib_position, gib_rotation, hit_zone_name, attack_direction, damage_profile, hit_zone_gib_template, optional_override_gib_forces)
	...
	if wounds_data then
		...
		WoundMaterials.apply(gib_unit, wounds_data, nil, , inverse_root_node_bind_pose)
	end
...
function MinionGibbing:_update_soft_oob(unit)
...
function MinionGibbing:gib(hit_zone_name_or_nil, attack_direction, damage_profile, override_gib_type, optional_override_gib_forces, optional_is_critical_strike)
	...
	if stump_settings then
		...
		if wounds_data then
			...
			WoundMaterials.apply(stump_unit, wounds_data, nil, , inverse_root_node_bind_pose)
		end
	end
...
function MinionGibbing:delete_gibs()
	local unit_spawner_manager = Managers.state.unit_spawner
	local flesh_gibs = self._flesh_gibs
...
function MinionGibbing:allow_gib_for_hit_zone(hit_zone, allowed)
	self._disallowed_hit_zones[hit_zone] = not allowed
end

function MinionGibbing:hit_zone_is_disallowed(hit_zone)
