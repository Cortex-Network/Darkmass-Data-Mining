...
function MultiplayerSession:init()
...
function MultiplayerSession:became_host(host_type, lobby_id)
...
function MultiplayerSession:stopped_being_host(is_error, source, reason, optional_error_details)
...
function MultiplayerSession:client_joined(channel_id, peer_id, player_sync_data)
...
function MultiplayerSession:client_disconnected(channel_id, peer_id, game_reason, engine_reason, host_became_empty)
	local server_manager = Managers.hub_server or Managers.mission_server
...
function MultiplayerSession:has_joined_host()
	return self._state == self.STATES.client
end

function MultiplayerSession:disconnection_info()
	return self._disconnection_info
end

function MultiplayerSession:vivox_backend_info()
	return self._vivox_backend_info
end

function MultiplayerSession:set_vivox_backend_info(vivox_backend_info)
	self._vivox_backend_info = vivox_backend_info
end

function MultiplayerSession:joined_host(channel_id, host_peer_id, host_type)
...
function MultiplayerSession:has_successfully_joined_host()
	return self._joined_host_peer_id ~= nil
end

function MultiplayerSession:disconnected_from_host(is_error, source, reason, optional_error_details)
	self._disconnection_info = {
		is_error = is_error,
		source = source,
		reason = reason,
...
function MultiplayerSession:failed_to_boot(is_error, source, reason, optional_error_details)
	self._disconnection_info = {
...
function MultiplayerSession:other_client_joined(peer_id, player_sync_data)
...
function MultiplayerSession:other_client_left(peer_id)
...
function MultiplayerSession:_remove_remote_players(peer_id, optional_out_player_data)
	local player_manager = Managers.player
...
function MultiplayerSession:is_dead()
	return self._state == self.STATES.dead
end

function MultiplayerSession:is_host()
	return self._state == self.STATES.host
end

function MultiplayerSession:host_type()
