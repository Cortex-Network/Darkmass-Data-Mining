...
function NavMeshManager:_require_nav_tag_volume_data(level_name, nav_tag_volume_data)
...
function NavMeshManager:_setup_nav_cost_map_lookup()
...
function NavMeshManager:_setup_nav_tag_layer_lookup(nav_tag_volume_layers)
...
function NavMeshManager:_initialize_client_traverse_logic(nav_world)
...
function NavMeshManager:initialize_nav_tag_cost_table(cost_table, layer_costs)
	local nav_tag_layer_lookup = self._nav_tag_layer_lookup
...
function NavMeshManager:initialize_nav_cost_map_multiplier_table(multiplier_table, map_multipliers)
...
function NavMeshManager:nav_tag_layer_id(layer_name)
	return self._nav_tag_layer_lookup[layer_name]
end

function NavMeshManager:nav_cost_map_id(map_name)
	return self._nav_cost_map_lookup[map_name]
end

function NavMeshManager:nav_tag_volume_data()
	return self._nav_tag_volume_data
end

function NavMeshManager:nav_tag_volume_layer_ids_by_volume_type(volume_type)
...
function NavMeshManager:_create_nav_cost_maps()
	local nav_cost_map_lookup = self._nav_cost_map_lookup
...
function NavMeshManager:_destroy_nav_cost_maps()
...
function NavMeshManager:_destroy_nav_tag_volumes()
	self._nav_tag_volume_data = nil
end

function NavMeshManager:nav_cost_map(cost_map_id)
	local nav_cost_maps_data = self._nav_cost_maps_data
...
function NavMeshManager:add_nav_cost_map_box_volume(transform, scale_vector, cost, cost_map_id)
...
function NavMeshManager:add_nav_cost_map_sphere_volume(position, radius, cost, cost_map_id)
	local volume_id_data = self._nav_cost_map_volume_id_data
	local volume_id = volume_id_data.current_id
	local volume_ids = volume_id_data.ids
	local volume_ids_size = volume_id_data.size
...
function NavMeshManager:set_nav_cost_map_volume_transform(volume_id, cost_map_id, transform)
	local volume_id_data = self._nav_cost_map_volume_id_data
	local volume_ids = volume_id_data.ids
	local cost_map_data = self._nav_cost_maps_data[cost_map_id]
	local volumes = cost_map_data.volumes
...
function NavMeshManager:set_nav_cost_map_volume_scale(volume_id, cost_map_id, scale)
	local volume_id_data = self._nav_cost_map_volume_id_data
	local volume_ids = volume_id_data.ids
	local cost_map_data = self._nav_cost_maps_data[cost_map_id]
	local volumes = cost_map_data.volumes
...
function NavMeshManager:remove_nav_cost_map_volume(volume_id, cost_map_id)
	local volume_id_data = self._nav_cost_map_volume_id_data
...
function NavMeshManager:client_traverse_logic()
	return self._traverse_logic
end

function NavMeshManager:update_time_slice_volumes_integration()
...
function NavMeshManager:hot_join_sync(sender, channel)
	local nav_tag_layer_lookup = self._nav_tag_layer_lookup
	local allowed_layers = self._nav_tag_allowed_layers
...
function NavMeshManager:is_nav_tag_volume_layer_allowed(layer_name)
	return self._nav_tag_allowed_layers[layer_name]
end

function NavMeshManager:allowed_nav_tag_layers()
	return self._nav_tag_allowed_layers
end

function NavMeshManager:rpc_set_allowed_nav_tag_layer(channel_id, layer_id, allowed)
