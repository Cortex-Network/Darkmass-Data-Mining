...
function NetworkStoryManager:init(world, is_server, network_event_delegate)
...
function NetworkStoryManager:_change_state(story, new_state, new_id)
...
function NetworkStoryManager:on_gameplay_post_init(level)
end

function NetworkStoryManager:destroy()
	if not self._is_server then
		self._network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function NetworkStoryManager:update(dt, t)
...
function NetworkStoryManager:hot_join_sync(peer, channel)
	self:_sync_stories(peer, channel)
end

function NetworkStoryManager:register_story(story_name, story_level, state_change_callback)
...
function NetworkStoryManager:set_position_level(story_level, unit)
...
function NetworkStoryManager:_create_story(story_name, story_level)
...
function NetworkStoryManager:unregister_story(story_name, story_level)
...
function NetworkStoryManager:play_story(story_name, story_level, speed)
	speed = speed or 1
...
function NetworkStoryManager:reset_story(story_name, story_level)
	local story = self._levels[story_level][story_name]
...
function NetworkStoryManager:play_story_from_start(story_name, story_level, speed)
	self:reset_story(story_name, story_level)

	return self:play_story(story_name, story_level, speed)
end

function NetworkStoryManager:get_story_time(story_name, story_level)
	local story = self._levels[story_level][story_name]
...
function NetworkStoryManager:get_story_length(story_name, story_level)
	local story = self._levels[story_level][story_name]
...
function NetworkStoryManager:get_story_state(story_name, story_level)
	local story = self._levels[story_level][story_name]
...
function NetworkStoryManager:get_story_id(story_name, story_level)
	local story = self._levels[story_level][story_name]
...
function NetworkStoryManager:rpc_network_story_sync(channel_id, level_id, story_name, story_speed, story_time)
...
function NetworkStoryManager:rpc_network_story_set_position_level(channel_id, level_id, unit_id, is_level_unit)
	local unit_spawner_manager = Managers.state.unit_spawner
...
function NetworkStoryManager:_sync_stories(peer, channel_id)
