...
function NetworkedFlowStateManager:init(world, is_server, network_event_delegate)
	self._story_lookup = {}
	self._level_lookup = {}
...
function NetworkedFlowStateManager:destroy()
	if not self._is_server then
		self._network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

function NetworkedFlowStateManager:flow_cb_create_story(node_id)
...
function NetworkedFlowStateManager:flow_cb_play_networked_story(client_call_event_name, start_time, start_from_stop_time, node_id)
	if not self._is_server then
		return nil
	end
...
function NetworkedFlowStateManager:rpc_flow_state_story_played(channel_id, level_id, start_time)
...
function NetworkedFlowStateManager:flow_cb_stop_networked_story(node_id)
	if not self._is_server then
		return nil
	end
...
function NetworkedFlowStateManager:rpc_flow_state_story_stopped(channel_id, level_id, stop_time)
	local story_data = self._story_lookup[level_id]
	local level = story_data.level
	local node_id = story_data.node_id
...
function NetworkedFlowStateManager:flow_cb_has_stopped_networked_story(node_id)
	if not self._is_server then
		return nil
	end
...
function NetworkedFlowStateManager:flow_cb_has_played_networked_story(node_id, story_id)
	if not self._is_server then
		return nil
	end
...
function NetworkedFlowStateManager:hot_join_sync(peer, channel)
	self:_sync_states(peer, channel)
	self:_sync_stories(peer, channel)
end

function NetworkedFlowStateManager:_sync_stories(peer, channel)
...
function NetworkedFlowStateManager:_sync_states(peer, channel)
...
function NetworkedFlowStateManager:flow_cb_create_state(unit, state_name, default_value, client_data_changed_event, hot_join_sync_event, is_game_object)
...
function NetworkedFlowStateManager:flow_cb_get_state(unit, state_name)
...
function NetworkedFlowStateManager:flow_cb_change_state(unit, state_name, new_state)
	if not self._is_server then
		return
	end
...
function NetworkedFlowStateManager:_clamp_state(state_name, type_data, new_state)
...
function NetworkedFlowStateManager:client_flow_state_changed(unit_id, state_network_id, new_state, only_set, is_game_object)
...
function NetworkedFlowStateManager:rpc_flow_state_bool_changed(channel_id, unit_id, state_network_id, new_state, only_set, is_game_object)
	self:client_flow_state_changed(unit_id, state_network_id, new_state, only_set, is_game_object)
end

function NetworkedFlowStateManager:rpc_flow_state_number_changed(channel_id, unit_id, state_network_id, new_state, only_set, is_game_object)
	self:client_flow_state_changed(unit_id, state_network_id, new_state, only_set, is_game_object)
end
