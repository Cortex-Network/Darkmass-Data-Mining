...
function MonsterPacing:init(nav_world)
...
function MonsterPacing:destroy()
end

function MonsterPacing:on_gameplay_post_init(level, template)
...
function MonsterPacing:_generate_spawns(template)
...
function MonsterPacing:fill_spawns_by_travel_distance(breed_name, spawn_type, monster_per_travel_distance)
...
function MonsterPacing:add_spawn_point(unit, position, path_position, travel_distance, section_index, spawn_type)
	local above = 1
	local below = 1
	local horizontal = 1
...
function MonsterPacing:_spawn_monster(monster, ahead_target_unit, side_id)
...
function MonsterPacing:_spawn_boss_patrol(boss_patrol, ahead_travel_distance, side_id)
	...
	for i = 1, num_positions do
		...
		local unit = minion_spawn_manager:spawn_minion(breed_name, spawn_position, Quaternion.identity(), side_id, perception_aggro_states.passive, nil, , group_id, nil, , "trickle_horde")
