...
function RoamerPacing:init(nav_world, level_path, seed, pacing_control)
...
function RoamerPacing:_create_zones(spawn_point_positions)
...
function RoamerPacing:_create_sub_zones(spawn_positions, density_setting, group_id, num_to_spawn)
	local sub_zones = {}
...
function RoamerPacing:_limit_roamer_breeds(breed_name, limit_settings, faction)
...
function RoamerPacing:_generate_roamers(zones, roamers)
	local num_zones = #zones
	local max_tries = 100
...
function RoamerPacing:update(dt, t, side_id, target_side_id)
...
function RoamerPacing:_alert_roamer_unit(roamer_unit, target_unit, alert_delay)
...
function RoamerPacing:_alert_roamer_group(aggroing_unit, target_unit, aggroing_roamer, side_id, target_side_id)
...
function RoamerPacing:_try_activate_roamer(roamer, side_id)
...
function RoamerPacing:_deactivate_roamer(roamer)
...
function RoamerPacing:_spawn_roamer(breed_name, position, rotation, group_id, side_id)
	local unit = Managers.state.minion_spawn:spawn_minion(breed_name, position, rotation, side_id, nil, , , group_id)
...
function RoamerPacing:_update_density_type_switch(ahead_travel_distance)
...
function RoamerPacing:_handle_group_ambience_sfx(roamer, activated)
...
function RoamerPacing:allow_nav_tag_layer(layer_name, layer_allowed)
...
function RoamerPacing:is_traverse_logic_initialized()
	return self._traverse_logic ~= nil
end

function RoamerPacing:traverse_logic()
	return self._traverse_logic
end

function RoamerPacing:override_faction(faction)
	self._override_faction = faction
end

function RoamerPacing:num_encampments_override(value, chance)
	self._num_encampments_override = value
	self._override_chance_of_encampment = chance
end

function RoamerPacing:current_faction()
	return self._current_faction
end

function RoamerPacing:current_density_type()
