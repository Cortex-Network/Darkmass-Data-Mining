...
function SpecialsPacing:init(nav_world)
...
function SpecialsPacing:_setup(template, optional_first_spawn_modifier)
	self._template = template
...
function SpecialsPacing:switch_template(new_template)
	self:_setup(new_template)
end

function SpecialsPacing:set_timer_modifier(modifier)
...
function SpecialsPacing:update(dt, t, side_id, target_side_id)
...
function SpecialsPacing:_spawn_special(specials_slot, side_id, target_side_id)
	local breed_name = self:_get_special_slot_breed_name(specials_slot)
	...
	local unit = Managers.state.minion_spawn:spawn_minion(breed_name, spawn_position, Quaternion.identity(), side_id, aggro_states.aggroed, target_unit, nil, , , )

	return true, unit
end

function SpecialsPacing:_on_special_spawned(specials_slot, spawned_unit)
	self._num_spawned_specials = self._num_spawned_specials + 1
...
function SpecialsPacing:_get_special_slot_breed_name(specials_slot)
...
function SpecialsPacing:_find_spawn_position(side_id, breed_name, target_side_id, check_ahead, optional_mainpath_offset)
	local main_path_manager = Managers.state.main_path
...
function SpecialsPacing:_find_nearby_spawner(target_side_id)
	local main_path_manager = Managers.state.main_path
...
function SpecialsPacing:_add_spawner_special(spawner, breed_name, side_id, target_side_id)
...
function SpecialsPacing:_check_stuck_special(unit, specials_slot, template, target_side_id, t)
...
function SpecialsPacing:_check_disabler_override(template, target_side_id, specials_slot)
...
function SpecialsPacing:_check_and_activate_coordinated_strike(template, current_special_slot)
...
function SpecialsPacing:_update_rush_prevention(target_side_id, template, t)
...
function SpecialsPacing:_update_speed_running_prevention(target_side_id, template, t)
...
function SpecialsPacing:try_inject_special(breed_name, optional_prefered_spawn_direction, optional_target_unit, optional_spawner_group, optional_ignore_allowance, optional_is_prevention)
...
function SpecialsPacing:freeze(should_freeze)
	self._frozen = should_freeze
end

function SpecialsPacing:set_max_alive_specials_multiplier(multiplier)
...
function SpecialsPacing:refund_special_slot()
	local specials_slots = self._specials_slots
	local chosen_slot = nil
