...
function PartyImmateriumManager:init()
...
function PartyImmateriumManager:destroy()
...
function PartyImmateriumManager:_resolve_join_permission(presence_entry, context)
...
function PartyImmateriumManager:_ensure_join_permission_on_current_party(dt)
...
function PartyImmateriumManager:_resolve_join_permission_on_whole_party(party)
...
function PartyImmateriumManager:start()
...
function PartyImmateriumManager:reset()
	self:_reset_party_data()
...
function PartyImmateriumManager:_reset_party_data()
...
function PartyImmateriumManager:other_members()
	return self._other_members
end

function PartyImmateriumManager:num_other_members()
	return #self._other_members
end

function PartyImmateriumManager:member_from_account_id(account_id)
...
function PartyImmateriumManager:member_from_peer_id(peer_id)
...
function PartyImmateriumManager:other_member_from_unique_id(unique_id)
	for _, member in ipairs(self._other_members) do
...
function PartyImmateriumManager:_generate_compabillity_string()
	return Managers.connection.combined_hash
end

function PartyImmateriumManager:debug_get_parties()
...
function PartyImmateriumManager:cached_debug_get_parties()
	return self._cached_debug_get_parties
end

function PartyImmateriumManager:_fetch_cached_debug_get_parties(dt, force)
	if BUILD == "release" then
		return
	end
...
function PartyImmateriumManager:leave_party()
...
function PartyImmateriumManager:_can_join_new_party_check(join_parameter)
...
function PartyImmateriumManager:join_party(join_parameter, is_reconnect)
...
function PartyImmateriumManager:_hot_join_ticket(mission_session_id)
...
function PartyImmateriumManager:_handle_party_update_event(event)
...
function PartyImmateriumManager:_handle_party_game_state_update_event(event)
	self:_execute_triggers_for_game_state()
end

function PartyImmateriumManager:_handle_party_client_event_trigger(event)
	Managers.event:trigger("party_immaterium_" .. event.trigger_event_name, unpack(event.args))
end

function PartyImmateriumManager:_handle_request_to_join(joiner_account_id, popup, context_account_id)
...
function PartyImmateriumManager:_check_for_invites()
...
function PartyImmateriumManager:update(dt, t)
...
function PartyImmateriumManager:is_in_matchmaking()
	return self:current_state() == PartyConstants.State.matchmaking
end

function PartyImmateriumManager:_reject_game_session_promises(error)
	for _, game_session_promise in ipairs(self._game_session_promises) do
...
function PartyImmateriumManager:_resolve_game_session_promises()
	for _, game_session_promise in ipairs(self._game_session_promises) do
...
function PartyImmateriumManager:_execute_triggers_for_game_state()
...
function PartyImmateriumManager:current_state()
	local game_state = self:party_game_state()
...
function PartyImmateriumManager:are_all_members_in_hub()
...
function PartyImmateriumManager:are_all_members_in_mission()
...
function PartyImmateriumManager:num_party_members_in_mission()
...
function PartyImmateriumManager:hot_join_party_hub_server()
...
function PartyImmateriumManager:create_single_player_game(mission_id)
	local profile = Managers.player:local_player_backend_profile()
	local character_id = profile and profile.character_id
...
function PartyImmateriumManager:request_vivox_party_token()
	return Managers.grpc:request_vivox_token(self:party_id(), "party")
end

function PartyImmateriumManager:game_session_in_progress()
	local game_state = self:party_game_state()

	return game_state.status == "GAME_SESSION_IN_PROGRESS"
end

function PartyImmateriumManager:join_game_session()
...
function PartyImmateriumManager:current_game_session_id()
	local game_state = self:party_game_state()
...
function PartyImmateriumManager:current_game_session_mission_data()
	local game_state = self:party_game_state()
...
function PartyImmateriumManager:consume_matched_hub_server_session_id()
...
function PartyImmateriumManager:cancel_matchmaking()
...
function PartyImmateriumManager:latched_hub_server_matchmaking()
	return Managers.backend.interfaces.matchmaker:fetch_queue_ticket_hub():next(function (response)
...
function PartyImmateriumManager:_table_tostring_or_string(table_or_string)
...
function PartyImmateriumManager:_game_session_promise()
...
function PartyImmateriumManager:wanted_mission_selected(backend_mission_id, private_session)
...
function PartyImmateriumManager:start_hub_group_up_vote()
	return Managers.voting:start_voting("new_hub_session_immaterium", {})
end

function PartyImmateriumManager:get_myself()
	return self._myself
end

function PartyImmateriumManager:_mission_matchmaking_started(mission_name)
end

function PartyImmateriumManager:_mission_matchmaking_completed(matched_game_session_id)
end

function PartyImmateriumManager:_mission_matchmaking_aborted(reason)
...
function PartyImmateriumManager:ready_voting_completed()
end

function PartyImmateriumManager:get_your_standing_invite_code()
	local party_id = self:party_id()
...
function PartyImmateriumManager:_retry_get_your_standing_invite_code(party_id, promise)
...
function PartyImmateriumManager:get_invite_code_for_platform_invite(platform, platform_user_id)
	local promise = Promise:new()
...
function PartyImmateriumManager:invite_to_party(invitee_account_id)
	local party_id = self:party_id()
...
function PartyImmateriumManager:cancel_party_invite(invite_token)
	local party_id = self:party_id()
...
function PartyImmateriumManager:get_invite_by_account_id(invitee_account_id)
	local invite_list = self:party_invite_list()
...
function PartyImmateriumManager:_decline_party_invite(party_id, invite_token, answer_code)
...
function PartyImmateriumManager:_get_presence_promise(account_id)
	local _, promise = Managers.presence:get_presence(account_id)

	return promise
end

function PartyImmateriumManager:_handle_member_joined(member_account_id)
...
function PartyImmateriumManager:_handle_member_left(member_account_id)
	self:_get_presence_promise(member_account_id):next(function (presence)
...
function PartyImmateriumManager:_handle_member_disconnected(member_account_id)
	self:_get_presence_promise(member_account_id):next(function (presence)
...
function PartyImmateriumManager:_handle_invite_canceled(invite_token, platform, platform_user_id, inviter_account_id, canceler_account_id, answer_code)
...
function PartyImmateriumManager:_handle_invite_timeout(invite_token, platform, platform_user_id, inviter_account_id)
	if self._myself:account_id() ~= inviter_account_id or platform ~= "" then
		return
	end
...
function PartyImmateriumManager:_handle_invite_accepted(invite_token, invitee_account_id, inviter_account_id)
...
function PartyImmateriumManager:_on_join_party(reconnect)
	if reconnect then
...
function PartyImmateriumManager:_on_join_party_error(error_code)
	Managers.event:trigger("event_add_notification_message", "alert", {
...
function PartyImmateriumManager:_handle_stay_in_party_voting_started(voting_id, new_party_id, new_party_invite_token)
...
function PartyImmateriumManager:_handle_stay_in_party_voting_completed(result)
...
function PartyImmateriumManager:_handle_stay_in_party_voting_aborted()
	self._active_party_vote = nil
end

function PartyImmateriumManager:active_stay_in_party_vote()
	return self._active_party_vote
end

function PartyImmateriumManager:_on_invite_party_error(error_code)
	Managers.event:trigger("event_add_notification_message", "alert", {
...
function PartyImmateriumManager:_handle_immaterium_invite(party_id, invite_token, inviter_account_id)
...
function PartyImmateriumManager:_handle_immaterium_invite_canceled(party_id, invite_token, inviter_account_id, canceler_account_id, answer_code)
...
function PartyImmateriumManager:_handle_immaterium_invite_timeout(party_id, invite_token, inviter_account_id)
	_info("invite timeout, party_id=%s, inviter_account_id=%s, inviter_account_id=%s", party_id, invite_token, inviter_account_id)
	self:_close_invite_popup(party_id)
end

function PartyImmateriumManager:_close_invite_popup(party_id)
...
function PartyImmateriumManager:_request_to_join_popup(joiner_account_id)
...
function PartyImmateriumManager:_handle_request_to_join_popup_cancel(joiner_account_id)
	_info("request to join canceled joiner_account_id=%s", joiner_account_id)
	self:_close_request_to_join_popup(joiner_account_id)
end

function PartyImmateriumManager:_close_request_to_join_popup(joiner_account_id)
...
function PartyImmateriumManager:_handle_game_session_aborted(payload)
	local context = {
