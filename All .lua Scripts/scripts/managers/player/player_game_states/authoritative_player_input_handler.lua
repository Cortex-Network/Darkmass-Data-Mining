...
function AuthoritativePlayerInputHandler:init(player, is_server)
...
function AuthoritativePlayerInputHandler:create_clock()
...
function AuthoritativePlayerInputHandler:fixed_update(dt, t, frame)
...
function AuthoritativePlayerInputHandler:update(dt, t)
	if not self._clock_handler then
		return
	end
...
function AuthoritativePlayerInputHandler:get_orientation(frame)
...
function AuthoritativePlayerInputHandler:get(action, frame)
	local index = math.max(math.min(frame, self._received_frame) - self._parsed_frame, 1)

	return self._input_cache[self._action_lookup[action]][index]
end

function AuthoritativePlayerInputHandler:rpc_player_input_array(channel_id, local_player_id, start_frame, end_frame_offset, ...)
...
function AuthoritativePlayerInputHandler:rewind_ms()
	return self._clock_handler:rewind_ms()
end

function AuthoritativePlayerInputHandler:had_received_input(fixed_frame)
