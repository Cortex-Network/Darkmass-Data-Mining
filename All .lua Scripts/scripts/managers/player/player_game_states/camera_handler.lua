...
function CameraHandler:init(player, world)
	self._player = player
...
function CameraHandler:on_reload(refreshed_resources)
...
function CameraHandler:update(dt, t, player_orientation, input)
...
function CameraHandler:_switch_follow_target(new_unit)
	local old_unit = self._camera_follow_unit
...
function CameraHandler:_post_update(dt, t, player_orientation)
	local camera_manager = Managers.state.camera
	local viewport_name = self._viewport_name
	local camera_follow_unit = self._camera_follow_unit
...
function CameraHandler:_update_follow(follow_unit_switch)
...
function CameraHandler:_update_player_mood(switched_target, follow_unit)
...
function CameraHandler:_update_follow_camera(unit, follow_unit_switch)
...
function CameraHandler:_update_wwise_state(unit)
...
function CameraHandler:_follow_owner()
	local player = self._player
...
function CameraHandler:_next_follow_unit(except_unit)
	local side_system = self._side_system
	local old_unit = self._camera_follow_unit
...
function CameraHandler:_get_theme_shading_environment(themes)
	if themes then
...
function CameraHandler:spawn_camera(level, themes)
	local viewport_name = self._viewport_name
	local camera_manager = Managers.state.camera
...
function CameraHandler:_load_node_trees(viewport_name)
	local camera_manager = Managers.state.camera
...
function CameraHandler:is_observing()
	return self._mode == CameraModes.observer and self._first_person_spectating_mode
end

function CameraHandler:camera_follow_unit()
	return self._camera_follow_unit
end

function CameraHandler:destroy_camera()
