...
function HumanGameplay:init(player, game_state_context)
...
function HumanGameplay:on_reload(refreshed_resources)
...
function HumanGameplay:_create_player_hud(player)
...
function HumanGameplay:_destroy_player_hud()
	local ui_manager = Managers.ui
...
function HumanGameplay:_create_spectator_hud(spectated_player)
	local ui_manager = Managers.ui
...
function HumanGameplay:_destroy_spectator_hud()
	local ui_manager = Managers.ui
...
function HumanGameplay:_create_player_orientation_classes(player)
	local orientation = player:get_orientation()
...
function HumanGameplay:_destroy_player_orientation_classes(all)
...
function HumanGameplay:destroy()
...
function HumanGameplay:_init_input()
	local input_manager = Managers.input
	self._input = input_manager:get_input_service("Ingame")
end

function HumanGameplay:_input_active()
...
function HumanGameplay:_get_input()
...
function HumanGameplay:pre_update(main_dt, main_t)
...
function HumanGameplay:_player_orientation_class()
...
function HumanGameplay:_cb_player_activate_emote(emote, player)
	self._emote = emote
end

function HumanGameplay:initialize_client_fixed_frame(fixed_frame)
...
function HumanGameplay:fixed_update(game_dt, game_t, fixed_frame)
	self._last_frame = fixed_frame
	local input = self:_get_input()
	local player_orientation_class = self:_player_orientation_class()
	local yaw = 0
...
function HumanGameplay:update(main_dt, main_t)
	local input = self:_get_input()
	local player = self._player
...
function HumanGameplay:_update_spectating(camera_follow_unit_or_nil)
...
function HumanGameplay:_handle_huds(camera_follow_unit_or_nil)
	local own_player = self._player
...
function HumanGameplay:post_update(main_dt, main_t)
	local player = self._player
	local player_orientation = self:_player_orientation_class()
...
function HumanGameplay:on_player_removed(player)
	if player == self._spectated_player then
		self:_stop_spectating()
	end
end

function HumanGameplay:_start_spectating(spectated_player)
	self._spectated_player = spectated_player
end

function HumanGameplay:_stop_spectating()
	if self._has_spectator_hud then
		self:_destroy_spectator_hud()
	end
