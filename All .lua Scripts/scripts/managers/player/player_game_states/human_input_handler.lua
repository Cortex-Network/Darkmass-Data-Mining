...
function HumanInputHandler:init(player, is_server, client_clock_handler)
	self._service = nil
...
function HumanInputHandler:initialize_client_fixed_frame(frame, input_service, yaw, pitch, roll)
	self._frame = frame
...
function HumanInputHandler:_buffer_index(frame)
...
function HumanInputHandler:pre_update(dt, t, input_service, ui_interaction_action)
...
function HumanInputHandler:fixed_update(dt, t, frame, input_service, yaw, pitch, roll)
...
function HumanInputHandler:get_orientation(frame)
	local index = self:_buffer_index(frame)
	local cache = self._input_cache
...
function HumanInputHandler:get(action, frame)
	local index = self:_buffer_index(frame)
...
function HumanInputHandler:_parse_input(input_cache, input_service, index)
	if index < 1 then
...
function HumanInputHandler:update(dt, t, input_service)
...
function HumanInputHandler:rpc_player_input_array_ack(channel_id, local_player_id, frame)
	self:frame_acknowledged(frame)
end

function HumanInputHandler:frame_parsed(frame, remainder_time, frame_time)
...
function HumanInputHandler:frame_acknowledged(frame)
	if self._last_frame_acknowledged < frame then
		self._last_frame_acknowledged = frame
	end
end

function HumanInputHandler:had_received_input(fixed_frame)
	return true
end

function HumanInputHandler:in_panic()
	return self._in_panic
end

function HumanInputHandler:set_in_panic(in_panic)
