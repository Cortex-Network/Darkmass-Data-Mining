...
function AdaptiveClockHandlerClient:init(network_event_delegate)
...
function AdaptiveClockHandlerClient:destroy()
...
function AdaptiveClockHandlerClient:rpc_sync_clock(clock_start, clock_offset, last_frame)
...
function AdaptiveClockHandlerClient:_frame_discrepancy_buffer()
	return math.max(self._max_frame_time, 0.006944444444444444)
end

function AdaptiveClockHandlerClient:_target_offset()
	return self._server_offset + self:_frame_discrepancy_buffer() + self._offset_correction
end

function AdaptiveClockHandlerClient:frame_parsed(frame, remainder_time, frame_time)
...
function AdaptiveClockHandlerClient:post_update(main_dt)
...
function AdaptiveClockHandlerClient:_update_max_frame_time(dt)
...
function AdaptiveClockHandlerClient:set_in_panic(in_panic)
	self._in_panic = in_panic
end

function AdaptiveClockHandlerClient:in_panic()
	return self._in_panic
end

function AdaptiveClockHandlerClient:diagnostics_dump()
	local dump = nil
	local diag = self._diagnostics
	local i = diag[0]
...
function AdaptiveClockHandlerClient:rpc_sync_clock_offset(channel_id, offset, server_gameplay_t)
