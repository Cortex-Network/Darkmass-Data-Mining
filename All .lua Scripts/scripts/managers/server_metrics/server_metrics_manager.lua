...
function ServerMetricsManager:init()
	self._metrics = {}
...
function ServerMetricsManager:destroy()
end

function ServerMetricsManager:add_annotation(type_name, metadata)
	local json_object = {
		ts = os.time(),
		type = type_name,
		metadata = metadata
...
function ServerMetricsManager:set_gauge(metric_name, value)
...
function ServerMetricsManager:add_to_counter(metric_name, value)
	local metric = self._metrics[metric_name]
...
function ServerMetricsManager:_flush_metric(metric, dt)
...
function ServerMetricsManager:update(dt)
