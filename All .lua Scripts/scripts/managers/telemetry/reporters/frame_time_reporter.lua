...
function FrameTimeReporter:init(params)
	self._measures = {}
	self._last_sample_time = 0
	self._mission_name = params.mission_name
end

function FrameTimeReporter:destroy()
end

function FrameTimeReporter:update(dt, t)
...
function FrameTimeReporter:_take_measure(dt)
	local dt_ms = dt * 1000
	self._measures[#self._measures + 1] = dt_ms
end

function FrameTimeReporter:report()
