...
function TerrorEventManager:init(world, is_server, network_event_delegate, mission, level_name)
...
function TerrorEventManager:on_gameplay_post_init(level)
	self._level = level
end

function TerrorEventManager:_load_mission_event_templates(mission)
...
function TerrorEventManager:_calculate_random_event_probabilities(random_events)
...
function TerrorEventManager:start_random_event(event_chunk_name)
	if not self._is_server then
		return
	end
...
function TerrorEventManager:stop_event(event_name)
	if not self._is_server then
		return
	end
...
function TerrorEventManager:num_active_events()
	return #self._active_events
end

function TerrorEventManager:update(dt, t)
	local active_events = self._active_events
...
function TerrorEventManager:start_terror_trickle(template_name, spawner_group, delay, use_occluded_positions, limit_spawners, proximity_spawners)
...
function TerrorEventManager:stop_terror_trickle()
	self._terror_trickle_data.active = false

	Managers.event:trigger("terror_event_stopped")
end

function TerrorEventManager:get_terror_trickle_data()
...
function TerrorEventManager:_update_terror_trickle(dt, t)
	local data = self._terror_trickle_data
...
function TerrorEventManager:_start_event(event_name, data)
	local nodes = self._event_templates[event_name]
...
function TerrorEventManager:current_event()
	return self._current_event
end

function TerrorEventManager:_update_event(event, t, dt)
