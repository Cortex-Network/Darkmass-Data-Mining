...
function UIHud:init(elements, visibility_groups, params)
	self._params = params
...
function UIHud:get_all_player_extensions(player, output)
	if player:unit_is_alive() then
		local player_unit = player.player_unit
...
function UIHud:using_input()
	return self._using_cursor or self._element_using_input
end

function UIHud:player()
	return self._player
end

function UIHud:player_unit()
...
function UIHud:player_extensions()
	return self._extensions
end

function UIHud:_peer_id()
	local params = self._params

	return params.peer_id
end

function UIHud:_verify_elements(element_definitions)
...
function UIHud:_setup_elements(element_definitions)
	for _, definition in ipairs(element_definitions) do
		self:_setup_element(definition)
	end
end

function UIHud:_unload_element_packages(element_definitions)
...
function UIHud:_setup_element(definition)
...
function UIHud:_add_element(definition, elements, elements_array)
	local class_name = definition.class_name
	local params = self._params
...
function UIHud:element(element_name)
	local elements = self._elements
...
function UIHud:_update_element_visibility()
...
function UIHud:_apply_hud_scale()
...
function UIHud:_abort_hud_scale()
	local render_settings = self._render_settings
	local scale = RESOLUTION_LOOKUP.scale
...
function UIHud:ui_renderer()
	return self._ui_renderer
end

function UIHud:update(dt, t, input_service)
	local ui_renderer = self._ui_renderer
...
function UIHud:draw(dt, t, input_service)
...
function UIHud:destroy(disable_world_bloom)
...
function UIHud:_activate_mouse_cursor()
...
function UIHud:_deactivate_mouse_cursor()
	local input_manager = Managers.input
	local name = self.__class_name
...
function UIHud:tactical_overlay_active()
	return self._tactical_overlay_active or false
end

function UIHud:event_set_tactical_overlay_state(active)
	self._tactical_overlay_active = active
end

function UIHud:event_set_communication_wheel_state(state)
	self._communication_wheel_state = state
end

function UIHud:communication_wheel_active()
	return self._communication_wheel_state == "active"
end

function UIHud:communication_wheel_wants_camera_control()
	local state = self._communication_wheel_state

	return state == "active" or state == "camera_lock"
end

function UIHud:event_update_hud_scale(value)
	self._hud_scale_modified = true
end

function UIHud:is_onboarding()
