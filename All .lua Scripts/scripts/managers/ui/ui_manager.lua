...
function UIManager:get_delta_time()
...
function UIManager:_setup_icon_renderers()
...
function UIManager:renderer_by_name(name)
	local data = self._renderers[name]

	return data.renderer
end

function UIManager:destroy_renderer(name)
	local data = self._renderers[name]
...
function UIManager:create_renderer(name, world, create_resource_target, gui, gui_retained, material_name)
...
function UIManager:load_hud_packages(element_definitions, complete_callback)
	self:_load_ui_element_packages(element_definitions, "hud", complete_callback)
end

function UIManager:_load_ui_element_packages(element_definitions, reference_key, on_loaded_callback)
...
function UIManager:unload_hud()
	self:_unload_ui_element_packages("hud")
end

function UIManager:_unload_ui_element_packages(reference_key)
	local package_references_data = self._ui_element_package_references[reference_key]
...
function UIManager:create_player_hud(peer_id, local_player_id, elements, visibility_groups)
...
function UIManager:create_spectator_hud(world_viewport_name, peer_id, local_player_id, elements, visibility_groups)
	visibility_groups = visibility_groups or require("scripts/ui/hud/hud_visibility_groups")
...
function UIManager:destroy_spectator_hud()
...
function UIManager:destroy_player_hud()
	if self._hud then
...
function UIManager:input_service()
...
function UIManager:get_input_alias_key(action, optional_service_name)
...
function UIManager:get_action_type(action, optional_service_name)
	local input_service_name = optional_service_name or self._input_service_name
	local input_manager = Managers.input
	local input_service = input_manager:get_input_service(input_service_name)
...
function UIManager:handle_view_hotkeys(hotkey_settings)
	self._update_hotkeys = hotkey_settings
end

function UIManager:_update_view_hotkeys()
...
function UIManager:open_view(view_name, transition_time, close_previous, close_all, close_transition_time, context, settings_override)
...
function UIManager:_show_error_popup(view_name)
	local popup_params = {}
	local view_settings = Views[view_name]
...
function UIManager:use_fullscreen_blur()
	return self._view_handler:use_fullscreen_blur()
end

function UIManager:disable_game_world()
	return self._view_handler:disable_game_world()
end

function UIManager:cb_on_game_state_change(previous_state_name, next_state_name)
	local view_handler = self._view_handler
...
function UIManager:cb_on_game_sub_state_change(previous_state_name, next_state_name)
	self._current_sub_state_name = next_state_name
end

function UIManager:create_viewport(world, viewport_name, viewport_type, viewport_layer, shading_environment_name, shading_callback)
	shading_environment_name = shading_environment_name or GameParameters.default_ui_shading_environment
	local viewport = ScriptWorld.create_viewport(world, viewport_name, viewport_type, viewport_layer, nil, , , , shading_environment_name, shading_callback)

	return viewport
end

function UIManager:create_world(world_name, optional_layer, optional_timer_name, optional_view_connection_name, optional_flags)
...
function UIManager:destroy_world(world)
	local world_manager = Managers.world
...
function UIManager:world()
	return self._world
end

function UIManager:using_cursor_navigation()
	return not InputDevice.gamepad_active
end

function UIManager:using_input()
...
function UIManager:communication_wheel_active()
	local hud = self._hud

	return hud and hud:communication_wheel_active()
end

function UIManager:communication_wheel_wants_camera_control()
	local hud = self._hud

	return hud and hud:communication_wheel_wants_camera_control()
end

function UIManager:wwise_music_state(wwise_state_group_name)
...
function UIManager:play_3d_sound(event_name, position)
	local world = self._world
...
function UIManager:play_unit_sound(event_name, unit, optional_node)
	local world = self._world
	local wwise_world = Managers.world:wwise_world(world)
...
function UIManager:play_2d_sound(event_name)
	local world = self._world
	local wwise_world = Managers.world:wwise_world(world)
...
function UIManager:stop_2d_sound(event_id)
	local world = self._world
	local wwise_world = Managers.world:wwise_world(world)
...
function UIManager:set_2d_sound_parameter(parameter_id, value)
	local world = self._world
	local wwise_world = Managers.world:wwise_world(world)
...
function UIManager:_handle_resolution_modified()
...
function UIManager:update(dt, t)
...
function UIManager:render(dt, t)
...
function UIManager:post_update(dt, t)
	local hud = self._hud
	local spectator_hud = self._spectator_hud
...
function UIManager:load_view(view_name, reference_name, loaded_callback)
...
function UIManager:unload_view(view_name, reference_name, frame_delay_count)
	reference_name = "UIManager_" .. reference_name
...
function UIManager:_unload_package(package_id, frame_delay_count)
...
function UIManager:_update_package_unload_delay()
...
function UIManager:world_extension_manager(world)
...
function UIManager:unregister_world_extension_manager_lookup(world)
	local world_name = World.get_data(world, "__world_name")
	self._ui_extension_managers[world_name] = nil
end

function UIManager:register_world_extension_manager_lookup(world, extension_manager)
	local world_name = World.get_data(world, "__world_name")
	self._ui_extension_managers[world_name] = extension_manager
end

function UIManager:render_black_background()
...
function UIManager:active_popups()
	return self._active_popups
end

function UIManager:event_show_ui_popup(data, callback)
...
function UIManager:event_remove_ui_popup(popup_id)
	local remove = true

	self:_popup_by_id(popup_id, remove)
end

function UIManager:_popup_by_id(popup_id, remove)
	local active_popups = self._active_popups
...
function UIManager:get_current_state_name()
	return self._current_state_name
end

function UIManager:get_current_sub_state_name()
	return self._current_sub_state_name
end

function UIManager:load_profile_portrait(profile, cb, render_context)
...
function UIManager:unload_profile_portrait(id)
	local instance = self._back_buffer_render_handlers.portraits

	instance:unload_profile_portrait(id)
end

function UIManager:event_player_profile_updated(peer_id, local_player_id, profile)
	local instance = self._back_buffer_render_handlers.portraits

	instance:profile_updated(profile)
end

function UIManager:load_appearance_portrait(profile, cb, render_context, prioritized)
...
function UIManager:unload_appearance_portrait(id)
	local instance = self._back_buffer_render_handlers.appearance

	instance:unload_profile_portrait(id)
end

function UIManager:update_player_appearance(profile, prioritized)
	local instance = self._back_buffer_render_handlers.appearance

	instance:profile_updated(profile, prioritized)
end

function UIManager:event_player_appearance_updated(profile)
	local instance = self._back_buffer_render_handlers.appearance

	instance:profile_updated(profile)
end

function UIManager:load_item_icon(real_item, cb, render_context, dummy_profile)
...
function UIManager:unload_item_icon(id)
...
function UIManager:item_icon_updated(item)
	local item_type = item.item_type
...
function UIManager:event_on_render_settings_applied()
...
function UIManager:view_is_available(view_name)
	local view_settings = Views[view_name]
...
function UIManager:ui_constant_elements()
	return self._ui_constant_elements
end

function UIManager:get_hud()
	return self._hud
end
