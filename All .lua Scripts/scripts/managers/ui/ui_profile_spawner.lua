...
function UIProfileSpawner:init(reference_name, world, camera, unit_spawner)
...
function UIProfileSpawner:reset()
...
function UIProfileSpawner:node_world_position(node_name)
...
function UIProfileSpawner:_node(node_name)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:spawn_profile(profile, position, rotation, scale, state_machine, animation_event, face_animation_event, force_highest_mip)
	if self._loading_profile_data then
		self._loading_profile_data.loader:destroy()
...
function UIProfileSpawner:_apply_pending_animation_data()
...
function UIProfileSpawner:assign_animation_variable(index, value)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:assign_animation_event(animation_event)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:assign_face_animation_event(face_animation_event)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:assign_state_machine(state_machine, optional_animation_event, optional_face_animation_event)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:_change_slot_item(slot_id, item)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:cb_on_single_slot_item_loaded(slot_id, item)
	local character_spawn_data = self._character_spawn_data
	local loading_profile_data = self._loading_profile_data
	local use_loader_version = loading_profile_data ~= nil
...
function UIProfileSpawner:destroy()
	if self._single_item_profile_loader then
		self._single_item_profile_loader:destroy()
...
function UIProfileSpawner:_sync_profile_changes()
	local loading_profile_data = self._loading_profile_data
	local use_loader_version = loading_profile_data ~= nil
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:update(dt, t, input_service)
...
function UIProfileSpawner:spawned()
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:loading()
	return self._loading_profile_data ~= nil
end

function UIProfileSpawner:spawned_character_unit()
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:set_position(position)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:_handle_input(input_service, dt)
...
function UIProfileSpawner:_despawn_current_character_profile()
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:_despawn_players_characters()
...
function UIProfileSpawner:_despawn_players_gear()
	local world = self._world
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:_equip_item_for_spawn_character(slot_id, item)
...
function UIProfileSpawner:ignore_slot(slot_id)
	self._ignored_slots[slot_id] = true
end

function UIProfileSpawner:_spawn_character_profile(profile, profile_loader, position, rotation, scale, state_machine, animation_event, face_animation_event, force_highest_mip)
	local loadout = profile.loadout
	local archetype = profile.archetype
	...
	local equipment_component = EquipmentComponent:new(self._world, self._item_definitions, self._unit_spawner, unit_3p, nil, , true)
...
function UIProfileSpawner:cb_on_unit_3p_streaming_complete_equip_item(unit_3p)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:cb_on_unit_3p_streaming_complete(unit_3p)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:wield_slot(slot_id)
	local spawn_data = self._character_spawn_data
...
function UIProfileSpawner:_update_items_visibility()
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:_update_input_rotation(dt)
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:_set_auto_rotation_return(enabled)
	self._auto_rotation_return = enabled
end

function UIProfileSpawner:_set_character_rotation(angle)
	self._rotation_angle = angle
end

function UIProfileSpawner:disable_rotation_input()
	self._rotation_input_disabled = true
end

local mouse_pos_temp = {}

function UIProfileSpawner:_mouse_rotation_input(input_service, dt)
...
function UIProfileSpawner:_controller_rotation_input(input_service, dt)
...
function UIProfileSpawner:_is_character_pressed(input_service)
...
function UIProfileSpawner:_get_raycast_hit(from, to, physics_world, collision_filter)
...
function UIProfileSpawner:character_scale()
	local character_spawn_data = self._character_spawn_data
...
function UIProfileSpawner:set_character_scale(scale_value)
	local character_spawn_data = self._character_spawn_data
