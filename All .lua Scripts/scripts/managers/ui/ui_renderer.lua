...
function UIRenderer.add_render_pass(ui_renderer, sort_key, name, clear, ...)
...
function UIRenderer.clear_render_pass_queue(ui_renderer)
	table.clear(ui_renderer.render_passes)
end

function UIRenderer.create_viewport_renderer(world, ...)
...
function UIRenderer.create_resource_renderer(world, gui, gui_retained, reference_name, material_name, ...)
...
function UIRenderer.create_ui_renderer(world, gui, gui_retained, name, render_target, render_target_material)
...
function UIRenderer:destroy(world)
...
function UIRenderer:create_material(material_name, retained_mode)
...
function UIRenderer:destroy_material(material, retained_mode)
	local gui = retained_mode and self.gui_retained or self.gui

	Gui.destroy_material(gui, material)
end

function UIRenderer:material(material_name, retained_mode)
	local gui = retained_mode and self.gui_retained or self.gui
	local render_settings = self.render_settings
	local material_flags = _get_material_flag(render_settings, nil, self.render_pass_flag)
...
function UIRenderer:script_draw_bitmap(material, gui_position, gui_size, color, retained_id)
...
function UIRenderer:script_draw_bitmap_uv(material, gui_position, gui_size, uvs, color, retained_id)
	table.clear(optional_gui_args)
...
function UIRenderer:script_draw_bitmap_3d(material, tm, gui_position, gui_layer, gui_size, color, optional_uvs, retained_id)
	table.clear(optional_gui_args)
...
function UIRenderer:script_draw_text(text, font_size, font_type, gui_position, gui_size, color, options, retained_id)
...
function UIRenderer:script_draw_text_3d(text, font_size, font_type, tm, gui_position, gui_layer, gui_size, color, options, retained_id)
	local render_settings = self.render_settings
	local retained_mode = not not retained_id
...
function UIRenderer:create_video_player(reference_name, world, resource, set_loop)
	local video_players = self.video_players
...
function UIRenderer:destroy_video_player(reference_name, world)
	local video_players = self.video_players
...
function UIRenderer:clear_scenegraph_queue()
...
function UIRenderer:begin_pass(ui_scenegraph, input_service, dt, render_settings)
...
function UIRenderer:draw_slug_icon(resource, index, position, size, color, optional_material, material_flags, retained_id)
	local render_settings = self.render_settings
	local retained_mode = not not retained_id
...
function UIRenderer:draw_slug_multi_icon(resource, index, position, size, color, axis, spacing, direction, draw_count, optional_material, retained_ids)
	axis = axis or 1
...
function UIRenderer:draw_slug_icon_rotated(resource, index, size, position, angle, pivot, color, optional_material, retained_id)
	local render_settings = self.render_settings
	local retained_mode = not not retained_id
...
function UIRenderer:draw_slug_picture(resource, position, size, color, optional_material, retained_id)
	local render_settings = self.render_settings
	local retained_mode = not not retained_id
...
function UIRenderer:draw_rect(gui_position, gui_size, color, retained_id)
	table.clear(optional_gui_args)
...
function UIRenderer:draw_triangle(position, size, ui_style, retained_id)
	local render_settings = self.render_settings
	local retained_mode = not not retained_id
...
function UIRenderer:draw_rect_rotated(size, position, angle, pivot, color)
	local scale = self.scale
	size = UIResolution.scale_vector(size, scale)
	local scaled_pivot = UIResolution.scale_vector(pivot, scale)
	local tm = Rotation2D(Vector3.zero(), angle, Vector2(scaled_pivot[1], scaled_pivot[2]))
...
function UIRenderer:draw_texture(material, position, size, color, retained_id)
	local scale = self.scale
...
function UIRenderer:draw_multi_texture(material, position, size, color, axis, spacing, direction, draw_count, retained_ids)
	axis = axis or 1
	direction = direction or 1
	local retained_mode = not not retained_ids
...
function UIRenderer:draw_texture_uv(material, position, size, uvs, color, retained_id)
	local scale = self.scale
	local gui_position = Vector3(position[1] * scale, position[2] * scale, position[3] or 0)
	local gui_size = Vector3(size[1] * scale, size[2] * scale, size[3] or 0)
...
function UIRenderer:draw_texture_rotated(material, size, position, angle, pivot, color, optional_uvs, retained_id)
	local scale = self.scale
...
function UIRenderer:draw_text(text, font_size, font_type, position, size, color, options, retained_id)
	local scale = self.scale
	position = UIResolution.scale_vector(position, scale)
...
function UIRenderer:word_wrap(text, font_type, font_size, width)
	local whitespace = " 。，"
	local soft_dividers = " -+&/*"
...
function UIRenderer:text_width(text, font_type, font_size, optional_size, options)
end

function UIRenderer:text_height(text, font_type, font_size, optional_size, options)
	local font_data = UIFonts.data_by_type(font_type)
	local font = font_data.path
	local flags = font_data.render_flags or 0
...
function UIRenderer:text_size(text, font_type, font_size, optional_size, options)
	local font_data = UIFonts.data_by_type(font_type)
	local font = font_data.path
	local flags = font_data.render_flags or 0
	optional_size = optional_size and Vector2(optional_size[1], optional_size[2])
...
function UIRenderer:draw_video(material_name, position, size, color, video_player_reference, y_slot_name, uv_slot_name, optional_video_player)
	local gui = self.gui
...
function UIRenderer:draw_circle(position, radius, size, color)
	local gui = self.gui
	local scale = self.scale
...
function UIRenderer:is_clipped(position, size)
...
function UIRenderer:clip_is_enclosing(position, size)
	local x0, y0, x1, y1 = unpack(self.current_clipping_rect)
...
function UIRenderer.crop_text(text, max_chars)
...
function UIRenderer:crop_text_width(text, font_type, font_size, max_width, crop_from_left)
	local scale = self.scale or 1
...
function UIRenderer:scaled_font_size_by_width(text, font_type, font_size, max_width)
	local scale = self.scale
...
function UIRenderer:destroy_bitmap(retained_id)
	Gui.destroy_bitmap(self.gui_retained, retained_id)
end

function UIRenderer:destroy_text(retained_id)
	Gui.destroy_text(self.gui_retained, retained_id)
end

function UIRenderer:destroy_slug_icon(retained_id)
	Gui.destroy_slug_icon(self.gui_retained, retained_id)
end

function UIRenderer:destroy_slug_picture(retained_id)
