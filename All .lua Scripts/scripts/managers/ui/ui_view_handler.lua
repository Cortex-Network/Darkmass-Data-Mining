...
function UIViewHandler:init(view_list, timer_name)
...
function UIViewHandler:cb_trigger_view_widget_pressed(view_name, widget_name, optional_content_id)
...
function UIViewHandler:open_view(view_name, transition_time, close_previous, close_all, close_transition_time, context, settings_override)
...
function UIViewHandler:can_close(view_name)
...
function UIViewHandler:allow_to_pass_input_for_view(view_name)
	local active_views_data = self._active_views_data
	local view_data = active_views_data[view_name]
...
function UIViewHandler:allow_close_hotkey_for_view(view_name)
	local active_views_data = self._active_views_data
	local view_data = active_views_data[view_name]
	local view_instance = view_data.instance
...
function UIViewHandler:close_all_views(force_close, optional_excepted_views)
	local active_views_array = self._active_views_array
...
function UIViewHandler:close_view(view_name, force_close)
...
function UIViewHandler:wwise_music_state(wwise_state_group_name)
	local active_views_array = self._active_views_array
...
function UIViewHandler:destroy()
...
function UIViewHandler:_update_transition_progress(dt, transition_progress)
...
function UIViewHandler:update(dt, t, allow_input)
...
function UIViewHandler:draw(dt, t)
...
function UIViewHandler:transitioning()
	return self._transitioning
end

function UIViewHandler:is_view_loading(view_name)
	local instance = self:view_instance(view_name)

	return instance and instance:loading()
end

function UIViewHandler:post_update(dt, t)
	self:_post_update_views(dt, t)
end

function UIViewHandler:view_active(view_name)
	local active_views_data = self._active_views_data

	return active_views_data[view_name] ~= nil
end

function UIViewHandler:is_view_closing(view_name)
	local view_data = self._active_views_data[view_name]

	return view_data and view_data.closing
end

function UIViewHandler:settings_by_view_name(view_name)
...
function UIViewHandler:view_instance(view_name)
	local view_data = self._active_views_data[view_name]
...
function UIViewHandler:active_views()
	local active_views_array = self._active_views_array
...
function UIViewHandler:_update_views(dt, t, allow_input)
	local active_views_array = self._active_views_array
	local active_views_data = self._active_views_data
				...
				if instance:on_exit_animation_done() then
					if view_data.use_transition_ui then
						if view_data.use_transition_ui_on_animation_on_exit_done then
							...
							function view_data.fade_in_callback()
...
function UIViewHandler:_post_update_views(dt, t)
	local active_views_array = self._active_views_array
	local active_views_data = self._active_views_data
	local num_active_views = self._num_active_views
...
function UIViewHandler:_can_draw_view(view_name)
	local active_views_data = self._active_views_data
	local view_data = active_views_data[view_name]
...
function UIViewHandler:_draw_views(dt, t, allow_input, transitioning_in, transitioning_out)
	local active_views_array = self._active_views_array
	local active_views_data = self._active_views_data
	local input_service, null_service, gamepad_active = self:_get_input()
...
function UIViewHandler:_get_time()
...
function UIViewHandler:_get_input()
	local ui_manager = Managers.ui

	return ui_manager:input_service()
end

function UIViewHandler:_force_close(view_name)
	local active_views_array = self._active_views_array
	local active_views_data = self._active_views_data
	local view_data = active_views_data[view_name]
	local instance = view_data.instance
...
function UIViewHandler:_open(view_name, opening_duration, context, settings_override)
	local active_views_array = self._active_views_array
	local active_views_data = self._active_views_data
...
function UIViewHandler:top_draw_layer()
	return self._top_draw_layer
end

function UIViewHandler:_set_game_world_disabled(disabled)
	self._game_world_disabled = disabled
end

function UIViewHandler:use_fullscreen_blur()
	return self._game_world_fullscreen_blur_enabled, self._game_world_fullscreen_blur_amount
end

function UIViewHandler:disable_game_world()
	return self._game_world_disabled
end

function UIViewHandler:register_view_world(view_name, world_name, layer)
...
function UIViewHandler:unregister_world(world_name)
	local registered_view_worlds = self._registered_view_worlds
...
function UIViewHandler:_set_view_worlds_enabled(view_name, enabled)
	local registered_view_worlds = self._registered_view_worlds
...
function UIViewHandler:_set_view_worlds_layer(view_name, layer)
	local registered_view_worlds = self._registered_view_worlds
	local worlds_table = registered_view_worlds[view_name]
