...
function UIWeaponSpawner:init(reference_name, world, camera, unit_spawner)
...
function UIWeaponSpawner:node_world_position(node_name)
...
function UIWeaponSpawner:_node(node_name)
	local weapon_spawn_data = self._weapon_spawn_data
...
function UIWeaponSpawner:cb_on_item_package_loaded(slot_id, item, on_spawn_cb)
	if on_spawn_cb then
		on_spawn_cb(slot_id, item)
	end
end

function UIWeaponSpawner:destroy()
...
function UIWeaponSpawner:update(dt, t, input_service)
	local weapon_spawn_data = self._weapon_spawn_data
...
function UIWeaponSpawner:_handle_input(input_service, dt)
	self:_mouse_rotation_input(input_service, dt)
end

function UIWeaponSpawner:_despawn_current_weapon()
	local weapon_spawn_data = self._weapon_spawn_data
...
function UIWeaponSpawner:_despawn_weapon()
...
function UIWeaponSpawner:set_position(position)
	local weapon_spawn_data = self._weapon_spawn_data
...
function UIWeaponSpawner:cb_on_unit_3p_streaming_complete(item_unit_3p)
	local weapon_spawn_data = self._weapon_spawn_data
...
function UIWeaponSpawner:spawned()
	return self._weapon_spawn_data ~= nil
end

function UIWeaponSpawner:_update_input_rotation(dt)
	local weapon_spawn_data = self._weapon_spawn_data
...
function UIWeaponSpawner:_set_rotation(angle)
	self._rotation_angle = angle
end

local mouse_pos_temp = {}

function UIWeaponSpawner:_mouse_rotation_input(input_service, dt)
...
function UIWeaponSpawner:set_forced_pressed()
	self._forced_pressed = true
end

function UIWeaponSpawner:_is_pressed(input_service)
...
function UIWeaponSpawner:activate_auto_spin(ignore_randomness)
	self._auto_spin_random_seed = ignore_randomness and 0 or math.random(5, 30000)
end

function UIWeaponSpawner:_auto_spin_values(dt, t)
