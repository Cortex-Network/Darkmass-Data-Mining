...
function VotingManagerImmateriumParty:init()
	self._current_vote_id = nil
	self._current_vote_state = nil
	self._casted_votes = {}
end

function VotingManagerImmateriumParty:_current_vote()
	return Managers.party_immaterium:party_vote_state()
end

function VotingManagerImmateriumParty:_can_start_matchmaking()
	return Managers.party_immaterium:current_game_session_id() == nil
end

function VotingManagerImmateriumParty:_get_myself()
	return Managers.party_immaterium:get_myself()
end

function VotingManagerImmateriumParty:_unwrap_vote_id(id)
	local p = "immaterium_party:"

	return string.sub(id, p:len() + 1, id:len())
end

function VotingManagerImmateriumParty:_wrap_vote_id(id)
	return string.format("immaterium_party:%s", id)
end

function VotingManagerImmateriumParty:can_start_voting(template_name, params)
...
function VotingManagerImmateriumParty:start_voting(template_name, params)
...
function VotingManagerImmateriumParty:can_cast_vote(voting_id, option, voter_peer_id)
...
function VotingManagerImmateriumParty:current_vote()
	return self:_current_vote()
end

function VotingManagerImmateriumParty:cast_vote(voting_id, option)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:update(dt, t)
...
function VotingManagerImmateriumParty:member_list(voting_id)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:votes(voting_id)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:time_left(voting_id)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:voting_template(voting_id)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:voting_exists(voting_id)
	local current_vote = self:_current_vote()

	return self:_wrap_vote_id(current_vote.id) == voting_id
end

function VotingManagerImmateriumParty:voting_result(voting_id)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:has_voted(voting_id, voter_peer_id)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:is_host(voting_id)
	local current_vote = self:_current_vote()
...
function VotingManagerImmateriumParty:complete_vote(voting_id)
end
