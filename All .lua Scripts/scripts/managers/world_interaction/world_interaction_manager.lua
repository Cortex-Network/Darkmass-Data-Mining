...
function WorldInteractionManager:init(world)
	self._world = world
	self._water_timer = 0
	self._water_ripples = {}
	self._units = {}
...
function WorldInteractionManager:_setup_gui()
...
function WorldInteractionManager:add_world_interaction(material, unit)
...
function WorldInteractionManager:remove_world_interaction(unit, material_to_ignore)
...
function WorldInteractionManager:_add_water_ripple(pos, angle, material, random_size_diff, stretch_multiplier, ref_time, size, multiplier)
...
function WorldInteractionManager:add_simple_effect(material, unit, position)
...
function WorldInteractionManager:_add_simple_water_effect(unit, position)
	local water_settings = WorldInteractionSettings.water
...
function WorldInteractionManager:update(dt, t)
...
function WorldInteractionManager:_update_water(dt, t)
...
function WorldInteractionManager:_cleanup_removed_units()
...
function WorldInteractionManager:_update_water_data(dt, t)
	local water_settings = WorldInteractionSettings.water
	local window_size = math.clamp(water_settings.window_size, 1, 100)
...
function WorldInteractionManager:_update_water_ripples(dt, t)
...
function WorldInteractionManager:destroy()
