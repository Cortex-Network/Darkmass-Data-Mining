...
function WorldLevelDespawnManager:init()
	self._state = STATES.idle
end

function WorldLevelDespawnManager:destroy()
	self:flush()
end

function WorldLevelDespawnManager:despawn(world, world_name, level, level_name, themes, use_time_slice)
...
function WorldLevelDespawnManager:flush()
	if self._state == STATES.idle then
		return
	end
...
function WorldLevelDespawnManager:update(dt, t)
...
function WorldLevelDespawnManager:despawning()
	return self._state ~= STATES.idle
end

function WorldLevelDespawnManager:_init_timeslice_despawn_units(level)
...
function WorldLevelDespawnManager:_update_time_slice_despawn_units()
...
function WorldLevelDespawnManager:_delete_unit(unit)
...
function WorldLevelDespawnManager:_check_orphaned_units()
	local world = self._world
