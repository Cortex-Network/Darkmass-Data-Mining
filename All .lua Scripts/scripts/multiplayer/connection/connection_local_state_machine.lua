...
function ConnectionLocalStateMachine:init(event_delegate, engine_lobby, host_peer_id, network_hash, host_type, profile_synchronizer_client, slots_to_reserve, jwt_ticket)
	local parent = nil
	local shared_state = {
		boot_complete = false,
		has_reserved = false,
...
function ConnectionLocalStateMachine:destroy()
	self:disconnect()
...
function ConnectionLocalStateMachine:update(dt)
	self._state_machine:update(dt)
end

function ConnectionLocalStateMachine:disconnect(optional_game_reason)
	self._state_machine:event("disconnected", {
		game_reason = optional_game_reason or "game_request"
	})
end

function ConnectionLocalStateMachine:channel_id()
	return self._shared_state.channel_id
end

function ConnectionLocalStateMachine:host_is_dedicated_server()
	return self._shared_state.is_dedicated
end

function ConnectionLocalStateMachine:max_members()
	return self._shared_state.max_members
end

function ConnectionLocalStateMachine:session_seed()
	return self._shared_state.session_seed
end

function ConnectionLocalStateMachine:has_disconnected()
	return self._state_machine:state().__class_name == "LocalDisconnectedState"
end

function ConnectionLocalStateMachine:has_reserved()
	return self._shared_state.has_reserved
end

function ConnectionLocalStateMachine:ready_to_join()
	self._shared_state.ready_to_claim_slots = true
end

function ConnectionLocalStateMachine:boot_complete()
	self._shared_state.boot_complete = true
end

function ConnectionLocalStateMachine:next_event()
