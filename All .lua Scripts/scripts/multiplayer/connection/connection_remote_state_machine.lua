...
function ConnectionRemoteStateMachine:init(event_delegate, engine_lobby, channel_id, network_hash, slot_reserver, optional_eac_server, is_dedicated_server, host_type, profile_synchronizer_host, max_members, session_seed)
...
function ConnectionRemoteStateMachine:destroy()
	self:disconnect()
...
function ConnectionRemoteStateMachine:update(dt)
	self._state_machine:update(dt)
end

function ConnectionRemoteStateMachine:is_connected()
	local state_name = self._state_machine:state().__class_name

	return state_name == "RemoteConnectedState"
end

function ConnectionRemoteStateMachine:is_disconnected()
	local state_name = self._state_machine:state().__class_name

	return state_name == "RemoteDisconnectedState"
end

function ConnectionRemoteStateMachine:is_connecting()
	return not self:is_connected() and not self:is_disconnected()
end

function ConnectionRemoteStateMachine:client_entered_connected_state()
	local shared_state = self._shared_state

	return shared_state.client_entered_connected_state
end

function ConnectionRemoteStateMachine:connection_notification_parameters()
	local shared_state = self._shared_state
...
function ConnectionRemoteStateMachine:peer_joined(peer_id)
...
function ConnectionRemoteStateMachine:peer_left(peer_id)
	local old = self._known_peers[peer_id] or false
...
function ConnectionRemoteStateMachine:peer_id()
	return self._peer_id
end

function ConnectionRemoteStateMachine:disconnect(optional_game_reason)
	self._state_machine:event("disconnected", {
		game_reason = optional_game_reason or "game_request"
	})
end

function ConnectionRemoteStateMachine:next_event()
