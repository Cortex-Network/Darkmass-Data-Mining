...
function RemoteAwaitConnectionBootedState:init(state_machine, shared_state)
...
function RemoteAwaitConnectionBootedState:destroy()
	local shared_state = self._shared_state

	shared_state.event_delegate:unregister_channel_events(shared_state.channel_id, unpack(RPCS))
end

function RemoteAwaitConnectionBootedState:update(dt)
	local shared_state = self._shared_state
...
function RemoteAwaitConnectionBootedState:rpc_connection_booted(channel_id)
