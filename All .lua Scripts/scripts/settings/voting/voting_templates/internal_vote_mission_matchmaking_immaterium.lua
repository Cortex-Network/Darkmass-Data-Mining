...
function internal_vote_mission_matchmaking_immaterium.fetch_my_vote_params(template, params, option)
	if option == "yes" then
...
function internal_vote_mission_matchmaking_immaterium.on_aborted(voting_id, template, params, abort_reason)
end

function internal_vote_mission_matchmaking_immaterium.on_vote_casted(voting_id, template, voter_peer_id, vote_option)
end
