...
function ItemPreviewer:init(world)
...
function ItemPreviewer:destroy()
	World.destroy_gui(self.previewer_world, self.gui)
end

function ItemPreviewer:update(dt, t)
...
function ItemPreviewer:cleanup()
...
function ItemPreviewer:preview(resource, return_data)
	self.preview_type = "3D"
...
function ItemPreviewer:_spawn_item(item_data, root_unit)
...
function ItemPreviewer:_build_bounding_box(item_data)
...
function ItemPreviewer:spawn_unit(base_unit, item_material_slot_overrides)
	return World.spawn_unit(self.previewer_world, base_unit, item_material_slot_overrides)
end

function ItemPreviewer:_select_root_unit_resource(item_data)
...
function ItemPreviewer:_select_hardcoded_bounding_box(item_data)
	local breeds = item_data.breeds
	local slots = item_data.slots
	local item_type = item_data.item_type
...
function ItemPreviewer:get_bounding_box()
...
function ItemPreviewer:get_supported_types()
	return {
		"item"
	}
end

function ItemPreviewer:allow_camera_horizontal_orbit()
	return true
end

function ItemPreviewer:allow_camera_vertical_orbit()
	return true
end
