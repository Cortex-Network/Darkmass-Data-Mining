...
function ConstantElementChat:init(parent, draw_layer, start_scale, definitions)
...
function ConstantElementChat:destroy()
...
function ConstantElementChat:update(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantElementChat:using_input()
	return self._input_field_widget.content.is_writing and self._is_visible
end

function ConstantElementChat:draw(dt, t, ui_renderer, render_settings, input_service)
	ConstantElementChat.super.draw(self, dt, t, ui_renderer, render_settings, input_service)
end

function ConstantElementChat:set_visible(visible, optional_visibility_parameters)
...
function ConstantElementChat:should_update()
	return self._num_connected_channels > 0
end

function ConstantElementChat:should_draw()
	return (self._is_visible or self._alpha > 0) and self:should_update()
end

function ConstantElementChat:cb_chat_manager_login_state_change(login_state)
...
function ConstantElementChat:cb_chat_manager_added_channel(channel_handle, channel)
	self._channels[channel_handle] = channel
end

function ConstantElementChat:cb_chat_manager_removed_channel(channel_handle)
...
function ConstantElementChat:cb_chat_manager_updated_channel_state(channel_handle, state)
...
function ConstantElementChat:cb_chat_manager_message_recieved(channel_handle, participant, message)
...
function ConstantElementChat:cb_chat_manager_participant_added(channel_handle, participant)
...
function ConstantElementChat:cb_chat_manager_participant_update(channel_handle, participant)
end

function ConstantElementChat:cb_chat_manager_participant_removed(channel_handle, participant_uri, participant)
	if not participant.is_current_user and not participant.is_text_muted_for_me then
		local channel = self._channels[channel_handle]
		local channel_tag = channel.tag
...
function ConstantElementChat:cb_chat_manager_connection_state_change(state)
...
function ConstantElementChat:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function ConstantElementChat:_draw_message_widgets(ui_renderer)
...
function ConstantElementChat:_setup_input()
...
function ConstantElementChat:_setup_input_labels()
...
function ConstantElementChat:_get_localized_input_text(action)
...
function ConstantElementChat:_handle_input(input_service, ui_renderer)
	local input_widget = self._input_field_widget
	local is_input_field_active = input_widget.content.is_writing
...
function ConstantElementChat:_handle_active_chat_input(input_service, ui_renderer)
	local input_widget = self._input_field_widget
...
function ConstantElementChat:_update_caret_position(ui_renderer, input_widget, caret_position, is_caret_visible)
...
function ConstantElementChat:_next_connected_channel(current_priority_index)
...
function ConstantElementChat:_update_input_field_selected_channel(ui_renderer, widget, selected_channel)
...
function ConstantElementChat:_update_message_widgets_sizes(ui_renderer)
	local spacing = ChatSettings.message_spacing
	local message_widgets = self._message_widgets
...
function ConstantElementChat:_scroll_to_latest_message()
...
function ConstantElementChat:_update_scrollbar(render_settings)
...
function ConstantElementChat:_calculate_first_visible_widget(scroll_length_from_top, chat_window_height)
	local message_widgets = self._message_widgets
...
function ConstantElementChat:_add_notification(message, channel_tag)
...
function ConstantElementChat:_add_message(message, sender, channel_tag)
...
function ConstantElementChat:_scrub(text)
...
function ConstantElementChat:_format_chat_message(message)
...
function ConstantElementChat:_add_message_widget_to_message_list(new_message, new_message_widget)
	local messages = self._messages
	local message_widgets = self._message_widgets
...
function ConstantElementChat:_calculate_widget_size(widget, ui_renderer)
...
function ConstantElementChat:_enable_mouse_cursor(is_mouse_cursor_enabled)
...
function ConstantElementChat:_on_connect_to_channel(channel_handle)
	local channel = self._channels[channel_handle]
	local channel_tag = channel.tag
	local connected_channels = self._connected_channels
...
function ConstantElementChat:_on_disconnect_from_channel(channel_handle)
	local connected_channels = self._connected_channels
...
function ConstantElementChat:_channel_name(channel_tag, colorize)
...
function ConstantElementChat:_channel_color(channel_tag)
	local channel_metadata = ChatSettings.channel_metadata[channel_tag]
...
function ConstantElementChat:_text_size(ui_renderer, text, font_style, max_width)
...
function ConstantElementChat:_cancel_animations_if_necessary(force_cancellation)
...
function ConstantElementChat:_apply_visibility_parameters(optional_visibility_parameters)
