...
function ConstantElementPopupHandler:init(parent, draw_layer, start_scale)
...
function ConstantElementPopupHandler:using_input()
	return self._handling_popups
end

function ConstantElementPopupHandler:_push_cursor()
...
function ConstantElementPopupHandler:_pop_cursor()
...
function ConstantElementPopupHandler:event_update_popup_message(popup_id, text, sound_event)
	self:_set_text(popup_id, text)
end

function ConstantElementPopupHandler:_destroy_default_gui()
...
function ConstantElementPopupHandler:_destroy_background()
...
function ConstantElementPopupHandler:_get_active_popup()
	local active_popups = Managers.ui:active_popups()

	return active_popups[1]
end

function ConstantElementPopupHandler:_setup_presentation(data, ui_renderer)
...
function ConstantElementPopupHandler:_cleanup_presentation(active_popup, ui_renderer)
...
function ConstantElementPopupHandler:_cb_on_button_pressed(widget)
...
function ConstantElementPopupHandler:trigger_widget_callback(widget)
	self:_cb_on_button_pressed(widget)
end

function ConstantElementPopupHandler:_create_popup_content(options, ui_renderer)
...
function ConstantElementPopupHandler:_set_title_text(text)
	local widgets_by_name = self._widgets_by_name
	widgets_by_name.title_text.content.text = text
end

function ConstantElementPopupHandler:_set_description_text(text)
	local widgets_by_name = self._widgets_by_name
...
function ConstantElementPopupHandler:_get_text_height(text, text_style, ui_renderer)
	local text_options = UIFonts.get_font_options_by_style(text_style)
...
function ConstantElementPopupHandler:_get_text_width(text, text_style, ui_renderer)
	local text_options = UIFonts.get_font_options_by_style(text_style)
...
function ConstantElementPopupHandler:_find_closest_neighbour_button_index(starting_index, direction)
	local content_widgets = self._content_widgets
...
function ConstantElementPopupHandler:_select_button_index(index)
...
function ConstantElementPopupHandler:_update_button_input(input_service)
...
function ConstantElementPopupHandler:update(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantElementPopupHandler:using_cursor_navigation()
	return self._using_cursor_navigation
end

function ConstantElementPopupHandler:_on_navigation_input_changed()
...
function ConstantElementPopupHandler:draw(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantElementPopupHandler:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function ConstantElementPopupHandler:_setup_default_gui()
	local ui_manager = Managers.ui
...
function ConstantElementPopupHandler:_setup_background_gui()
	local ui_manager = Managers.ui
	local class_name = self.__class_name
	local timer_name = "ui"
...
function ConstantElementPopupHandler:cb_shading_callback(world, shading_env, viewport, default_shading_environment_name)
...
function ConstantElementPopupHandler:_set_background_blur(fraction)
...
function ConstantElementPopupHandler:widgets_by_name()
