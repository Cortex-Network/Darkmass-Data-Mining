...
function ConstantElementSubtitles:init(parent, draw_layer, start_scale)
...
function ConstantElementSubtitles:event_player_authenticated()
	self:_initialize_settings()
end

function ConstantElementSubtitles:_initialize_settings()
...
function ConstantElementSubtitles:event_update_subtitles_enabled()
	self:_setup_subtitles_enabled()
end

function ConstantElementSubtitles:event_update_subtitle_text_opacity()
	self:_setup_text_opacity()
end

function ConstantElementSubtitles:event_update_subtitles_font_size()
	self:_setup_font_size()
end

function ConstantElementSubtitles:event_update_subtitle_speaker_enabled()
...
function ConstantElementSubtitles:event_update_subtitles_background_opacity()
	self:_setup_letterbox()
end

function ConstantElementSubtitles:_setup_subtitles_enabled()
...
function ConstantElementSubtitles:_setup_font_size()
	local save_data = Managers.save:account_data()
	local interface_settings = save_data.interface_settings
...
function ConstantElementSubtitles:_update_letterbox_size()
...
function ConstantElementSubtitles:_setup_letterbox()
	local save_data = Managers.save:account_data()
	local interface_settings = save_data.interface_settings
...
function ConstantElementSubtitles:_setup_text_opacity()
	local save_data = Managers.save:account_data()
	local interface_settings = save_data.interface_settings
...
function ConstantElementSubtitles:_set_text_opacity(alpha)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.subtitles
...
function ConstantElementSubtitles:_set_letterbox_opacity(alpha)
	local widget = self._letterbox_widget
	widget.style.rect.color[1] = alpha or 0
	self._draw_letterbox = alpha ~= nil
end

function ConstantElementSubtitles:destroy()
	self:_unregister_events()
	ConstantElementSubtitles.super.destroy(self)
end

function ConstantElementSubtitles:_debug_add_subtitle_lines()
...
function ConstantElementSubtitles:_get_active_dialogue_system()
...
function ConstantElementSubtitles:update(dt, t, ui_renderer, render_settings, input_service)
...
function ConstantElementSubtitles:_debug_trigger_subtitle(text, duration)
	if self._line_duration then
...
function ConstantElementSubtitles:_set_font_size(new_size)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.subtitles
	local style = widget.style
	local text_style = style.text
...
function ConstantElementSubtitles:_display_text_line(text, duration)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.subtitles
	local content = widget.content
	content.text = text
...
function ConstantElementSubtitles:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function ConstantElementSubtitles:_register_events()
...
function ConstantElementSubtitles:_unregister_events()
	local event_manager = Managers.event
	local events = ConstantElementSubtitlesSettings.events
