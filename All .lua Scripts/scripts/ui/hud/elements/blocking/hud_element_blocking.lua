...
function HudElementBlocking:init(parent, draw_layer, start_scale)
...
function HudElementBlocking:destroy()
	HudElementBlocking.super.destroy(self)
end

function HudElementBlocking:_add_shield()
	self._shields[#self._shields + 1] = {}
end

function HudElementBlocking:_remove_shield()
	self._shields[#self._shields] = nil
end

function HudElementBlocking:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementBlocking:_update_shield_amount()
...
function HudElementBlocking:_update_visibility(dt)
	local draw = false
	local parent = self._parent
	local player_extensions = parent:player_extensions()
...
function HudElementBlocking:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementBlocking:_draw_shields(dt, t, ui_renderer)
