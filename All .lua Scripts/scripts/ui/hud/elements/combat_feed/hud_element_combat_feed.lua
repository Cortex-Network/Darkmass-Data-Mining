...
function HudElementCombatFeed:init(parent, draw_layer, start_scale)
...
function HudElementCombatFeed:_can_spawn_message()
...
function HudElementCombatFeed:_get_unit_presentation_name(unit)
...
function HudElementCombatFeed:_color_by_enemy_tags(tags)
	local color = nil
...
function HudElementCombatFeed:event_combat_feed_kill(attacking_unit, attacked_unit)
...
function HudElementCombatFeed:event_add_combat_feed_message(text)
	self:_add_combat_feed_message(text)
end

function HudElementCombatFeed:_add_combat_feed_message(text)
	local _, notification_id = self:_add_notification_message("default")

	self:_set_text(notification_id, text)
end

function HudElementCombatFeed:destroy()
...
function HudElementCombatFeed:_add_notification_message(type)
...
function HudElementCombatFeed:_remove_notification(notification_id)
	local remove = true

	self:_notification_by_id(notification_id, remove)
end

function HudElementCombatFeed:_create_notification_entry(notification_type)
...
function HudElementCombatFeed:_notification_by_id(notification_id, remove)
	local notifications = self._notifications
...
function HudElementCombatFeed:_set_text(notification_id, text)
	local notification = self:_notification_by_id(notification_id)
	local widget = notification.widget
	widget.content.text = text
end

function HudElementCombatFeed:_set_icon(notification_id, icon)
	local notification = self:_notification_by_id(notification_id)
	local widget = notification.widget
	widget.content.icon = icon
end

function HudElementCombatFeed:_get_notifications_text_height(notification, ui_renderer)
	local widget = notification.widget
...
function HudElementCombatFeed:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementCombatFeed:_align_notification_widgets(dt)
...
function HudElementCombatFeed:_get_height_of_notification_index(index)
	local notifications = self._notifications
...
function HudElementCombatFeed:_set_widget_position(widget, x, y)
	local widget_offset = widget.offset
...
function HudElementCombatFeed:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
