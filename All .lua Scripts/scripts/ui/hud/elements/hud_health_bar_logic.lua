...
function HudHealthBarLogic:init(settings)
...
function HudHealthBarLogic:_verify_settings(settings)
	for i = 1, #settings_list do
		local setting_name = settings_list[i]
	end
end

function HudHealthBarLogic:_sync_player_health(t, bar_progress, bar_max_progress)
...
function HudHealthBarLogic:update(dt, t, bar_progress, bar_max_progress)
...
function HudHealthBarLogic:_set_bar_fraction(t, name, target_fraction, delta_fraction, total_duration, force_update, animation_delay, refresh_delay_on_override)
...
function HudHealthBarLogic:_update_bar_animations(dt, t)
	local bar_animations = self._bar_animations
...
function HudHealthBarLogic:_reset_bar_alpha()
	self:_set_alpha(255)
...
function HudHealthBarLogic:_update_bar_alpha(dt, t)
...
function HudHealthBarLogic:_set_alpha(alpha)
	local alpha_fraction = alpha / 255
	self._alpha_multiplier = alpha_fraction
end

function HudHealthBarLogic:_update_bar_animation(anim_data, dt, t)
...
function HudHealthBarLogic:alpha_multiplier()
	return self._alpha_multiplier
end

function HudHealthBarLogic:animated_health_fractions()
	return self._health_fraction, self._health_ghost_fraction, self._health_max_fraction
end

function HudHealthBarLogic:_set_animated_health_fractions(health_fraction, health_ghost_fraction, health_max_fraction)
