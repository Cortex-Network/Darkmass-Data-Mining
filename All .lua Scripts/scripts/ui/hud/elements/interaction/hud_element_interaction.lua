...
function HudElementInteraction:init(parent, draw_layer, start_scale)
...
function HudElementInteraction:_set_event_popup_visibility(visible)
	self._widgets_by_name.event_background.content.visible = visible
end

function HudElementInteraction:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementInteraction:_interaction_extension_scan()
...
function HudElementInteraction:_on_interaction_marker_spawned(unit, id)
	self._interaction_units[unit].marker_id = id
end

function HudElementInteraction:destroy()
	local event_manager = Managers.event
	local interaction_units = self._interaction_units
...
function HudElementInteraction:_update_can_interact_target()
	local parent = self._parent
	local player = parent:player()
	local ALIVE = ALIVE
	local player_unit = player.player_unit
...
function HudElementInteraction:_update_tag_input_information(interactee_unit)
	local parent = self._parent
	local player = parent:player()
	local player_unit = player.player_unit
...
function HudElementInteraction:_cb_world_markers_list_request(marker_list)
	local active_presentation_data = self._active_presentation_data
...
function HudElementInteraction:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementInteraction:_update_progress_complete_anim(dt, t)
...
function HudElementInteraction:_update_target_interaction_hold_progress(dt, t)
	local active_presentation_data = self._active_presentation_data
...
function HudElementInteraction:_update_target_interaction_animation(dt, t)
	local active_presentation_data = self._active_presentation_data
...
function HudElementInteraction:_update_interaction_hud_position(dt, t)
	local active_presentation_data = self._active_presentation_data
	local marker = active_presentation_data.marker
...
function HudElementInteraction:_present_interaction_hud()
	return self._show_interaction_hud
end

function HudElementInteraction:is_synchronized_with_interactee(interactee_unit, interactor_extension)
...
function HudElementInteraction:_setup_interaction_information(interactee_unit, interactee_extension, interactor_extension)
...
function HudElementInteraction:_play_3d_sound(event_name, position)
