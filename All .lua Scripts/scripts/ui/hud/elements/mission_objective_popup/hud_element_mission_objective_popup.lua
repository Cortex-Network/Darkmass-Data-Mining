...
function HudElementMissionObjectivePopup:destroy()
	self:_unregister_events()
	HudElementMissionObjectivePopup.super.destroy(self)
end

function HudElementMissionObjectivePopup:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementMissionObjectivePopup:_present_popup(popup_data)
...
function HudElementMissionObjectivePopup:event_mission_objective_update(mission_name)
	if not self:_can_present_mission(mission_name) then
		return
	end
...
function HudElementMissionObjectivePopup:event_mission_objective_complete(mission_name)
	if not self:_can_present_mission(mission_name) then
		return
	end
...
function HudElementMissionObjectivePopup:_setup_mission_update_texts(widget, update_text)
...
function HudElementMissionObjectivePopup:_setup_mission_popup_texts(widget, title_text, description_text)
	local parent = self._parent
	local ui_renderer = parent:ui_renderer()
	local content = widget.content
	local style = widget.style
...
function HudElementMissionObjectivePopup:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementMissionObjectivePopup:_register_events()
...
function HudElementMissionObjectivePopup:_unregister_events()
	local event_manager = Managers.event
	local events = HudElementMissionObjectivePopupSettings.events
