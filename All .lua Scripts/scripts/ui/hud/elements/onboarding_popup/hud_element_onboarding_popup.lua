...
function HudElementOnboardingPopup:init(parent, draw_layer, start_scale, definitions)
...
function HudElementOnboardingPopup:destroy()
	self:_unregister_events()
	HudElementOnboardingPopup.super.destroy(self)
end

function HudElementOnboardingPopup:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementOnboardingPopup:event_player_display_onboarding_message(player, text, optional_duration, on_close_callback)
...
function HudElementOnboardingPopup:event_player_hide_onboarding_message(player)
	if not self._player or self._player ~= player then
		return
	end
...
function HudElementOnboardingPopup:_present_new_message(text, duration, on_close_callback)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.popup
	local content = widget.content
	content.text = text
...
function HudElementOnboardingPopup:_hide_message()
...
function HudElementOnboardingPopup:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementOnboardingPopup:_register_events()
...
function HudElementOnboardingPopup:_unregister_events()
	local event_manager = Managers.event
	local events = HudElementOnboardingPopupSettings.events
...
function HudElementOnboardingPopup:_set_widget_size_from_content(title)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.popup
