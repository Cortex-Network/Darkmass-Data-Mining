...
function HudElementPlayerAbility:init(parent, draw_layer, start_scale, data)
...
function HudElementPlayerAbility:destroy()
	self:_unregister_events()
	HudElementPlayerAbility.super.destroy(self)
end

function HudElementPlayerAbility:_update_input()
...
function HudElementPlayerAbility:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementPlayerAbility:set_charges_amount(amount)
...
function HudElementPlayerAbility:_set_widget_state_colors(on_cooldown, uses_charges, has_charges_left)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.ability
...
function HudElementPlayerAbility:set_input_text(text)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.ability
...
function HudElementPlayerAbility:set_icon(icon)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.ability
	local style = widget.style
...
function HudElementPlayerAbility:_set_progress(progress)
...
function HudElementPlayerAbility:set_visible(visible, ui_renderer, use_retained_mode)
...
function HudElementPlayerAbility:_destroy_widgets(ui_renderer)
...
function HudElementPlayerAbility:_register_events()
...
function HudElementPlayerAbility:_unregister_events()
	local event_manager = Managers.event
	local events = HudElementPlayerAbilitySettings.events
...
function HudElementPlayerAbility:event_on_input_changed()
	self:_update_input()
end
