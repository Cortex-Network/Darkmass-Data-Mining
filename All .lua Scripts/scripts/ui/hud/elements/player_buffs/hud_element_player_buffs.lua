...
function HudElementPlayerBuffs:init(parent, draw_layer, start_scale, definitions)
...
function HudElementPlayerBuffs:event_player_buff_proc_start(player, buff_instance)
	self:event_player_buff_added(player, buff_instance)
end

function HudElementPlayerBuffs:event_player_buff_proc_stop(player, buff_instance)
	self:event_player_buff_removed(player, buff_instance)
end

function HudElementPlayerBuffs:event_player_buff_added(player, buff_instance)
...
function HudElementPlayerBuffs:event_player_buff_removed(player, buff_instance)
	if not self._player or self._player ~= player then
		return
	end
...
function HudElementPlayerBuffs:_sync_current_active_buffs(buffs)
	local active_buffs_data = self._active_buffs_data
...
function HudElementPlayerBuffs:_add_buff(buff_instance)
	local active_buffs_data = self._active_buffs_data
...
function HudElementPlayerBuffs:_remove_buff_at_index(index)
	local active_buffs_data = self._active_buffs_data
...
function HudElementPlayerBuffs:_get_available_widget()
...
function HudElementPlayerBuffs:_setup_buff_widget_array()
...
function HudElementPlayerBuffs:_update_buff_alignments(force_update)
	local active_buffs_data = self._active_buffs_data
	local num_active_buffs = #active_buffs_data
...
function HudElementPlayerBuffs:_update_buffs()
	local active_buffs_data = self._active_buffs_data
...
function HudElementPlayerBuffs:_set_widget_state_colors(widget, is_negative, is_inactive)
...
function HudElementPlayerBuffs:destroy()
...
function HudElementPlayerBuffs:set_visible(visible, ui_renderer, use_retained_mode)
...
function HudElementPlayerBuffs:_destroy_widgets(ui_renderer)
...
function HudElementPlayerBuffs:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementPlayerBuffs:event_present_new_area(area_id)
...
function HudElementPlayerBuffs:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementPlayerBuffs:_register_events()
...
function HudElementPlayerBuffs:_unregister_events()
	local event_manager = Managers.event
	local events = HudElementPlayerBuffsSettings.events
