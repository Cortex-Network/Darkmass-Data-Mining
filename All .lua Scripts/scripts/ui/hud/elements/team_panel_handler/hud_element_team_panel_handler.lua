...
function HudElementTeamPanelHandler:init(parent, draw_layer, start_scale)
...
function HudElementTeamPanelHandler:_composition_changed(composition_name)
	if composition_name ~= self._player_composition_name then
		-- Nothing
	end
end

function HudElementTeamPanelHandler:_setup_position_scenegraphs()
	local scenegraphs = {}
...
function HudElementTeamPanelHandler:_num_other_player_panels()
...
function HudElementTeamPanelHandler:_get_available_scenegraph()
...
function HudElementTeamPanelHandler:_player_scan(ui_renderer)
...
function HudElementTeamPanelHandler:_add_panel(unique_id, ui_renderer, fixed_scenegraph_id)
	local player_composition_name = self._player_composition_name
...
function HudElementTeamPanelHandler:_remove_panel(unique_id, ui_renderer)
	local player_panels_array = self._player_panels_array
...
function HudElementTeamPanelHandler:_on_panels_removed()
	self:_refresh_assigned_scenegraph_ids()
	self:_align_panels()
end

function HudElementTeamPanelHandler:_refresh_assigned_scenegraph_ids()
	local index = 1
	local scenegraphs = self._position_scenegraphs
	local player_panels_array = self._player_panels_array
	local num_player_panels = #player_panels_array
...
function HudElementTeamPanelHandler:set_scenegraph_position(scenegraph_id, x, y, z, horizontal_alignment, vertical_alignment)
...
function HudElementTeamPanelHandler:_align_panels()
...
function HudElementTeamPanelHandler:destroy(ui_renderer)
	local player_panels_array = self._player_panels_array
...
function HudElementTeamPanelHandler:set_visible(visible, ui_renderer, use_retained_mode)
...
function HudElementTeamPanelHandler:set_retained_visibility(visible, ui_renderer)
...
function HudElementTeamPanelHandler:on_resolution_modified()
...
function HudElementTeamPanelHandler:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementTeamPanelHandler:draw(dt, t, ui_renderer, render_settings, input_service)
