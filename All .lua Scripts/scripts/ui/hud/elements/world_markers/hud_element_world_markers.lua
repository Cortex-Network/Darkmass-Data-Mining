...
function HudElementWorldMarkers:init(parent, draw_layer, start_scale)
...
function HudElementWorldMarkers:destroy()
...
function HudElementWorldMarkers:_template_by_type(marker_type, clone)
	return clone and table.clone(self._marker_templates[marker_type]) or self._marker_templates[marker_type]
end

function HudElementWorldMarkers:event_request_world_markers_list(response_callback)
	if response_callback then
		response_callback(self._markers)
	end
end

function HudElementWorldMarkers:event_remove_world_marker(id)
...
function HudElementWorldMarkers:event_add_world_marker_unit(marker_type, unit, callback, data)
	local marker = {
		scale = 1,
		type = marker_type,
		unit = unit,
...
function HudElementWorldMarkers:event_add_world_marker_position(marker_type, world_position, callback, data)
	local marker = {
		scale = 1,
		type = marker_type,
...
function HudElementWorldMarkers:_register_marker(marker)
	local markers = self._markers
...
function HudElementWorldMarkers:_unregister_marker(marker)
	local markers = self._markers
	local markers_by_id = self._markers_by_id
	local markers_by_type = self._markers_by_type
	local id = marker.id
...
function HudElementWorldMarkers:_create_widget_by_type(name, template)
...
function HudElementWorldMarkers:update(dt, t, ui_renderer, render_settings, input_service)
...
function HudElementWorldMarkers:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementWorldMarkers:_calculate_markers(dt, t, input_service, ui_renderer, render_settings)
...
function HudElementWorldMarkers:_draw_markers(dt, t, input_service, ui_renderer, render_settings)
	local camera = self._camera
...
function HudElementWorldMarkers:_get_fade(fade_settings, distance)
...
function HudElementWorldMarkers:_get_scale(scale_settings, distance)
...
function HudElementWorldMarkers:_apply_scale(widget, scale)
...
function HudElementWorldMarkers:_get_screen_offset(scale)
	local position = UIScenegraph.world_position(self._ui_scenegraph, "screen", scale)

	return Vector3.to_elements(position)
end

function HudElementWorldMarkers:_convert_world_to_screen_position(camera, world_position)
	if camera then
...
function HudElementWorldMarkers:_clamp_to_screen(x, y, screen_margins, is_behind, is_under, world_position, camera_position_center, camera_position_left, camera_position_right, camera_position_up, camera_position_down)
	local root_size = UIScenegraph.size_scaled(self._ui_scenegraph, "screen")
...
function HudElementWorldMarkers:_physics_world()
...
function HudElementWorldMarkers:physics_cb_line_of_sight_hit(marker, id, hit, hit_position)
	marker.raycast_result = hit
	marker.raycast_frame_count = 0
end

function HudElementWorldMarkers:_raycast_markers(marker_raycast_queue)
...
function HudElementWorldMarkers:_async_raycast_result_cb(id, hit, hit_position, data)
