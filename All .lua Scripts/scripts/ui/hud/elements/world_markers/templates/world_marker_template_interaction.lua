...
function template.get_smart_tag_id(marker)
...
function template:create_widget_defintion(scenegraph_id)
...
function template.on_enter(widget, marker, self)
	local content = widget.content
	content.spawn_progress_timer = 0
end

function template.update_function(parent, ui_renderer, widget, marker, self, dt, t)
	local content = widget.content
	local style = widget.style
