...
function PortraitUI:update_all()
...
function PortraitUI:profile_updated(profile, prioritized)
...
function PortraitUI:get_profile_grid_index(id)
...
function PortraitUI:_profile_request_by_id(id, ignore_assert)
	local profile_requests = self._profile_requests
...
function PortraitUI:has_request(id)
	local data = self:_profile_request_by_id(id, true)

	return data ~= nil
end

function PortraitUI:load_profile_portrait(profile, on_load_callback, optional_render_context, prioritized)
	local character_id = profile.character_id
...
function PortraitUI:unload_profile_portrait(id)
	local data = self:_profile_request_by_id(id)
...
function PortraitUI:_handle_request_queue()
...
function PortraitUI:_handle_next_request_in_queue()
...
function PortraitUI:_spawn_profile(profile, render_context)
	if self._profile_spawner then
		self._profile_spawner:destroy()
...
function PortraitUI:_initialize_world()
...
function PortraitUI:event_register_spawn_point_character_portrait(spawn_point_unit)
	Managers.event:unregister(self, "event_register_spawn_point_character_portrait")

	self._spawn_point_unit = spawn_point_unit
end

function PortraitUI:event_register_portrait_camera_human(camera_unit)
...
function PortraitUI:event_register_portrait_camera_ogryn(camera_unit)
...
function PortraitUI:_setup_viewport(camera_unit, render_targets)
...
function PortraitUI:_resume_rendering()
	local world_spawner = self._world_spawner

	world_spawner:set_world_disabled(false)
end

function PortraitUI:_pause_rendering()
	local world_spawner = self._world_spawner

	world_spawner:set_world_disabled(true)
end

function PortraitUI:destroy()
...
function PortraitUI:active()
	return self._active
end

function PortraitUI:update(dt, t)
...
function PortraitUI:_create_uv_grid()
...
function PortraitUI:_get_free_grid_index()
...
function PortraitUI:_grid_index_uvs(index)
	return self._uv_grid[index]
end

function PortraitUI:_get_camera_unit_by_key(key)
	return self:_get_unit_by_value_key("camera_gear_slot_name", key)
end

function PortraitUI:_get_unit_by_value_key(key, value)
	local world_spawner = self._world_spawner
