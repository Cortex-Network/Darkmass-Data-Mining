...
function ViewElementCraftingRecipe:init(parent, draw_layer, start_scale, optional_menu_settings)
...
function ViewElementCraftingRecipe:present_recipe_navigation(recipes, left_click_callback, optional_on_present_callback)
	local layout = {
		{
	...
	self:present_grid_layout(layout, CraftingRecipeBlueprints, left_click_callback, nil, , , optional_on_present_callback)
end

function ViewElementCraftingRecipe:present_recipe(recipe, raw_costs, left_click_callback, optional_on_present_callback)
	local layout = {
		[#layout + 1] = {
			widget_type = "spacing_vertical"
		},
...
function ViewElementCraftingRecipe:_pre_present_height_adjust()
...
function ViewElementCraftingRecipe:set_overlay_texture(texture)
...
function ViewElementCraftingRecipe:play_craft_animation(callback)
	return self:_start_animation("on_craft", self._widgets_by_name, nil, callback)
end

function ViewElementCraftingRecipe:_on_present_grid_layout_changed(layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction)
