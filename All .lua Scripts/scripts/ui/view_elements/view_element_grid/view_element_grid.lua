...
function ViewElementGrid:init(parent, draw_layer, start_scale, optional_menu_settings, optional_definitions)
...
function ViewElementGrid:set_expire_time(time)
...
function ViewElementGrid:setup_sort_button(options, sort_callback, start_index)
...
function ViewElementGrid:trigger_sort_index(index)
	self:_cb_on_sort_button_pressed(index)
end

function ViewElementGrid:_cb_on_sort_button_pressed(start_index)
...
function ViewElementGrid:_apply_sort_button_text()
...
function ViewElementGrid:set_pivot_offset(x, y)
...
function ViewElementGrid:disable_input(disabled)
	self._input_disabled = disabled
end

function ViewElementGrid:input_disabled()
	return self._input_disabled
end

function ViewElementGrid:_setup_grid_gui()
...
function ViewElementGrid:_destroy_grid_gui()
...
function ViewElementGrid:update(dt, t, input_service)
	self._drawn = false
...
function ViewElementGrid:widgets()
	return self._grid_widgets
end

function ViewElementGrid:draw(dt, t, ui_renderer, render_settings, input_service)
...
function ViewElementGrid:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
end

function ViewElementGrid:_draw_render_target(render_settings)
	local ui_grid_renderer = self._ui_grid_renderer
...
function ViewElementGrid:_draw_grid(dt, t, input_service, render_settings)
...
function ViewElementGrid:_update_grid_widgets(dt, t, input_service)
	local widgets = self._grid_widgets
...
function ViewElementGrid:_destroy_grid()
	if self._grid then
		self._grid:destroy()
...
function ViewElementGrid:_destroy_grid_widgets()
	local widgets = self._grid_widgets
	local content_blueprints = self._content_blueprints
...
function ViewElementGrid:_clear_widgets(widgets)
	if widgets then
		for i = 1, #widgets do
			local widget = widgets[i]
			local widget_name = widget.name
...
function ViewElementGrid:_create_entry_widget_from_config(config, suffix, callback_name, secondary_callback_name, double_click_callback_name)
...
function ViewElementGrid:update_grid_height(grid_height, mask_height)
...
function ViewElementGrid:grid_height()
	return self._menu_settings.grid_size[2]
end

function ViewElementGrid:_update_window_size()
	local menu_settings = self._menu_settings
...
function ViewElementGrid:_assign_display_name(display_name)
...
function ViewElementGrid:present_grid_layout(layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction, optional_on_present_callback, optional_left_double_click_callback)
...
function ViewElementGrid:_on_present_grid_layout_changed(layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction, optional_left_double_click_callback)
...
function ViewElementGrid:update_dividers(top_divider_material, top_divider_size, top_divider_position, bottom_divider_material, bottom_divider_size, bottom_divider_position)
...
function ViewElementGrid:widget_by_name(widget_name)
	return self._widgets_by_name[widget_name]
end

function ViewElementGrid:focus_grid_index(index, scrollbar_animation_progress, instant_scroll)
	local grid = self._grid

	grid:focus_grid_index(index, scrollbar_animation_progress, instant_scroll)
end

function ViewElementGrid:focused_grid_index()
	local grid = self._grid
...
function ViewElementGrid:select_grid_widget(widget, scrollbar_animation_progress, instant_scroll)
	local grid = self._grid
...
function ViewElementGrid:select_grid_index(index, scrollbar_animation_progress, instant_scroll)
	local grid = self._grid

	grid:select_grid_index(index, scrollbar_animation_progress, instant_scroll)
end

function ViewElementGrid:select_first_index()
	local grid = self._grid
...
function ViewElementGrid:selected_grid_index()
	local grid = self._grid
...
function ViewElementGrid:widget_index(widget)
	local grid = self._grid
...
function ViewElementGrid:widget_by_index(widget)
	local grid = self._grid
...
function ViewElementGrid:first_interactable_grid_index()
	local grid = self._grid
...
function ViewElementGrid:last_interactable_grid_index()
	local grid = self._grid
...
function ViewElementGrid:selected_grid_widget()
	local grid = self._grid
...
function ViewElementGrid:force_update_list_size()
	self:_update_window_size()
...
function ViewElementGrid:get_scrollbar_percentage_by_index(index, start_from_bottom)
	return self._grid:get_scrollbar_percentage_by_index(index, start_from_bottom)
end

function ViewElementGrid:scroll_to_grid_index(index, instant_scroll, start_from_bottom)
	local grid = self._grid
...
function ViewElementGrid:scroll_to_grid_widget(widget, instant_scroll, start_from_bottom)
	local grid = self._grid
	local index = grid:index_by_widget(widget)
	local scroll_progress = grid:get_scrollbar_percentage_by_index(index, start_from_bottom)
...
function ViewElementGrid:_on_navigation_input_changed()
...
function ViewElementGrid:cb_on_grid_entry_right_pressed(widget, element)
	if self._right_click_callback then
		self._right_click_callback(widget, element)
	end
end

function ViewElementGrid:cb_on_grid_entry_left_pressed(widget, element)
	if self._left_click_callback then
		self._left_click_callback(widget, element)
	end
end

function ViewElementGrid:cb_on_grid_entry_double_click_pressed(widget, element)
	if self._left_double_click_callback then
		self._left_double_click_callback(widget, element)
	end
end

function ViewElementGrid:update_grid_widgets_visibility()
	local widgets = self._grid_widgets
...
function ViewElementGrid:remove_widget(widget)
	self._grid:remove_widget(widget)
end

function ViewElementGrid:grid()
	return self._grid
end

function ViewElementGrid:destroy()
	if self._present_grid_layout then
		self._present_grid_layout = nil
	end
...
function ViewElementGrid:element_by_index(index)
	local widgets = self._grid_widgets
...
function ViewElementGrid:index_by_element(element)
	local widgets = self._grid_widgets
...
function ViewElementGrid:grid_length()
