...
function ViewElementKeybindPopup:init(parent, draw_layer, start_scale)
...
function ViewElementKeybindPopup:cb_shading_callback(world, shading_env, viewport, default_shading_environment_name)
...
function ViewElementKeybindPopup:_setup_default_gui()
...
function ViewElementKeybindPopup:_setup_background_gui()
	local ui_manager = Managers.ui
	local class_name = self.__class_name
	local timer_name = "ui"
...
function ViewElementKeybindPopup:destroy()
...
function ViewElementKeybindPopup:_set_background_blur(fraction)
...
function ViewElementKeybindPopup:update(dt, t, input_service)
...
function ViewElementKeybindPopup:draw(dt, t, ui_renderer, render_settings, input_service)
...
function ViewElementKeybindPopup:_pulse_action_text()
...
function ViewElementKeybindPopup:_get_color_string_by_color(color)
	return "{#color(" .. color[2] .. "," .. color[3] .. "," .. color[4] .. ")}"
end

function ViewElementKeybindPopup:set_value_text(value)
	local widgets_by_name = self._widgets_by_name
	local value_text = widgets_by_name.value_text
	value_text.content.text = value
end

function ViewElementKeybindPopup:set_action_text(action)
	local widgets_by_name = self._widgets_by_name
	local action_text = widgets_by_name.action_text
	action_text.content.text = action
end

function ViewElementKeybindPopup:set_description_text(description)
	local widgets_by_name = self._widgets_by_name
