...
function ViewElementPlayerPanel:init(parent, draw_layer, start_scale, context)
...
function ViewElementPlayerPanel:set_pivot_offset(x, y)
	self:_set_scenegraph_position("pivot", x, y)
end

function ViewElementPlayerPanel:update(dt, t, input_service)
...
function ViewElementPlayerPanel:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	ViewElementPlayerPanel.super._draw_widgets(self, dt, t, input_service, ui_renderer, render_settings)
end

function ViewElementPlayerPanel:_request_player_insignia(item)
	self:_unload_insignia()
	self:_load_insignia(item)
end

function ViewElementPlayerPanel:_load_insignia(item)
...
function ViewElementPlayerPanel:_unload_insignia()
...
function ViewElementPlayerPanel:_cb_set_player_insignia(item)
...
function ViewElementPlayerPanel:_request_player_frame(item)
	self:_unload_portrait_frame()
	self:_load_portrait_frame(item)
end

function ViewElementPlayerPanel:_load_portrait_frame(item)
...
function ViewElementPlayerPanel:_unload_portrait_frame()
...
function ViewElementPlayerPanel:_cb_set_player_frame(item)
	local loadout = self._preview_profile.loadout
...
function ViewElementPlayerPanel:_request_player_icon()
	local material_values = self._widgets_by_name.character_portrait.style.texture.material_values
...
function ViewElementPlayerPanel:_load_portrait_icon()
...
function ViewElementPlayerPanel:_unload_portrait_icon()
...
function ViewElementPlayerPanel:_cb_set_player_icon(grid_index, rows, columns)
	local material_values = self._widgets_by_name.character_portrait.style.texture.material_values
