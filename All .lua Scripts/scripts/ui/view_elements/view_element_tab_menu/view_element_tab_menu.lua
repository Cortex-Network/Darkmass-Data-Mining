...
function ViewElementTabMenu:init(parent, draw_layer, start_scale, optional_menu_settings)
...
function ViewElementTabMenu:_on_navigation_input_changed()
	ViewElementTabMenu.super._on_navigation_input_changed(self)
	self:_update_input_action_texts()
end

function ViewElementTabMenu:set_pivot_offset(x, y)
...
function ViewElementTabMenu:add_entry(display_name, on_pressed_callback, pass_template, optional_display_icon, optional_update_function, no_localization)
...
function ViewElementTabMenu:set_selected_index(index)
	self._selected_index = index
end

function ViewElementTabMenu:selected_index(index)
	return self._selected_index
end

function ViewElementTabMenu:_update_widget_size(widget, ui_renderer)
	local content = widget.content
...
function ViewElementTabMenu:set_tab_disabled(id, is_disabled)
...
function ViewElementTabMenu:tab_disabled(id)
	local widget = self:_widget_by_id(id)

	return widget and widget.content.hotspot.disabled or false
end

function ViewElementTabMenu:remove_entry(id)
...
function ViewElementTabMenu:remove_all_entries()
...
function ViewElementTabMenu:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function ViewElementTabMenu:get_total_width()
	return self._total_width or 0
end

function ViewElementTabMenu:_get_input_text(input_action)
...
function ViewElementTabMenu:set_input_actions(input_action_left, input_action_right)
...
function ViewElementTabMenu:_update_input_action_texts()
...
function ViewElementTabMenu:_widget_by_id(id)
	local entries = self._entries
