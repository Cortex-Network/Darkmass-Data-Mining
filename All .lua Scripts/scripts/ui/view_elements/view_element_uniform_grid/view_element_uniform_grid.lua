...
function ViewElementUniformGrid:init(parent, draw_layer, start_scale)
	self._num_cells = {
		0,
		0
	}
...
function ViewElementUniformGrid:update(dt, t, input_service)
...
function ViewElementUniformGrid:on_resolution_modified(scale)
	ViewElementUniformGrid.super.on_resolution_modified(self, scale)

	self._needs_grid_recalculation = true
end

function ViewElementUniformGrid:grid_area_position()
	local position = self:scenegraph_position(GRID_AREA_SCENEGRAPH_ID)

	return position[1], position[2], position[3]
end

function ViewElementUniformGrid:set_grid_area_position(position, horizontal_alignment, vertical_alignment)
...
function ViewElementUniformGrid:grid_area_size()
...
function ViewElementUniformGrid:set_grid_area_size(size)
	self:_set_scenegraph_size(GRID_AREA_SCENEGRAPH_ID, size[1], size[2])

	self._needs_grid_recalculation = true
end

function ViewElementUniformGrid:grid_size()
	local num_cells = self._num_cells

	return num_cells[1], num_cells[2]
end

function ViewElementUniformGrid:num_cells()
	local num_cells = self._num_cells

	return num_cells[1] * num_cells[2]
end

function ViewElementUniformGrid:set_grid_size(num_columns, num_rows)
	local num_cells = self._num_cells
...
function ViewElementUniformGrid:is_hexagonal()
	return self._is_hexagonal
end

function ViewElementUniformGrid:set_is_hexagonal(flag)
...
function ViewElementUniformGrid:set_grid_spacing(column_spacing, row_spacing)
...
function ViewElementUniformGrid:cell_size()
	local cell_size = self._cell_size

	return cell_size[1], cell_size[2]
end

function ViewElementUniformGrid:force_grid_recalculation()
	self:_recalculate_grid()

	self._needs_grid_recalculation = false
end

function ViewElementUniformGrid:add_widget_to_grid(column, row, widget_name, widget_blueprint, init_callback, ...)
...
function ViewElementUniformGrid:remove_widget(widget)
	local widgets = self._widgets
...
function ViewElementUniformGrid:grid_widget(column, row)
	local widgets = self._widgets
...
function ViewElementUniformGrid:focused_cell()
	local focused_widget = self._focused_widget
...
function ViewElementUniformGrid:set_focused_grid_cell(column, row)
	self._focused_widget = nil
...
function ViewElementUniformGrid:focused_grid_widget()
	return self._focused_widget
end

function ViewElementUniformGrid:set_focused_grid_widget(widget_to_focus)
...
function ViewElementUniformGrid:grid_cell_world_position(column, row, scale)
	local scenegraph_id = string.format(GRID_CELL_ID_FORMAT, column, row)
...
function ViewElementUniformGrid:focus_first_index(start_column, start_row)
...
function ViewElementUniformGrid:select_first_index(start_column, start_row)
	start_column = start_column or 1
	start_row = start_row or 1
	local widget = self:_find_next_cell(start_column, start_row)
...
function ViewElementUniformGrid:_handle_input(input_service)
	local focused_widget = self._focused_widget
	local widget_content = focused_widget.content
...
function ViewElementUniformGrid:_find_next_cell(start_column, start_row, direction)
...
function ViewElementUniformGrid:_recalculate_grid()
...
function ViewElementUniformGrid:_clear_navigation_history()
	self._last_focused_widget = nil
	self._last_navigation_direction = nil
end

function ViewElementUniformGrid:_insert_widget_widget_in_grid(widget, column, row, start_index, end_index)
