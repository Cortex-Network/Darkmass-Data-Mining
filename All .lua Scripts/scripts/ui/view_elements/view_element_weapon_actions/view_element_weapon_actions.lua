...
function ViewElementWeaponActions:init(parent, draw_layer, start_scale, optional_menu_settings)
...
function ViewElementWeaponActions:_hide_dividers()
...
function ViewElementWeaponActions:destroy()
...
function ViewElementWeaponActions:present_item(item)
	local menu_settings = self._menu_settings
	local grid_size = menu_settings.grid_size
	local layout = {
		{
...
function ViewElementWeaponActions:stop_presenting()
	self:_destroy_grid_widgets()
	self:_destroy_grid()
end

function ViewElementWeaponActions:present_grid_layout(layout)
...
function ViewElementWeaponActions:_on_present_grid_layout_changed(layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction)
...
function ViewElementWeaponActions:update(dt, t, input_service)
	if self._weapon_icon_renderer then
...
function ViewElementWeaponActions:load_item_icon(item, cb, render_context)
...
function ViewElementWeaponActions:unload_item_icon(id)
	local weapon_icon_renderer = self._weapon_icon_renderer
...
function ViewElementWeaponActions:item_icon_updated(peer_id, local_player_id, item)
	local item_type = item.item_type
