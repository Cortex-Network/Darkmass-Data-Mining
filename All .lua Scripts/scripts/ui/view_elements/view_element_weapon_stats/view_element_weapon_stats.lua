...
function ViewElementWeaponStats:init(parent, draw_layer, start_scale, optional_menu_settings)
...
function ViewElementWeaponStats:destroy()
...
function ViewElementWeaponStats:present_item(item, is_equipped, on_present_callback)
	local menu_settings = self._menu_settings
	local grid_size = menu_settings.grid_size
...
function ViewElementWeaponStats:stop_presenting()
	self:_destroy_grid_widgets()
	self:_destroy_grid()
end

function ViewElementWeaponStats:present_grid_layout(layout, on_present_callback)
...
function ViewElementWeaponStats:_on_present_grid_layout_changed(layout, content_blueprints, left_click_callback, right_click_callback, display_name, optional_grow_direction)
...
function ViewElementWeaponStats:update(dt, t, input_service)
	if self._weapon_icon_renderer then
...
function ViewElementWeaponStats:load_item_icon(item, cb, render_context, dummy_profile)
	local item_name = item.name
...
function ViewElementWeaponStats:unload_item_icon(id)
	local weapon_icon_renderer = self._weapon_icon_renderer
	local cosmetics_icon_renderer = self._cosmetics_icon_renderer
...
function ViewElementWeaponStats:item_icon_updated(item)
	local item_type = item.item_type
