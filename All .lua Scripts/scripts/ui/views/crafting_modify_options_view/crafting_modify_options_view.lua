...
function CraftingModifyOptionsView:init(settings, context)
...
function CraftingModifyOptionsView:upgrade_item()
	self._wanted_tab_index = 1
end

function CraftingModifyOptionsView:extract_trait()
	self._wanted_tab_index = 3
end

function CraftingModifyOptionsView:replace_trait()
	self._wanted_tab_index = 2
end

function CraftingModifyOptionsView:on_enter()
...
function CraftingModifyOptionsView:_fetch_inventory()
	local local_player_id = 1
...
function CraftingModifyOptionsView:_get_wallet()
...
function CraftingModifyOptionsView:_on_panel_option_pressed(index)
		...
		if options_type == "upgrade" then
			...
			if not item.rarity then
				...
				if upgrade_count > 4 or not RaritySettings[next_rarity] or not costs then
					...
					function self._widgets_by_name.upgrade_item_button.content.hotspot.pressed_callback()
			...
			elseif #self._weapon_traits == 0 then
				...
				Promise.all(unpack(widgets_promise)):next(function (costs)
					...
					for i = 1, #costs do
						...
						function widget.content.hotspot.pressed_callback()
							if avaialable then
...
function CraftingModifyOptionsView:setup_prices_widget(costs, afford_by_type)
...
function CraftingModifyOptionsView:_can_afford(costs)
...
function CraftingModifyOptionsView:_remove_traits_widgets()
...
function CraftingModifyOptionsView:_reset_state()
...
function CraftingModifyOptionsView:_remove_trait_tooltip()
...
function CraftingModifyOptionsView:_set_trait_tooltip(trait_info, direction)
...
function CraftingModifyOptionsView:_set_tooltip_position(parent_widget, direction)
...
function CraftingModifyOptionsView:_select_trait_extract(selected_widget)
	self:_remove_selected_widget()
	self:_remove_traits_widgets()
	...
	function self._widgets_by_name.extract_trait_button.content.hotspot.pressed_callback()
		local context = {
...
function CraftingModifyOptionsView:_select_trait_modify(selected_widget)
	self:_remove_selected_widget()
	self:_remove_traits_widgets()
...
function CraftingModifyOptionsView:_remove_selected_widget()
...
function CraftingModifyOptionsView:_update_item_grid_position()
	if not self._inventory_traits_grid then
		return
	end
...
function CraftingModifyOptionsView:_generate_weapon_traits()
	local traits = {}
...
function CraftingModifyOptionsView:_setup_trait_tabs()
	...
	for i = 1, 4 do
		...
		local tab_id = tab_menu_element:add_entry(name, pressed_callback, tab_button_template, nil, , true)
		tab_ids[i] = tab_id
	end
...
function CraftingModifyOptionsView:_update_tab_bar_position()
...
function CraftingModifyOptionsView:cb_switch_trait_tab(index)
...
function CraftingModifyOptionsView:_present_layout_by_trait_rarity(trait_rarity)
	local traits = {}
...
function CraftingModifyOptionsView:cb_on_grid_entry_left_pressed(widget, element)
...
function CraftingModifyOptionsView:_destroy_weapon_preview()
...
function CraftingModifyOptionsView:_set_preview_widgets_visibility(visible)
	CraftingModifyOptionsView.super._set_preview_widgets_visibility(self, visible)
end

function CraftingModifyOptionsView:update(dt, t, input_service)
...
function CraftingModifyOptionsView:on_back_pressed(force_close)
...
function CraftingModifyOptionsView:on_exit()
...
function CraftingModifyOptionsView:draw(dt, t, input_service, layer)
...
function CraftingModifyOptionsView:_on_navigation_input_changed()
	CraftingModifyOptionsView.super._on_navigation_input_changed(self)
end

function CraftingModifyOptionsView:_handle_input(input_service)
	local selected_trait = self._selected_trait
...
function CraftingModifyOptionsView:dialogue_system()
