...
function CraftingModifyView:init(settings, context)
...
function CraftingModifyView:on_enter()
	...
	self._item_grid:update_dividers("content/ui/materials/frames/item_list_top_hollow", {
		...
		-95,
		200
	}, "content/ui/materials/frames/item_info_lower", nil, )
...
function CraftingModifyView:crafting_costs()
	return self._crafting_costs
end

function CraftingModifyView:_setup_crafting_recipe(reference_name, scenegraph_id)
	local layer = 10
...
function CraftingModifyView:on_resolution_modified(scale)
...
function CraftingModifyView:_update_weapon_stats_position(scenegraph_id, weapon_stats)
...
function CraftingModifyView:on_back_pressed()
	return not self._using_cursor_navigation and self._selected_grid == "crafting_recipe"
end

function CraftingModifyView:_set_selected_grid(new_selected_grid)
...
function CraftingModifyView:update(dt, t, input_service)
...
function CraftingModifyView:_handle_input(input_service)
	if self._using_cursor_navigation then
		return
	end
...
function CraftingModifyView:cb_on_recipe_button_pressed(_, config)
	self._parent:go_to_crafting_view(config.recipe.view_name, self._previewed_item)
end

function CraftingModifyView:_preview_item(item)
...
function CraftingModifyView:on_exit()
...
function CraftingModifyView:_cb_fetch_inventory_items(items)
...
function CraftingModifyView:_setup_menu_tabs(content)
	local id = "tab_menu"
	local layer = 10
...
function CraftingModifyView:_on_navigation_input_changed()
...
function CraftingModifyView:character_level()
...
function CraftingModifyView:dialogue_system()
	return self._parent:dialogue_system()
end

function CraftingModifyView:ui_renderer()
