...
function CraftingRerollPerkView:init(settings, context)
...
function CraftingRerollPerkView:_update_reroll_costs(reroll_count)
...
function CraftingRerollPerkView:on_enter()
...
function CraftingRerollPerkView:_present_item_upgrade(item)
...
function CraftingRerollPerkView:_update_weapon_stats_position(scenegraph_id, weapon_stats)
...
function CraftingRerollPerkView:on_resolution_modified(scale)
...
function CraftingRerollPerkView:on_back_pressed()
...
function CraftingRerollPerkView:_set_selected_grid(new_selected_grid)
...
function CraftingRerollPerkView:update(dt, t, input_service)
...
function CraftingRerollPerkView:_handle_input(input_service)
...
function CraftingRerollPerkView:get_warning()
	return "BLAHBLAH"
end

function CraftingRerollPerkView:cb_on_main_button_pressed(recipe)
...
function CraftingRerollPerkView:_on_navigation_input_changed()
	if self._using_cursor_navigation then
		self._crafting_recipe:disable_input(false)
		self._crafting_recipe:select_grid_index(nil)
		self._weapon_stats:disable_input(false)
...
function CraftingRerollPerkView:_setup_crafting_recipe(reference_name, scenegraph_id)
	local layer = 10
...
function CraftingRerollPerkView:_setup_weapon_stats(reference_name, scenegraph_id)
	local layer = 10
...
function CraftingRerollPerkView:on_perk_selected(perk_index, perk_display_name)
	self._perk_display_name = perk_display_name
	self._ingredients.existing_perk_index = perk_index
end

function CraftingRerollPerkView:can_afford(amount, currency)
	return self._parent:can_afford(amount, currency)
end

function CraftingRerollPerkView:can_craft()
	if self._craft_promise then
...
function CraftingRerollPerkView:dialogue_system()
