...
function CreditsView:init(settings)
...
function CreditsView:cb_on_close_pressed()
	Managers.ui:close_view(self.view_name)
end

function CreditsView:update(dt, t, input_service)
...
function CreditsView:cb_credits_speed()
	self._speed_multiplier = credits_settigns.ffw_speed
	self._triggered_action = true
end

function CreditsView:cb_credits_pause()
	self._should_stop = true
	self._triggered_action = true
end

function CreditsView:draw(dt, t, input_service, layer)
	CreditsView.super.draw(self, dt, t, input_service, layer)
	self:_handle_credits(dt, t, input_service, layer)
end

function CreditsView:_handle_credits(dt, t, input_service, layer)
	...
	local current_offset = self._should_stop and self._current_offset or math.min(0, self._current_offset + dt * 50 - self._speed_multiplier * 30)
...
function CreditsView:_handle_carousel(dt, t)
...
function CreditsView:_setup_input_legend()
