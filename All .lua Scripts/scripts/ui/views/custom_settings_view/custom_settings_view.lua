...
function CustomSettingsView:init(settings, context)
...
function CustomSettingsView:on_enter()
...
function CustomSettingsView:on_exit()
	self:_destroy_offscreen_renderer()
	Application.save_user_settings()
	CustomSettingsView.super.on_exit(self)
end

function CustomSettingsView:_setup_offscreen_renderer()
...
function CustomSettingsView:_destroy_offscreen_renderer()
...
function CustomSettingsView:_setup_buttons_interactions()
	self._widgets_by_name.next_button.content.hotspot.pressed_callback = callback(self, "_on_forward_pressed")
end

function CustomSettingsView:_change_settings_page(next_index)
...
function CustomSettingsView:cb_on_settings_pressed(widget, entry)
...
function CustomSettingsView:_on_forward_pressed()
	local index = self._current_index + 1

	self:_change_settings_page(index)
end

function CustomSettingsView:draw(dt, t, input_service, layer)
	if self._grid then
...
function CustomSettingsView:_draw_grid(dt, t, input_service)
...
function CustomSettingsView:_handle_input(input_service)
...
function CustomSettingsView:_on_navigation_input_changed()
	self:_focus_selected_widget()
end

function CustomSettingsView:_focus_selected_widget()
	local is_gamepad = not self:using_cursor_navigation()
