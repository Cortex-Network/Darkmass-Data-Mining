...
function EndPartyView:init(settings, context)
	self._player_panels = {}
...
function EndPartyView:on_exit()
	EndPartyView.super.on_exit(self)

	self._spawned_accolades_array = nil
end

function EndPartyView:_set_game_data_fields(text_1, text_2, text_3)
...
function EndPartyView:_get_text_widget_length(widget)
...
function EndPartyView:_register_end_of_round_camera(camera_unit)
	self._camera_unit = camera_unit
end

function EndPartyView:_convert_world_to_screen_position(camera, world_position)
	if camera then
...
function EndPartyView:_get_viewport()
...
function EndPartyView:_draw_widgets(dt, t, input_service, ui_renderer)
...
function EndPartyView:_on_panel_pressed(panel, index)
...
function EndPartyView:_update_player_accolade_positions(dt, t)
	local camera = self._camera
...
function EndPartyView:_spawn_player_accolade(player, spawn_position, spawn_delay)
	local draw_layer = 10
...
function EndPartyView:_spawn_player_panel(player, spawn_position)
...
function EndPartyView:_setup_players_ui()
...
function EndPartyView:_set_player_name(panel)
	local widget = panel.widget
	local player = panel.player
...
function EndPartyView:can_exit()
	return self._can_exit
end

function EndPartyView:update(dt, t)
...
function EndPartyView:_on_entry_pressed_cb(widget, entry)
...
function EndPartyView:_continue()
...
function EndPartyView:_set_timer_text(time)
