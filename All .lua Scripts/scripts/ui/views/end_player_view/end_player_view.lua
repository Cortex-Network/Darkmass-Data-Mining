...
function EndPlayerView:init(settings, context)
	self._card_widgets = {}
...
function EndPlayerView:on_enter()
...
function EndPlayerView:on_exit()
...
function EndPlayerView:can_exit()
	return self._can_exit
end

function EndPlayerView:trigger_on_exit_animation()
...
function EndPlayerView:on_exit_animation_done()
	local current_state_data = self._carousel_state_data
	local animation_id = current_state_data.animation_id
...
function EndPlayerView:on_resolution_modified(scale)
...
function EndPlayerView:update(dt, t, input_service)
...
function EndPlayerView:event_trigger_current_end_presentation_skip()
	self._skip_to_next_animation_state = true
end

function EndPlayerView:start_card_animation(card_widget, animation_name)
	local animation_id = self:_start_animation(animation_name, card_widget, nil, , animation_speed)

	return animation_id
end

function EndPlayerView:is_animation_done(animation_id)
	return not EndPlayerView.super._is_animation_active(self, animation_id)
end

function EndPlayerView:update_xp_bar(new_xp_value)
	self:_update_experience_bar(new_xp_value)
end

function EndPlayerView:update_wallet(new_value, wallet_type)
	self:_update_wallet(new_value, wallet_type)
end

function EndPlayerView:belate_wallet_update(new_value, wallet_type)
	self:_belate_wallet_update(new_value, wallet_type)
end

function EndPlayerView:update_belated_wallet(progress)
	self:_update_belated_wallets(progress)
end

function EndPlayerView:retract_currency_gain_widgets(progress)
	self:_retract_currency_gain_widgets(progress)
end

function EndPlayerView:trigger_currency_gain_widgets_timeout()
	self:_trigger_currency_gain_widgets_timeout()
end

function EndPlayerView:_handle_input(input_service, dt, t)
	EndPlayerView.super._handle_input(self, input_service, dt, t)
end

function EndPlayerView:play_sound(sound_event)
	EndPlayerView.super._play_sound(self, sound_event)
end

function EndPlayerView:stop_sound(sound_event)
	EndPlayerView.super._stop_sound(self, sound_event)
end

function EndPlayerView:set_sound_parameter(parameter_id, value)
	EndPlayerView.super._set_sound_parameter(self, parameter_id, value)
end

function EndPlayerView:_draw_widgets(dt, t, input_service, ui_renderer)
...
function EndPlayerView:_continue()
...
function EndPlayerView:_finish_current_animation_state()
...
function EndPlayerView:_setup_progress_bar()
...
function EndPlayerView:_create_cards()
	local card_widgets = self._card_widgets
	local session_report = self._session_report
...
function EndPlayerView:_create_card_widget(index, card_type, card_data)
	local blueprints = self._definitions.blueprints
...
function EndPlayerView:_setup_wallets(wallet_data, salary_rewards)
...
function EndPlayerView:_get_item(card_reward)
...
function EndPlayerView:_get_unlocked_talents(card_data)
	local player = self:_player()
	local profile = player:profile()
...
function EndPlayerView:_set_carousel_state(state_id)
	local state_data = self._carousel_state_data
...
function EndPlayerView:_update_carousel(dt, t, input_service)
	local carousel_state = self._current_carousel_state
...
function EndPlayerView:_update_timed_visibility_widgets(dt)
...
function EndPlayerView:_update_experience_bar(new_experience)
	local widgets_by_name = self._widgets_by_name
...
function EndPlayerView:_update_wallet(new_value, wallet_type)
...
function EndPlayerView:_belate_wallet_update(new_value, wallet_type)
	local wallet_widget = self._wallet_widgets[wallet_type]
...
function EndPlayerView:_update_belated_wallets(amount_progress)
	local currency_gain_widgets = self._currency_gain_widgets
...
function EndPlayerView:_retract_currency_gain_widgets(progress)
	local currency_gain_widgets = self._currency_gain_widgets
...
function EndPlayerView:_trigger_currency_gain_widgets_timeout()
