...
function EndView:init(settings, context)
...
function EndView:can_exit()
	return self._can_exit
end

function EndView:allow_close_hotkey()
	return self._can_exit
end

local _update_alpha_fade_time = EndViewSettings.alpha_fade_time

function EndView:update(dt, t, input_service)
	...
	if session_report and end_time and show_player_view_time and show_player_view_time < server_time and not is_showing_player_view then
		...
		Managers.ui:open_view(SUMMARY_VIEW_NAME, nil, , , , summary_view_context)
...
function EndView:draw(dt, t, input_service, layer)
	layer = _draw_layer

	EndView.super.draw(self, dt, t, input_service, layer)
end

function EndView:on_resolution_modified(scale)
...
function EndView:_on_navigation_input_changed()
	self:_update_buttons()
end

function EndView:event_register_end_of_round_camera(camera_unit)
...
function EndView:event_state_game_score_continue()
	local ui_manager = Managers.ui
	local is_showing_player_view = ui_manager:view_active(SUMMARY_VIEW_NAME)
...
function EndView:event_stay_in_party_voting_started(voting_id)
...
function EndView:_set_stay_in_party_voting_init_step_done()
...
function EndView:_stay_in_party_voting_started()
...
function EndView:event_stay_in_party_voting_completed()
	self._stay_in_party_voting_active = false
	self._stay_in_party_voting_id = nil
end

function EndView:event_stay_in_party_voting_aborted()
	self._stay_in_party_voting_active = false
	self._stay_in_party_voting_id = nil
end

function EndView:event_stay_in_party_vote_casted()
...
function EndView:_handle_input(input_service, dt, t)
...
function EndView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function EndView:_create_game_mode_condition_widgets()
...
function EndView:_update_continue_button_time(end_time, server_time)
...
function EndView:_update_voting_button_visibility(dt)
...
function EndView:_update_buttons()
...
function EndView:_get_timer_text(time)
	local timer_text = string.format("%.2d:%.2d", _math_floor(time / 60) % 60, time % 60)

	return timer_text
end

function EndView:_trigger_current_presentation_skip()
...
function EndView:_setup_stay_in_party_vote()
...
function EndView:_setup_spawn_slots(players)
...
function EndView:_get_free_slot_id()
	local spawn_slots = self._spawn_slots
...
function EndView:_player_slot_id(account_id)
	local spawn_slots = self._spawn_slots
...
function EndView:_assign_player_to_slot(player_info, slot, more_than_one_party)
...
function EndView:_create_player_widget(player_info, slot, more_than_one_party)
	local session_report = self._session_report
...
function EndView:_end_of_round_blur_background_world(blur_amount)
	self._world_spawner:set_camera_blur(blur_amount, EndViewSettings.total_blur_duration)
end

function EndView:_update_player_slots(dt, t, input_service)
	local spawn_slots = self._spawn_slots
...
function EndView:_reset_spawn_slot(slot)
	local profile_spawner = slot.profile_spawner
...
function EndView:_set_character_names()
	local session_report = self._session_report
...
function EndView:_set_mission_key(mission_key, session_report, render_scale)
...
function EndView:_get_participant_progression(participant_reports, account_id)
...
function EndView:_level_from_xp(experience_settings, xp)
...
function EndView:_load_portrait_icon(widget, profile)
	if not profile then
		return
	end
...
function EndView:_unload_portrait_icon(widget, ui_renderer)
...
function EndView:_unload_portrait_frame(widget, ui_renderer)
...
function EndView:_unload_insignia(widget)
...
function EndView:_cb_on_continue_pressed()
	self:_trigger_current_presentation_skip()
end

function EndView:_cb_on_stay_in_party_pressed()
	local voting_id = self._stay_in_party_voting_id
...
function EndView:_cb_set_player_icon(widget, grid_index, rows, columns, render_target)
	local portrait_style = widget.style.character_portrait
	local material_values = portrait_style.material_values
