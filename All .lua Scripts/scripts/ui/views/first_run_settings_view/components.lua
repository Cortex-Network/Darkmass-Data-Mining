...
function components.dropdown(template)
	...
	function entry.on_activated(new_value, current_value, new_option)
...
function components.checkbox(template)
	local entry = table.clone(template)
	entry.on_value_changed = nil
	local default_value = nil
	...
	function entry.on_activated(new_value, current_value)
		if not _is_same(current_value, new_value) and template.on_value_changed then
...
function components.percent_slider(template)
...
function components.value_slider(template)
	local min_value = template.min_value or 0
	local max_value = template.max_value or 100
