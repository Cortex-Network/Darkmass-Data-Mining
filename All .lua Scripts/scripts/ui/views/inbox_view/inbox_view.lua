...
function InboxView:_setup_input_legend()
...
function InboxView:_request_player_icon()
...
function InboxView:_load_portrait_icon()
...
function InboxView:_cb_set_player_icon(grid_index, rows, columns)
	local material_values = self._widgets_by_name.character_portrait.style.texture.material_values
...
function InboxView:_set_experience_bar(experience_fraction, duration)
	if duration then
...
function InboxView:_fetch_character_progression(player)
...
function InboxView:_create_offscreen_renderer()
...
function InboxView:_create_grid(entries)
...
function InboxView:_apply_item_icon_cb_func(widget, grid_index, rows, columns)
	local material_values = widget.style.icon.material_values
	material_values.use_placeholder_texture = 0
	material_values.rows = rows
	material_values.columns = columns
...
function InboxView:_remove_item_icon_cb_func(widget, ui_renderer)
	local material_values = widget.style.icon.material_values
	material_values.use_placeholder_texture = 1
end

function InboxView:_on_back_pressed()
	Managers.ui:close_view(self.view_name)
end

function InboxView:_destroy_renderer()
...
function InboxView:update(dt, t, input_service)
...
function InboxView:_update_grid_widgets_visibility()
...
function InboxView:draw(dt, t, input_service, layer)
	self:_draw_grid(dt, t, input_service)
	InboxView.super.draw(self, dt, t, input_service, layer)
end

function InboxView:_draw_grid(dt, t, input_service, layer)
	if self._inbox_grid then
...
function InboxView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
...
function InboxView:_destroy_grid_widgets()
...
function InboxView:on_exit()
	InboxView.super.on_exit(self)
	self:_destroy_grid_widgets()
	self:_destroy_renderer()
end

function InboxView:_fetch_store_items()
...
function InboxView:_create_grid_layout(item_offers)
	local layout = {}
...
function InboxView:cb_on_grid_entry_pressed(widget, element)
...
function InboxView:_preview_element(element)
	local item = element.item
...
function InboxView:_set_loading_visible(visible)
	local widgets_by_name = self._widgets_by_name
...
function InboxView:_set_empty_list()
	local widgets_by_name = self._widgets_by_name
	widgets_by_name.loading.content.text = "No Items available"
end

function InboxView:_set_preview_widgets_visibility(visible)
	local widgets_by_name = self._widgets_by_name
...
function InboxView:_setup_stats_preview_widgets()
	local num_stats = 6
	local max_rows = 2
	local stats_size = {
		216.66666666666666,
...
function InboxView:_setup_item_stats(item)
...
function InboxView:_set_stat_bar_value(stat_index, value, duration, should_reset)
...
function InboxView:_update_stat_bar_animations(dt)
	local stats_animation_progress = self._stats_animation_progress
