...
function InventoryCosmeticsView:init(settings, context)
...
function InventoryCosmeticsView:on_enter()
...
function InventoryCosmeticsView:_setup_input_legend()
...
function InventoryCosmeticsView:_set_preview_widgets_visibility(visible, allow_equip_button)
...
function InventoryCosmeticsView:_stop_previewing()
...
function InventoryCosmeticsView:_spawn_profile(profile, initial_rotation, disable_rotation_input)
...
function InventoryCosmeticsView:_preview_element(element)
	self:_stop_previewing()
...
function InventoryCosmeticsView:_update_player_panel_position()
...
function InventoryCosmeticsView:_register_button_callbacks()
	local widgets_by_name = self._widgets_by_name
...
function InventoryCosmeticsView:cb_on_camera_zoom_toggled(id, instant)
...
function InventoryCosmeticsView:_trigger_zoom_logic(instant, optional_slot_name)
	local selected_slot = self._selected_slot
...
function InventoryCosmeticsView:cb_on_equip_pressed()
	local selected_slot = self._selected_slot
...
function InventoryCosmeticsView:on_exit()
...
function InventoryCosmeticsView:_fetch_inventory_items(selected_slots)
...
function InventoryCosmeticsView:_calc_text_size(widget, text_and_style_id)
...
function InventoryCosmeticsView:_get_item_from_inventory(wanted_item)
...
function InventoryCosmeticsView:_item_valid_by_current_profile(item)
	local player = self._preview_player
	local profile = player:profile()
...
function InventoryCosmeticsView:_on_double_click(widget, element)
	local selected_slot = self._selected_slot
...
function InventoryCosmeticsView:_equip_item(slot_name, item)
...
function InventoryCosmeticsView:equipped_item_in_slot(slot_name)
...
function InventoryCosmeticsView:_clear_widgets(widgets)
	if widgets then
		for i = 1, #widgets do
...
function InventoryCosmeticsView:_handle_input(input_service)
	if input_service:get("confirm_pressed") then
		self:cb_on_equip_pressed()
	end
end

function InventoryCosmeticsView:_handle_back_pressed()
	local view_name = "inventory_cosmetics_view"

	Managers.ui:close_view(view_name)
end

function InventoryCosmeticsView:_update_equip_button_status()
	local previewed_item = self._previewed_item
...
function InventoryCosmeticsView:cb_on_close_pressed()
	self:_handle_back_pressed()
end

function InventoryCosmeticsView:update(dt, t, input_service)
...
function InventoryCosmeticsView:_get_weapon_spawn_position_normalized()
...
function InventoryCosmeticsView:on_resolution_modified(scale)
	InventoryCosmeticsView.super.on_resolution_modified(self, scale)
end

function InventoryCosmeticsView:_setup_background_world()
	local player = self._preview_player
...
function InventoryCosmeticsView:world_spawner()
	return self._world_spawner
end

function InventoryCosmeticsView:spawn_point_unit()
	return self._spawn_point_unit
end

function InventoryCosmeticsView:event_register_cosmetics_preview_character_spawn_point(spawn_point_unit)
...
function InventoryCosmeticsView:_set_camera_item_slot_focus(slot_name, time, func_ptr)
	local world_spawner = self._world_spawner
...
function InventoryCosmeticsView:_set_camera_node_focus(node_name, time, func_ptr)
	if node_name then
		local profile_spawner = self._profile_spawner
		local world_spawner = self._world_spawner
...
function InventoryCosmeticsView:_set_camera_position_axis_offset(axis, value, animation_time, func_ptr)
	self._world_spawner:set_camera_position_axis_offset(axis, value, animation_time, func_ptr)
end

function InventoryCosmeticsView:_set_camera_rotation_axis_offset(axis, value, animation_time, func_ptr)
