...
function InventoryView:init(settings, context)
...
function InventoryView:_display_title_text(text)
...
function InventoryView:_set_active_layout_by_index(index)
...
function InventoryView:_switch_active_layout(layout, is_grid_layout, camera_settings, ui_animation)
...
function InventoryView:on_exit()
	self:_destroy_loadout_widgets(self._ui_renderer)
...
function InventoryView:_setup_offscreen_gui()
...
function InventoryView:_destroy_offscreen_gui()
...
function InventoryView:_fetch_inventory_items()
...
function InventoryView:_item_valid_by_current_profile(item)
...
function InventoryView:_get_items_layout_by_slot(slot)
...
function InventoryView:event_inventory_view_set_camera_focus(slot_name)
	self:_set_camera_focus_by_slot_name(slot_name)
end

function InventoryView:_set_camera_focus_by_slot_name(slot_name, optional_camera_settings)
...
function InventoryView:cb_on_grid_entry_right_pressed(widget, element)
...
function InventoryView:equipped_item_in_slot(slot_name)
	return self._preview_profile_equipped_items[slot_name]
end

function InventoryView:cb_on_grid_entry_pressed(widget, element)
	if not self._is_own_player or self._is_readonly then
		return
	end
...
function InventoryView:_play_voice_preview(slot_name, item)
...
function InventoryView:_clear_widgets(widgets)
	if widgets then
		for i = 1, #widgets do
...
function InventoryView:_set_group_header_highlight(group_header, text)
	self._highlighted_group_header = group_header
	self._highlighted_group_header_text = text
end

function InventoryView:get_group_header_highlight_text(group_header)
...
function InventoryView:_reset_grid_layout(ui_renderer)
...
function InventoryView:_setup_individual_layout(layout)
...
function InventoryView:_setup_grid_layout(layout)
...
function InventoryView:cb_switch_tab(index)
	if index ~= self._selected_tab_index then
		self:_set_active_layout_by_index(index)
	end
end

function InventoryView:_destroy_previous_tab()
	if self._tab_menu_element then
...
function InventoryView:_setup_menu_tabs(content)
	local id = "tab_menu"
	local layer = 10
...
function InventoryView:_update_tab_bar_position()
...
function InventoryView:_setup_grid(widgets, alignment_list, grid_scenegraph_id, spacing)
	local direction = "down"

	return UIWidgetGrid:new(widgets, alignment_list, self._ui_scenegraph, grid_scenegraph_id, direction, spacing)
end

function InventoryView:_create_entry_widget_from_config(config, suffix, callback_name, secondary_callback_name, optional_scenegraph_id)
...
function InventoryView:_draw_loadout_widgets(dt, t, input_service, ui_renderer)
...
function InventoryView:_draw_wallet_widgets(dt, t, input_service, ui_renderer)
...
function InventoryView:_draw_grid(dt, t, input_service, ui_renderer)
...
function InventoryView:_destroy_loadout_widgets(ui_renderer)
	local widgets = self._loadout_widgets
...
function InventoryView:_destroy_grid_widgets(ui_renderer)
	local widgets = self._grid_widgets
...
function InventoryView:_update_blueprint_widgets(widgets, dt, t, input_service, ui_renderer)
	if widgets then
...
function InventoryView:_handle_input(input_service)
...
function InventoryView:_get_next_array_index(direction, start_index, array)
...
function InventoryView:_handle_individual_widget_selection(input_service)
...
function InventoryView:_on_navigation_input_changed()
...
function InventoryView:_select_individual_widget_index(index)
...
function InventoryView:on_resolution_modified(scale)
...
function InventoryView:update(dt, t, input_service)
...
function InventoryView:draw(dt, t, input_service, layer)
...
function InventoryView:_request_wallets_update()
...
function InventoryView:_update_wallets_presentation(wallets_data, hide_presentation)
...
function InventoryView:profile_level()
	local preview_player = self._preview_player
