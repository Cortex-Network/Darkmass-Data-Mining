...
function InventoryWeaponDetailsView:on_enter()
...
function InventoryWeaponDetailsView:_setup_debug_weapon_stats(item)
	self._weapon_statistics = WeaponStats:new(item)
end

function InventoryWeaponDetailsView:_setup_input_legend()
...
function InventoryWeaponDetailsView:_setup_weapon_stats()
...
function InventoryWeaponDetailsView:_update_weapon_stats_position()
	if not self._weapon_stats then
		return
	end
...
function InventoryWeaponDetailsView:_setup_weapon_actions()
...
function InventoryWeaponDetailsView:_update_weapon_actions_position()
	if not self._weapon_actions then
		return
	end
...
function InventoryWeaponDetailsView:_cb_on_close_pressed()
	Managers.ui:close_view("inventory_weapon_details_view")
end

function InventoryWeaponDetailsView:_setup_default_gui()
...
function InventoryWeaponDetailsView:_preview_item(item)
...
function InventoryWeaponDetailsView:_destroy_weapon_preview()
...
function InventoryWeaponDetailsView:_setup_weapon_preview()
...
function InventoryWeaponDetailsView:_update_weapon_preview_viewport()
...
function InventoryWeaponDetailsView:on_back_pressed()
	Managers.ui:close_view("inventory_weapon_details_view")
end

function InventoryWeaponDetailsView:on_exit()
...
function InventoryWeaponDetailsView:_get_weapon_spawn_position_normalized()
...
function InventoryWeaponDetailsView:_set_weapon_zoom(fraction)
	self._weapon_zoom_fraction = fraction

	self:_update_weapon_preview_viewport()
end

function InventoryWeaponDetailsView:_is_trait_slot_unlocked(slot_index)
...
function InventoryWeaponDetailsView:_cb_on_ui_visibility_toggled(id)
...
function InventoryWeaponDetailsView:_handle_input(input_service)
...
function InventoryWeaponDetailsView:update(dt, t, input_service)
	return InventoryWeaponDetailsView.super.update(self, dt, t, input_service)
end

function InventoryWeaponDetailsView:draw(dt, t, input_service, layer)
...
function InventoryWeaponDetailsView:_draw_elements(dt, t, ui_renderer, render_settings, input_service)
...
function InventoryWeaponDetailsView:_draw_widgets(dt, t, input_service, ui_renderer, render_settings)
	local alpha_multiplier = self._alpha_multiplier or 1
...
function InventoryWeaponDetailsView:_clear_widgets(widgets)
	if widgets then
		for i = 1, #widgets do
			local widget = widgets[i]
			local widget_name = widget.name
...
function InventoryWeaponDetailsView:_setup_offscreen_gui()
	local ui_manager = Managers.ui
...
function InventoryWeaponDetailsView:_destroy_offscreen_gui()
...
function InventoryWeaponDetailsView:on_resolution_modified(scale)
