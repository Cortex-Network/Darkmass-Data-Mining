...
function InventoryWeaponsView:init(settings, context)
...
function InventoryWeaponsView:character_level()
...
function InventoryWeaponsView:on_enter()
...
function InventoryWeaponsView:_setup_background_frames_by_archetype(archetype_name)
...
function InventoryWeaponsView:_setup_item_grid_materials()
...
function InventoryWeaponsView:_setup_weapon_actions()
...
function InventoryWeaponsView:on_resolution_modified(scale)
	InventoryWeaponsView.super.on_resolution_modified(self, scale)
	self:_update_weapon_actions_position()
end

function InventoryWeaponsView:_update_weapon_actions_position()
	if not self._weapon_actions then
		return
	end
...
function InventoryWeaponsView:_setup_input_legend()
...
function InventoryWeaponsView:_register_button_callbacks()
	local widgets_by_name = self._widgets_by_name
...
function InventoryWeaponsView:cb_on_discard_held()
...
function InventoryWeaponsView:is_selected_item_equipped()
	local selected_grid_widget = self:selected_grid_widget()
...
function InventoryWeaponsView:cb_on_close_pressed()
	self:_handle_back_pressed()
end

function InventoryWeaponsView:_set_preview_widgets_visibility(visible)
...
function InventoryWeaponsView:_preview_element(element)
	InventoryWeaponsView.super._preview_element(self, element)
end

function InventoryWeaponsView:sort_items()
...
function InventoryWeaponsView:cb_on_sort_button_pressed(option)
...
function InventoryWeaponsView:update(dt, t, input_service)
...
function InventoryWeaponsView:_handle_input(input_service)
	if input_service:get("confirm_pressed") then
		self:cb_on_equip_pressed()
	end
end

function InventoryWeaponsView:_handle_back_pressed()
	local view_name = "inventory_weapons_view"

	Managers.ui:close_view(view_name)
end

function InventoryWeaponsView:cb_on_customize_pressed()
	if not Managers.ui:view_active("inventory_weapon_cosmetics_view") then
		self._customize_view_opened = true

		Managers.ui:open_view("inventory_weapon_cosmetics_view", nil, , , , {
...
function InventoryWeaponsView:cb_on_inspect_pressed()
...
function InventoryWeaponsView:cb_on_equip_pressed()
	local selected_slot = self._selected_slot
...
function InventoryWeaponsView:_setup_background_world()
...
function InventoryWeaponsView:world_spawner()
	return self._world_spawner
end

function InventoryWeaponsView:on_exit()
...
function InventoryWeaponsView:_fetch_inventory_items(selected_slot)
...
function InventoryWeaponsView:_get_item_from_inventory(wanted_item)
...
function InventoryWeaponsView:_item_valid_by_current_profile(item)
	local player = self._preview_player
...
function InventoryWeaponsView:_on_double_click(widget, element)
	local selected_slot = self._selected_slot
...
function InventoryWeaponsView:_fetch_item_compare_slot_name(item)
	local selected_slot = self._selected_slot
...
function InventoryWeaponsView:_equip_item(slot_name, item)
...
function InventoryWeaponsView:has_equipped_item_in_any_slot(slots)
	for i = 1, #slots do
...
function InventoryWeaponsView:is_item_equipped_in_any_slot(item, slots)
	for i = 1, #slots do
		local slot_name = slots[i]
...
function InventoryWeaponsView:equipped_item_in_slot(slot_name)
...
function InventoryWeaponsView:_update_grid_widgets(dt, t, input_service)
...
function InventoryWeaponsView:_discard_items(item)
	local gear_id = item.gear_id
...
function InventoryWeaponsView:_mark_item_for_discard(grid_index)
	...
	function self._on_discard_anim_complete_cb()
...
function InventoryWeaponsView:_update_equip_button_status()
	local previewed_item = self._previewed_item
...
function InventoryWeaponsView:_update_item_discard_progress(dt)
