...
function LobbyView:init(settings, context)
...
function LobbyView:event_register_lobby_camera(camera_unit)
...
function LobbyView:event_lobby_vote_started(context)
...
function LobbyView:_setup_mission_descriptions()
...
function LobbyView:_setup_menu_list()
...
function LobbyView:_setup_input_legend()
...
function LobbyView:cb_on_close_pressed()
	Managers.ui:close_view("lobby_view")
end

function LobbyView:cb_on_open_main_menu_pressed()
	Managers.ui:open_view("system_view")
end

function LobbyView:cb_on_inventory_pressed()
	Managers.ui:open_view(INVENTORY_VIEW_NAME)
end

function LobbyView:cb_on_loadout_pressed()
	self._show_loadout = not self._show_loadout
end

function LobbyView:_setup_menu_list_entries(config)
...
function LobbyView:_setup_list_content_widgets(content, scenegraph_id, callback_name)
...
function LobbyView:cb_on_list_entry_pressed(widget, entry)
...
function LobbyView:on_resolution_modified()
...
function LobbyView:_setup_spawn_slots()
...
function LobbyView:_destroy_spawn_slots()
...
function LobbyView:can_exit()
	return self._can_exit and self._navigation_state == "menu"
end

function LobbyView:on_exit()
...
function LobbyView:update(dt, t, input_service)
...
function LobbyView:_all_ready_countdown(dt)
...
function LobbyView:_update_player_slots(dt, t, input_service)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_update_menu_list(dt, t)
...
function LobbyView:_on_navigation_input_changed()
...
function LobbyView:_get_previous_occupied_slot_index(from_index)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_get_next_occupied_slot_index(from_index)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_handle_input(input_service, dt, t)
	local is_mouse = self._using_cursor_navigation
...
function LobbyView:_reset_spawn_slot(slot)
	local profile_spawner = slot.profile_spawner
...
function LobbyView:_get_free_slot_id()
	local spawn_slots = self._spawn_slots
...
function LobbyView:_player_slot_id(unique_id)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_assign_player_to_slot(player, slot)
...
function LobbyView:_load_portrait_icon(slot)
...
function LobbyView:_unload_all_portrait_icon()
	local spawn_slots = self._spawn_slots
...
function LobbyView:_unload_portrait_icon(slot)
...
function LobbyView:_sync_players()
...
function LobbyView:_cb_set_player_icon(slot, grid_index, rows, columns, render_target)
...
function LobbyView:_sync_local_player()
...
function LobbyView:_sync_player(unique_id, player)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_update_synced_slots()
	local spawn_slots = self._spawn_slots
...
function LobbyView:draw(dt, t, input_service, layer)
	self:_draw_menu_list_grid(dt, t, input_service)
	LobbyView.super.draw(self, dt, t, input_service, layer)
end

function LobbyView:_draw_widgets(dt, t, input_service, ui_renderer)
...
function LobbyView:_draw_menu_list_grid(dt, t, input_service)
...
function LobbyView:_convert_world_to_screen_position(camera, world_position)
	if camera then
...
function LobbyView:_set_slot_focused_by_index(index)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_is_slot_focused()
	return not not self._focused_slot_index
end

function LobbyView:_is_button_focused()
	return not not self._focused_button_index
end

function LobbyView:_get_slot_index_by_player(player)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_slot_by_index(index)
	local spawn_slots = self._spawn_slots
...
function LobbyView:_in_preview()
	return self._preview
end

function LobbyView:_set_own_player_ready_status(is_ready)
	if self._preview then
		return
	end
...
function LobbyView:event_lobby_ready_vote_casted()
	self:_sync_votes()
end

function LobbyView:_sync_votes()
	if self._preview then
		return
	end
...
function LobbyView:_set_slot_ready_status(slot, is_ready)
...
function LobbyView:_open_inventory_by_slot(slot)
	local player = slot.player
	local context = {
		parent = self,
		player = player,
	...
	Managers.ui:open_view(INVENTORY_VIEW_NAME, nil, , , , context)
end

function LobbyView:cb_on_slot_inspect_pressed(slot)
	local profile_spawner = slot.profile_spawner
...
function LobbyView:set_own_player_ready_status(is_ready)
	self:_set_own_player_ready_status(is_ready)
end

function LobbyView:_check_loadout_changes()
	local search_slot = {
		"slot_primary",
		"slot_secondary"
	}
...
function LobbyView:_setup_loadout_widgets(spawn_slot)
	local size = {
		370,
		0
	}
	...
	for i = 1, #search_slot do
		...
		if widget_definition then
			...
			if init then
				init(self, weapon_widget, config, nil, , ui_renderer)
			end
...
function LobbyView:_set_loadout_visibility()
...
function LobbyView:trigger_on_exit_animation()
...
function LobbyView:on_exit_animation_done()
	return not self._is_animating_on_exit and self:triggered_on_exit_animation()
end

function LobbyView:_update_presentation_wield_item(spawn_slot)
...
function LobbyView:_start_animation_ready(spawn_slot)
	if not spawn_slot.profile_spawner then
		return
	end
...
function LobbyView:_start_animation_unready(spawn_slot)
	if not spawn_slot.profile_spawner then
		return
	end
