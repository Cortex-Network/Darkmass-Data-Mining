...
function MainMenuView:init(settings, context)
...
function MainMenuView:on_enter()
...
function MainMenuView:_setup_input_legend()
...
function MainMenuView:_update_counts_refreshes(dt)
...
function MainMenuView:cb_on_open_main_menu_pressed()
	Managers.ui:open_view("system_view")
end

function MainMenuView:cb_update_friends_count(friends)
...
function MainMenuView:cb_update_strike_team_count(party)
...
function MainMenuView:cb_on_list_entry_pressed(widget, entry)
...
function MainMenuView:_set_waiting_for_characters(waiting)
...
function MainMenuView:event_main_menu_load_news()
	self:_update_news_list()
end

function MainMenuView:_event_profiles_changed(profiles)
...
function MainMenuView:_event_selected_profile_changed(profile)
...
function MainMenuView:_on_character_widget_selected(index)
...
function MainMenuView:_set_selected_character_list_index(index)
	local character_list_widgets = self._character_list_widgets
...
function MainMenuView:_reset_news_list()
	return {
		slides = {},
		viewed_slides_id = {}
	}
...
function MainMenuView:_populate_news_list()
...
function MainMenuView:_get_news_starting_slide_index(slides, viewed_slides)
...
function MainMenuView:_is_processing_backend_request()
	return self._news_list_requested
end

function MainMenuView:_update_news_list()
	self._news_list_requested = true

	self:_populate_news_list()
end

function MainMenuView:_event_profile_sync_changed(is_active)
	self._character_wait_overlay_active = is_active

	self:_set_waiting_for_characters(is_active)
end

function MainMenuView:update(dt, t, input_service)
	...
	if not self:_is_processing_backend_request() and self._news_list.starting_slide_index then
		...
		Managers.ui:open_view("news_view", nil, , , , {
...
function MainMenuView:draw(dt, t, input_service, layer)
	if self._character_wait_overlay_active then
		input_service = input_service:null_service()
	end
...
function MainMenuView:_draw_widgets(dt, t, input_service, ui_renderer)
	return MainMenuView.super._draw_widgets(self, dt, t, input_service, ui_renderer)
end

function MainMenuView:_on_navigation_input_changed()
...
function MainMenuView:_handle_input(input_service)
...
function MainMenuView:_on_delete_selected_character_pressed()
...
function MainMenuView:_destroy_character_list_renderer()
...
function MainMenuView:_sync_character_slots()
	self:_destroy_character_grid()
...
function MainMenuView:_draw_character_list(dt, t, input_service, layer)
...
function MainMenuView:_convert_world_to_screen_position(camera, world_position)
	if camera then
...
function MainMenuView:character_profiles()
	return self._character_profiles
end

function MainMenuView:character_wait_overlay_active()
	return self._character_wait_overlay_active
end

function MainMenuView:on_character_widget_selected(index)
	self:_on_character_widget_selected(index)
end

function MainMenuView:on_create_character_pressed()
	self:_on_create_character_pressed()
end

function MainMenuView:on_play_pressed()
	self:_on_play_pressed()
end

function MainMenuView:_create_character_list_renderer()
...
function MainMenuView:_destroy_character_grid()
	local widgets = self._character_list_widgets
...
function MainMenuView:_setup_interactions()
	...
	function self._widgets_by_name.counts_background.content.hotspot.pressed_callback()
		Managers.ui:open_view("social_menu_view")
	end
end

function MainMenuView:_show_character_details(show, profile)
...
function MainMenuView:_are_slots_full()
	local profiles = self._character_profiles
	local num_characters = profiles and #profiles or 0
...
function MainMenuView:_set_player_profile_information(profile, widget)
...
function MainMenuView:_load_portrait_icon(profile, widget)
...
function MainMenuView:_cb_set_player_icon(widget, grid_index, rows, columns, render_target)
	local material_values = widget.style.character_portrait.material_values
...
function MainMenuView:_unload_portrait_icon(widget)
