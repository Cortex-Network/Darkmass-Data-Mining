...
function OptionsView:init(settings)
...
function OptionsView:cb_on_back_pressed()
...
function OptionsView:cb_reset_category_to_default()
...
function OptionsView:_setup_input_legend()
...
function OptionsView:_setup_content_grid_scrollbar(grid, widget_id, grid_scenegraph_id, grid_pivot_scenegraph_id)
...
function OptionsView:_setup_offscreen_gui()
...
function OptionsView:_setup_content_widgets(content, scenegraph_id, callback_name)
...
function OptionsView:_draw_widgets(dt, t, input_service, ui_renderer)
	local widgets_by_name = self._widgets_by_name
...
function OptionsView:_draw_elements(dt, t, ui_renderer, render_settings, input_service)
...
function OptionsView:draw(dt, t, input_service, layer)
...
function OptionsView:_draw_grid(grid, widgets, interaction_widget, dt, t, input_service)
...
function OptionsView:_setup_grid(widgets, alignment_list, grid_scenegraph_id, spacing, use_is_focused)
	local ui_scenegraph = self._ui_scenegraph
...
function OptionsView:set_render_scale(scale)
...
function OptionsView:update(dt, t, input_service, view_data)
...
function OptionsView:on_resolution_modified()
...
function OptionsView:_on_navigation_input_changed()
...
function OptionsView:_reset_options_view(reset_all)
	if reset_all then
...
function OptionsView:settings_grid_length()
...
function OptionsView:settings_scroll_amount()
	local grid = self._settings_content_grid
...
function OptionsView:set_exclusive_focus_on_grid_widget(widget_name)
	self:_set_exclusive_focus_on_grid_widget(widget_name)
end

function OptionsView:_handle_input(input_service)
	local selected_settings_widget = self._selected_settings_widget
...
function OptionsView:_update_grid_navigation_selection()
...
function OptionsView:present_category_widgets(category)
...
function OptionsView:_setup_category_config(config)
	if self._category_content_widgets then
		for i = 1, #self._category_content_widgets do
			local widget = self._category_content_widgets[i]
...
function OptionsView:_setup_settings_config(config)
...
function OptionsView:_update_category_content_widgets(dt, t)
...
function OptionsView:_set_tooltip_data(widget)
...
function OptionsView:_update_settings_content_widgets(dt, t, input_service)
...
function OptionsView:_create_settings_widget_from_config(config, category, suffix, callback_name)
...
function OptionsView:_handle_keybind_rebind(dt, t, input_service)
...
function OptionsView:_handling_keybinding()
	return self._handling_keybind or self._close_keybind_popup_duration ~= nil
end

function OptionsView:show_keybind_popup(widget, entry)
...
function OptionsView:close_keybind_popup(force_close)
	if force_close then
		Managers.input:stop_key_watch()
...
function OptionsView:_set_warning_text()
	local widgets_by_name = self._widgets_by_name
...
function OptionsView:cb_on_category_pressed(widget, entry)
...
function OptionsView:cb_on_settings_pressed(widget, entry)
...
function OptionsView:_enable_settings_overlay(enable)
	local widgets_by_name = self._widgets_by_name
	local settings_overlay_widget = widgets_by_name.settings_overlay
	settings_overlay_widget.content.visible = enable
end

function OptionsView:_set_exclusive_focus_on_grid_widget(widget_name)
...
function OptionsView:_change_navigation_column(column_index)
...
function OptionsView:_set_default_navigation_widget()
	local navigation_widgets = self._navigation_widgets
...
function OptionsView:_set_selected_navigation_widget(widget)
	...
	for column_index = 1, #navigation_grids do
		...
		navigation_grid:select_grid_index(selected_grid and selected_row or nil, nil, , column_index == CATEGORIES_GRID)
	end
...
function OptionsView:_set_focused_grid_widget(widgets, widget_name)
...
function OptionsView:_set_selected_grid_widget(widgets, widget_name)
	local selected_widget, selected_widget_index = nil
