...
function ResultView:init(settings, context)
...
function ResultView:can_exit()
	return self._can_exit
end

function ResultView:on_exit()
	ResultView.super.on_exit(self)
end

function ResultView:_set_overlay_opacity(opacity)
	local widget = self._widgets_by_name.overlay
	widget.alpha_multiplier = opacity
end

function ResultView:_set_icon_backround_progress(progress)
...
function ResultView:update(dt, t)
...
function ResultView:_set_text(text, text_id, background_id)
