...
function SocialMenuRosterView:init(settings, context)
...
function SocialMenuRosterView:on_enter()
...
function SocialMenuRosterView:on_exit()
...
function SocialMenuRosterView:set_can_exit(value, apply_next_frame)
...
function SocialMenuRosterView:update(dt, t, input_service)
...
function SocialMenuRosterView:draw(dt, t, input_service, layer)
...
function SocialMenuRosterView:on_resolution_modified()
	SocialMenuRosterView.super.on_resolution_modified(self)
	self:_position_tab_bar()
end

function SocialMenuRosterView:_on_navigation_input_changed()
...
function SocialMenuRosterView:set_next_roster_filter(list_index)
...
function SocialMenuRosterView:formatted_character_name(player_info)
...
function SocialMenuRosterView:cb_switch_roster_filter(index)
	if self._using_cursor_navigation then
		self:set_next_roster_filter(index)
	end
end

function SocialMenuRosterView:cb_close_popup_menu()
	self:_close_popup_menu()
end

function SocialMenuRosterView:cb_on_popup_menu_closed()
	self:_remove_popup_menu()
end

function SocialMenuRosterView:cb_leave_party()
	self:_close_popup_menu()
	self:_show_confirmation_popup(nil, "leave_party")
end

function SocialMenuRosterView:cb_vote_to_kick_player_from_party(player_info)
...
function SocialMenuRosterView:cb_invite_player_to_party(player_info)
	social_service:send_party_invite(player_info)
	self:_close_popup_menu()
end

function SocialMenuRosterView:cb_cancel_party_invite(player_info)
	social_service:cancel_party_invite(player_info)
	self:_close_popup_menu()
end

function SocialMenuRosterView:cb_join_players_party(player_info)
	self:_close_popup_menu()
...
function SocialMenuRosterView:cb_show_player_profile(player_info)
	self:_close_popup_menu()
end

function SocialMenuRosterView:cb_show_xbox_profile(player_info)
	self:_close_popup_menu()
...
function SocialMenuRosterView:cb_invite_player_to_guild(player_info)
	self:_close_popup_menu()
end

function SocialMenuRosterView:cb_send_friend_request(player_info)
...
function SocialMenuRosterView:cb_cancel_friend_request(player_info)
	local account_id = player_info:account_id()
	local friend_status = player_info:friend_status()
...
function SocialMenuRosterView:cb_accept_friend_request(player_info)
	local account_id = player_info:account_id()
	local friend_status = player_info:friend_status()
...
function SocialMenuRosterView:cb_reject_friend_request(player_info)
	local account_id = player_info:account_id()
	local friend_status = player_info:friend_status()
...
function SocialMenuRosterView:cb_unfriend_player(player_info)
	local account_id = player_info:account_id()
	local friend_status = player_info:friend_status()
...
function SocialMenuRosterView:cb_mute_text_chat(player_info, is_muted)
	local account_id = player_info:account_id()
...
function SocialMenuRosterView:cb_mute_voice_chat(player_info, is_muted)
	local account_id = player_info:account_id()
	local friend_status = player_info:friend_status()
...
function SocialMenuRosterView:cb_unblock_player(player_info)
	self:_close_popup_menu()
	self:_show_confirmation_popup(player_info, "unblock_account")
end

function SocialMenuRosterView:cb_block_player(player_info)
	self:_close_popup_menu()
	self:_show_confirmation_popup(player_info, "block_account")
end

function SocialMenuRosterView:cb_report_player(player_info)
	self:_close_popup_menu()
end

function SocialMenuRosterView:cb_update_roster(results)
...
function SocialMenuRosterView:_fetch_platform_player_data(results)
...
function SocialMenuRosterView:cb_show_popup_menu_for_player(player_info)
	local popup_menu = self._popup_menu
...
function SocialMenuRosterView:on_back_pressed()
...
function SocialMenuRosterView:_cb_set_player_icon(widget, grid_index, rows, columns, render_target)
...
function SocialMenuRosterView:_cb_set_player_frame(widget, item)
	local widget_content = widget.content
...
function SocialMenuRosterView:_update_list_refreshes(dt)
...
function SocialMenuRosterView:_update_portraits()
	local widgets_with_portraits = self._widgets_with_portraits
	...
	for i = #widgets_with_portraits, 1, -1 do
		...
		elseif profile.hash ~= content.profile_hash then
			...
			if character_name ~= "" then
				...
				Managers.event:trigger("event_player_profile_updated", nil, , profile)
...
function SocialMenuRosterView:_update_view_visibility(dt, t, input_service)
...
function SocialMenuRosterView:_draw_widgets(dt, t, input_service, ui_renderer)
	if self._popup_menu then
...
function SocialMenuRosterView:_draw_roster_grid(dt, t, input_service, ui_renderer)
...
function SocialMenuRosterView:_position_tab_bar()
...
function SocialMenuRosterView:_create_offscreen_renderer()
...
function SocialMenuRosterView:_destroy_renderer()
...
function SocialMenuRosterView:_setup_tab_bar(tab_bar, params)
...
function SocialMenuRosterView:_create_party_grid()
	local party_widgets = self._party_widgets
...
function SocialMenuRosterView:_check_can_exit()
...
function SocialMenuRosterView:_update_portrait_frame(widget, profile)
	local widget_content = widget.content
...
function SocialMenuRosterView:_load_widget_portrait(widget, profile, portrait_renderer)
	local widget_content = widget.content
...
function SocialMenuRosterView:_unload_widget_portrait(widget)
	local widget_content = widget.content
	local portrait_style = widget.style.portrait
...
function SocialMenuRosterView:_setup_roster_grid(widget_data)
...
function SocialMenuRosterView:_assign_roster_grid_scrollbar(roster_grid, widgets)
...
function SocialMenuRosterView:_create_roster_widgets(widget_data)
	local widgets = {}
...
function SocialMenuRosterView:get_platform_profile(xuid)
	return self._platform_id_to_display_name_lut[xuid]
end

function SocialMenuRosterView:_get_roster_widget(context, blueprint_name, grid_id, optional_unique_id)
...
function SocialMenuRosterView:_handle_input(input_service, dt, t)
	if self._popup_menu then
		return
	end
...
function SocialMenuRosterView:_get_roster_grid_first_visible_row()
...
function SocialMenuRosterView:_prepare_list(roster_list_data, roster_list, new_list_items)
...
function SocialMenuRosterView:_close_popup_menu()
	self._popup_menu:close(callback(self, "cb_on_popup_menu_closed"))
	self:_fade_widgets("in", true)
end

function SocialMenuRosterView:_remove_popup_menu()
	self._popup_menu = nil

	self:_remove_element(POPUP_NAME)
end

function SocialMenuRosterView:_fade_widgets(direction, should_delay)
	self._widgets_are_fading = direction
	self._fade_time = 0
	self._fade_delay = should_delay and SocialMenuSettings.widget_fade_delay or 0
end

function SocialMenuRosterView:_update_roster_list(list_index, new_list_items, should_update_widgets)
...
function SocialMenuRosterView:_update_party_list(party_members, force_update)
...
function SocialMenuRosterView:_update_current_list_widgets()
...
function SocialMenuRosterView:_add_to_party(unique_id, player_info)
	local party_widgets = self._party_widgets
...
function SocialMenuRosterView:_refresh_party_list()
...
function SocialMenuRosterView:_refresh_roster_lists(force_refresh)
...
function SocialMenuRosterView:_show_confirmation_popup(player_info, command_name, callback_parameter)
