...
function TabbedMenuViewBase:init(definitions, settings, context)
...
function TabbedMenuViewBase:on_enter()
...
function TabbedMenuViewBase:on_exit()
...
function TabbedMenuViewBase:get_current_tabs()
	return self._tab_bar_views
end

function TabbedMenuViewBase:update(dt, t, input_service)
...
function TabbedMenuViewBase:_blur_fade_in(duration, anim_func)
	if self._world_spawner then
...
function TabbedMenuViewBase:_blur_fade_out(duration, anim_func)
	if self._world_spawner then
...
function TabbedMenuViewBase:set_active_view_instance(view)
	self._active_view_instance = view
end

function TabbedMenuViewBase:set_can_navigate(value)
	local has_tab_bar_menu = self._elements.tab_bar ~= nil
	self._can_navigate = value and has_tab_bar_menu
end

function TabbedMenuViewBase:cb_on_close_pressed()
	self:_handle_back_pressed()
end

function TabbedMenuViewBase:cb_switch_tab(index)
	if self._can_navigate then
		self._next_tab_index = index
	end
end

function TabbedMenuViewBase:event_register_camera(camera_unit)
...
function TabbedMenuViewBase:_setup_background_world(world_params)
...
function TabbedMenuViewBase:_setup_tab_bar(tab_bar_params, additional_context)
...
function TabbedMenuViewBase:_setup_input_legend(input_legend_params)
...
function TabbedMenuViewBase:add_input_legend_entry(entry_params)
...
function TabbedMenuViewBase:remove_input_legend_entry(id)
	local input_legend = self:_element("input_legend")

	return input_legend and input_legend:remove_entry(id)
end

function TabbedMenuViewBase:input_legend_entry_set_display_name(entry_id, display_name, suffix)
	local input_legend = self:_element("input_legend")
...
function TabbedMenuViewBase:_switch_tab(index)
	...
	if view ~= current_view or not current_view then
		...
		if view then
			...
			ui_manager:open_view(view, nil, , , , context)
...
function TabbedMenuViewBase:_handle_input(input_service)
	local tab_bar_menu = self._elements.tab_bar
...
function TabbedMenuViewBase:_handle_back_pressed()
...
function TabbedMenuViewBase:_close_active_view()
	local ui_manager = Managers.ui
...
function TabbedMenuViewBase:selected_index()
	local tab_bar_menu = self._elements.tab_bar
...
function TabbedMenuViewBase:set_selected_panel_index(index)
	local tab_bar_menu = self._elements.tab_bar
...
function TabbedMenuViewBase:set_is_handling_navigation_input(is_enabled)
	local tab_bar_menu = self._elements.tab_bar
