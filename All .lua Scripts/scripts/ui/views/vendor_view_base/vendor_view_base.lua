...
function VendorViewBase:init(definitions, settings, context)
...
function VendorViewBase:on_enter()
...
function VendorViewBase:character_level()
...
function VendorViewBase:equipped_item_in_slot(slot_name)
	local player = self:_player()
	local profile = player:profile()
...
function VendorViewBase:_register_button_callbacks()
...
function VendorViewBase:cb_switch_tab(index, ignore_item_selection)
	self._next_tab_index = index
	self._next_tab_index_ignore_item_selection = ignore_item_selection
end

function VendorViewBase:_switch_tab(index, ignore_item_selection)
...
function VendorViewBase:_set_preview_widgets_visibility(visible)
...
function VendorViewBase:_preview_element(element)
...
function VendorViewBase:on_exit()
...
function VendorViewBase:_get_store()
end

function VendorViewBase:_clear_list()
...
function VendorViewBase:_fetch_store_items()
	self._current_rotation_end = nil
...
function VendorViewBase:_update_grid_height(use_tab_menu)
...
function VendorViewBase:cb_on_sort_button_pressed(option)
	VendorViewBase.super.cb_on_sort_button_pressed(self, option)
end

function VendorViewBase:_item_valid_by_current_profile(item)
	local player = self:_player()
	local profile = player:profile()
...
function VendorViewBase:_convert_offers_to_layout_entries(item_offers)
	local layout = {}
...
function VendorViewBase:update(dt, t, input_service)
...
function VendorViewBase:_set_display_price(price_data)
...
function VendorViewBase:_purchase_item(offer)
...
function VendorViewBase:_update_wallets()
	local store_service = Managers.data_service.store
...
function VendorViewBase:_update_wallets_presentation(wallets_data)
...
function VendorViewBase:can_afford(amount, type)
	return amount <= (self._current_balance[type] or 0)
end

function VendorViewBase:_handle_input(input_service)
