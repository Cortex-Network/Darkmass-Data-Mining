...
function UIWidgetGrid:init(widgets, alignment_list, scenegraph, area_scenegraph_id, direction, spacing, fill_section_spacing, use_is_focused_for_navigation, use_select_on_focused)
...
function UIWidgetGrid:_get_area_size()
...
function UIWidgetGrid:get_spacing()
	return self._spacing
end

function UIWidgetGrid:assign_scrollbar(scrollbar_widget, pivot_scenegraph_id, interaction_scenegraph_id)
...
function UIWidgetGrid:can_scroll()
...
function UIWidgetGrid:set_scrollbar_progress(progress, animate_scroll)
...
function UIWidgetGrid:scrollbar_progress()
	if self._scrollbar_active then
...
function UIWidgetGrid:set_render_scale(scale)
	self._render_scale = scale
end

function UIWidgetGrid:_update_animations(dt, t)
...
function UIWidgetGrid:update(dt, t, input_service)
...
function UIWidgetGrid:on_resolution_modified(scale)
	self._render_scale = scale
	self._area_size = nil
...
function UIWidgetGrid:_update_scrollbar_sizes()
	local scrollbar_widget = self._scrollbar_widget
...
function UIWidgetGrid:set_scroll_step_length(step_length)
	self._scroll_step_length = step_length

	self:_update_scrollbar_sizes()
end

function UIWidgetGrid:_update_grid_position(dt, t)
...
function UIWidgetGrid:scroll_progress_by_length(length)
	local scroll_length = self:scroll_length()
...
function UIWidgetGrid:clear_scroll_progress()
	self._scroll_progress = nil
end

function UIWidgetGrid:length_scrolled()
...
function UIWidgetGrid:scroll_length()
	local axis = self._axis
...
function UIWidgetGrid:area_length()
	local axis = self._axis
	local area_size = self:_get_area_size()
...
function UIWidgetGrid:length()
	return self._total_grid_length
end

function UIWidgetGrid:_find_closest_neighbour_vertical(index, input_direction)
...
function UIWidgetGrid:_find_closest_neighbour_horizontal(index, input_direction)
	local widgets = self._widgets
	local direction = self._direction
	local growing_vertically = direction == DIRECTION.UP or direction == DIRECTION.DOWN
	local current_widget = widgets[index]
...
function UIWidgetGrid:force_update_list_size()
...
function UIWidgetGrid:remove_widget(widget)
	local widgets = self._widgets
	local alignment_list = self._alignment_list
...
function UIWidgetGrid:_align_grid_widgets(alignment_list)
	local axis = self._axis
	local stale_axis = axis % 2 + 1
	local direction = self._direction
	local area_size = self:_get_area_size()
...
function UIWidgetGrid:handle_grid_selection(input_service)
	local current_index = self._selected_grid_index
	local widgets = self._widgets
...
function UIWidgetGrid:widget_neighbour_by_index(widget, index)
	local content = widget.content
...
function UIWidgetGrid:widgets()
	return self._widgets
end

function UIWidgetGrid:index_by_widget(widget)
	local widgets = self._widgets
...
function UIWidgetGrid:first_interactable_grid_index()
	return self._first_interactable_widget_index
end

function UIWidgetGrid:last_interactable_grid_index()
	return self._last_interactable_widget_index
end

function UIWidgetGrid:select_first_index(use_is_focused)
...
function UIWidgetGrid:select_last_index(use_is_focused)
...
function UIWidgetGrid:select_widget(widget, scrollbar_animation_progress, scroll_to_widget)
...
function UIWidgetGrid:selected_grid_index()
	return self._selected_grid_index
end

function UIWidgetGrid:widget_by_index(index)
	local widgets = self._widgets
...
function UIWidgetGrid:focus_grid_index(index, scrollbar_animation_progress, is_instant)
	self:select_grid_index(index, scrollbar_animation_progress, is_instant, not self._use_select_on_focused)
end

function UIWidgetGrid:focused_grid_index()
	return self._focused_grid_index
end

function UIWidgetGrid:select_grid_index(index, scrollbar_animation_progress, is_instant, use_is_focused)
...
function UIWidgetGrid:get_scrollbar_percentage_by_index(index, start_from_bottom)
...
function UIWidgetGrid:is_widget_visible(widget, extra_margin)
	if self._scrollbar_active then
