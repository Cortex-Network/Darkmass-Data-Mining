...
function ActionHandler:init(unit, data)
...
function ActionHandler:add_component(component_name)
...
function ActionHandler:set_tweak_component(component_name)
	local component = self._unit_data_extension:write_component(component_name)
	self._tweak_component = component
end

function ActionHandler:set_action_context(action_context)
	self._action_context = action_context
end

function ActionHandler:set_active_template(id, template_name)
...
function ActionHandler:block_actions()
	self._block_actions = true
end

function ActionHandler:unblock_actions()
	self._block_actions = false
end

function ActionHandler:wanted_character_state_transition()
...
function ActionHandler:update(dt, t)
	local registered_components = self._registered_components
...
function ActionHandler:_update_action(handler_data, action, dt, t)
	local component = handler_data.component
...
function ActionHandler:_update_timeline_anims(action, t, start_t, end_t)
...
function ActionHandler:start_action(id, action_objects, action_name, action_params, action_settings, used_input, t, transition_type, condition_func_params, automatic_input, reset_combo_override)
	local handler_data = self._registered_components[id]
	local component = handler_data.component
	local running_action = handler_data.running_action
...
function ActionHandler:_calculate_action_total_time(action_settings, action_params, time_scale)
...
function ActionHandler:_create_action(action_context, action_params, action_settings)
	local action_kind = action_settings.kind
...
function ActionHandler:_calculate_time_scale(action_settings)
...
function ActionHandler:_anim_event(action_settings, action, is_chain, condition_func_params)
...
function ActionHandler:server_correction_occurred(id, action_objects, action_params, actions)
	local handler_data = self._registered_components[id]
	local component = handler_data.component
...
function ActionHandler:_handle_action_input_on_stop_action(id, t, running_action)
	local action_settings = running_action:action_settings()
	local stop_input = action_settings.stop_input
...
function ActionHandler:has_running_action(id)
	local handler_data = self._registered_components[id]

	return handler_data.running_action ~= nil
end

function ActionHandler:running_action_name(id)
	local handler_data = self._registered_components[id]
	local running_action = handler_data.running_action
...
function ActionHandler:update_actions(fixed_frame, id, condition_func_params, actions, action_objects, action_params)
...
function ActionHandler:_validate_action(action_settings, condition_func_params, t, used_input)
...
function ActionHandler:_check_chain_actions(handler_data, current_action_settings, current_action_start_t, current_action_end_t, t, actions, condition_func_params, action_params, template, action_input, used_input)
	local component = handler_data.component
	local allowed_chain_actions = current_action_settings.allowed_chain_actions or EMPTY_TABLE
...
function ActionHandler:_validate_chain_action(chain_action, t, current_action_t, time_scale, actions, condition_func_params, used_input)
...
function ActionHandler:_valid_action_from_action_input(actions, action_input, t, condition_func_params, used_input)
...
function ActionHandler:_check_start_actions(handler_data, t, actions, condition_func_params, action_params, template, action_input, used_input)
	if not actions then
		return nil, , , , 
	end
...
function ActionHandler:_check_new_actions(handler_data, actions, condition_func_params, t, action_params)
	local running_action = handler_data.running_action
...
function ActionHandler:_update_stop_input(id, handler_data, t)
	local running_action = handler_data.running_action
	local has_running_action = running_action ~= nil
...
function ActionHandler:sensitivity_modifier(id, t)
	local handler_data = self._registered_components[id]
	local running_action = handler_data.running_action
...
function ActionHandler:rotation_contraints(id, t)
	local handler_data = self._registered_components[id]
	local running_action = handler_data.running_action
...
function ActionHandler:running_action_settings(id)
	local handler_data = self._registered_components[id]
	local running_action = handler_data.running_action
...
function ActionHandler:action_input_is_currently_valid(id, actions, condition_func_params, t, action_input, used_input)
	local handler_data = self._registered_components[id]
