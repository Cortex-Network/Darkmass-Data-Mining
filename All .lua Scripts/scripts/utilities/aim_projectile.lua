...
function AimProjectile.aim_parameters(initial_position, initial_rotation, look_rotation, locomotion_template, throw_type, time_in_action)
	...
	if rotation_charge_duration == 0 then
		...
		local look_rotation = Quaternion.multiply(look_rotation, maximal_offset)
		direction = Quaternion.forward(look_rotation)
	else
		...
		local maximal_ypr = rotation_offset_maximal
		local maximal_offset = Quaternion.from_yaw_pitch_roll(maximal_ypr.x, maximal_ypr.y, maximal_ypr.z)
		local offset = Quaternion.lerp(initial_offset, maximal_offset, alpha)
...
function AimProjectile.get_spawn_parameters_from_current_aim(action_settings, shoot_position, shoot_rotation, projectile_locomotion_template)
...
function AimProjectile.get_spawn_parameters_from_aim_component(action_aim_projectile)
