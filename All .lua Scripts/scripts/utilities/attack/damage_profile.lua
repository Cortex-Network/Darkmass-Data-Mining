...
function DamageProfile.armor_damage_modifier(power_type, damage_profile, target_settings, damage_profile_lerp_values, armor_type, is_critical_strike, dropoff_scalar, armor_penetrating, charge_level)
...
function DamageProfile.dropoff_scalar(hit_distance, damage_profile, lerp_values)
...
function DamageProfile.boost_curve_multiplier(target_settings, boost_multiplier_name, damage_profile_lerp_values)
...
function DamageProfile.suppression_value(damage_profile, damage_profile_lerp_values, num_suppressions)
...
function DamageProfile.suppression_attack_delay(damage_profile, damage_profile_lerp_values)
...
function DamageProfile.lerp_values(damage_profile, attacking_unit_or_nil, target_index_or_nil)
...
function DamageProfile.lerp_value_from_path(lerp_values, ...)
...
function DamageProfile.progress_lerp_values_path(lerp_values, ...)
	local default_lerp_value_or_nil = lerp_values[DEFAULT_LERP_VALUE]
...
function DamageProfile.lerp_damage_profile_entry(entry, lerp_value)
	local min = entry[1]
	local max = entry[2]
	local t = lerp_value
