...
function DamageTakenCalculation.get_calculation_parameters(attacked_unit, attacked_breed_or_nil, damage_profile, attacking_unit, attacking_unit_owner_unit, hit_actor)
...
function _calculate_toughness_damage_player(damage_amount, damage_profile, attack_type, attack_direction, toughness_template, weapon_toughness_template, current_toughness_damage, movement_state, attacked_unit_stat_buffs, attacked_unit_keywords, instakill, attacked_unit)
	...
	if melee_attack then
		...
		remaining_damage = math.lerp(damage_amount, damage_amount * 0.1, current_toughness_percent) * toughness_melee_spillover_modifier
		...
		if absorbed_attack or toughness_before_damage <= 0 then
