...
function Explosion.create_explosion(world, physics_world, source_position, impact_normal, attacking_unit, explosion_template, power_level, charge_level, attack_type, is_critical_strike, ignore_cover, item_or_nil, origin_slot_or_nil, optional_attack_result_table)
		...
		if hit_unit ~= attacking_unit_owner_unit or override_friendly_fire then
			...
			if damage_allowed then
				...
				if Health.is_ragdolled(hit_unit) and not hit_units[hit_unit] then
					if close_hit then
						MinionDeath.attack_ragdoll(hit_unit, direction, explosion_template.close_damage_profile, nil, hit_zone_name_or_nil, nil, )
					else
						MinionDeath.attack_ragdoll(hit_unit, direction, explosion_template.damage_profile, nil, hit_zone_name_or_nil, nil, )
					end
...
function Explosion.lerp_values(attacking_unit, explosion_template_name_or_nil)
...
function Explosion.lerp_value_from_path(lerp_values, ...)
...
function Explosion.lerp_entry(entry, lerp_value)
	local min = entry[1]
	local max = entry[2]
	local t = lerp_value
	...
	if sfx then
		...
		for i = 1, num_sfx do
			...
			if type(event_name_or_table) == "string" then
				fx_system:trigger_wwise_event(event_name_or_table, source_position, nil, , charge_wwise_parameter_name, charge_level)
			else
