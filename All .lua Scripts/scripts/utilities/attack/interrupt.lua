...
function Interrupt.ability(t, unit, reason, reason_data_or_nil, ignore_immunity)
...
function Interrupt.action(t, unit, reason, reason_data_or_nil, ignore_immunity)
	if not ignore_immunity and _is_interrupt_immune_from_buff(unit) then
		return
	end
	...
	if sprinting and action_settings then
		...
		if not allowed_during_sprint or sprint_requires_press_to_interrupt then
