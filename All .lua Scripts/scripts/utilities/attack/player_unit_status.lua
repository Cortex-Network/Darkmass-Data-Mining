...
function PlayerUnitStatus.is_disabled(character_state_component)
...
function PlayerUnitStatus.requires_help(character_state_component)
	local state_name = character_state_component.state_name

	return REQUIRES_HELP[state_name]
end

function PlayerUnitStatus.requires_immediate_help(character_state_component)
	local state_name = character_state_component.state_name

	return REQUIRES_IMMEDIATE_HELP[state_name]
end

function PlayerUnitStatus.requires_allied_interaction_help(character_state_component)
	local state_name = character_state_component.state_name

	return REQUIRES_ALLIED_INTERACTION_HELP[state_name]
end

function PlayerUnitStatus.end_zone_conditions_fulfilled(character_state_component)
	local state_name = character_state_component.state_name

	return VALID_END_ZONE_STATES[state_name]
end

function PlayerUnitStatus.is_dead_for_mission_failure(character_state_component)
	local state_name = character_state_component.state_name

	return MISSION_FAILURE_DEAD_STATES[state_name]
end

function PlayerUnitStatus.is_disabled_for_mission_failure(character_state_component)
	local state_name = character_state_component.state_name

	return MISSION_FAILURE_DISABLED_STATES[state_name]
end

function PlayerUnitStatus.is_knocked_down(character_state_component)
	local state_name = character_state_component.state_name

	return state_name == KNOCKED_DOWN_STATE_NAME
end

function PlayerUnitStatus.is_hogtied(character_state_component)
	local state_name = character_state_component.state_name

	return state_name == HOGTIED_STATE_NAME
end

function PlayerUnitStatus.is_catapulted(character_state_component)
	local state_name = character_state_component.state_name

	return state_name == CATAPULTED_STATE_NAME
end

function PlayerUnitStatus.is_pounced(disabled_character_state_component)
...
function PlayerUnitStatus.is_netted(disabled_character_state_component)
	local is_disabled = disabled_character_state_component.is_disabled
...
function PlayerUnitStatus.is_warp_grabbed(disabled_character_state_component)
	local is_disabled = disabled_character_state_component.is_disabled
...
function PlayerUnitStatus.is_mutant_charged(disabled_character_state_component)
	local is_disabled = disabled_character_state_component.is_disabled
...
function PlayerUnitStatus.is_consumed(disabled_character_state_component)
	local is_disabled = disabled_character_state_component.is_disabled
...
function PlayerUnitStatus.is_stunned(character_state_component)
	local state_name = character_state_component.state_name
...
function PlayerUnitStatus.is_ledge_hanging(character_state_component)
	local state_name = character_state_component.state_name
...
function PlayerUnitStatus.is_climbing_ladder(character_state_component)
	local state_name = character_state_component.state_name
...
function PlayerUnitStatus.is_in_mission_board(character_state_component, interacting_character_state_component)
	local state_name = character_state_component.state_name
...
function PlayerUnitStatus.is_assisted(assisted_state_input)
	local is_assisted = assisted_state_input.in_progress

	return is_assisted
end

function PlayerUnitStatus.is_dead(character_state_component)
	local state_name = character_state_component.state_name

	return state_name == "dead"
end

function PlayerUnitStatus.is_in_critical_health(health_extension, toughness_extension)
...
function PlayerUnitStatus.is_in_lunging_aim_or_combat_ability(lunge_character_state_component)
...
function PlayerUnitStatus.can_interact_with_objective(player_unit)
...
function PlayerUnitStatus.no_toughness_left(toughness_extension)
	local toughness_percent = toughness_extension:current_toughness_percent()

	return toughness_percent <= 0
end

function PlayerUnitStatus.has_corruption_damage(health_extension)
