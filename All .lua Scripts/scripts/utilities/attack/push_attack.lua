...
function PushAttack.push(physics_world, push_position, push_direction, rewind_ms, power_level, push_settings, attacking_unit, is_predicted, optional_weapon_item, weak_push)
	...
	for i = 1, num_actors do
		...
		if not owned_by_death_manager and not hit_units[unit] then
			...
			if damage_profile then
				...
				local damage_dealt, attack_result, damage_efficiency, stagger_result = Attack.execute(unit, damage_profile, "attacking_unit", attacking_unit, "attack_direction", attack_direction, "hit_world_position", hit_world_position, "hit_actor", actor, "power_level", power_level, "hit_zone_name", hit_zone_name, "damage_type", damage_type, "item", optional_weapon_item)

				ImpactEffect.play(unit, actor, damage_dealt, damage_type, hit_zone_name, attack_result, hit_world_position, nil, attack_direction, attacking_unit, impact_fx_data, nil, , damage_efficiency, damage_profile)
