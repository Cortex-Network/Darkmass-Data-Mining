...
function Suppression.apply_suppression(hit_unit, attacking_unit, damage_profile, hit_position, optional_same_side_supression_enabled, num_suppressions)
...
function Suppression.apply_area_explosion_suppression(attacking_unit, suppression_settings, from_position, optional_relation, optional_include_self, optional_lerp_values)
...
function Suppression.apply_area_minion_suppression(attacking_unit, suppression_settings, from_position, optional_relation, optional_include_self, optional_lerp_values)
...
function Suppression.apply_area_player_suppression(attacking_unit, suppression_settings, from_position, stagger_category, optional_relation, optional_include_self, optional_lerp_values)
	local broadphase_system = Managers.state.extension:system("broadphase_system")
	local broadphase = broadphase_system.broadphase
	local side_system = Managers.state.extension:system("side_system")
	local side = side_system.side_by_unit[attacking_unit]
...
function Suppression.add_immediate_suppression(t, suppression_movement_template, suppression_component, suppression_index)
...
function Suppression.apply_suppression_offsets_to_sway(suppression_component, pitch, yaw)
...
function Suppression.apply_suppression_offsets_to_spread(suppression_component, pitch, yaw)
...
function Suppression.is_suppressed(unit)
...
function Suppression.clear_suppression(unit)
	local suppression_extension = ScriptUnit.has_extension(unit, "suppression_system")
