...
function BotTargetSelection.slot_weight(unit, target_unit, target_distance_sq, target_breed, target_ally)
...
function BotTargetSelection.threat_weight(target_unit, threat_units)
...
function BotTargetSelection.opportunity_weight(unit, target_unit, target_breed, t)
	...
	if legacy_v2_proximity_extension then
		...
		if not react_at then
			legacy_v2_proximity_extension.bot_reaction_times[unit] = t + BotTargetSelection.reaction_time(true, OPPORTUNITY_TARGET_REACTION_TIMES, nil, )
...
function BotTargetSelection.priority_weight(target_unit, bot_group)
...
function BotTargetSelection.monster_weight(unit, target_unit, target_breed, t)
	local tags = target_breed.tags
...
function BotTargetSelection.current_target_weight(target_unit, current_target_enemy)
...
function BotTargetSelection.melee_distance_weight(target_distance_sq)
...
function BotTargetSelection.ranged_distance_weight(target_distance_sq)
...
function BotTargetSelection.line_of_sight_weight(unit, target_unit)
...
function BotTargetSelection.reaction_time(use_difficulty_reaction_times, difficulty_reaction_times, default_min, default_max)
...
function BotTargetSelection.allowed_ranged_target(bot_blackboard, target_unit)
