...
function BreedQueries.pick_random_minion_by_points(minion_breeds, points)
...
function BreedQueries.add_spawns_single_breed(spawners, breed_name, breed_amount, spawn_side_id, target_side_id, spawned_minion_data, optional_mission_objective_id, optional_attack_selection_template_name, optional_aggro_state, optional_group_id, optional_max_health_modifier)
	local num_spawners = #spawners
	local breed_lists = {}
	local breed_index = 1

	while breed_amount >= breed_index do
...
function BreedQueries.minion_breeds()
