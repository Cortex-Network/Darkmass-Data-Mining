...
function CharacterCreate:init(item_definitions, owned_gear, optional_real_profile)
...
function CharacterCreate:_reset_loadout()
	self._profile.loadout = {}
end

function CharacterCreate:reset_backstory()
	self._profile.lore.backstory = {}

	self:_randomize_lore_properties()
end

function CharacterCreate:_randomize_lore_properties()
...
function CharacterCreate:_randomize_backstory_properties()
...
function CharacterCreate:randomize_backstory_properties()
	self:_randomize_backstory_properties()
end

function CharacterCreate:_randomize_archetype_properties()
	local randomized_gender = self:_randomize_gender()

	self:set_gender(randomized_gender)
end

function CharacterCreate:profile()
	return self._profile
end

function CharacterCreate:_setup_default_values()
...
function CharacterCreate:get_height_values_range()
...
function CharacterCreate:_setup_appearance_presets(verified_items)
	local presets = {}
...
function CharacterCreate:_randomize_archetype()
...
function CharacterCreate:_randomize_gender()
...
function CharacterCreate:_presets_options()
...
function CharacterCreate:_is_fallback_item(slot, item_name)
...
function CharacterCreate:_verify_items(source_items, owned_gear)
...
function CharacterCreate:height()
	return self._character_height
end

function CharacterCreate:set_height(scale_factor)
	self._character_height = scale_factor
end

function CharacterCreate:breed()
...
function CharacterCreate:gender()
	return self._profile.gender
end

function CharacterCreate:randomize_presets()
...
function CharacterCreate:set_gender(gender)
...
function CharacterCreate:set_archetype(archetype)
...
function CharacterCreate:reset_height()
...
function CharacterCreate:set_specialization(specialization)
...
function CharacterCreate:set_item_per_slot(slot_name, item)
	local profile = self._profile
...
function CharacterCreate:set_item_per_slot_preview(slot_name, item, profile)
	local loadout = profile.loadout
	loadout[slot_name] = item
...
function CharacterCreate:gender_options()
	local breed = self._profile.breed
...
function CharacterCreate:archetype_options()
	return self._archetypes_array
end

function CharacterCreate:personality()
	return self._profile.lore.backstory.personality
end

function CharacterCreate:set_personality(option)
	self._profile.lore.backstory.personality = option

	self:_increase_value_version("personality")
end

function CharacterCreate:personality_options()
	local profile = self._profile
...
function CharacterCreate:randomize_name()
...
function CharacterCreate:set_name(name)
	self._profile.name = name
end

function CharacterCreate:name()
	return self._profile.name
end

function CharacterCreate:crime_options()
	local profile = self._profile
...
function CharacterCreate:_randomize_crime()
	local crimes = self:crime_options()

	return crimes[math.random(1, #crimes)]
end

function CharacterCreate:slot_item(slot_name)
	local profile = self._profile
	local loadout = profile.loadout
...
function CharacterCreate:slot_item_options(slot_name)
	local profile = self._profile
	local archetype = profile.archetype
	local archetype_name = archetype.name
	local breed = profile.breed
...
function CharacterCreate:set_gear_visible(visible)
...
function CharacterCreate:check_name(name)
	local profiles_service = Managers.data_service.profiles

	return profiles_service:check_name(name)
end

function CharacterCreate:destroy()
end

function CharacterCreate:profile_value_versions()
	return self._profile_value_versions
end

function CharacterCreate:_increase_value_version(value_keys)
...
function CharacterCreate:_generate_backend_profile()
	local profile = self._profile
...
function CharacterCreate:completed()
	return self._done
end

function CharacterCreate:failed()
	return self._failed
end

function CharacterCreate:upload_profile()
...
function CharacterCreate:created_character_profile()
