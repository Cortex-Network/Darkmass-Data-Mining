...
function MinionAttack.aim_at_target(unit, scratchpad, t, action_data)
...
function MinionAttack.aim_at_position(unit, scratchpad, t, action_data, target_position)
...
function MinionAttack.update_scope_reflection(unit, scratchpad, t, action_data)
...
function MinionAttack.trigger_shoot_sfx_and_vfx(unit, scratchpad, action_data, optional_end_position)
...
function MinionAttack.shoot_hit_scan(world, physics_world, unit, target_unit, weapon_item, fx_source_name, shoot_position, shoot_template, optional_spread_multiplier, perception_component)
	...
	if should_hit then
		end_position = HitScan.process_hits(is_server, world, physics_world, unit, shoot_template, hits, from_position, spread_direction, power_level, charge_level, IMPACT_FX_DATA, range, nil, , , , , )
	elseif hits then
		end_position = HitScan.process_hits(is_server, world, physics_world, unit, shoot_template, hits, from_position, spread_direction, power_level * 0.175, charge_level, IMPACT_FX_DATA, range, nil, , , , , )
	end
...
function MinionAttack.init_scratchpad_shooting_variables(unit, scratchpad, action_data, blackboard, breed)
...
function MinionAttack.check_and_start_scope_reflection_timing(scratchpad, action_data, aim_duration)
	local shoot_template = action_data.shoot_template
...
function MinionAttack.start_shooting(unit, scratchpad, t, action_data, optional_shoot_timing, optional_ignore_add_intensity)
...
function MinionAttack.update_shooting(unit, scratchpad, t, action_data)
...
function MinionAttack.shoot(unit, scratchpad, action_data)
	local perception_component = scratchpad.perception_component
...
function MinionAttack.stop_shooting(unit, scratchpad)
	local global_effect_id = scratchpad.global_effect_id
...
function MinionAttack.get_attack_delay(unit)
...
function MinionAttack.check_and_trigger_backstab_sound(attacking_unit, action_data, target_unit, wwise_event, dot_threshold, ignore_attack_intensity)
	local target_unit_data_extension = ScriptUnit.extension(target_unit, "unit_data_system")
	local target_breed = target_unit_data_extension:breed()
...
function MinionAttack.push_nearby_enemies(unit, scratchpad, action_data, ignored_unit, optional_only_ahead_targets)
...
function MinionAttack.push_friendly_minions(unit, scratchpad, action_data, t, optional_from_position)
...
function MinionAttack.sweep(unit, breed, scratchpad, blackboard, target_unit, action_data, physics_world, sweep_hit_units_cache, override_damage_profile_or_nil, override_damage_type_or_nil, attack_event)
...
function MinionAttack.melee(unit, breed, scratchpad, blackboard, target_unit, action_data, physics_world, override_damage_profile_or_nil, override_damage_type_or_nil, attack_event)
	local hit = nil
...
function MinionAttack.update_lag_compensation_melee(unit, breed, scratchpad, blackboard, t, action_data)
...
function MinionAttack.melee_oobb_extents(unit, action_data)
...
function MinionAttack.melee_broadphase_extents(unit, action_data)
...
function _melee_hit(unit, breed, scratchpad, blackboard, target_unit, hit_position, action_data, override_damage_profile_or_nil, override_damage_type_or_nil, offtarget_hit_or_nil)
	...
	ImpactEffect.play(target_unit, nil, damage, damage_type, nil, result, hit_position, nil, attack_direction, unit, nil, , , damage_efficiency, damage_profile)
