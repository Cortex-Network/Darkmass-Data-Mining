...
function PlayerCompositions.trigger_change_event(composition_name)
	Managers.event:trigger(PlayerCompositions.player_composition_changed_event, composition_name)
end

function PlayerCompositions.player_from_unique_id(composition_name, unique_id)
...
function PlayerCompositions.party_member_by_peer_id(peer_id)
	if GameParameters.prod_like_backend then
