...
function Scanning.get_scannable_units_position(scannable_unit, scannable_extension)
...
function Scanning.calculate_score(scannable_unit, first_person_component, scan_settings, scannable_extension)
...
function Scanning.check_direct_line_of_sight(physics_world, first_person_component, scan_distance)
	local rotation = first_person_component.rotation
...
function Scanning.check_line_of_sight_to_unit(physics_world, first_person_component, scannable_unit, scan_settings, scannable_extension)
	local look_pos = first_person_component.position
	local rotation = first_person_component.rotation
	local look_forward = rotation and Quaternion.forward(rotation)
...
function Scanning.find_scannable_unit(physics_world, first_person_component, scan_settings)
...
function Scanning.calculate_current_scores(scanning_compomnent, weapon_action_component, first_person_component)
	local total_score = 0
	local angle_score = 0
...
function Scanning.scan_confirm_progression(scanning_compomnent, weapon_action_component, t)
...
function Scanning.calculate_color_lerp(lerp_value)
	return math.ilerp(0.25, 1, lerp_value)
end

function Scanning.has_active_scanning_zone()
	local mission_objective_zone_system = Managers.state.extension:system("mission_objective_zone_system")
	local current_scan_mission_zone = mission_objective_zone_system:current_active_zone()

	return not not current_scan_mission_zone
end
